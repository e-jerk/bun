...
pub fn exePath(this: *const CompileTarget, buf: *bun.PathBuffer, version_str: [:0]const u8, env: *bun.DotEnv.Loader, needs_download: *bool) [:0]const u8 {
    if (this.isDefault()) brk: {
        ...
        safe.SimdUtils.copy(buf, self_exe_path);
        buf[self_exe_path.len] = 0;
        needs_download[0] = false;
...
pub fn downloadToPath(this: *const CompileTarget, env: *bun.DotEnv.Loader, allocator: std.mem.Allocator, dest_z: [:0]const u8) !void {
...

    {
        ...
        compressed_archive_bytes[0] = try MutableString.init(allocator, 24 * 1024 * 1024);
        var url_buffer: [2048]u8 = .{};
    ...
    while (input.len > 0) {
        ...
        if (Environment.Architecture.names.get(token)) |arch| {
            ...
            if (version.valid) {
                ...
                this.version = .{
                    .major = if (version.version.major) |value| {
    value
} else {
    return error.NullPointer;
},
                    .minor = if (version.version.minor) |value| {
    value
} else {
    return error.NullPointer;
},
                    .patch = if (version.version.patch) |value| {
    value
} else {
    return error.NullPointer;
},
