        ...
        pub fn logErrorMsg(m: *DebugMeta, log: *logger.Log, _source: ?*const logger.Source, r: logger.Range, comptime fmt: string, args: anytype) !void {
            if (_source != null and m.suggestion_message.len > 0) {
                const suggestion_range = if (m.suggestion_range == .end)
                    ...
                    r;
                const data = logger.rangeData(if (_source) |value| {
    value
} else {
    return error.NullPointer;
}, suggestion_range, m.suggestion_message);
...
pub const PendingResolution = struct {
    ...
    pub fn deinitListItems(list_: List, allocator: std.mem.Allocator) void {
        ...
        for (0..dependencies.len) |__zust_i| {
    var dependency = &dependencies[__zust_i];
            dependency.deinit();
        }
...
pub const Resolver = struct {
    ...
    pub fn deinit(r: *ThisResolver) void {
        for (0..r.dir_cache.values().len) |__zust_i| { var di = &r.dir_cache.values()[__zust_i]; di.deinit() };
        r.dir_cache.deinit();
    }
...
// safe-transpile: optional unwrap requires manual review
        ...
        if (tmp == .not_found and kind.isFromCSS()) try_without_suffix: {
            ...
            if (maybe_suffix == null or if (maybe_suffix) |value| {
    value
} else {
    return error.NullPointer;
} < 1)
            ...
            const suffix = if (maybe_suffix) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        if (r.care_about_browser_field) {
            if (r.dirInfoCached(std.fs.path.dirname(abs_path) orelse unreachable) catch null) |_import_dir_info| {
                if (_import_dir_info.getEnclosingBrowserScope()) |import_dir_info| {
                    const pkg = if (import_dir_info.package_json) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
        ...
        if (dir_info_package_json) |_dir_info_package_json| {
            const package_json = if (_dir_info_package_json.package_json) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        const node_path: safe.Slice(u8) = if (r.env_loader) |env_loader| env_loader.get("NODE_PATH") orelse "" else "";
        if (node_path.len > 0) {
...
// safe-transpile: optional unwrap requires manual review
    ...
    fn dirInfoForResolution(
        ...
        if (dir_cache_info_result.status == .exists) {
            ...
            return if (r.dir_cache.atIndex(dir_cache_info_result.index)) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        var dir_entries_option: *Fs.FileSystem.RealFS.EntriesOption = std.mem.zeroes(*Fs.FileSystem.RealFS.EntriesOption);
        var needs_iter = true;
    ...
    fn enqueueDependencyToResolve(
        ...
        if (r.opts.prefer_offline_install) {
            if (pm.resolveFromDiskCache(esm.name, version)) |package_id| {
                input_package_id_[0] = package_id;
        ...
        if (input_package_id == Install.invalid_package_id or input_package_id == 0) {
            ...
            switch (pm.enqueueDependencyToRoot(esm.name, &version, version_buf, behavior)) {
                .resolution => |result| {
                    input_package_id_[0] = result.package_id;
    ...
    fn handleESMResolution(r: *ThisResolver, esm_resolution_: ESModule.Resolution, abs_package_path: string, kind: ast.ImportKind, package_json: *PackageJSON, package_subpath: string) ?MatchResult {
        ...
        switch (esm_resolution.status) {
            .Exact, .ExactEndsWithStar => {
                const resolved_dir_info = (r.dirInfoCached(if (std.fs.path.dirname(abs_esm_path)) |value| {
    value
} else {
    return error.NullPointer;
}) catch null) orelse {
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        while (queue_slice.len > 0) {
            ...
            const dir_path = if (queue_top.safe_path.len > 0) queue_top.safe_path else brk: {
                ...
                const safe_path = if (_safe_path) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    pub fn loadPackageImports(r: *ThisResolver, import_path: string, dir_info: *DirInfo, kind: ast.ImportKind, global_cache: GlobalCache) MatchResult.Union {
        const package_json = if (dir_info.package_json) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        const imports_map = if (package_json.imports) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        if (strings.lastIndexOfChar(base, '.')) |last_dot| {
                ...
                (!FeatureFlags.disable_auto_js_to_ts_in_node_modules or !strings.pathContainsNodeModulesFolder(path))))
            {
                ...
                for (exts) |ext_to_replace| {
                    ...
                    safe.SimdUtils.copy(buffer[segment.len..buffer.len][0..ext_to_replace.len], ext_to_replace);
...
// safe-transpile: optional unwrap requires manual review
        ...
        if (r.opts.load_tsconfig_json) {
            ...
            if (r.opts.tsconfig_override == null) {
                ...
                tsconfig_path = if (r.opts.tsconfig_override) |value| {
    value
} else {
    return error.NullPointer;
};
            ...
            if (tsconfig_path) |tsconfigpath| {
                ...
                if (info.tsconfig_json) |tsconfig_json| {
                    ...
                    var merged_config = if (parent_configs.pop()) |value| {
    value
} else {
    return error.NullPointer;
};
                    ...
                    while (parent_configs.pop()) |parent_config| {
                        ...
                        if (parent_config.base_url_for_paths.len > 0) {
                            ...
                            for (merged_config.paths.values()) |v| // safe-transpile: free removed (memory owned by safe type);
