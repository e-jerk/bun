...
pub fn runImmediateTask(this: *TimerObjectInternals, vm: *VirtualMachine) bool {
    ...
    vm.eventLoop().enter();
    const callback = if (ImmediateObject.js.callbackGetCached(timer)) |value| {
    value
} else {
    return error.NullPointer;
};
    const arguments = if (ImmediateObject.js.argumentsGetCached(timer)) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub fn fire(this: *TimerObjectInternals, _: *const timespec, vm: *jsc.VirtualMachine) void {
    ...
    const callback: JSValue = undefined, const arguments: JSValue = undefined, var idle_timeout: JSValue = undefined, var repeat: JSValue = undefined = switch (kind) {
        .setImmediate => .{
            if (ImmediateObject.js.callbackGetCached(this_object)) |value| {
    value
} else {
    return error.NullPointer;
},
            if (ImmediateObject.js.argumentsGetCached(this_object)) |value| {
    value
} else {
    return error.NullPointer;
},
        ...
        .setTimeout, .setInterval => .{
            if (TimeoutObject.js.callbackGetCached(this_object)) |value| {
    value
} else {
    return error.NullPointer;
},
            if (TimeoutObject.js.argumentsGetCached(this_object)) |value| {
    value
} else {
    return error.NullPointer;
},
            if (TimeoutObject.js.idleTimeoutGetCached(this_object)) |value| {
    value
} else {
    return error.NullPointer;
},
            if (TimeoutObject.js.repeatGetCached(this_object)) |value| {
    value
} else {
    return error.NullPointer;
},
    ...
    vm.eventLoop().enter();
    {
        ...
        switch (kind) {
            .setTimeout, .setInterval => {
                idle_timeout = if (TimeoutObject.js.idleTimeoutGetCached(this_object)) |value| {
    value
} else {
    return error.NullPointer;
};
                repeat = if (TimeoutObject.js.repeatGetCached(this_object)) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub fn reschedule(this: *TimerObjectInternals, timer: JSValue, vm: *VirtualMachine, globalThis: *JSGlobalObject) void {
    ...
    const idle_timeout = if (TimeoutObject.js.idleTimeoutGetCached(timer)) |value| {
    value
} else {
    return error.NullPointer;
};
    const repeat = if (TimeoutObject.js.repeatGetCached(timer)) |value| {
    value
} else {
    return error.NullPointer;
};
