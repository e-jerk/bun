...
// NOTE: not @import("std-net-shim").Address b/c .un is huge and we don't use it.
...
pub fn parse(global: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!jsc.JSValue {
    ...
    const url_str = switch (input.is8Bit()) {
        inline else => |is_8_bit| with_prefix: {
            ...
            safe.SimdUtils.copy(to_chars[0..prefix.len], bun.strings.literal(enc.Byte(), prefix));
            safe.SimdUtils.copy(to_chars[prefix.len..], from_chars);
...
pub fn initIPv4(addr: [4]u8, port_: u16) SocketAddress {
    ...
    return .{ ._addr = sockaddr.v4(std.mem.nativeToBig(u16, port_), // safe-transpile: @bitCast requires manual review
    @bitCast(addr)) };
}
...
pub fn address(this: *SocketAddress) bun.String {
    ...
    var buf: [inet.INET6_ADDRSTRLEN]u8 = .{};
...
pub fn flowLabel(this: *const SocketAddress) ?u32 {
    if (this.family() == AF.INET6) {
        const in6: inet.sockaddr_in6 = // safe-transpile: @bitCast requires manual review
    @bitCast(this._addr);
        return in6.flowinfo;
    } else {
        return null;
...
// bindings though.
extern "c" const IPv4: StaticStringImpl = undefined;
extern "c" const IPv6: StaticStringImpl = undefined;
...
pub const sockaddr = extern union {
    ...
    pub fn fmt(self: *const sockaddr, buf: *[inet.INET6_ADDRSTRLEN]u8) [:0]const u8 {
        const addr_src: *const anyopaque = if (self.family() == AF.INET) // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&self.sin.addr) else // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&self.sin6.addr);
    ...
    pub const @"127.0.0.1": sockaddr = sockaddr.v4(0, // safe-transpile: @bitCast requires manual review
    @bitCast([_]u8{ 127, 0, 0, 1 }));
...
const WellKnownAddress = struct {
    extern "c" const INET_LOOPBACK: StaticStringImpl = undefined;
    extern "c" const INET6_ANY: StaticStringImpl = undefined;
