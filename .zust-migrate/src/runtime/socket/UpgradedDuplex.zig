...
fn callWriteOrEnd(this: *UpgradedDuplex, data: ?[]const u8, msg_more: bool) void {
    ...
    if (this.origin.get()) |duplex| {
        const globalThis = if (this.global) |value| {
    value
} else {
...
fn onReceivedData(
    ...
    if (jsc.host_fn.getFunctionData(function)) |self| {
        const this = @as(*UpgradedDuplex, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
...
pub fn sslError(this: *UpgradedDuplex) us_bun_verify_error_t {
    return .{
        ...
        .code = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.ssl_error.code.ptr),
        .reason = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.ssl_error.reason.ptr),
    };
}
