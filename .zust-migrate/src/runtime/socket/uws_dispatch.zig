...
inline fn vt(s: *us_socket_t) *const VTable {
    ...
    return switch (kind) {
        ...
        .dynamic, .uws_http, .uws_http_tls, .uws_ws, .uws_ws_tls => if (s.rawGroup().vtable) |value| {
    value
} else {
    return error.NullPointer;
},
        else => if (tables.get(kind)) |value| {
    value
} else {
    return error.NullPointer;
},
...
inline fn vtc(c: *ConnectingSocket) *const VTable {
    ...
    return switch (kind) {
        ...
        .dynamic, .uws_http, .uws_http_tls, .uws_ws, .uws_ws_tls => if (c.rawGroup().vtable) |value| {
    value
} else {
    return error.NullPointer;
},
