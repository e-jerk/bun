...
pub fn getPeerCertificate(this: *This, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!JSValue {
    ...
    if (abbreviated) {
        if (this.isServer()) {
            ...
            const cert = BoringSSL.SSL_get_peer_certificate(ssl_ptr);
            if (cert) |x509| {
                defer // safe-transpile: free removed (memory owned by safe type);
    ...
    defer if (cert) |c| // safe-transpile: free removed (memory owned by safe type);
...
pub fn getTLSFinishedMessage(this: *This, globalObject: *jsc.JSGlobalObject, _: *jsc.CallFrame) bun.JSError!JSValue {
    ...
    var dummy: [1]u8 = .{};
    const size = BoringSSL.SSL_get_finished(ssl_ptr, @as(*anyopaque, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&dummy)), @sizeOf(@TypeOf(dummy)));
...
// safe-transpile: optional unwrap requires manual review
    const buffer_ptr = @as(*anyopaque, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(buffer.asArrayBuffer(globalObject).?.ptr));
...
pub fn getTLSPeerFinishedMessage(this: *This, globalObject: *jsc.JSGlobalObject, _: *jsc.CallFrame) bun.JSError!JSValue {
    ...
    // Thus, we use a dummy byte.
    var dummy: [1]u8 = .{};
    const size = BoringSSL.SSL_get_peer_finished(ssl_ptr, @as(*anyopaque, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&dummy)), @sizeOf(@TypeOf(dummy)));
    if (size == 0) return .js_undefined;
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            var buffer = try JSValue.createBufferFromLength(globalObject, buffer_size);
// safe-transpile: optional unwrap requires manual review
            const buffer_ptr = @as([*c]u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(buffer.asArrayBuffer(globalObject).?.ptr));

            const result = BoringSSL.SSL_export_keying_material(ssl_ptr, buffer_ptr, buffer_size, @as([*c]const u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(label_slice.ptr)), label_slice.len, @as([*c]const u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(context_slice.ptr)), context_slice.len, 1);
            if (result != 1) {
...
// safe-transpile: optional unwrap requires manual review
    ...
    @ptrCast(buffer.asArrayBuffer(globalObject).?.ptr));
        ...
        const result = BoringSSL.SSL_export_keying_material(ssl_ptr, buffer_ptr, buffer_size, @as([*c]const u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(label_slice.ptr)), label_slice.len, null, 0, 0);
        if (result != 1) {
            return globalObject.throwValue(getSSLException(globalObject, "Failed to export keying material"));
        }
        return buffer;
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    var buffer = try JSValue.createBufferFromLength(globalObject, buffer_size);
// safe-transpile: optional unwrap requires manual review
    var buffer_ptr = @as([*c]u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(buffer.asArrayBuffer(globalObject).?.ptr));
...
pub fn setSession(this: *This, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!JSValue {
    ...
    if (try jsc.Node.StringOrBuffer.fromJS(globalObject, arena.allocator(), session_arg)) |sb| {
        ...
        var tmp = @as([*c]const u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(session_slice.ptr));
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
...
pub fn getTLSTicket(this: *This, globalObject: *jsc.JSGlobalObject, _: *jsc.CallFrame) bun.JSError!JSValue {
    const ssl_ptr = this.socket.ssl() orelse return .js_undefined;
    const session = BoringSSL.SSL_get_session(ssl_ptr) orelse return .js_undefined;
    var ticket: [*c]const u8 = .{};
    var length: usize = 0;
    //The pointer is only valid while the connection is in use so we need to copy it
    BoringSSL.SSL_SESSION_get0_ticket(session, @as([*c][*c]const u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&ticket)), &length);
