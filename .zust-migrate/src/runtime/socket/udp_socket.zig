...
fn onClose(socket: *uws.udp.Socket) callconv(.c) void {
    ...
    const this: *UDPSocket = bun.cast(*UDPSocket, if (socket.user()) |value| {
    value
} else {
...
fn onData(socket: *uws.udp.Socket, buf: *uws.udp.PacketBuffer, packets: c_int) callconv(.c) void {
    ...
    const udpSocket: *UDPSocket = bun.cast(*UDPSocket, if (socket.user()) |value| {
    value
} else {
    return error.NullPointer;
});
    ...
    while (i < packets) : (i += 1) {
        ...
        var addr_buf: [INET6_ADDRSTRLEN + 1:0]u8 = .{};
        ...
        switch (peer.family) {
            std.posix.AF.INET => {
                const peer4: *std.posix.sockaddr.in = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(peer);
                hostname = inet_ntop(peer.family, &peer4.addr, &addr_buf, addr_buf.len);
                port = ntohs(peer4.port);
            },
            std.posix.AF.INET6 => {
                const peer6: *std.posix.sockaddr.in6 = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(peer);
        ...
        const span = std.mem.span((if (hostname) |value| {
    value
} else {
    return error.NullPointer;
}));
...
pub const UDPSocket = struct {
    ...
    pub fn udpSocket(globalThis: *JSGlobalObject, options: JSValue) bun.JSError!JSValue {
        ...
        this.socket = uws.udp.Socket.create(
            ...
            if (err != 0) {
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                const code = @tagName(if (bun.sys.SystemErrno.init(@as(c_int, @intCast(err)))) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: optional unwrap requires manual review
        ...
        var source_addr: std.posix.sockaddr.storage = std.mem.zeroes(std.posix.sockaddr.storage);
...
// safe-transpile: optional unwrap requires manual review
        ...
        var group_addr: std.posix.sockaddr.storage = std.mem.zeroes(std.posix.sockaddr.storage);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        var addr4: *std.posix.sockaddr.in = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(storage);
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
            ...
            var addr6: *std.posix.sockaddr.in6 = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(storage);
            addr6.scope_id = 0;
