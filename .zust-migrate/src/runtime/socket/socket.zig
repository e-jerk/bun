...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        pub fn handleConnectError(this: *This, errno: c_int) bun.JSError!void {
            ...
            if (callback == .zero) {
                ...
                if (handlers.promise.trySwap()) |promise| {
                    ...
                    const js_promise = if (promise.asPromise()) |value| {
    value
} else {
    return error.NullPointer;
            ...
            if (result.toError()) |err_val| {
                ...
                var promise = if (val.asPromise()) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        pub fn getLocalFamily(this: *This, globalThis: *jsc.JSGlobalObject) bun.JSError!JSValue {
            ...
            const address_bytes: safe.Slice(u8) = this.socket.localAddress(&buf) orelse return .js_undefined;
        ...
        pub fn getLocalAddress(this: *This, globalThis: *jsc.JSGlobalObject) JSValue {
            ...
            const address_bytes: safe.Slice(u8) = this.socket.localAddress(&buf) orelse return .js_undefined;
            const address: @import("std-net-shim").Address = switch (address_bytes.len) {
        ...
        pub fn getRemoteFamily(this: *This, globalThis: *jsc.JSGlobalObject) bun.JSError!JSValue {
            ...
            var buf: [64]u8 = [_]u8{0} ** 64;
            const address_bytes: safe.Slice(u8) = this.socket.remoteAddress(&buf) orelse return .js_undefined;
            return switch (address_bytes.len) {
                4 => globalThis.commonStrings().IPv4(),
                16 => globalThis.commonStrings().IPv6(),
                else => return .js_undefined,
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
        ...
        pub fn upgradeTLS(this: *This, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!JSValue {
            ...
            const new_raw = raw_socket.adoptTLS(group, .bun_socket_tls, if (tls.owned_ssl_ctx) |value| {
    value
} else {
    return error.NullPointer;
}, sni, @sizeOf(*anyopaque), @sizeOf(*anyopaque)) orelse {
...
pub fn jsUpgradeDuplexToTLS(globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!JSValue {
    ...
    duplexContext.upgrade = uws.UpgradedDuplex.from(globalObject, duplex, .{
        .onOpen = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&DuplexUpgradeContext.onOpen),
        .onData = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&DuplexUpgradeContext.onData),
        .onHandshake = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&DuplexUpgradeContext.onHandshake),
        .onClose = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&DuplexUpgradeContext.onClose),
        .onEnd = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&DuplexUpgradeContext.onEnd),
        .onWritable = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&DuplexUpgradeContext.onWritable),
        .onError = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&DuplexUpgradeContext.onError),
        .onTimeout = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&DuplexUpgradeContext.onTimeout),
        .ctx = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(duplexContext),
    });
