...
pub fn listen(globalObject: *jsc.JSGlobalObject, opts: JSValue) bun.JSError!JSValue {
    ...
    if (Environment.isWindows and port == null) {
        ...
        if (normalizePipeName(hostname_or_unix.slice(), buf[0..])) |pipe_name| {
            ...
            errdefer {
                ...
                if (this.protos) |protos| // safe-transpile: free removed (memory owned by safe type);
                defer _ = this.deinit();
            }
    ...
    errdefer if (listener_allocated) {
        ...
        if (this.protos) |p| // safe-transpile: free removed (memory owned by safe type);
...
pub fn onCreate(comptime ssl: bool, listener: *Listener, socket: uws.NewSocketHandler(ssl)) *NewSocket(ssl) {
    ...
    if (socket.ext(**anyopaque)) |ctx| {
        ctx[0] = bun.cast(**anyopaque, this_socket);
    }
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        if (isNamedPipe) {
            ...
            handlers_ptr[0] = handlers.*;
            handlers_ptr.mode = .client;

            var promise = safe.Box(globalObject).init(allocator, undefined);
...
pub fn getsockname(this: *Listener, globalThis: *jsc.JSGlobalObject, callFrame: *jsc.CallFrame) bun.JSError!JSValue {
    ...
    var text_buf: [512]u8 = .{};
    const address_bytes: safe.Slice(u8) = socket.getLocalAddress(&buf) catch return .js_undefined;
...
fn normalizePipeName(pipe_name: []const u8, buffer: []u8) ?[]const u8 {
    if (Environment.isWindows) {
        ...
        safe.SimdUtils.copy(buffer[0..9], "\\\\.\\pipe\\");
        safe.SimdUtils.copy(buffer[9..pipe_name.len], pipe_name[9..]);
...
pub const WindowsNamedPipeListeningContext = if (Environment.isWindows) struct {
    ...
    fn onClientConnect(this: *WindowsNamedPipeListeningContext, status: uv.ReturnCode) void {
        ...
        const listener = if (this.listener) |value| {
    value
} else {
    return error.NullPointer;
};
    ...
    fn onPipeClosed(pipe: *uv.Pipe) callconv(.c) void {
        const this: *WindowsNamedPipeListeningContext = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(pipe.data));
        this.deinit();
    }
