...
pub fn ping(this: *JSValkeyClient, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!JSValue {
    ...
    defer {
        for (0..args_slice.len) |__zust_i| {
    var item = &args_slice[__zust_i];
            item.deinit();
        }
    }
...
pub fn subscribe(
    ...
    defer {
        for (0..redis_channels.items.len) |__zust_i| {
    var item = &redis_channels.items[__zust_i];
            item.deinit();
        }
