...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    fn hashToBytes(globalThis: *JSGlobalObject, evp: *EVP, input: jsc.Node.BlobOrStringOrBuffer, output: ?jsc.ArrayBuffer) bun.JSError!jsc.JSValue {
        var output_digest_buf: Digest = undefined;
        var output_digest_slice = try safe.OffsetPtr.init(allocator, &output_digest_buf); defer output_digest_slice.deinit();
        defer input.deinit();
...
const CryptoHasherZig = struct {
    ...
    fn hashByNameInnerToString(globalThis: *JSGlobalObject, comptime Algorithm: type, input: jsc.Node.BlobOrStringOrBuffer, encoding: jsc.Node.Encoding) bun.JSError!jsc.JSValue {
        ...
        var out: [digestLength(Algorithm)]u8 = .{};
        h.final(&out);
    ...
    fn update(self: *CryptoHasherZig, bytes: []const u8) void {
        inline for (algo_map) |item| {
            if (self.algorithm == @field(EVP.Algorithm, item[0])) {
                return item[1].update(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(self.state)), bytes);
            }
        }
        @panic("unreachable");
    }
        ...
        inline for (algo_map) |item| {
            if (self.algorithm == @field(EVP.Algorithm, item[0])) {
                return .{
                    ...
                    .state = bun.dupe(item[1], // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(self.state))),
    ...
    fn finalWithLen(self: *CryptoHasherZig, output_digest_slice: []u8, res_len: usize) []u8 {
        inline for (algo_map) |pair| {
            ...
            if (self.algorithm == @field(EVP.Algorithm, name)) {
                T.final(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(self.state)), // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(output_digest_slice));
                const reset: *T = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(self.state));
                reset[0] = T.init(.{});
...
// safe-transpile: function returns small constant slice — consider safe.String
    ...
    fn deinit(self: *CryptoHasherZig) void {
        inline for (algo_map) |item| {
            if (self.algorithm == @field(EVP.Algorithm, item[0])) {
                return bun.destroy(@as(*item[1], // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(self.state))));
            }
        }
        @panic("unreachable");
...
fn StaticCryptoHasher(comptime Hasher: type, comptime name: [:0]const u8) type {
    return struct {
        ...
        fn hashToEncoding(globalThis: *JSGlobalObject, input: jsc.Node.BlobOrStringOrBuffer, encoding: jsc.Node.Encoding) bun.JSError!jsc.JSValue {
            var output_digest_buf: Hasher.Digest = std.mem.zeroes(Hasher.Digest);
        ...
        fn digestToEncoding(this: *@This(), globalThis: *JSGlobalObject, encoding: jsc.Node.Encoding) bun.JSError!jsc.JSValue {
            var output_digest_buf: Hasher.Digest = comptime brk: {
                var bytes: Hasher.Digest = std.mem.zeroes(Hasher.Digest);
                var i: usize = 0;
