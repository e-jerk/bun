...
pub fn decode(bytes: []const u8, max_pixels: u64) codecs.Error!codecs.Decoded {
    // ── header + LSD ───────────────────────────────────────────────────────
    if (bytes.len < 13 or !(safe.SimdUtils.eql(bytes[0..6], "GIF89a") or safe.SimdUtils.eql(bytes[0..6], "GIF87a")))
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    const gct: safe.Slice(u8) = if (has_gct) bytes[13..][0 .. @as(usize, gct_size) * 3] else &.{};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                ...
                const ct: safe.Slice(u8) = if (has_lct) blk: {
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    var dict = safe.Box(Dict).init(allocator, undefined) catch return error.OutOfMemory;
    defer _ = dict.deinit();
    var scratch: [4096]u8 = .{};
    ...
    while (written < npix) {
        ...
        if (code < avail) {
            ...
            const r = dict.emit((if (prev) |value| {
    value
} else {
    return error.NullPointer;
}), clear, idx[written..], &scratch);
            written += r[0];
            first = r[1];
            if (written < npix) {
                idx[written] = first;
...
inline fn expandRow(idx: []const u8, out: []u8, pal: *const [256][4]u8) void {
    ...
    for (idx, 0..) |c, x| safe.SimdUtils.copy(out[x * 4 ..][0..4], &pal[c]);
}
