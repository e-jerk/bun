...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn sniff(bytes: []const u8) ?Format {
        if (bytes.len >= 3 and bytes[0] == 0xFF and bytes[1] == 0xD8 and bytes[2] == 0xFF)
            return .jpeg;
        if (bytes.len >= 8 and safe.SimdUtils.eql(bytes[0..8], "\x89PNG\r\n\x1a\n"))
            return .png;
        if (bytes.len >= 12 and safe.SimdUtils.eql(bytes[0..4], "RIFF") and safe.SimdUtils.eql(bytes[8..12], "WEBP"))
            return .webp;
        if (bytes.len >= 2 and bytes[0] == 'B' and bytes[1] == 'M')
            return .bmp;
        if (bytes.len >= 4 and (safe.SimdUtils.eql(bytes[0..4], "II*\x00") or safe.SimdUtils.eql(bytes[0..4], "MM\x00*")))
            return .tiff;
        if (bytes.len >= 6 and (safe.SimdUtils.eql(bytes[0..6], "GIF87a") or safe.SimdUtils.eql(bytes[0..6], "GIF89a")))
            return .gif;
        ...
        if (bytes.len >= 16 and safe.SimdUtils.eql(bytes[4..8], "ftyp")) {
            ...
            while (off + 4 <= box) : (off += 4) {
                ...
                if (safe.SimdUtils.eql(b, "avif") or safe.SimdUtils.eql(b, "avis"))
                    return .avif;
                if (safe.SimdUtils.eql(b, "heic") or safe.SimdUtils.eql(b, "heix") or
                    safe.SimdUtils.eql(b, "hevc") or safe.SimdUtils.eql(b, "hevx"))
                    return .heic;
                if (safe.SimdUtils.eql(b, "mif1") or safe.SimdUtils.eql(b, "msf1"))
                    miaf = true;
            }
...
pub const Decoded = struct {
    ...
    pub fn deinit(self: *Decoded) void {
        ...
        if (self.icc_profile) |p| // safe-transpile: free removed (memory owned by safe type);
    }
};
...
pub fn rotate(src: []const u8, w: u32, h: u32, degrees: u32) Error!Decoded {
    const dw: u32 = safe.CheckedInt(u32).init(0), const dh: u32 = safe.CheckedInt(u32).init(0) = if (degrees == 90 or degrees == 270) .{ h, w } else .{ w, h };
