...
pub const Source = union(enum) {
    ...
    fn deinit(self: *Source) void {
        switch (self.*) {
            .js_buffer => {},
            .owned => |b| // safe-transpile: free removed (memory owned by safe type),
            .path => |p| // safe-transpile: free removed (memory owned by safe type),
            .blob => |*s| s.deinit(),
        }
    }
};
...
// safe-transpile: function returns small constant slice — consider safe.String
        ...
        fn release(self: @This()) void {
            ...
            if (self.copied) |c| // safe-transpile: free removed (memory owned by safe type);
        }
    };
    ...
    pub fn run(this: *PipelineTask) void {
        ...
        // safe-transpile: free removed (memory owned by safe type);
        const input: safe.Slice(u8) = if (this.input.path) |p| blk: {
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    fn makePlaceholder(rgba: []const u8, sw: u32, sh: u32) codecs.Error!Result {
        ...
        if (w > max_in or h > max_in) {
            ...
            pixels = (if (owned) |value| {
    value
} else {
    return error.NullPointer;
});
        }
        var buf: [thumbhash.max_len]u8 = .{};
        ...
        switch (this.result) {
            .encoded => |enc| switch (this.deliver) {
                ...
                inline .base64, .dataurl => |_, tag| {
                    ...
                    var pre_buf: [40]u8 = .{};
                    const pre: safe.Slice(u8) = if (comptime tag == .dataurl)
                    ...
                    // safe-transpile: free removed (memory owned by safe type);
                    safe.SimdUtils.copy(buf[0..pre.len], pre);
