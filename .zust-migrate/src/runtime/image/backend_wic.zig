...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    if (f.vt.CreateDecoderFromStream(f, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(stream), null, 0, &dec) < 0 or dec == null)
        return error.DecodeFailed;
    defer release(dec);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    const istream: *IStream = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast((if (stream) |v| v else unreachable)));
    var pos: u64 = 0;
    ...
    @ptrCast(GlobalLock((if (hg) |v| v else unreachable)) orelse return error.EncodeFailed);
...
inline fn release(p: anytype) void {
    if (p) |obj| {
        const unk: *IUnknown = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
...
fn loadFactory() void {
    ...
    wicConvertBitmapSource = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    ...
    factory_ptr = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
...
fn dupGlobal(h: *anyopaque, comptime prefix: usize) error{OutOfMemory}!?[]u8 {
    ...
    const ptr: [*]const u8 = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(GlobalLock(h) orelse return null);
    defer _ = GlobalUnlock(h);
    const out = try bun.default_allocator.alloc(u8, prefix + size);
    safe.SimdUtils.copy(out[prefix..], ptr[0..size]);
    return out;
}
