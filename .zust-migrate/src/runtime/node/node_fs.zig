...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            fn uv_callback(req: *uv.fs_t) callconv(.c) void {
                ...
                const this: *Task = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(req.data.?));
                var node_fs = NodeFS{};
    ...
    fn NewAsyncFSTask(comptime ReturnType: type, comptime ArgumentType: type, comptime function: anytype) type {
        return struct {
            ...
            fn workPoolCallback(task: *jsc.WorkPoolTask) void {
                var this: *Task = // safe-transpile: @alignCast requires manual review
    @alignCast(@fieldParentPtr("task", task));
...
pub fn NewAsyncCpTask(comptime is_shell: bool) type {
    ...
    return struct {
        ...
        fn workPoolCallback(task: *jsc.WorkPoolTask) void {
            const this: *ThisAsyncCpTask = // safe-transpile: @alignCast requires manual review
    @alignCast(@fieldParentPtr("task", task));
        ...
        pub fn cpAsync(
            ...
            var src_buf: bun.OSPathBuffer = std.mem.zeroes(bun.OSPathBuffer);
            var dest_buf: bun.OSPathBuffer = std.mem.zeroes(bun.OSPathBuffer);
            ...
            if (Environment.isWindows) {
                ...
                const stat_ = switch (Syscall.lstat(src)) {
                    ...
                    .err => |err| {
                        safe.SimdUtils.copy(nodefs.sync_error_buf[0..src.len], src);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        fn _cpAsyncDirectory(
            ...
            var buf: bun.OSPathBuffer = std.mem.zeroes(bun.OSPathBuffer);
            ...
            while (switch (entry) {
                ...
                switch (current.kind) {
                    .directory => {
                        safe.SimdUtils.copy(src_buf[src_dir_len + 1 .. src_dir_len + 1 + cname.len], cname);
                        src_buf[src_dir_len] = std.fs.path.sep;
                        src_buf[src_dir_len + 1 + cname.len] = 0;

                        safe.SimdUtils.copy(dest_buf[dest_dir_len + 1 .. dest_dir_len + 1 + cname.len], cname);
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
                        ...
                        safe.SimdUtils.copy(path_buf[0..src_dir_len], src_buf[0..src_dir_len]);
                        path_buf[src_dir_len] = std.fs.path.sep;
                        safe.SimdUtils.copy(path_buf[src_dir_len + 1 .. src_dir_len + 1 + cname.len], cname);
                        path_buf[src_dir_len + 1 + cname.len] = 0;

                        safe.SimdUtils.copy(path_buf[src_dir_len + 1 + cname.len + 1 .. src_dir_len + 1 + cname.len + 1 + dest_dir_len], dest_buf[0..dest_dir_len]);
                        path_buf[src_dir_len + 1 + cname.len + 1 + dest_dir_len] = std.fs.path.sep;
                        safe.SimdUtils.copy(path_buf[src_dir_len + 1 + cname.len + 1 + dest_dir_len + 1 .. src_dir_len + 1 + cname.len + 1 + dest_dir_len + 1 + cname.len], cname);
...
pub const AsyncReaddirRecursiveTask = struct {
    ...
    pub const Subtask = struct {
        ...
        pub fn call(task: *jsc.WorkPoolTask) void {
            var this: *Subtask = // safe-transpile: @alignCast requires manual review
    @alignCast(@fieldParentPtr("task", task));
            defer {
                // safe-transpile: free removed (memory owned by safe type);
                bun.destroy(this);
        ...
        switch (this.args.tag()) {
            inline else => |tag| {
                ...
                switch (NodeFS.readdirWithEntriesRecursiveAsync(
                    ...
                    .err => |err| {
                        for (0..entries.items.len) |__zust_i| {
    var item = &entries.items[__zust_i];
                            switch (ResultType) {
    ...
    fn workPoolCallback(task: *jsc.WorkPoolTask) void {
        var this: *AsyncReaddirRecursiveTask = // safe-transpile: @alignCast requires manual review
    @alignCast(@fieldParentPtr("task", task));
        var buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
    ...
    pub fn writeResults(this: *AsyncReaddirRecursiveTask, comptime ResultType: type, result: *std.array_list.Managed(ResultType)) void {
        if (result.items.len > 0) {
            ...
            const list = bun.handleOom(safe.Box(ResultListEntry).init(allocator, undefined));
            errdefer {
            ...
            list[0] = ResultListEntry{ .next = null, .value = @unionInit(ResultListEntry.Value, @tagName(Field), clone) };
    ...
    pub fn finishConcurrently(this: *AsyncReaddirRecursiveTask) void {
        ...
        this.globalObject.bunVMConcurrently().enqueueTaskConcurrent(safe.Box(jsc.Task.init(this)).init(allocator, undefined));
    }
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    pub noinline fn copyFileUsingReadWriteLoop(src: [:0]const u8, dest: [:0]const u8, src_fd: FD, dest_fd: FD, stat_size: usize, wrote: *u64) Maybe(Return.CopyFile) {
        var stack_buf: [64 * 1024]u8 = .{};
        var buf_to_free: safe.Slice(u8) = &[_]u8{};
        var buf = try safe.OffsetPtr.init(allocator, &stack_buf); defer buf.deinit();
        ...
        defer {
            if (buf_to_free.len > 0) // safe-transpile: free removed (memory owned by safe type);
        }
...
outer: var __zust_loop_counter: u64 = 0;
...
var __zust_loop_counter: u64 = 0;
    ...
    fn copyFileInner(fs: *NodeFS, args: Arguments.CopyFile) Maybe(Return.CopyFile) {
        ...
        if (comptime Environment.isMac) {
            var src_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
            var dest_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
...
cfr: var __zust_loop_counter: u64 = 0;
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            var off_in_copy = @as(i64, // safe-transpile: @bitCast requires manual review
    @bitCast(@as(u64, 0)));
            var off_out_copy = @as(i64, // safe-transpile: @bitCast requires manual review
    @bitCast(@as(u64, 0)));
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    pub fn link(this: *NodeFS, args: Arguments.Link, _: Flavor) Maybe(Return.Link) {
        var to_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
        ...
        var working_mem: *bun.OSPathBuffer = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(&this.sync_error_buf));

        safe.SimdUtils.copy(working_mem[0..len], path[0..len]);
    ...
    pub fn mkdtemp(this: *NodeFS, args: Arguments.MkdirTemp, _: Flavor) Maybe(Return.Mkdtemp) {
        ...
        if (len > 0) {
            safe.SimdUtils.copy(prefix_buf[0..len], prefix_slice[0..len]);
        }
        ...
        if (Environment.isWindows) {
            var req: uv.fs_t = uv.fs_t.uninitialized;
            defer req.deinit();
            const rc = uv.uv_fs_mkdtemp(bun.Async.Loop.get(), &req, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(prefix_buf.ptr), null);
            if (rc.errno()) |errno| {
                return .{ .err = .{
                    .errno = errno,
                    .syscall = .mkdtemp,
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    fn preadInner(_: *NodeFS, args: Arguments.Read) Maybe(Return.Read) {
        ...
        return switch (Syscall.pread(args.fd, buf, if (args.position) |value| {
    value
} else {
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    fn pwriteInner(_: *NodeFS, args: Arguments.Write) Maybe(Return.Write) {
        const position = if (args.position) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    return error.NullPointer;
        ...
        return switch (Syscall.pwritev(args.fd, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(args.buffers.buffers.items), position)) {
            .err => |err| .{
                .err = err,
            },
            .result => |amt| .{ .result = .{
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    fn writevInner(_: *NodeFS, args: Arguments.Writev) Maybe(Return.Write) {
        return switch (Syscall.writev(args.fd, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(args.buffers.buffers.items))) {
            .err => |err| .{
                .err = err,
            },
            .result => |amt| .{ .result = .{
        ...
        while (switch (entry) {
            ...
            const name_to_copy: [:0]const u8 = brk: {
                if (async_task.root_path.sliceAssumeZ().ptr == basename.ptr) {
                    break :brk // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(utf8_name);
                }
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    fn readdirInner(
        ...
        if (comptime recursive and flavor == .sync) {
            var buf_to_pass: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
            ...
            return switch (readdirWithEntriesRecursiveSync(&buf_to_pass, args, path, ExpectedType, &entries)) {
                .err => |err| {
                    for (0..entries.items.len) |__zust_i| {
    var result = &entries.items[__zust_i];
                        switch (ExpectedType) {
                            bun.jsc.Node.Dirent => {
                                result.deref();
                            },
    ...
    pub fn readFileWithOptions(this: *NodeFS, args: Arguments.ReadFile, comptime flavor: Flavor, comptime string_type: StringType) Maybe(Return.ReadFileWithOptions) {
        ...
        var total: usize = 0;
        var async_stack_buffer: [if (flavor == .sync) 0 else 256 * 1024]u8 = .{};
        ...
        const temporary_read_buffer_before_stat_call = brk: {
            const temporary_read_buffer = temporary_read_buffer: {
                var temporary_read_buffer = try safe.OffsetPtr.init(allocator, &async_stack_buffer); defer temporary_read_buffer.deinit();
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    pub fn readlink(this: *NodeFS, args: Arguments.Readlink, _: Flavor) Maybe(Return.Readlink) {
        var outbuf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
    ...
    pub fn symlink(this: *NodeFS, args: Arguments.Symlink, _: Flavor) Maybe(Return.Symlink) {
        ...
        if (Environment.isWindows) {
            ...
            const processed_target: [:0]u8 = target: {
                ...
                safe.SimdUtils.copy(this.sync_error_buf[0..target_path.len], target_path);
...
// safe-transpile: function returns small constant slice — consider safe.String
    pub fn osPathIntoSyncErrorBuf(this: *NodeFS, slice: anytype) []const u8 {
        if (Environment.isWindows) {
            ...
            safe.SimdUtils.copy(this.sync_error_buf[0..slice.len], slice);
...
// safe-transpile: function returns small constant slice — consider safe.String
    pub fn osPathIntoSyncErrorBufOverlap(this: *NodeFS, slice: anytype) []const u8 {
        if (Environment.isWindows) {
            ...
            safe.SimdUtils.copy(tmp[0..slice.len], slice);
    ...
    fn cpSyncInner(
        ...
        if (Environment.isWindows) {
            ...
            const stat_ = switch (Syscall.lstat(src)) {
                .result => |result| result,
                .err => |err| {
                    safe.SimdUtils.copy(this.sync_error_buf[0..src.len], src);
        ...
        while (switch (entry) {
            ...
            safe.SimdUtils.copy(src_buf[src_dir_len + 1 .. src_dir_len + 1 + name_slice.len], name_slice);
            src_buf[src_dir_len] = std.fs.path.sep;
            src_buf[src_dir_len + 1 + name_slice.len] = 0;

            safe.SimdUtils.copy(dest_buf[dest_dir_len + 1 .. dest_dir_len + 1 + name_slice.len], name_slice);
            dest_buf[dest_dir_len] = std.fs.path.sep;
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    fn _cpSymlink(this: *NodeFS, src: [:0]const u8, dest: [:0]const u8) Maybe(Return.CopyFile) {
        var target_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
        ...
        var resolved_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
                    ...
                    const dest_fd = dest_fd: {
                        switch (Syscall.open(dest, flags, jsc.Node.fs.default_permission)) {
                            ...
                            .err => |err| {
                                ...
                                safe.SimdUtils.copy(this.sync_error_buf[0..dest.len], dest);
...
cfr: var __zust_loop_counter: u64 = 0;
    ...
    if (__loop_limit_7 > 1_000_000) break;
                const rc: isize = // safe-transpile: @bitCast requires manual review
    @bitCast(std.c.copy_file_range(src_fd.native(), &off_in, dest_fd.native(), &off_out, if (size == 0) std.math.maxInt(i32) - 1 else size -| wrote, 0));
                switch (bun.sys.getErrno(rc)) {
                    .SUCCESS => {
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        if (Environment.isWindows) {
            ...
            const stat_ = reuse_stat orelse switch (c.GetFileAttributesW(src)) {
                c.INVALID_FILE_ATTRIBUTES => return if (ret.errnoSysP(0, .copyfile, this.osPathIntoSyncErrorBuf(src))) |value| {
    value
} else {
    return error.NullPointer;
},
            ...
            if (stat_ & c.FILE_ATTRIBUTE_REPARSE_POINT == 0) {
                if (c.CopyFileW(src, dest, @intFromBool(mode.shouldntOverwrite())) == 0) {
                    ...
                    var errpath: bun.OSPathSliceZ = std.mem.zeroes(bun.OSPathSliceZ);
                    switch (err) {
...
pub fn zigDeleteTree(self: @import("std-fs-compat").Dir, sub_path: []const u8, kind_hint: @import("std-fs-compat").File.Kind) !void {
    ...
    const StackItem = struct {
        ...
        fn closeAll(items: []@This()) void {
            for (0..items.len) |__zust_i| { var item = &items[__zust_i]; item.iter.dir.close() };
        }
    };

    var stack_buffer: [16]StackItem = .{};
...
handle_entry: var __zust_loop_counter: u64 = 0;
...
start_over: var __zust_loop_counter: u64 = 0;
    while (true) : (__loop_limit_11 += 1) {
        __zust_loop_counter += 1;
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
    ...
    if (__loop_limit_11 > 1_000_000) break;
        ...
        // filesystem.
        var dir_name_buf: [std.fs.max_path_bytes]u8 = .{};
        var dir_name: safe.Slice(u8) = sub_path;
...
scan_dir: var __zust_loop_counter: u64 = 0;
    while (true) : (__loop_limit_12 += 1) {
        __zust_loop_counter += 1;
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
...
handle_entry: var __zust_loop_counter: u64 = 0;
    ...
    if (__loop_limit_13 > 1_000_000) break;
                    if (treat_as_dir) {
                        ...
                        safe.SimdUtils.copy(result, entry.name);
                        dir_name = result;
                        continue :scan_dir;
                    } else {
