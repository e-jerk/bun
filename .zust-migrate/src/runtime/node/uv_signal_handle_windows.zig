...
fn Bun__UVSignalHandle__init(
    ...
    @ptrCast(signal), &freeWithDefaultAllocator);
        return null;
    }

    libuv.uv_unref(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(signal));
...
fn freeWithDefaultAllocator(signal: *anyopaque) callconv(.c) void {
    bun.destroy(@as(*libuv.uv_signal_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(signal))));
}
