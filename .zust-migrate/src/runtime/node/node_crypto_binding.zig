...
fn ExternCryptoJob(comptime name: []const u8) type {
    return struct {
        ...
        pub fn runTask(task: *jsc.WorkPoolTask) void {
            ...
            var vm = job.vm;
            defer vm.enqueueTaskConcurrent(safe.Box(job.any_task.task()).init(allocator, undefined));
        ...
        pub fn runFromJS(this: *@This()) void {
            ...
            this.ctx.runFromJS(vm.global, callback) catch |err| {
                _ = vm.global.reportUncaughtException(if (vm.global.takeException(err).asException(vm.global.vm())) |value| {
    value
} else {
...
const random = struct {
    ...
    fn randomInt(global: *jsc.JSGlobalObject, callFrame: *jsc.CallFrame) bun.JSError!jsc.JSValue {
        ...
        const res = prng.random().intRangeLessThan(i64, min, max);
...
fn forEachHash(_: *const BoringSSL.EVP_MD, maybe_from: ?[*:0]const u8, _: ?[*:0]const u8, ctx: *anyopaque) callconv(.c) void {
    ...
    const hashes: *bun.CaseInsensitiveASCIIStringArrayHashMap(void) = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
...
fn getHashes(global: *JSGlobalObject, _: *jsc.CallFrame) JSError!JSValue {
    ...
    BoringSSL.EVP_MD_do_all_sorted(&forEachHash, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(&hashes)));
...
const Scrypt = struct {
    ...
    pub fn fromJS(global: *JSGlobalObject, callFrame: *jsc.CallFrame, comptime is_async: bool) JSError!if (is_async) struct { @This(), JSValue } else @This() {
        const password_value, const salt_value, const keylen_value, var maybe_options_value: ?JSValue = undefined, var callback =
            callFrame.argumentsAsArray(5);

        if (is_async) {
            if (callback.isUndefined()) {
                callback = (if (maybe_options_value) |value| {
    value
} else {
    return error.NullPointer;
});
    ...
    return error.NullPointer;
        ...
        if (maxmem == null or (if (maxmem) |value| {
    value
} else {
    return error.NullPointer;
}) == 0) {
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            .maxmem = @intCast((if (maxmem) |value| {
    value
} else {
    return error.NullPointer;
})),
