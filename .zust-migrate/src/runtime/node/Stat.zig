...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
            const tv_sec = if (Environment.isWindows) @as(u32, // safe-transpile: @bitCast requires manual review
    @bitCast(@as(i32, @truncate(ts.sec)))) else ts.sec;
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
            const tv_nsec = if (Environment.isWindows) @as(u32, // safe-transpile: @bitCast requires manual review
    @bitCast(@as(i32, @truncate(ts.nsec)))) else ts.nsec;
            if (big) {
