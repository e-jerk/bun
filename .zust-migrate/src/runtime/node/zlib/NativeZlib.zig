...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        switch (this.mode) {
            ...
            .UNZIP => {
                if (this.state.avail_in > 0) {
                    next_expected_header_byte = if (this.state.next_in) |value| {
    value
} else {
    return error.NullPointer;
                ...
                if (this.gzip_id_bytes_read == 0) {
                    ...
                    if ((if (next_expected_header_byte) |value| {
    value
} else {
    return error.NullPointer;
})[0] == GZIP_HEADER_ID1) {
                        this.gzip_id_bytes_read = 1;
                        (if (next_expected_header_byte) |value| {
    value
} else {
    return error.NullPointer;
}) += 1;
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    pub fn updateWriteResult(this: *Context, avail_in: *u32, avail_out: *u32) void {
        avail_in[0] = this.state.avail_in;
        avail_out[0] = this.state.avail_out;
    }
