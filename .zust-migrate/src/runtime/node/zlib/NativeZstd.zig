...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        if (err_.isError()) {
            this.stream.close();
            return globalThis.ERR(.ZLIB_INITIALIZATION_FAILED, "{s}", .{std.mem.sliceTo(if (err_.msg) |value| {
    value
} else {
...
const Context = struct {
    ...
    pub fn setParams(this: *Context, key: c_uint, value: u32) Error {
        switch (this.mode) {
            .ZSTD_COMPRESS => {
                const result = c.ZSTD_CCtx_setParameter(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.state), key, // safe-transpile: @bitCast requires manual review
    @bitCast(value));
                if (c.ZSTD_isError(result) > 0) return .init("Setting parameter failed", -1, "ERR_ZSTD_PARAM_SET_FAILED");
                return .ok;
            },
            .ZSTD_DECOMPRESS => {
                const result = c.ZSTD_DCtx_setParameter(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.state), key, // safe-transpile: @bitCast requires manual review
    @bitCast(value));
                if (c.ZSTD_isError(result) > 0) return .init("Setting parameter failed", -1, "ERR_ZSTD_PARAM_SET_FAILED");
                return .ok;
    ...
    fn deinitState(this: *Context) void {
        _ = switch (this.mode) {
            .ZSTD_COMPRESS => c.ZSTD_freeCCtx(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.state)),
            .ZSTD_DECOMPRESS => c.ZSTD_freeDCtx(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.state)),
            else => unreachable,
        };
        this.state = null;
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            .ZSTD_COMPRESS => c.ZSTD_compressStream2(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.state), &this.output, &this.input, @intCast(this.flush)),
            .ZSTD_DECOMPRESS => c.ZSTD_decompressStream(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.state), &this.output, &this.input),
            else => @panic("unreachable"),
        };
    }
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        avail_in[0] = @intCast(this.input.size - this.input.pos);
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        avail_out[0] = @intCast(this.output.size - this.output.pos);
    }
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    pub fn close(this: *Context) void {
        _ = switch (this.mode) {
            .ZSTD_COMPRESS => c.ZSTD_CCtx_reset(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.state), c.ZSTD_reset_session_and_parameters),
            .ZSTD_DECOMPRESS => c.ZSTD_DCtx_reset(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.state), c.ZSTD_reset_session_and_parameters),
            else => unreachable,
        };
        this.deinitState();
