...
const Context = struct {
    ...
    last_result: extern union { e: c_int, d: c.BrotliDecoderResult } = // safe-transpile: @bitCast requires manual review
    @bitCast(@as(u32, 0)),
    ...
    pub fn init(this: *Context) Error {
        switch (this.mode) {
            .BROTLI_ENCODE => {
                ...
                this.state = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast((state.?));
                return Error.ok;
            },
            .BROTLI_DECODE => {
    ...
    pub fn setParams(this: *Context, key: c_uint, value: u32) Error {
        switch (this.mode) {
            .BROTLI_ENCODE => {
                if (c.BrotliEncoderSetParameter(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.state), key, value) == 0) {
                    return Error.init("Setting parameter failed", -1, "ERR_BROTLI_PARAM_SET_FAILED");
                }
                return Error.ok;
            },
            .BROTLI_DECODE => {
                if (c.BrotliDecoderSetParameter(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.state), key, value) == 0) {
                    return Error.init("Setting parameter failed", -1, "ERR_BROTLI_PARAM_SET_FAILED");
                }
                return Error.ok;
    ...
    fn deinitState(this: *Context) void {
        switch (this.mode) {
            .BROTLI_ENCODE => c.BrotliEncoderDestroyInstance(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(this.state))),
            .BROTLI_DECODE => c.BrotliDecoderDestroyInstance(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(this.state))),
            else => unreachable,
        }
        this.state = null;
    ...
    pub fn doWork(this: *Context) void {
        switch (this.mode) {
            .BROTLI_ENCODE => {
                ...
                this.last_result.e = c.BrotliEncoderCompressStream(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.state), this.flush, &this.avail_in, &next_in, &this.avail_out, &this.next_out, null);
                if (this.next_in) |value| {
    value
} else {
    return error.NullPointer;
} += @intFromPtr((if (next_in) |value| {
    value
} else {
    return error.NullPointer;
})) - @intFromPtr(if (this.next_in) |value| {
    value
} else {
    return error.NullPointer;
});
            ...
            .BROTLI_DECODE => {
                var next_in = this.next_in;
                this.last_result.d = c.BrotliDecoderDecompressStream(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.state), &this.avail_in, &next_in, &this.avail_out, &this.next_out, null);
                if (this.next_in) |value| {
    value
} else {
    return error.NullPointer;
    ...
    return error.NullPointer;
                ...
                if (this.last_result.d == .err) {
                    this.error_ = c.BrotliDecoderGetErrorCode(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.state));
                }
            },
            else => unreachable,
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        avail_in[0] = @intCast(this.avail_in);
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        avail_out[0] = @intCast(this.avail_out);
    }
