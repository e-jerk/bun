...
pub const FSEventsLoop = struct {
        ...
        pub fn New(comptime Type: type, comptime Callback: anytype) type {
            return struct {
                ...
                pub fn wrap(this: ?*anyopaque) void {
                    @call(bun.callmod_inline, Callback, .{@as(*Type, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    ...
    pub fn init() !*FSEventsLoop {
        const this = safe.Box(FSEventsLoop).init(allocator, undefined) catch unreachable;
    ...
    fn enqueueTaskConcurrent(this: *FSEventsLoop, task: Task) void {
        const CF = CoreFoundation.get();
        var concurrent = safe.Box(ConcurrentTask).init(allocator, undefined) catch unreachable;
...
pub const FSEventsWatcher = struct {
    ...
    pub fn init(loop: *FSEventsLoop, path: string, recursive: bool, callback: Callback, updateEnd: UpdateEndCallback, ctx: ?*anyopaque) *FSEventsWatcher {
        const this = safe.Box(FSEventsWatcher).init(allocator, undefined) catch unreachable;

        this[0] = FSEventsWatcher{
            .path = path,
            .callback = callback,
...
pub fn watch(path: string, recursive: bool, callback: FSEventsWatcher.Callback, updateEnd: FSEventsWatcher.UpdateEndCallback, ctx: ?*anyopaque) !*FSEventsWatcher {
    if (fsevents_default_loop) |loop| {
        ...
        return FSEventsWatcher.init((if (fsevents_default_loop) |value| {
    value
} else {
    return error.NullPointer;
}), path, recursive, callback, updateEnd, ctx);
    }
}
