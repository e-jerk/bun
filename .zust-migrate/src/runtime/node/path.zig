...
pub fn getCwdWindowsU16(buf: []u16) MaybeBuf(u16) {
    ...
    if (len == 0) {
        ...
        return if (MaybeBuf(u16).errnoSys(0, Syscall.Tag.getcwd)) |value| {
    value
} else {
...
pub fn getCwdU8(buf: []u8) MaybeBuf(u8) {
    ...
    safe.SimdUtils.copy(buf[0..cached_cwd.len], cached_cwd);
...
pub fn basenamePosixT(comptime T: type, path: []const T, suffix: ?[]const T) []const T {
    ...
    if (suffix != null and _suffixLen > 0 and _suffixLen <= len) {
        if (safe.SimdUtils.eql(_suffix, path)) {
            return &.{};
        }
        // We use an optional value instead of -1, as in Node code, for easier number type use.
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        if (end) |_end| {
            if (start == _end) {
                return path[start..if (firstNonSlashEnd) |value| {
    value
} else {
    return error.NullPointer;
}];
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        if (byte == CHAR_DOT) {
            ...
            if (startDot == null) {
                startDot = i;
            } else if (preDotState != null and (if (preDotState) |value| {
    value
} else {
    return error.NullPointer;
}) != 1) {
...
fn _formatT(comptime T: type, pathObject: PathParsed(T), sep: T, buf: []T) []const T {
    ...
    const dirIsRoot = dir.len == 0 or safe.SimdUtils.eql(dir, root);
...
pub fn format(globalObject: *jsc.JSGlobalObject, isWindows: bool, args_ptr: [*]jsc.JSValue, args_len: u16) bun.JSError!jsc.JSValue {
    ...
    if (try pathObject_ptr.getTruthy(globalObject, "root")) |jsValue| {
        ...
        root = (if (root_slice) |value| {
    value
} else {
    return error.NullPointer;
}).slice();
    }
    var dir: safe.Slice(u8) = "";
    ...
    if (try pathObject_ptr.getTruthy(globalObject, "dir")) |jsValue| {
        ...
        dir = (if (dir_slice) |value| {
    value
} else {
    return error.NullPointer;
}).slice();
    }
    var base: safe.Slice(u8) = "";
    ...
    if (try pathObject_ptr.getTruthy(globalObject, "base")) |jsValue| {
        ...
        base = (if (base_slice) |value| {
    value
} else {
    return error.NullPointer;
}).slice();
    }
    var _name: safe.Slice(u8) = "";
    ...
    if (try pathObject_ptr.getTruthy(globalObject, "name")) |jsValue| {
        ...
        _name = (if (_name_slice) |value| {
    value
} else {
    return error.NullPointer;
}).slice();
    }
    var ext: safe.Slice(u8) = "";
...
pub fn isAbsoluteWindowsZigString(pathZStr: jsc.ZigString) bool {
    return if (pathZStr.len > 0 and pathZStr.is16Bit())
        isAbsoluteWindowsT(u16, // safe-transpile: @alignCast requires manual review
    @alignCast(pathZStr.utf16Slice()))
    else
    ...
    while (i <= len) : (i += 1) {
        ...
        if (isSepT(T, byte)) {
            ...
            if ((lastSlash == null and i == 0) or
                (lastSlash != null and i > 0 and (if (lastSlash) |value| {
    value
} else {
    return error.NullPointer;
}) == i - 1) or
                (dots != null and (if (dots) |value| {
    value
} else {
    return error.NullPointer;
}) == 1))
            {
                // NOOP
            } else if (dots != null and (if (dots) |value| {
    value
} else {
    return error.NullPointer;
}) == 2) {
                    ...
                    buf[bufSize - 2] != CHAR_DOT)
                {
                    if (bufSize > 2) {
                        ...
                        if (lastSlashIndex == null) {
                            ...
                            bufSize = (if (lastSlashIndex) |value| {
    value
} else {
    return error.NullPointer;
});
                            ...
                            if (lastIndexOfSep == null) {
                                ...
                                lastSegmentLength = bufSize - 1 - (if (lastIndexOfSep) |value| {
    value
} else {
    return error.NullPointer;
});
                ...
                const sliceStart = if (lastSlash != null) (if (lastSlash) |value| {
    value
} else {
    return error.NullPointer;
}) + 1 else 0;
                ...
                const subtract = if (lastSlash != null) (if (lastSlash) |value| {
    value
} else {
    return error.NullPointer;
}) + 1 else 2;
            ...
            dots = if (dots != null) (if (dots) |value| {
    value
} else {
    return error.NullPointer;
}) + 1 else 0;
...
pub fn normalizeWindowsT(comptime T: type, path: []const T, buf: []T) []const T {
    ...
    if (isSepT(T, byte0)) {
        ...
        if (isSepT(T, path[1])) {
            ...
            if (j < len and j != last) {
                const firstPart: safe.Slice(u8) = path[last..j];
                // Matched!
                last = j;
                // Match 1 or more path separators
                while (j < len and
...
pub fn relativePosixT(comptime T: type, from: []const T, to: []const T, buf: []T, buf2: []T, buf3: []T) MaybeSlice(T) {
    ...
    if (safe.SimdUtils.eql(fromOrig, toOrig)) {
        return MaybeSlice(T){ .result = &.{} };
    }
    ...
    // Add a block to isolate `i`.
    {
        ...
        var i: usize = fromStart + (if (lastCommonSep != null) (if (lastCommonSep) |value| {
    value
} else {
    return error.NullPointer;
}) + 1 else 0);
    ...
    toStart = if (lastCommonSep != null) toStart + (if (lastCommonSep) |value| {
    value
} else {
    return error.NullPointer;
}) else 0;
...
pub fn relativeWindowsT(comptime T: type, from: []const T, to: []const T, buf: []T, buf2: []T, buf3: []T) MaybeSlice(T) {
    ...
    if (safe.SimdUtils.eql(fromOrig, toOrig) or
    ...
    if (lastCommonSep == null) {
        ...
        toStart += (if (lastCommonSep) |value| {
    value
} else {
    return error.NullPointer;
});
...
pub fn resolveWindowsT(comptime T: type, paths: []const []const T, buf: []T, buf2: []T) MaybeSlice(T) {
    ...
    const isSepT = isSepWindowsT;
    var tmpBuf: [MAX_PATH_SIZE(T):0]T = .{};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            if (comptime Environment.isWindows) {
                var u16Buf: bun.WPathBuffer = std.mem.zeroes(bun.WPathBuffer);
