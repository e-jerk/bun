...
pub fn check(this: *@This(), globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!jsc.JSValue {
    ...
    this.mutex.lock();
    defer this.mutex.unlock();
    for (0..this.da_rules.items.len) |__zust_i| {
    var item = &this.da_rules.items[__zust_i];
        switch (item.*) {
            .addr => |*a| {
...
pub fn rules(this: *@This(), globalThis: *jsc.JSGlobalObject) bun.JSError!jsc.JSValue {
    ...
    defer this.mutex.unlock();
    for (0..this.da_rules.items.len) |__zust_i| {
    var rule = &this.da_rules.items[__zust_i];
        switch (rule.*) {
            .addr => |*a| {
...
fn _compare_ipv6(l: *const sockaddr.in6, r: *const sockaddr.in6) std.math.Order {
    return std.math.order(@byteSwap((@as(u128, // safe-transpile: @bitCast requires manual review
    @bitCast(l.addr)))), @byteSwap((@as(u128, // safe-transpile: @bitCast requires manual review
    @bitCast(r.addr)))));
}
