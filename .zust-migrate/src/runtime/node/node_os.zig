...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        while (line_iter.next()) |line| {
            ...
            if (cpu_name == null or !std.mem.startsWith(u8, (if (cpu_name) |value| {
    value
} else {
    return error.NullPointer;
}), "cpu")) break; // done with CPUs
...
fn cpusImplDarwin(globalThis: *jsc.JSGlobalObject) !jsc.JSValue {
    ...
    var info: [*]bun.c.processor_cpu_load_info = .{};
    ...
    if (bun.c.host_processor_info(
        ...
        &num_cpus,
        @as(*bun.c.processor_info_array_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&info)),
        &info_size,
    ) != 0) {
...
pub fn cpusImplWindows(globalThis: *jsc.JSGlobalObject) !jsc.JSValue {
    var cpu_infos: [*]libuv.uv_cpu_info_t = .{};
...
pub fn homedir(global: *jsc.JSGlobalObject) !bun.String {
    ...
    if (Environment.isWindows) {
        var out: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
        ...
        var stack_string_bytes: [4096]u8 = .{};
        var string_bytes = try safe.OffsetPtr.init(allocator, &stack_string_bytes); defer string_bytes.deinit();
        // safe-transpile: free removed (memory owned by safe type) if (string_bytes.ptr != &stack_string_bytes)
            // safe-transpile: free removed (memory owned by safe type);

        var pw: bun.c.passwd = std.mem.zeroes(bun.c.passwd);
        var result: ?*bun.c.passwd = null;

        const ret = var __zust_loop_counter: u64 = 0;
    while (true) {
...
pub fn hostname(global: *jsc.JSGlobalObject) bun.JSError!jsc.JSValue {
    if (Environment.isWindows) {
        ...
        var result: std.os.windows.ws2_32.WSADATA = std.mem.zeroes(std.os.windows.ws2_32.WSADATA);
        ...
        var name_buffer: [bun.HOST_NAME_MAX]u8 = .{};
...
pub fn loadavg(global: *jsc.JSGlobalObject) bun.JSError!jsc.JSValue {
    const result = switch (bun.Environment.os) {
        .mac => loadavg: {
            var avg: c.struct_loadavg = std.mem.zeroes(c.struct_loadavg);
        ...
        .linux => loadavg: {
            var info: c.struct_sysinfo = std.mem.zeroes(c.struct_sysinfo);
...
// safe-transpile: optional unwrap requires manual review
    ...
    while (it) |iface| : (it = iface.ifa_next) {
        ...
        const addr = @import("std-net-shim").Address.initPosix(// safe-transpile: @alignCast requires manual review
    @alignCast(@as(*std.posix.sockaddr, @ptrCast(iface.ifa_addr))));
        const netmask = @import("std-net-shim").Address.initPosix(// safe-transpile: @alignCast requires manual review
    @alignCast(@as(*std.posix.sockaddr, @ptrCast(iface.ifa_netmask))));
        ...
        // cidr <string> The assigned IPv4 or IPv6 address with the routing prefix in CIDR notation. If the netmask is invalid, this property is set to null.
        {
            ...
            const maybe_suffix: ?u8 = switch (addr.any.family) {
                ...
                std.posix.AF.INET6 => netmaskToCIDRSuffix(@as(u128, // safe-transpile: @bitCast requires manual review
    @bitCast(netmask.in6.addr))),
                else => null,
            };
        ...
        // mac <string> The MAC address of the network interface
        {
            ...
            const maybe_ll_addr = while (ll_it) |ll_iface| : (ll_it = ll_iface.ifa_next) {
                ...
                if (comptime Environment.isLinux) {
                    break @as(?*std.posix.sockaddr.ll, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(ll_iface.ifa_addr)));
                } else if (comptime Environment.isMac or Environment.isFreeBSD) {
                    break @as(?*c.sockaddr_dl, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(ll_iface.ifa_addr)));
                } else {
                    @compileError("unreachable");
                }
...
fn networkInterfacesWindows(globalThis: *jsc.JSGlobalObject) bun.JSError!jsc.JSValue {
    var ifaces: [*]libuv.uv_interface_address_t = .{};
    var count: c_int = undefined;
    ...
    for (ifaces[0..@intCast(count)]) |iface| {
        ...
        var cidr = jsc.JSValue.null;
        {
            ...
            const maybe_suffix: ?u8 = switch (iface.address.address4.family) {
                ...
                std.posix.AF.INET6 => netmaskToCIDRSuffix(@as(u128, // safe-transpile: @bitCast requires manual review
    @bitCast(iface.netmask.netmask6.addr))),
                else => null,
            };
            ...
            const addr_str = bun.fmt.formatIp(
                ...
                @import("std-net-shim").Address.initPosix(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&iface.address.address4)),
                &ip_buf,
            ) catch unreachable;
            if (maybe_suffix) |suffix| {
                //NOTE addr_str might not start at buf[0] due to slicing in formatIp
        ...
        // netmask
        {
            const str = bun.fmt.formatIp(
                // @import("std-net-shim").Address will do ptrCast depending on the family so this is ok
                @import("std-net-shim").Address.initPosix(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&iface.netmask.netmask4)),
                &ip_buf,
            ) catch unreachable;
            interface.put(globalThis, jsc.ZigString.static("netmask"), jsc.ZigString.init(str).withEncoding().toJS(globalThis));
        }
...
pub fn release() bun.String {
    ...
    const value = switch (Environment.os) {
        ...
        .windows => slice: {
            var info: bun.windows.libuv.uv_utsname_s = std.mem.zeroes(bun.windows.libuv.uv_utsname_s);
            const err = bun.windows.libuv.uv_os_uname(&info);
            if (err != 0) {
                break :slice "unknown";
            }
            const value = bun.sliceTo(&info.release, 0);
            safe.SimdUtils.copy(name_buffer[0..value.len], value);
...
pub fn totalmem() u64 {
    switch (bun.Environment.os) {
        ...
        .linux => {
            var info: c.struct_sysinfo = std.mem.zeroes(c.struct_sysinfo);
            if (c.sysinfo(&info) == @as(c_int, 0)) return @as(u64, // safe-transpile: @bitCast requires manual review
    @bitCast(info.totalram)) *% @as(c_ulong, // safe-transpile: @bitCast requires manual review
    @bitCast(@as(c_ulong, info.mem_unit)));
            return 0;
        },
        .freebsd => {
            var physmem: u64 = 0;
...
pub fn uptime(global: *jsc.JSGlobalObject) bun.JSError!f64 {
    switch (Environment.os) {
        ...
        .mac, .freebsd => {
            var boot_time: std.posix.timeval = std.mem.zeroes(std.posix.timeval);
...
pub fn version() bun.JSError!bun.String {
    ...
    const slice: safe.Slice(u8) = switch (Environment.os) {
        .mac, .freebsd => slice: {
            @memset(&name_buffer, 0);
        ...
        .windows => slice: {
            ...
            safe.SimdUtils.copy(name_buffer[0..slice.len], slice);
