...
pub const PathWatcherManager = struct {
    ...
    pub fn get() bun.sys.Maybe(*PathWatcherManager) {
        ...
        const m = bun.handleOom(safe.Box(PathWatcherManager).init(allocator, undefined));
        m[0] = .{};
...
// safe-transpile: function returns small constant slice — consider safe.String
    fn makeKey(buf: []u8, resolved_path: []const u8, recursive: bool) []const u8 {
        safe.SimdUtils.copy(buf[0..resolved_path.len], resolved_path);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    while (switch (it.next()) {
        ...
        const child_rel: safe.Slice(u8) = if (rel_dir.len == 0)
            name
        else
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        for (0..gop.value_ptr.items.len) |__zust_i| {
    var o = &gop.value_ptr.items[__zust_i];
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    fn threadMain(manager: *PathWatcherManager) void {
        ...
        var buf: [64 * 1024]u8 align(@alignOf(InotifyEvent)) = .{};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            var i: usize = 0;
            while (i < n) {
                const ev: *align(1) const InotifyEvent = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(buf[i..].ptr);
                ...
                const name: safe.Slice(u8) = if (ev.name_len > 0) blk: {
                    const name_ptr: [*:0]const u8 = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(buf[i - ev.name_len ..].ptr);
    ...
    while (true) : (oi += 1) {
        ...
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
                    ...
                    const rel: safe.Slice(u8) = if (watcher.is_file)
...
const Darwin = struct {
    ...
    fn onFSEvent(ctx: ?*anyopaque, event: Event, is_file: bool) void {
        const watcher: *PathWatcher = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                ...
                const rel: safe.Slice(u8) = if (entry.is_file and entry.subpath.len == 0)
                    bun.path.basename(watcher.path)
                else
                    entry.subpath;
