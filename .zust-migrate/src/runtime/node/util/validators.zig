...
pub fn validateNumber(globalThis: *JSGlobalObject, value: JSValue, name: string, maybe_min: ?f64, maybe_max: ?f64) bun.JSError!f64 {
    ...
    if (!valid) {
        if (maybe_min != null and maybe_max != null) {
            return throwRangeError(globalThis, "The value of \"{s}\" is out of range. It must be >= {d} && <= {d}. Received {d}", .{ name, (if (maybe_min) |value| {
    value
} else {
    ...
    return error.NullPointer;
            ...
            return throwRangeError(globalThis, "The value of \"{s}\" is out of range. It must be >= {d}. Received {d}", .{ name, (if (maybe_min) |value| {
    value
} else {
    return error.NullPointer;
}), num });
        } else if (maybe_max != null) {
            return throwRangeError(globalThis, "The value of \"{s}\" is out of range. It must be <= {d}. Received {d}", .{ name, (if (maybe_max) |value| {
    value
} else {
    return error.NullPointer;
}), num });
