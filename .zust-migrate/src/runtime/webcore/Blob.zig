...
pub fn doReadFile(this: *Blob, comptime Function: anytype, global: *JSGlobalObject) JSValue {
    ...
    if (Environment.isWindows) {
        var promise = safe.Box(global).init(allocator, undefined);
        const promise_value = promise.toJS();
        promise_value.ensureStillAlive();
        handler.promise.strong.set(global, promise_value);

        read_file.ReadFileUV.start(handler.globalThis.bunVM().eventLoop(), if (this.store) |value| {
    value
} else {
    return error.NullPointer;
}, this.offset, this.size, Handler, handler);
    ...
    if (this.isS3()) {
        const Task = struct {
            ...
            fn cb(result: S3.S3DownloadResult, opaque_self: *anyopaque) bun.JSTerminated!void {
                const t: *@This() = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(opaque_self));
                switch (result) {
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            try S3.downloadSlice(cred, path, @intCast(this.offset), len, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&Task.cb), t, proxy, payer);
        } else {
            try S3.download(cred, path, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&Task.cb), t, proxy, payer);
        }
        return;
    }
...
pub fn doReadFileInternal(this: *Blob, comptime Handler: type, ctx: Handler, comptime Function: anytype, global: *JSGlobalObject) void {
    if (Environment.isWindows) {
        ...
        return read_file.ReadFileUV.start(global.bunVM().eventLoop(), if (this.store) |value| {
    value
} else {
    return error.NullPointer;
}, this.offset, this.size, ReadFileHandler, ctx);
    }
...
fn writeFloat(
    ...
    const bytes: [@sizeOf(FloatType)]u8 = // safe-transpile: @bitCast requires manual review
    @bitCast(value);
...
fn readFloat(
    ...
    var bytes_buf: [@sizeOf(FloatType)]u8 = .{};
...
fn _onStructuredCloneDeserialize(
    ...
    var content_type: safe.Slice(u8) = try readSlice(reader, content_type_len, allocator);
    ...
    const blob: *Blob = switch (store_tag) {
        .bytes => bytes: {
            ...
            versions: {
                ...
                if (!name_consumed) // safe-transpile: free removed (memory owned by safe type);
...
pub fn writeFormat(this: *Blob, comptime Formatter: type, formatter: *Formatter, writer: anytype, comptime enable_ansi_colors: bool) !void {
    ...
    if (this.isDetached()) {
        ...
        const store = if (this.store) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn writeFileWithEmptySourceToDestination(ctx: *jsc.JSGlobalObject, destination_blob: *Blob, options: WriteFileOptions) bun.JSError!jsc.JSValue {
    ...
    const destination_store = if (destination_blob.store) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                    ...
                    .NOENT => {
                        ...
                        // permissions issue
                        const dirpath: safe.Slice(u8) = switch (file.pathlike) {
...
pub fn writeFileWithSourceDestination(ctx: *jsc.JSGlobalObject, source_blob: *Blob, destination_blob: *Blob, options: WriteFileOptions) bun.JSError!jsc.JSValue {
    ...
    if (destination_type == .file and source_type == .bytes) {
        ...
        if (comptime Environment.isWindows) {
            var promise = safe.Box(ctx).init(allocator, undefined);
...
fn writeStringToFileFast(
    ...
    const fd: bun.FD = if (comptime !needs_open) pathlike.fd else brk: {
        var file_path: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
        switch (bun.sys.open(
    ...
    if (check_s3) {
        if (path_or_fd.* == .path) {
            if (strings.startsWith(path_or_fd.path.slice(), "s3://")) {
                ...
                path_or_fd[0] = .{ .path = .{ .string = bun.PathString.empty } };
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            try S3.downloadSlice(credentials, path, offset, len, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&S3BlobDownloadTask.onS3DownloadResolved), this, if (env.getHttpProxy(true, null, null)) |proxy| proxy.href else null, s3_store.request_payer);
        } else if (blob.size == Blob.max_size) {
            try S3.download(credentials, path, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&S3BlobDownloadTask.onS3DownloadResolved), this, if (env.getHttpProxy(true, null, null)) |proxy| proxy.href else null, s3_store.request_payer);
        } else {
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
...
// safe-transpile: optional unwrap requires manual review
    ...
    signal[0] = jsc.WebCore.FileSink.JSSink.SinkSignal.init(.zero);
    ...
    const assignment_result: jsc.JSValue = jsc.WebCore.FileSink.JSSink.assignToStream(
        ...
        file_sink,
        @as(**anyopaque, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&signal.ptr)),
    );
...
export fn Blob__fromBytes(globalThis: *jsc.JSGlobalObject, ptr: ?[*]const u8, len: usize) callconv(.c) *Blob {
    ...
    const bytes = bun.handleOom(bun.default_allocator.dupe(u8, (if (ptr) |value| {
    value
} else {
    return error.NullPointer;
})[0..len]));
...
pub fn resolveSize(this: *Blob) void {
    if (this.store) |store| {
        if (store.data == .bytes) {
            ...
            if (store.data.file.seekable != null and if (store.data.file.seekable) |value| {
    value
} else {
    return error.NullPointer;
} == false) {
...
fn resolveFileStat(store: *Store) void {
    if (store.data.file.pathlike == .path) {
        var buffer: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
...
pub fn constructor(globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!*Blob {
    ...
    switch (args.len) {
        0 => {
            const empty: safe.Slice(u8) = &[_]u8{};
...
pub fn tryCreate(
    ...
    if (comptime Environment.isLinux) {
        if (bun.linux.MemFdAllocator.shouldUse(bytes_)) {
            switch (safe.Box(bytes_).init(allocator, undefined)) {
                .err => {},
                .result => |result| {
...
pub fn toStringWithBytes(this: *Blob, global: *JSGlobalObject, raw_bytes: []const u8, comptime lifetime: Lifetime) bun.JSError!JSValue {
    ...
    if (buf.len == 0) {
        ...
        if (lifetime == .temporary) // safe-transpile: free removed (memory owned by safe type);
...
// safe-transpile: optional unwrap requires manual review
    ...
    if (could_be_all_ascii == null or !(if (could_be_all_ascii) |value| {
    value
} else {
    return error.NullPointer;
})) {
...
// safe-transpile: optional unwrap requires manual review
            ...
            return ZigString.init(buf).external(global, if (this.store) |value| {
    value
} else {
    return error.NullPointer;
}, Store.external);
...
pub fn toFormData(this: *Blob, global: *JSGlobalObject, comptime lifetime: Lifetime) bun.JSTerminated!JSValue {
    ...
    if (view_.len == 0)
        return safe.Box(global).init(allocator, undefined);
    ...
    switch (current.jsTypeLoose()) {
        .Array, .DerivedArray => {
            ...
            if (might_only_be_one_thing) {
                top_value = if ((try top_iter.next())) |value| {
    value
} else {
    return error.NullPointer;
};
                    ...
                    if (!any_arrays) {
                        switch (item.jsTypeLoose()) {
                            ...
                            => {
                                ...
                                var buf = if (item.asArrayBuffer(global)) |value| {
    value
} else {
    return error.NullPointer;
};
            ...
            .DataView,
            => {
                var buf = if (current.asArrayBuffer(global)) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        if (this.* == .Blob) {
            if (this.Blob.store) |s| {
                if (s.data == .bytes and s.hasOneRef()) {
                    this[0] = .{ .InternalBlob = s.data.bytes.toInternalBlob() };
                    s.deref();
                    return;
                }
            }
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn concat(first: []const u8, second: []const u8) Inline {
        ...
        if (first.len > 0)
            safe.SimdUtils.copy(bytes_slice[0..first.len], first);

        if (second.len > 0)
            safe.SimdUtils.copy(bytes_slice[first.len..][0..second.len], second);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        if (data.len > 0)
            safe.SimdUtils.copy(blob.bytes[0..data.len], data);
        return blob;
    }
            ...
            if (Environment.isWindows) {
                const WrappedCallback = struct {
                    pub fn callback(req: *libuv.fs_t) callconv(.c) void {
                        var self: *This = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
...
// safe-transpile: optional unwrap requires manual review
                ...
                this.req.data = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this);
                return;
            }
...
pub fn FileCloser(comptime This: type) type {
    return struct {
        fn scheduleClose(request: *io.Request) io.Action {
            var this: *This = // safe-transpile: @alignCast requires manual review
    @alignCast(@fieldParentPtr("io_request", request));
            return io.Action{
                .close = .{
                    .ctx = this,
                    .fd = this.opened_fd,
                    .onDone = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&onIORequestClosed),
                    .poll = &this.io_poll,
                    .tag = This.io_tag,
                },
            };
