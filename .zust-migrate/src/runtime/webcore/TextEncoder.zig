...
pub export fn TextEncoder__encode8(
    ...
    if (slice.len <= buf.len / 2) {
        ...
        safe.SimdUtils.copy(array_buffer.byteSlice()[0..result.written], buf[0..result.written]);
        return uint8array;
    } else {
...
pub export fn TextEncoder__encode16(
    ...
    if (slice.len <= buf.len / 4) {
        ...
        if (result.read == 0 or result.written == 0) {
            ...
            const array_buffer = if (uint8array.asArrayBuffer(globalThis)) |value| {
    value
} else {
    return error.NullPointer;
};
            const replacement_char = [_]u8{ 239, 191, 189 };
            safe.SimdUtils.copy(array_buffer.slice()[0..replacement_char.len], &replacement_char);
            return uint8array;
        }
        const uint8array = jsc.JSValue.createUninitializedUint8Array(globalThis, result.written) catch return .zero;
        bun.assert(result.written <= buf.len);
    ...
    return error.NullPointer;
        ...
        bun.assert(result.written == array_buffer.len);
        safe.SimdUtils.copy(array_buffer.slice()[0..result.written], buf[0..result.written]);
        return uint8array;
    } else {
...
const RopeStringEncoder = struct {
    ...
    pub fn append8(it: *jsc.JSString.Iterator, ptr: [*]const u8, len: u32) callconv(.c) void {
        var this = bun.cast(*RopeStringEncoder, if (it.data) |value| {
    value
} else {
    return error.NullPointer;
});
...
pub export fn TextEncoder__encodeRopeString(
    ...
    var stack_buf: [2048]u8 = .{};
    var buf_to_use = try safe.OffsetPtr.init(allocator, &stack_buf); defer buf_to_use.deinit();
...
// safe-transpile: optional unwrap requires manual review
    ...
    if (array == .zero) {
        ...
        array.ensureStillAlive();
// safe-transpile: optional unwrap requires manual review
        safe.SimdUtils.copy(array.asArrayBuffer(globalThis).?.byteSlice(), buf_to_use[0..length]);
    }
...
pub export fn TextEncoder__encodeInto16(
    ...
    if (output.len >= 3 and (result.read == 0 or result.written == 0)) {
        const replacement_char = [_]u8{ 239, 191, 189 };
        safe.SimdUtils.copy(buf_ptr[0..replacement_char.len], &replacement_char);
        result.read = 1;
        result.written = 3;
    }
