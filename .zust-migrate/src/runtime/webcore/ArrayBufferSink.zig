...
pub fn writeUTF16(this: *@This(), data: streams.Result) streams.Result.Writable {
    ...
    const len = this.bytes.writeUTF16(this.allocator, @as([*]const u16, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(data.slice().ptr)))[0..std.mem.bytesAsSlice(u16, data.slice()).len]) catch {
        return .{ .err = Syscall.Error.oom };
    };
    this.signal.ready(null, null);
    return .{ .owned = len };
