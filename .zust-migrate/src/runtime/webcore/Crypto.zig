...
fn throwInvalidParams(globalThis: *jsc.JSGlobalObject, comptime error_type: anytype, comptime message: [:0]const u8, fmt: anytype) bun.JSError {
...
pub fn getRandomValuesWithoutTypeChecks(
    ...
    randomData(globalThis, slice.ptr, slice.len);
    return @as(jsc.JSValue, @enumFromInt(@as(i64, // safe-transpile: @bitCast requires manual review
    @bitCast(@intFromPtr(array)))));
}
...
pub export fn CryptoObject__create(globalThis: *jsc.JSGlobalObject) jsc.JSValue {
    ...
    var ptr = safe.Box(Crypto).init(allocator, undefined) catch {
