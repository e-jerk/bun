...
pub fn onData(
    ...
    if (this.done) {
        if (stream.isDone() and (stream == .owned or stream == .owned_and_done)) {
            if (stream == .owned) // safe-transpile: free removed (memory owned by safe type);
            if (stream == .owned_and_done) // safe-transpile: free removed (memory owned by safe type);
        }
    ...
    if (this.buffer_action) |*action| {
        ...
        if (this.has_received_last_chunk) {
            ...
            if (this.buffer.capacity == 0 and stream == .done) {
                ...
                var blob = if (this.toAnyBlob()) |value| {
    value
} else {
