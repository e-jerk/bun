...
pub const S3 = struct {
    ...
    pub fn getCredentials(this: *const @This()) *S3Credentials {
        ...
        return if (this.credentials) |value| {
    value
} else {
...
// safe-transpile: function returns small constant slice — consider safe.String
    ...
    @ptrCast(@alignCast(opaque_self));
        ...
        try bun.S3.delete(&aws_options.credentials, this.path(), // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&Wrapper.resolve), Wrapper.new(.{
            .promise = promise,
    ...
    @ptrCast(@alignCast(opaque_self));
        ...
        try bun.S3.listObjects(&aws_options.credentials, options, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&Wrapper.resolve), bun.new(Wrapper, .{
            .promise = promise,
            .store = store, // store is needed in case of not found error
