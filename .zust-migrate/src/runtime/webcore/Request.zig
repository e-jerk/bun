...
pub fn getSignal(this: *Request, globalThis: *jsc.JSGlobalObject) jsc.JSValue {
    ...
    if (this.signal) |signal| {
        ...
        const js_signal = safe.Box(globalThis).init(allocator, undefined);
    ...
    var body_ = brk: {
        if (this._js_ref.tryGet()) |js_ref| {
            if (js.gc.stream.get(js_ref)) |stream| {
                ...
                if (readable != null) {
                    break :brk try this._body.value.cloneWithReadableStream(globalThis, &if (readable) |value| {
    value
} else {
    return error.NullPointer;
});
