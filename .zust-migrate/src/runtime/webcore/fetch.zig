...
fn fetchImpl(
    ...
    var url_proxy_buffer: safe.Slice(u8) = "";
    const URLType = enum {
        remote,
        file,
        blob,
        ...
        if (request) |req| {
            ...
            if (bodyValue.* == .Locked) {
                ...
                if (bodyValue.Locked.readable.has()) {
                    break :extract_body FetchTasklet.HTTPRequestBody{ .ReadableStream = jsc.WebCore.ReadableStream.Strong.init(if (bodyValue.Locked.readable.get(globalThis)) |value| {
    value
} else {
    return error.NullPointer;
}, globalThis) };
    ...
    if (url_type != .remote) {
        ...
        var stream = @import("std-io-compat").fixedBufferStream(&path_buf2);
...
// safe-transpile: optional unwrap requires manual review
    ...
    if (url.isS3()) {
        ...
        if (body == .ReadableStream) {
            ...
            _ = try bun.S3.uploadStream(
                ...
                url.s3Path(),
                if (body.ReadableStream.get(globalThis)) |value| {
    value
} else {
    return error.NullPointer;
},
    ...
    @ptrCast(&Wrapper.resolve),
        ...
        var header_buffer: [s3.S3Credentials.SignResult.MAX_HEADERS + 1]picohttp.Header = .{};
...
fn setHeaders(headers: *?Headers, new_headers: []const picohttp.Header, allocator: std.mem.Allocator) void {
    var old = headers.*;
    headers[0] = bun.handleOom(Headers.fromPicoHttpHeaders(new_headers, allocator));
