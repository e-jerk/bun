...
pub fn toBunStringFromOwnedSlice(input: []u8, encoding: Encoding) bun.String {
    ...
    switch (encoding) {
        ...
        .ucs2, .utf16le => {
            ...
            return bun.String.createExternalGloballyAllocated(.utf16, // safe-transpile: @alignCast requires manual review
    @alignCast(as_u16));
        },
    ...
    switch (comptime encoding) {
        ...
        .ucs2, .utf16le => {
            ...
            if (std.mem.isAligned(@intFromPtr(to_ptr), @alignOf([*]u16))) {
                ...
                const output = @as([*]u16, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(to_ptr)))[0 .. to_len / 2];
                const written = strings.copyLatin1IntoUTF16([]u16, output, buf).written;
                return written * 2;
            } else {
                const buf = input[0..len];
                const output = @as([*]align(1) u16, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(to_ptr))[0 .. to_len / 2];
    ...
    switch (comptime encoding) {
        ...
        .ucs2, .utf16le => {
            if (allow_partial_write) {
                ...
                const input_u8 = @as([*]const u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(input));
