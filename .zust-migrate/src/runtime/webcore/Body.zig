...
// safe-transpile: optional unwrap requires manual review
    ...
    pub fn setPromise(value: *PendingValue, globalThis: *jsc.JSGlobalObject, action: Action, owned_readable: ?jsc.WebCore.ReadableStream) bun.JSError!JSValue {
        ...
        {
            var promise = safe.Box(globalThis).init(allocator, undefined);
            ...
            if (value.onStartBuffering) |onStartBuffering| {
                ...
                onStartBuffering(if (value.task) |value| {
    value
} else {
...
pub const Value = union(Tag) {
    ...
    pub const ValueError = union(enum) {
        ...
        pub fn toJS(this: *@This(), globalObject: *jsc.JSGlobalObject) jsc.JSValue {
            ...
            this[0] = .{ .JSValue = .create(js_value, globalObject) };
            return js_value;
        }
    ...
    pub fn toReadableStream(this: *Value, globalThis: *JSGlobalObject) bun.JSError!JSValue {
        ...
        switch (this.*) {
            ...
            .InternalBlob, .Blob, .WTFStringImpl => {
                ...
                const stream = if ((try jsc.WebCore.ReadableStream.fromJS(value, globalThis))) |value| {
    value
} else {
    return error.NullPointer;
};
            ...
            .Locked => {
                ...
                if (locked.onStartStreaming) |drain| {
                    ...
                    drain_result = drain(if (locked.task) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
                ...
                if (locked.onReadableStreamAvailable) |onReadableStreamAvailable| {
                    onReadableStreamAvailable(if (locked.task) |value| {
    value
} else {
    return error.NullPointer;
}, globalThis, if (locked.readable.get(globalThis)) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: optional unwrap requires manual review
    ...
    return error.NullPointer;
            ...
            if (locked.promise) |promise_| {
                const promise = if (promise_.asAnyPromise()) |value| {
    value
} else {
    return error.NullPointer;
};
    ...
    pub fn toErrorInstance(this: *Value, err: ValueError, global: *JSGlobalObject) bun.JSTerminated!void {
        if (this.* == .Locked) {
            ...
            if (locked.onReceiveValue) |onReceiveValue| {
                locked.onReceiveValue = null;
                onReceiveValue(if (locked.task) |value| {
    value
} else {
    return error.NullPointer;
}, this);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    fn createJSSink(sink: *@This(), stream: jsc.WebCore.ReadableStream) !void {
        ...
        var buffer_stream = try safe.Box(ArrayBufferSink.JSSink).init(allocator, undefined);
        var globalThis = sink.global;
        buffer_stream[0] = ArrayBufferSink.JSSink{
        ...
        signal[0] = ArrayBufferSink.JSSink.SinkSignal.init(JSValue.zero);
    ...
    fn bufferLockedBodyValue(sink: *@This(), value: *jsc.WebCore.Body.Value, owned_readable_stream: ?jsc.WebCore.ReadableStream) !void {
        ...
        locked.task = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(sink);
