    ...
    pub fn onProgressUpdate(this: *FetchTasklet) bun.JSTerminated!void {
        ...
        if (this.result.certificate_info) |certificate_info| {
            ...
            if (this.reject_unauthorized and !this.checkServerIdentity(certificate_info)) {
                ...
                const promise = if (promise_value.asAnyPromise()) |value| {
    value
} else {
    return error.NullPointer;
        ...
        const Holder = struct {
            ...
            pub fn resolve(self: *@This()) bun.JSTerminated!void {
                ...
                // resolve the promise
                var prom = if (self.promise.swap().asAnyPromise()) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                if (BoringSSL.d2i_X509(null, &cert_ptr, @intCast(cert.len))) |x509| {
                    ...
                    const js_cert = X509.toJS(x509, globalObject) catch |err| {
                        ...
                        const check_result = if (globalObject.tryTakeException()) |value| {
    value
} else {
    return error.NullPointer;
};
                    ...
                    const js_hostname = hostname.toJS(globalObject) catch |err| {
                        ...
                        const hostname_err_result = if (globalObject.tryTakeException()) |value| {
    value
} else {
    return error.NullPointer;
};
    ...
    pub fn onReject(this: *FetchTasklet) Body.Value.ValueError {
        ...
        switch (if (this.result.fail) |value| {
    value
} else {
    return error.NullPointer;
}) {
        ...
        const fetch_error = jsc.SystemError{
            .code = bun.String.static(switch (if (this.result.fail) |value| {
    value
} else {
    return error.NullPointer;
}) {
            ...
            .message = switch (if (this.result.fail) |value| {
    value
} else {
    return error.NullPointer;
}) {
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    fn onStreamCancelledCallback(ctx: ?*anyopaque) void {
        const this = bun.cast(*FetchTasklet, (if (ctx) |value| {
    value
} else {
    return error.NullPointer;
}));
    ...
    fn toResponse(this: *FetchTasklet) Response {
        ...
        const metadata = if (this.metadata) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    pub fn get(
        ...
        var fetch_tasklet = try safe.Box(FetchTasklet).init(allocator, undefined);
        ...
        fetch_tasklet[0] = .{
            ...
            .http = try safe.Box(http.AsyncHTTP).init(allocator, undefined),
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn writeRequestData(this: *FetchTasklet, data: []const u8) ResumableSinkBackpressure {
        ...
        defer if (needs_schedule) {
            ...
            http.http_thread.scheduleRequestWrite(if (this.http) |value| {
    value
} else {
    return error.NullPointer;
}, .data);
        ...
        if (this.skipChunkedFraming()) {
            ...
            var formated_size_buffer: [18]u8 = .{};
