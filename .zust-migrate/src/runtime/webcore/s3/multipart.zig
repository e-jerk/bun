...
// safe-transpile: function returns small constant slice — consider safe.String
        ...
        fn perform(this: *@This()) bun.JSTerminated!void {
            ...
            }, .{ .part = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&onPartResponse) }, this);
        }
        ...
        switch (result) {
            .failure => |err| {
                if (this.options.retry > 0) {
                    ...
                    }, .{ .upload = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&singleSendUploadResponse) }, this);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    fn getCreatePart(this: *@This(), chunk: []const u8, allocated_size: usize, needs_clone: bool) ?*UploadPart {
        ...
        const queue_item = &if (this.queue) |value| {
    value
} else {
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    fn drainEnqueuedParts(this: *@This(), flushed: u64) bun.JSTerminated!void {
        ...
        if (this.state == .multipart_completed) {
            if (this.queue) |queue| {
                for (0..queue.len) |__zust_i| {
    var part = &queue[__zust_i];
    ...
    fn commitMultiPartRequest(this: *@This()) bun.JSTerminated!void {
        ...
        try executeSimpleS3Request(this.credentials, .{
            ...
            .request_payer = this.request_payer,
        }, .{ .commit = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&onCommitMultiPartRequest) }, this);
    }
    ...
    @ptrCast(&onRollbackMultiPartRequest) }, this);
    }
// safe-transpile: function uses raw slice parameter — consider safe.String
    fn enqueuePart(this: *@This(), chunk: []const u8, allocated_size: usize, needs_clone: bool) bun.JSTerminated!bool {
        ...
        if (this.state == .not_started) {
            ...
            try executeSimpleS3Request(this.credentials, .{
                ...
                .storage_class = this.storage_class,
                .request_payer = this.request_payer,
            }, .{ .download = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&startMultiPartRequestResult) }, this);
    ...
    @ptrCast(&singleSendUploadResponse) }, this) catch {}; // TODO: properly propagate exception upwards
        } else {
            // we need to split
        ...
        if (is_last) {
            ...
            if (chunk.len > 0) {
                switch (encoding) {
                    ...
                    .utf16 => try this.buffered.writeUTF16(// safe-transpile: @alignCast requires manual review
    @alignCast(std.mem.bytesAsSlice(u16, chunk))),
                }
            }
            this.processBuffered(this.partSizeInBytes());
        } else {
