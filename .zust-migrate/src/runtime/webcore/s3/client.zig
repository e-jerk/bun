...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    task.callback_context = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(response_stream);
    task.onWritable = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&jsc.WebCore.NetworkSink.onWritable);
    var signal = &response_stream.sink.signal;

    signal[0] = jsc.WebCore.NetworkSink.JSSink.SinkSignal.init(.zero);
...
pub fn uploadStream(
    ...
    const task = bun.new(MultiPartUpload, .{
        ...
        .content_encoding = if (content_encoding) |ce| bun.handleOom(bun.default_allocator.dupe(u8, ce)) else null,
        .callback = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&S3UploadStreamWrapper.resolve),
        .callback_context = undefined,
        .globalThis = globalThis,
    ...
    @ptrCast(ctx);
    task.onWritable = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&S3UploadStreamWrapper.onWritable);
    task.continueStream();
...
pub fn downloadStream(
    ...
    var result = this.signRequest(.{
        ...
        if (range) |range_| // safe-transpile: free removed (memory owned by safe type);
        const error_code_and_message = Error.getSignErrorCodeAndMessage(sign_err);
    ...
    var header_buffer: [S3Credentials.SignResult.MAX_HEADERS + 1]picohttp.Header = .{};
    const headers = brk: {
        if (range) |range_| {
...
pub fn readableStream(
    ...
    const S3DownloadStreamWrapper = struct {
        ...
        fn onStreamCancelled(ctx: ?*anyopaque) void {
            const self: *@This() = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
