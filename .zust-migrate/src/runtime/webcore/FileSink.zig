...
fn Bun__ForceFileSinkToBeSynchronousForProcessObjectStdio(_: *jsc.JSGlobalObject, jsvalue: jsc.JSValue) callconv(.c) void {
    var this: *FileSink = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(JSSink.fromJS(jsvalue) orelse return));
    ...
    if (comptime !Environment.isWindows) {
        ...
        if (this.writer.source) |*source| {
            switch (source.*) {
                .pipe => |pipe| {
                    if (uv.uv_stream_set_blocking(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(pipe), 1) == .zero) {
                        return;
                    }
                },
...
pub const FlushPendingTask = struct {
    ...
    pub fn runFromJSThread(flush_pending: *FlushPendingTask) void {
        ...
        const this: *FileSink = // safe-transpile: @alignCast requires manual review
    @alignCast(@fieldParentPtr("run_pending_later", flush_pending));
        defer this.deref();
        if (had)
            this.runPending();
    }
...
pub fn assignToStream(this: *FileSink, stream: *jsc.WebCore.ReadableStream, globalThis: *JSGlobalObject) jsc.JSValue {
    ...
    signal[0] = jsc.WebCore.FileSink.JSSink.SinkSignal.init(JSValue.zero);
    this.ref();
    defer this.deref();
    ...
    const promise_result = jsc.WebCore.FileSink.JSSink.assignToStream(globalThis, stream.value, this, @as(**anyopaque, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&signal.ptr)));
