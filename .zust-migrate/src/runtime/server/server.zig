...
const ServePlugins = struct {
    ...
    for (plugin_list, bunstring_array) |raw_plugin, *out| {
            out[0] = bun.String.init(raw_plugin);
        }
...
// safe-transpile: optional unwrap requires manual review
        ...
        pub fn jsValueAssertAlive(server: *ThisServer) jsc.JSValue {
            ...
            return if (server.js_value.tryGet()) |value| {
    value
} else {
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        pub fn onUpgrade(this: *ThisServer, globalThis: *jsc.JSGlobalObject, object: jsc.JSValue, optional: ?JSValue) bun.JSError!JSValue {
            ...
            const resp = if (upgrader.resp) |value| {
    value
} else {
    return error.NullPointer;
};
            const ctx = if (upgrader.upgrade_context) |value| {
    value
} else {
    return error.NullPointer;
};
            ...
            if (optional) |opts| {
                getter: {
                    ...
                    if (try opts.fastGet(globalThis, .headers)) |headers_value| {
                        ...
                        if ((if (fetch_headers_to_use) |value| {
    value
} else {
    return error.NullPointer;
}).fastGet(.SecWebSocketProtocol)) |protocol| {
                            // Clone before fastRemove frees the backing StringImpl.
                            sec_websocket_protocol_owned.deinit();
                            sec_websocket_protocol_owned = bun.handleOom(protocol.toSliceClone(bun.default_allocator));
                            sec_websocket_protocol = sec_websocket_protocol_owned.toZigString();
                            // Remove from headers so it's not written twice (once here and once by upgrade())
                            (if (fetch_headers_to_use) |value| {
    value
} else {
    return error.NullPointer;
}).fastRemove(.SecWebSocketProtocol);
                        }
    ...
    return error.NullPointer;
}).fastGet(.SecWebSocketExtensions)) |extensions| {
                            // Clone before fastRemove frees the backing StringImpl.
    ...
    return error.NullPointer;
}).fastRemove(.SecWebSocketExtensions);
                        }
                    }
            ...
            if (fetch_headers_to_use != null or cookies_to_write != null) {
                ...
                if (cookies_to_write) |cookies| {
                    try cookies.write(globalThis, uws.ResponseKind.from(ssl_enabled, false), // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(resp));
                }
            }
...
// safe-transpile: optional unwrap requires manual review
            ...
            for (0..static_routes.items.len) |__zust_i| {
    var route = &static_routes.items[__zust_i];
                route.deinit();
            }
            ...
            if (new_config.had_routes_object) {
                for (0..this.config.user_routes_to_build.items.len) |__zust_i| {
    var route = &this.config.user_routes_to_build.items[__zust_i];
                    route.deinit();
                }
                this.config.user_routes_to_build.clearAndFree();
                this.config.user_routes_to_build = new_config.user_routes_to_build;
                for (0..this.user_routes.items.len) |__zust_i| {
    var route = &this.user_routes.items[__zust_i];
                    route.deinit();
                }
...
// safe-transpile: optional unwrap requires manual review
        ...
        pub fn onFetch(this: *ThisServer, ctx: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!jsc.JSValue {
            ...
            var first_arg = if (args.nextEat()) |value| {
    value
} else {
    return error.NullPointer;
};
            var body: jsc.WebCore.Body.Value = .{ .Null = {} };
            var existing_request: WebCore.Request = std.mem.zeroes(WebCore.Request);
            ...
            if (first_arg.isString()) {
                ...
                const owned_url_buf: safe.Slice(u8) = if (url.hostname.len == 0)
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
        ...
        pub fn getAllClosedPromise(this: *ThisServer, globalThis: *jsc.JSGlobalObject) jsc.JSValue {
            ...
            prom[0] = jsc.JSPromise.Strong.init(globalThis);
            return prom.value();
        }
...
// safe-transpile: optional unwrap requires manual review
        ...
        pub fn scheduleDeinit(this: *ThisServer) void {
            ...
            if (!this.flags.terminated) {
                ...
                this.flags.terminated = true;
                const task = safe.Box(jsc.AnyTask).init(allocator, undefined) catch unreachable;
                task[0] = jsc.AnyTask.New(App, App.close).init(if (this.app) |value| {
    value
} else {
    return error.NullPointer;
});
            ...
            const task = safe.Box(jsc.AnyTask).init(allocator, undefined) catch unreachable;
            task[0] = jsc.AnyTask.New(ThisServer, deinit).init(this);
            this.vm.enqueueTask(jsc.Task.init(task));
        }
        ...
        pub fn deinit(this: *ThisServer) void {
            ...
            for (0..this.user_routes.items.len) |__zust_i| {
    var user_route = &this.user_routes.items[__zust_i];
                user_route.deinit();
            }
        ...
        pub fn init(config: *ServerConfig, global: *JSGlobalObject) bun.JSOOM!*ThisServer {
            ...
            server.request_pool_allocator = if (RequestContext.pool) |value| {
    value
} else {
    return error.NullPointer;
};
            ...
            if (comptime has_h3) {
                ...
                server.h3_request_pool_allocator = if (H3RequestContext.pool) |value| {
    value
} else {
    return error.NullPointer;
};
            ...
            if (comptime ssl_enabled) {
                ...
                while (ssl_error != 0 and written < output_buf.len) : (ssl_error = BoringSSL.ERR_get_error()) {
                    ...
                    if (BoringSSL.ERR_reason_error_string(
                        ...
                        safe.SimdUtils.copy(output_buf[written..][0..reason.len], reason);
                        written += reason.len;
                    }
        ...
        pub fn onListen(this: *ThisServer, socket: ?*App.ListenSocket) void {
            ...
            if (!ssl_enabled)
                this.vm.addListeningSocketForWatchMode((if (socket) |value| {
    value
} else {
    return error.NullPointer;
}).socket().fd());
                ...
                .exception, .rejection => |err| {
                    ...
                    if (node_http_response) |node_response| {
                        if (!node_response.flags.upgraded and node_response.raw_response != null) {
                            const raw_response = if (node_response.raw_response) |value| {
    value
} else {
    return error.NullPointer;
};
    ...
    return error.NullPointer;
        ...
        pub fn setUsingCustomExpectHandler(this: *ThisServer, value: bool) void {
            NodeHTTP_setUsingCustomExpectHandler(ssl_enabled, if (this.app) |value| {
    value
} else {
    return error.NullPointer;
}, value);
        ...
        fn onUserRouteRequestFor(comptime Ctx: type, user_route: *UserRoute, req: *Ctx.Req, resp: *Ctx.Resp) void {
            ...
            const server_request_list = if (js.routeListGetCached(server.jsValueAssertAlive())) |value| {
    value
} else {
    return error.NullPointer;
};
    ...
    return error.NullPointer;
        ...
        pub fn onWebSocketUpgrade(this: *ThisServer, resp: *App.Response, req: *uws.Request, upgrade_ctx: *uws.WebSocketUpgradeContext, id: usize) void {
            ...
            if (id == 1) {
                ...
                upgradeWebSocketUserRoute(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this), resp, req, upgrade_ctx, null);
                return;
            }
        ...
        fn onChromeDevToolsJSONRequest(this: *ThisServer, req: *uws.Request, resp: *App.Response) void {
            ...
            const first_hash_segment: [8]u8 = brk: {
                ...
                break :brk // safe-transpile: @bitCast requires manual review
    @bitCast(bun.hash(bun.strings.copyLowercase(main[0..len], buffer[0..len])));
            };
...
// safe-transpile: optional unwrap requires manual review
                ...
                break :brk // safe-transpile: @bitCast requires manual review
    @bitCast(bun.hash(bun.strings.copyLowercase(root[0..len], buffer[0..len])));
            };
...
// safe-transpile: optional unwrap requires manual review
    ...
    return error.NullPointer;
            ...
            if (this.config.user_routes_to_build.items.len > 0) {
                ...
                defer {
                    for (0..old_user_routes.items.len) |__zust_i| { var r = &old_user_routes.items[__zust_i]; r.route.deinit() };
    ...
    for (user_routes_to_build_list.items, paths_zig, callbacks_js, 0..) |*builder, *p_zig, *cb_js, i| {
                    p_zig[0] = ZigString.init(builder.route.path);
                    cb_js[0] = if (builder.callback.get()) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
                ...
                for (0..user_routes_to_build_list.items.len) |__zust_i| { var builder = &user_routes_to_build_list.items[__zust_i]; builder.deinit() };
    ...
    var user_route = &this.user_routes.items[__zust_i];
            ...
            if (this.config.static_routes.items.len > 0) {
                for (0..this.config.static_routes.items.len) |__zust_i| {
    var entry = &this.config.static_routes.items[__zust_i];
        ...
        pub fn listen(this: *ThisServer) jsc.JSValue {
            ...
            if (ssl_enabled) {
                ...
                app = safe.Box(ssl_options).init(allocator, undefined) orelse {
                    if (!globalThis.hasException()) {
                ...
                if (this.config.sni) |*sni| {
                    for (0..sni.slice().len) |__zust_i| {
    var sni_ssl_config = &sni.slice()[__zust_i];
                ...
                app = safe.Box(.{}).init(allocator, undefined) orelse {
                    if (!globalThis.hasException()) {
                        globalThis.throw("Failed to create HTTP server", .{}) catch {};
                    }
                    this.deinit();
