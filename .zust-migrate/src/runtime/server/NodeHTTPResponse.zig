...
pub const UpgradeCTX = struct {
    ...
    pub fn deinit(this: *UpgradeCTX) void {
        ...
        this[0] = .{};
        if (sec_websocket_extensions.len > 0) // safe-transpile: free removed (memory owned by safe type);
        if (sec_websocket_protocol.len > 0) // safe-transpile: free removed (memory owned by safe type);
        if (sec_websocket_key.len > 0) // safe-transpile: free removed (memory owned by safe type);
    }
...
// safe-transpile: optional unwrap requires manual review
    return Bun__getNodeHTTPResponseThisValue(if (this.raw_response) |value| {
    value
} else {
    return error.NullPointer;
} == .SSL, this.raw_response.?.socket());
}
...
// safe-transpile: optional unwrap requires manual review
    return Bun__getNodeHTTPServerSocketThisValue(if (this.raw_response) |value| {
    value
} else {
    return error.NullPointer;
} == .SSL, this.raw_response.?.socket());
...
pub fn upgrade(this: *NodeHTTPResponse, data_value: JSValue, sec_websocket_protocol: ZigString, sec_websocket_extensions: ZigString) bool {
    ...
    const sec_websocket_protocol_value = brk: {
        ...
        break :brk (if (sec_websocket_protocol_str) |value| {
    value
} else {
    return error.NullPointer;
}).slice();
    ...
    const sec_websocket_extensions_value = brk: {
        ...
        break :brk (if (sec_websocket_extensions_str) |value| {
    value
} else {
    return error.NullPointer;
}).slice();
...
pub fn create(
    ...
    if (method.hasRequestBody() or method == HTTP.Method.GET) {
        ...
        has_body[0] = req_len > 0 or request.header("transfer-encoding") != null;
    }
    ...
    const response = bun.new(NodeHTTPResponse, .{
        ...
        .upgrade_context = .{
            .context = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(upgrade_ctx),
            .request = request,
        },
        .server = AnyServer{ .ptr = AnyServer.Ptr.from(@ptrFromInt(any_server_tag)) },
        .raw_response = switch (is_ssl != 0) {
            true => uws.AnyResponse{ .SSL = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(response_ptr) },
            false => uws.AnyResponse{ .TCP = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(response_ptr) },
        },
...
pub fn getFlags(this: *const NodeHTTPResponse, _: *jsc.JSGlobalObject) jsc.JSValue {
    return jsc.JSValue.jsNumber(@as(u8, // safe-transpile: @bitCast requires manual review
    @bitCast(this.flags)));
}
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            if (HTTPStatusText.get(@intCast(status_code))) |status_message| {
                writeHeadInternal(if (this.raw_response) |value| {
    value
} else {
    return error.NullPointer;
}, globalObject, status_message, headers_object_value);
                break :do_it;
            }
        }
...
fn writeHeadInternal(response: uws.AnyResponse, globalObject: *jsc.JSGlobalObject, status_message: []const u8, headers: jsc.JSValue) void {
    ...
    switch (response) {
        .TCP => NodeHTTPServer__writeHead_http(globalObject, status_message.ptr, status_message.len, headers, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(response.TCP)),
        .SSL => NodeHTTPServer__writeHead_https(globalObject, status_message.ptr, status_message.len, headers, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(response.SSL)),
...
pub export fn Bun__NodeHTTPRequest__onResolve(globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) callconv(jsc.conv) jsc.JSValue {
    ...
    const this: *NodeHTTPResponse = if (arguments[1].as(NodeHTTPResponse)) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn getBytes(this: *NodeHTTPResponse, globalThis: *jsc.JSGlobalObject, chunk: []const u8) jsc.JSValue {
    ...
    const bytes: jsc.JSValue = brk: {
        if (chunk.len > 0 and this.buffered_request_body_data_during_pause.len > 0) {
            ...
            const array_buffer = if (buffer.asArrayBuffer(globalThis)) |value| {
    value
} else {
    return error.NullPointer;
};
            ...
            safe.SimdUtils.copy(input[0..this.buffered_request_body_data_during_pause.len], this.buffered_request_body_data_during_pause.slice());
            safe.SimdUtils.copy(input[this.buffered_request_body_data_during_pause.len..], chunk);
            break :brk buffer;
        }
...
fn writeOrEnd(
    ...
    const raw_response = if (this.raw_response) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn handleCorked(globalObject: *jsc.JSGlobalObject, function: jsc.JSValue, result: *JSValue, is_exception: *bool) void {
    result[0] = function.call(globalObject, .js_undefined, &.{}) catch |err| {
        result[0] = globalObject.takeException(err);
        is_exception[0] = true;
        return;
    };
}
