...
pub fn memoryCost(this: *const ServerConfig) usize {
    ...
    var cost: usize = 0;
    for (0..this.static_routes.items.len) |__zust_i| {
    var entry = &this.static_routes.items[__zust_i];
...
pub fn deinit(this: *ServerConfig) void {
    ...
    var ssl_config = &sni.slice()[__zust_i];
            ssl_config.deinit();
        }
    ...
    for (0..this.user_routes_to_build.items.len) |__zust_i| {
    var builder = &this.user_routes_to_build.items[__zust_i];
        builder.deinit();
    }
...
pub fn fromJS(
    ...
    if (arguments.next()) |arg| {
        ...
        if (try getRoutesObject(global, arg)) |static| {
            ...
            var init_ctx = try safe.OffsetPtr.init(allocator, &init_ctx_); defer init_ctx.deinit();
            errdefer {
            ...
            errdefer {
                for (0..args.static_routes.items.len) |__zust_i| {
    var static_route = &args.static_routes.items[__zust_i];
                    static_route.deinit();
                }
                ...
                if (value_iter.len == 0) {
                    ...
                    while (try value_iter.next()) |item| {
                        ...
                        if (args.ssl_config == null) {
                            ...
                            if (ssl_config.server_name == null or if (ssl_config.server_name) |value| {
    value
} else {
    return error.NullPointer;
}[0] == 0) {
...
// safe-transpile: optional unwrap requires manual review
    ...
    if (args.base_uri.len > 0) {
        ...
        const hostname: string =
            if (has_hostname) std.mem.span(if (args.address.tcp.hostname) |value| {
    value
} else {
    return error.NullPointer;
}) else "0.0.0.0";
