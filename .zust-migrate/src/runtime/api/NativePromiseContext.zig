...
pub fn take(comptime T: type, cell: jsc.JSValue) ?*T {
    ...
    @ptrCast(@alignCast(Bun__NativePromiseContext__take(cell)));
}
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    pub fn runFromJSThread(packed_ptr: usize) void {
        ...
        switch (tag) {
            .HTTPServerRequestContext => @as(*server.HTTPServer.RequestContext, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(ctx))).deref(),
            .HTTPSServerRequestContext => @as(*server.HTTPSServer.RequestContext, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(ctx))).deref(),
            .DebugHTTPServerRequestContext => @as(*server.DebugHTTPServer.RequestContext, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(ctx))).deref(),
            .DebugHTTPSServerRequestContext => @as(*server.DebugHTTPSServer.RequestContext, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(ctx))).deref(),
            .BodyValueBufferer => {
                ...
                // release it there.
                const bufferer: *bun.webcore.Body.ValueBufferer = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(ctx));
                @as(*HTMLRewriter.BufferOutputSink, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(bufferer.ctx))).deref();
            },
            .HTTPSServerH3RequestContext => @as(*server.HTTPSServer.H3RequestContext, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(ctx))).deref(),
            .DebugHTTPSServerH3RequestContext => @as(*server.DebugHTTPSServer.H3RequestContext, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(ctx))).deref(),
        }
    }
};
