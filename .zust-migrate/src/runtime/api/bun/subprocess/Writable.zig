...
const zust = @import("safe");
    ...
    pub fn init(
                ...
                .pipe, .readable_stream => {
                    if (result == .buffer) {
                        ...
                        if (stdio.* == .readable_stream) {
                            ...
                            promise_for_stream[0] = assign_result;
                        }
        ...
        if (comptime Environment.isPosix) {
            if (stdio.* == .pipe) {
                _ = bun.sys.setNonblocking((if (result) |value| {
    value
} else {
    return error.NullPointer;
        ...
        switch (stdio.*) {
            ...
            .pipe, .readable_stream => {
                const pipe = jsc.WebCore.FileSink.create(event_loop, (if (result) |value| {
    value
} else {
    return error.NullPointer;
}));
    ...
    pub fn finalize(this: *Writable) void {
        ...
        return switch (this.*) {
            .pipe => |pipe| {
                if (pipe.signal.ptr == @as(*anyopaque, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this))) {
                    pipe.signal.clear();
                }
