...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    return error.NullPointer;
            ...
            return .{
                .signaled = @enumFromInt((if (signal) |value| {
    value
} else {
    return error.NullPointer;
})),
...
const WaiterThreadPosix = struct {
                        ...
                        switch (process.event_loop) {
                            .js => |event_loop| {
                                event_loop.enqueueTaskConcurrent(
                                    safe.Box(jsc.Task.init().init(allocator, undefined)jsc.Task.init(
                                        ResultTask.new(
                                            .{
                                                .result = result,
                                                .subprocess = process,
    ...
    pub fn append(process: anytype) void {
        ...
        if (comptime Environment.isLinux) {
            const one = @as([8]u8, // safe-transpile: @bitCast requires manual review
    @bitCast(@as(usize, 1)));
    ...
    pub fn loop() void {
        ...
        reloadHandlers();
        var this = try safe.OffsetPtr.init(allocator, &instance); defer this.deinit();
...
pub const WindowsSpawnResult = struct {
    ...
    pub fn toProcess(
        ...
        var process = if (this.process_) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    const argv0 = options.argv0 orelse if (argv[0]) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub fn spawnProcessWindows(
    ...
    uv_process_options.file = options.argv0 orelse if (argv[0]) |value| {
    value
} else {
    return error.NullPointer;
};
    ...
    safe.SimdUtils.copy(cwd_buf[0..options.cwd.len], options.cwd);
    ...
    inline for (0..3) |fd_i| {
        ...
        if (fd_i == 1 and stdio_options[2] == .dup2) {
            ...
            .buffer => |my_pipe| {
                ...
                stdio.data.stream = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(my_pipe);
            },
            .pipe => |fd| {
                stdio.flags = uv.UV_INHERIT_FD;
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    inline for (0..3) |i| {
        ...
        if (dup_src != null and i == (if (dup_src) |value| {
    value
} else {
    return error.NullPointer;
})) {
            result_stdio[0] = .unavailable;
        } else if (dup_tgt != null and i == (if (dup_tgt) |value| {
    value
} else {
    return error.NullPointer;
})) {
            result_stdio[0] = .{ .buffer_fd = .fromUV(dup_fds[0]) };
        } else switch (stdio_options[i]) {
            .buffer => {
                result_stdio[0] = .{ .buffer = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(stdio.data.stream) };
            },
            else => {
                result_stdio[0] = .unavailable;
            },
    ...
    for (options.extra_fds, 0..) |*input, i| {
        switch (input.*) {
            .ipc, .buffer => {
                result.extra_pipes.appendAssumeCapacity(.{ .buffer = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(stdio_containers.items[3 + i].data.stream) });
            },
            else => {
...
// safe-transpile: function returns small constant slice — consider safe.String
    fn flattenOwnedChunks(total_allocator: std.mem.Allocator, chunks_allocator: std.mem.Allocator, chunks: []const []u8) ![]u8 {
        ...
        var remain = result;
        for (chunks) |chunk| {
            safe.SimdUtils.copy(remain[0..chunk.len], chunk);
    ...
    pub fn spawn(
        ...
        // [*:null]?[*:0]const u8
        // [*:null]?[*:0]u8
        const envp = options.envp orelse @as([*:null]?[*:0]const u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(std.c.environ));
        ...
        return spawnWithArgv(options, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(args.items.ptr), // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(envp));
    }
    ...
    extern "c" var Bun__currentSyncPID: i64 = safe.CheckedInt(i64).init(0);
    ...
    fn spawnPosix(
        ...
        var siblings_buf: [64]std.c.pid_t = .{};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    while (true) switch (PosixSpawn.wait4(-1, std.posix.W.NOHANG, null)) {
        ...
        defer {
            ...
            if (!success) {
                for (0..&out.len) |__zust_i| {
    var array_list = &&out[__zust_i];
    ...
    for (&out_fds_to_wait_for, &out, &out_fds) |*fd, *bytes, *out_fd| {
                ...
                var poll_fds_buf: [2]std.c.pollfd = .{};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    for (process.memfds[1..], &out, out_fds) |memfd, *bytes, out_fd| {
                if (memfd) {
                    bytes[0] = bun.sys.File.from(out_fd).readToEnd(bun.default_allocator).bytes;
                }
            }
        }
    ...
    fn waitMacKqueue(
        ...
        var changes_buf: [5]std.c.Kevent = .{};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            if (chld_fd != bun.invalid_fd and pfds[chld_idx].revents != 0) {
                var si: linux.signalfd_siginfo = std.mem.zeroes(linux.signalfd_siginfo);
    ...
    for (out_fds_to_wait_for, out, out_fds) |*fd, *bytes, *out_fd| _ = drainFd(fd, out_fd, bytes);
        return .{ .result = (if (child_status) |value| {
    value
} else {
    return error.NullPointer;
}) };
