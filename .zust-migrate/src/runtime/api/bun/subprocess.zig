...
pub fn onAbortSignal(subprocess_ctx: ?*anyopaque, _: jsc.JSValue) callconv(.c) void {
    var this: *Subprocess = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(subprocess_ctx.?));
    ...
    return ResourceUsage.create(
        brk: {
            if (this.pid_rusage != null) {
                break :brk &if (this.pid_rusage) |value| {
    value
} else {
    ...
    switch (kind) {
        ...
        inline .stdout, .stderr => |tag| {
            ...
            switch (out.*) {
                .pipe => |pipe| {
                    if (pipe.state == .done) {
                        out[0] = .{ .buffer = CowString.initOwned(pipe.state.done, bun.default_allocator) };
                        pipe.state = .{ .done = &.{} };
                    } else {
                        out[0] = .{ .ignore = {} };
                    }
        ...
        if (jsc.Codegen.JSSubprocess.stdinGetCached(this_jsvalue)) |existing_value| {
            if (existing_value.isCell()) {
                if (stdin == null) {
                    ...
                    stdin = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(jsc.WebCore.FileSink.JSSink.fromJS(existing_value)));
                }
...
pub fn getExited(
    ...
    switch (this.process.status) {
        ...
        .err => |err| {
            ...
            const promise = safe.Box(globalThis).init(allocator, undefined).toJS();
