...
fn createOverlappedPipePair(
    ...
    var name_utf8_buf: [96]u8 = .{};
    const name = std.fmt.bufPrint(
        &name_utf8_buf,
        "\\\\.\\pipe\\bun-conpty-{d}-{d}",
        .{ pid, counter },
    ) catch return error.OpenPtyFailed;
    var name_w_buf: [96:0]u16 = .{};
...
fn createPtyWindows(cols: u16, rows: u16) CreatePtyError!PtyResult {
    ...
    {
        var pc: w.HPCON = std.mem.zeroes(w.HPCON);
        if (w.CreatePseudoConsole(size, (if (in_client) |value| {
    value
} else {
    ...
    _ = w.CloseHandle((if (in_client) |value| {
    value
} else {
    return error.NullPointer;
}));
    in_client = null;
    _ = w.CloseHandle((if (out_client) |value| {
    value
} else {
    return error.NullPointer;
}));
    ...
    const read_fd = bun.FD.fromNative((if (out_server) |value| {
    value
} else {
    return error.NullPointer;
})).makeLibUVOwned() catch return error.DupFailed;
    out_server = null;
    errdefer read_fd.close();

    const write_fd = bun.FD.fromNative((if (in_server) |value| {
    value
} else {
    return error.NullPointer;
})).makeLibUVOwned() catch return error.DupFailed;
    ...
    const result_hpcon = (if (hpcon) |value| {
    value
} else {
    return error.NullPointer;
});
...
fn getTermiosFlag(this: *Terminal, comptime field: enum { iflag, oflag, lflag, cflag }) JSValue {
    ...
    const Int = if (@typeInfo(@TypeOf(flag)).@"struct".backing_integer) |value| {
    value
} else {
    return error.NullPointer;
};
    return JSValue.jsNumber(@as(f64, @floatFromInt(@as(Int, // safe-transpile: @bitCast requires manual review
    @bitCast(flag)))));
}
...
fn setTermiosFlag(this: *Terminal, globalObject: *jsc.JSGlobalObject, comptime field: enum { iflag, oflag, lflag, cflag }, value: JSValue) bun.JSError!void {
    ...
    const Int = if (@typeInfo(FlagType).@"struct".backing_integer) |value| {
    value
} else {
    return error.NullPointer;
};
    ...
    @field(termios_data, @tagName(field)) = // safe-transpile: @bitCast requires manual review
    @bitCast(@as(Int, @intFromFloat(clamped)));
