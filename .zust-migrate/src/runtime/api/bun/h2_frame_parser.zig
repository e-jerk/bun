...
pub const H2FrameParser = struct {
    ...
    threadlocal var CORK_BUFFER: [16386]u8 = .{};
    ...
    threadlocal var shared_request_buffer: [16384]u8 = .{};
                    ...
                    const no_backpressure = brk: {
                        ...
                        defer {
                            if (!is_flow_control_limited) {
                                ...
                                var _frame = if (this.dataFrameQueue.dequeue()) |value| {
    value
} else {
        ...
        pub fn queueFrame(this: *Stream, client: *H2FrameParser, bytes: []const u8, callback: jsc.JSValue, end_stream: bool) void {
            ...
            if (this.dataFrameQueue.peekLast()) |last_frame| {
                ...
                if (remaining > 0) {
                    ...
                    safe.SimdUtils.copy(last_frame.buffer[last_frame.len .. last_frame.len + consumed_len], merge);
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            if (bytes.len > 0) {
                safe.SimdUtils.copy(frame.buffer[0..bytes.len], bytes);
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    pub fn setSettings(this: *H2FrameParser, settings: FullSettingsPayload) bool {
        ...
        var buffer: [FrameHeader.byteSize + FullSettingsPayload.byteSize]u8 = .{};
        @memset(&buffer, 0);
    ...
    pub fn abortStream(this: *H2FrameParser, stream: *Stream, abortReason: jsc.JSValue) void {
        ...
        var buffer: [FrameHeader.byteSize + 4]u8 = .{};
        @memset(&buffer, 0);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn sendGoAway(this: *H2FrameParser, streamIdentifier: u32, rstCode: ErrorCode, debug_data: []const u8, lastStreamID: u32, emitError: bool) void {
        ...
        var buffer: [FrameHeader.byteSize + 8]u8 = .{};
        @memset(&buffer, 0);
        var stream = @import("std-io-compat").fixedBufferStream(&buffer);
        const writer = stream.writer();
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn sendAltSvc(this: *H2FrameParser, streamIdentifier: u32, origin_str: []const u8, alt: []const u8) void {
        ...
        var buffer: [FrameHeader.byteSize + 2]u8 = .{};
        @memset(&buffer, 0);
        var stream = @import("std-io-compat").fixedBufferStream(&buffer);
        const writer = stream.writer();
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    pub fn sendPrefaceAndSettings(this: *H2FrameParser) void {
        ...
        var preface_buffer: [24 + FrameHeader.byteSize + FullSettingsPayload.byteSize]u8 = .{};
    ...
    pub fn sendSettingsACK(this: *H2FrameParser) void {
        ...
        var buffer: [FrameHeader.byteSize]u8 = .{};
        @memset(&buffer, 0);
        var stream = @import("std-io-compat").fixedBufferStream(&buffer);
        const writer = stream.writer();
        var settingsHeader: FrameHeader = .{
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
                ...
                safe.SimdUtils.copy(available[0..bytes.len], bytes);
                return true;
            }
        } else {
            return this._write(bytes);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            var buffer: [FrameHeader.byteSize + 16384]u8 = .{};
            @memset(&buffer, 0);
            var stream = @import("std-io-compat").fixedBufferStream(&buffer);
            const writer = stream.writer();
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    pub fn altsvc(this: *H2FrameParser, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!JSValue {
        ...
        var origin_str: safe.Slice(u8) = "";
        var value_str: safe.Slice(u8) = "";
        var stream_id: u32 = 0;
        ...
        if (!origin_string.isEmptyOrUndefinedOrNull()) {
            ...
            origin_str = (if (origin_slice) |value| {
    value
} else {
    return error.NullPointer;
}).slice();
        ...
        if (!value_string.isEmptyOrUndefinedOrNull()) {
            ...
            value_str = (if (value_slice) |value| {
    value
} else {
    return error.NullPointer;
}).slice();
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn write(this: *MemoryWriter, data: []const u8) !usize {
            ...
            safe.SimdUtils.copy(pending[0..data.len], data);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        var single_value_headers: [SingleValueHeaders.keys().len]bool = .{};
        ...
        var this = brk: {
            if (ENABLE_ALLOCATOR_POOL) {
                if (H2FrameParser.pool == null) {
                    H2FrameParser.pool = bun.handleOom(safe.Box(H2FrameParser.H2FrameParserHiveAllocator).init(allocator, undefined));
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
                ...
                if (try settings_js.get(globalObject, "maxSendHeaderBlockLength")) |max_send_header_block_length| {
                    if (max_send_header_block_length.isNumber()) {
                        this.maxSendHeaderBlockLength = // safe-transpile: @bitCast requires manual review
    @bitCast(max_send_header_block_length.toInt32());
                    }
                }
