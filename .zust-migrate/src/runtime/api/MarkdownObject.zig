    ...
    if (opts_value.isObject()) {
        ...
        inline for (@typeInfo(md.Options).@"struct".fields) |field| {
            comptime if (field.type != bool or
                safe.SimdUtils.eql(field.name, "permissive_autolinks") or
                safe.SimdUtils.eql(field.name, "permissive_url_autolinks") or
                safe.SimdUtils.eql(field.name, "permissive_www_autolinks") or
                safe.SimdUtils.eql(field.name, "permissive_email_autolinks") or
                safe.SimdUtils.eql(field.name, "heading_ids") or
                safe.SimdUtils.eql(field.name, "autolink_headings")) continue;

            if (try opts_value.getBooleanLoose(globalThis, comptime camelCaseOf(field.name))) |val| {
                @field(options, field.name) = val;
            } else if (comptime !safe.SimdUtils.eql(camelCaseOf(field.name), field.name)) {
...
const ParseRenderer = struct {
    ...
    _stack: std.ArrayListUnmanaged(StackEntry) = .empty,
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    fn enterBlockImpl(ptr: *anyopaque, block_type: md.BlockType, data: u32, flags: u32) bun.JSError!void {
        const self: *ParseRenderer = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    ...
    @ptrCast(@alignCast(ptr));
        ...
        const entry = if (self._stack.pop()) |value| {
    value
} else {
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    fn deinit(self: *JsCallbackRenderer) void {
        for (0..self._stack.items.len) |__zust_i| {
    var entry = &self._stack.items[__zust_i];
...
// safe-transpile: function returns small constant slice — consider safe.String
    ...
    fn enterBlockImpl(ptr: *anyopaque, block_type: md.BlockType, data: u32, flags: u32) bun.JSError!void {
        const self: *JsCallbackRenderer = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(ptr));
        if (!self._stack_check.isSafeToRecurse()) return self._globalObject.throwStackOverflow();
        if (block_type == .doc) return;
        if (block_type == .h) {
    ...
    fn createBlockMeta(self: *JsCallbackRenderer, block_type: md.BlockType, data: u32, flags: u32) bun.JSError!?JSValue {
        ...
        switch (block_type) {
            ...
            .li => {
                ...
                const is_ordered = parent != null and (if (parent) |value| {
    value
} else {
    return error.NullPointer;
}).block_type == .ol;
                ...
                const start_js = if (is_ordered) JSValue.jsNumber((if (parent) |value| {
    value
} else {
    return error.NullPointer;
}).data) else JSValue.js_undefined;
