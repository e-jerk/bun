...
const ScanOpts = struct {
    ...
    fn parseCWD(globalThis: *JSGlobalObject, allocator: std.mem.Allocator, cwdVal: jsc.JSValue, absolute: bool, comptime fnName: string) bun.JSError![]const u8 {
        ...
        const cwd_str: safe.Slice(u8) = cwd_str: {
            ...
            var path_buf2: [bun.MAX_PATH_BYTES * 2]u8 = .{};
...
pub const WalkTask = struct {
    ...
    pub fn create(
        ...
        const walkTask = try safe.Box(WalkTask).init(allocator, undefined);
        walkTask[0] = .{
            .walker = globWalker,
            .global = globalThis,
            .alloc = alloc,
...
fn makeGlobWalker(
    ...
    var globWalker = try safe.Box(GlobWalker).init(allocator, undefined);
...
pub fn constructor(globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!*Glob {
    ...
    const pat_str: safe.Slice(u8) = (try pat_arg.toSliceClone(globalThis)).slice();
