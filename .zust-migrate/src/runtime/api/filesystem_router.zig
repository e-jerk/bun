...
pub const FileSystemRouter = struct {
    ...
    pub fn constructor(globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!*FileSystemRouter {
        ...
        var out_buf: [bun.MAX_PATH_BYTES * 2]u8 = .{};
        ...
        var arena = safe.Box(bun.ArenaAllocator).init(allocator, undefined) catch unreachable;
        arena[0] = bun.ArenaAllocator.init(globalThis.allocator());
        ...
        var fs_router = safe.Box(FileSystemRouter).init(allocator, undefined) catch unreachable;
        fs_router[0] = .{
...
pub const MatchedRoute = struct {
    ...
    threadlocal var query_string_values_buf: [256]string = .{};
    threadlocal var query_string_value_refs_buf: [256]ZigString = .{};
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn getScriptSrcString(
        ...
        var entry_point_tempbuf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
    ...
    pub fn getParams(
        ...
        return createQueryObject(globalThis, &if (this.param_map) |value| {
    value
} else {
    return error.NullPointer;
});
