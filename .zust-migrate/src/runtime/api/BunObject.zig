...
pub fn openInEditor(globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!JSValue {
    ...
    const editor = editor_choice orelse edit.editor orelse brk: {
        ...
        break :brk if (edit.editor) |value| {
    value
} else {
...
export fn Bun__resolve(global: *JSGlobalObject, specifier: JSValue, source: JSValue, is_esm: bool) jsc.JSValue {
    ...
    const value = doResolveWithArgs(global, specifier_str, source_str, is_esm, true, false) catch {
        const err = if (global.tryTakeException()) |value| {
    value
} else {
    return error.NullPointer;
};
...
export fn Bun__resolveSyncWithPaths(
    ...
    const paths: []const bun.String = if (paths_len == 0) &.{} else (if (paths_ptr) |value| {
    value
} else {
    return error.NullPointer;
})[0..paths_len];
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    const S = struct {
        pub fn x(ptr: ?*anyopaque, size: ?*anyopaque) callconv(.c) void {
            _ = bun.sys.munmap(@as([*]align(std.heap.page_size_min) const u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(ptr)))[0..@intFromPtr(size)]);
        }
    };
...
pub fn getCSRFObject(globalObject: *jsc.JSGlobalObject, _: *jsc.JSObject) jsc.JSValue {
    return safe.Box(globalObject).init(allocator, undefined);
}
...
pub const EnvironmentVariables = struct {
    pub export fn Bun__getEnvCount(globalObject: *jsc.JSGlobalObject, ptr: *[*][]const u8) usize {
        ...
        ptr[0] = bunVM.transpiler.env.map.map.keys().ptr;
    ...
    pub export fn Bun__getEnvValueBunString(globalObject: *jsc.JSGlobalObject, name: *bun.String, value: *bun.String) bool {
        ...
        value[0] = bun.String.borrowUTF8(val);
        return true;
    }
    ...
    pub export fn Bun__setEnvValue(globalObject: *jsc.JSGlobalObject, name: *bun.String, value: *bun.String) void {
        ...
        const new_val = safe.Box(value_slice.slice()).init(allocator, undefined);
        slot.ptr.* = new_val;
...
pub const JSZlib = struct {
    export fn reader_deallocator(_: ?*anyopaque, ctx: ?*anyopaque) void {
        var reader: *zlib.ZlibReaderArrayList = bun.cast(*zlib.ZlibReaderArrayList, (if (ctx) |value| {
    value
} else {
    return error.NullPointer;
}));
    ...
    export fn compressor_deallocator(_: ?*anyopaque, ctx: ?*anyopaque) void {
        var compressor: *zlib.ZlibCompressorArrayList = bun.cast(*zlib.ZlibCompressorArrayList, (if (ctx) |value| {
    value
} else {
    return error.NullPointer;
}));
    ...
    pub fn gunzipOrInflateSync(globalThis: *JSGlobalObject, buffer: jsc.Node.StringOrBuffer, options_val_: ?JSValue, is_gzip: bool) bun.JSError!JSValue {
        ...
        var list = brk: {
            if (is_gzip and compressed.len > 64) {
                ...
                //  |     CRC32     |     ISIZE     |
                //  +---+---+---+---+---+---+---+---+
                const estimated_size: u32 = // safe-transpile: @bitCast requires manual review
    @bitCast(compressed[compressed.len - 4 ..][0..4].*);
...
pub const JSZstd = struct {
    ...
    pub const ZstdJob = struct {
        ...
        pub fn runTask(task: *jsc.WorkPoolTask) void {
            ...
            defer job.vm.enqueueTaskConcurrent(safe.Box(job.any_task.task()).init(allocator, undefined));
