...
pub const HTMLRewriter = struct {
    ...
    pub fn constructor(_: *JSGlobalObject, _: *jsc.CallFrame) bun.JSError!*HTMLRewriter {
        const rewriter = bun.handleOom(safe.Box(HTMLRewriter).init(allocator, undefined));
        rewriter[0] = HTMLRewriter{
    ...
    pub fn on_(
        ...
        const handler = bun.handleOom(safe.Box(ElementHandler).init(allocator, undefined));
        handler[0] = handler_;
        errdefer {
            handler.deinit();
            bun.default_allocator.destroy(handler);
    ...
    pub fn onDocument_(
        ...
        const handler = bun.handleOom(safe.Box(DocumentHandler).init(allocator, undefined));
        handler[0] = handler_;
        errdefer {
            handler.deinit();
            bun.default_allocator.destroy(handler);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    pub const BufferOutputSink = struct {
        ...
        pub fn init(context: *LOLHTMLContext, global: *JSGlobalObject, original: *Response, builder: *LOLHTML.HTMLRewriter.Builder) bun.JSError!jsc.JSValue {
            ...
            sink.ref();
            sink.bodyValueBufferer = jsc.WebCore.Body.ValueBufferer.init(sink, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&onFinishedBuffering), sink.global, bun.default_allocator);
...
// safe-transpile: optional unwrap requires manual review
        ...
        pub fn done(this: *BufferOutputSink) void {
            ...
            bodyValue[0] = jsc.WebCore.Body.Value{
                .InternalBlob = .{
...
fn HandlerCallback(
    ...
    return struct {
        pub fn callback(this: *HandlerType, value: *LOLHTMLType) bool {
            ...
            var scope: bun.jsc.TopExceptionScope = std.mem.zeroes(bun.jsc.TopExceptionScope);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    fn contentHandler(this: *TextChunk, comptime Callback: (fn (*LOLHTML.TextChunk, []const u8, bool) LOLHTML.Error!void), thisObject: JSValue, globalObject: *JSGlobalObject, content: ZigString, contentOptions: ?ContentOptions) JSValue {
        ...
        Callback(
            text_chunk,
            content_slice.slice(),
            contentOptions != null and (if (contentOptions) |value| {
    value
} else {
...
pub const Element = struct {
    ...
    pub fn onEndTag_(
        ...
        const end_tag_handler = bun.handleOom(safe.Box(EndTag.Handler).init(allocator, undefined));
        end_tag_handler[0] = .{ .global = globalObject, .callback = function };
