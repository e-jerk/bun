...
const LibInfo = struct {
    ...
    pub fn lookup(this: *Resolver, query: GetAddrInfo, globalThis: *jsc.JSGlobalObject) jsc.JSValue {
        ...
        const errno = getaddrinfo_async_start_(
            ...
            name_z.ptr,
            null,
            if (hints != null) &(if (hints) |v| v else unreachable) else null,
        ...
        const rc = poll.registerWithFd(
            if (this.vm.event_loop_handle) |value| {
    value
} else {
    return error.NullPointer;
            ...
            .machport,
            .one_shot,
            .fromNative(// safe-transpile: @bitCast requires manual review
    @bitCast(request.backend.libinfo.machport)),
        );
...
const LibUVBackend = struct {
    ...
    fn onRawLibUVComplete(uv_info: *libuv.uv_getaddrinfo_t, _: c_int, _: ?*libuv.addrinfo) callconv(.c) void {
        ...
        const this: *GetAddrInfoRequest = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(uv_info.data));
        const Holder = struct {
        ...
        var holder = bun.handleOom(safe.Box(Holder).init(allocator, undefined));
        holder[0] = .{
            .uv_info = uv_info,
            .task = undefined,
        };
    ...
    pub fn lookup(this: *Resolver, query: GetAddrInfo, globalThis: *jsc.JSGlobalObject) !jsc.JSValue {
        ...
        var hostname: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn init(
            ...
            var request = try safe.Box(@This()).init(allocator, undefined);
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
                    .pos_in_pending = @as(u5, @truncate(if (@field((if (resolver) |v| v else unreachable), cache_field).indexOf(cache.new)) |value| {
    value
} else {
    return error.NullPointer;
})),
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn init(globalThis: *jsc.JSGlobalObject, allocator: std.mem.Allocator, name: []const u8) !*@This() {
        const this = try safe.Box(@This()).init(allocator, undefined);
    ...
    pub fn processResolve(this: *@This(), err_: ?c_ares.Error, _: i32, result: ?c_ares.struct_nameinfo) void {
        ...
        var name_info = (if (result) |v| v else unreachable);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn init(
        ...
        log("init", .{});
        var request = try safe.Box(GetAddrInfoRequest).init(allocator, undefined);
        var poll_ref = Async.KeepAlive.init();
        poll_ref.ref(globalThis.bunVM());
        if (resolver) |resolver_| resolver_.ref();
        request[0] = .{
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    pub fn getAddrInfoAsyncCallback(
        ...
        const this = @as(*GetAddrInfoRequest, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    ...
    pub const Backend = union(enum) {
            ...
            union(enum) {
                ...
                pub fn run(this: *@This()) void {
                    ...
                    if (@intFromEnum(err) != 0 or addrinfo == null) {
                        this[0] = .{ .err = @intFromEnum(err) };
                        return;
                    }

                    // do not free addrinfo when err != 0
                    // https://github.com/ziglang/zig/pull/14242
                    defer std.c.freeaddrinfo((if (addrinfo) |v| v else unreachable));

                    this[0] = .{ .success = bun.handleOom(GetAddrInfo.Result.toList(default_allocator, (if (addrinfo) |v| v else unreachable))) };
                }
            },
        ...
        pub const LibInfo = struct {
            ...
            pub fn onMachportChange(this: *GetAddrInfoRequest) void {
                ...
                if (!getaddrinfo_send_reply(this.backend.libinfo.machport, if (dns.LibInfo.getaddrinfo_async_handle_reply()) |value| {
    value
} else {
    return error.NullPointer;
})) {
    ...
    pub fn then(this: *GetAddrInfoRequest, _: *jsc.JSGlobalObject) void {
        ...
        switch (this.backend.libc) {
            .success => |result| {
                ...
                defer any.deinit();
                if (this.resolver_for_caching) |resolver| {
                    // if (this.cache.entry_cache and result != null and (if (result) |v| v else unreachable).node != null) {
                    //     resolver.putEntryInCache(this.hash, this.cache.name_len, (if (result) |v| v else unreachable));
                    // }
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    pub fn processResolve(this: *@This(), err_: ?c_ares.Error, _: i32, result: ?*c_ares.struct_hostent) void {
        ...
        var node = (if (result) |v| v else unreachable);
...
pub const DNSLookup = struct {
    ...
    pub fn init(resolver: *Resolver, globalThis: *jsc.JSGlobalObject, allocator: std.mem.Allocator) !*DNSLookup {
        ...
        const this = try safe.Box(DNSLookup).init(allocator, undefined);
        var poll_ref = Async.KeepAlive.init();
        poll_ref.ref(globalThis.bunVM());
    ...
    pub fn onCompleteNative(this: *DNSLookup, result: GetAddrInfo.Result.Any) void {
        ...
        const array = if ((result.toJS(this.globalThis) catch .zero)) |value| {
    value
} else {
    return error.NullPointer;
    ...
    pub fn processGetAddrInfo(this: *DNSLookup, err_: ?c_ares.Error, _: i32, result: ?*c_ares.AddrInfo) void {
        ...
        if (result == null or (if (result) |v| v else unreachable).node == null) {
            c_ares.Error.ENOTFOUND.toDeferred("getaddrinfo", null, &this.promise).rejectLater(this.globalThis);
            this.deinit();
            return;
        }
        this.onComplete((if (result) |v| v else unreachable));
    }
...
pub const GlobalData = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, vm: *jsc.VirtualMachine) *GlobalData {
        const global = bun.handleOom(safe.Box(GlobalData).init(allocator, undefined));
        global[0] = .{
    ...
    pub const Request = struct {
        ...
        pub const MacAsyncDNS = struct {
            ...
            pub fn onMachportChange(this: *Request) void {
                if (!getaddrinfo_send_reply(this.libinfo.machport, if (LibInfo.getaddrinfo_async_handle_reply()) |value| {
    value
} else {
    return error.NullPointer;
})) {
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
        ...
        fn tryPush(this: *This, entry: *Request) bool {
            ...
            if (this.len >= this.cache.len) {
                ...
                for (0..this.cache[0..this.len].len) |__zust_i| {
    var e = &this.cache[0..this.len][__zust_i];
                    if (e.*.refcount == 0) {
                        e.*.deinit();
                        e.* = entry;
                        return true;
        ...
        while (info_) |ai| {
            ...
            if (ai.addr) |addr| {
                if (ai.family == std.c.AF.INET) {
                    const addr_in: *std.c.sockaddr.in = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&results[i].addr);
                    addr_in[0] = @as(*std.c.sockaddr.in, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(addr))).*;
                } else if (ai.family == std.c.AF.INET6) {
                    const addr_in: *std.c.sockaddr.in6 = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&results[i].addr);
                    addr_in[0] = @as(*std.c.sockaddr.in6, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(addr))).*;
                }
            } else {
    ...
    for (results, 0..) |*entry, idx| {
            ...
            if (entry.info.addr != null) {
                entry.info.addr = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(&entry.addr));
            }
        }
    ...
    fn workPoolCallback(req: *Request) void {
        var service_buf: [bun.fmt.fastDigitCount(std.math.maxInt(u16)) + 2]u8 = .{};
        ...
        if (Environment.isWindows) {
            ...
            afterResult(req, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(addrinfo), err);
        } else {
            var addrinfo: ?*std.c.addrinfo = null;
            var hints = getHints();
    ...
    pub fn lookupLibinfo(req: *Request, loop: jsc.EventLoopHandle) bool {
        ...
        var poll = bun.Async.FilePoll.init(loop, .fromNative(// safe-transpile: @bitCast requires manual review
    @bitCast(machport)), .{}, InternalDNSRequest, req);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        if (status == @intFromEnum(std.c.EAI.NONAME) and req.can_retry_for_addrconfig) retry: {
            ...
            const poll = if (req.libinfo.file_poll) |value| {
    value
} else {
    return error.NullPointer;
};
            poll.fd = .fromNative(// safe-transpile: @bitCast requires manual review
    @bitCast(machport));
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        if (!bun.feature_flag.BUN_FEATURE_FLAG_DISABLE_DNS_CACHE.get()) {
            if (global_cache.get(key, &timestamp_to_store)) |entry| {
                ...
                if (entry.result != null) {
                    (if (is_cache_hit) |v| v else unreachable).* = true;
    ...
    fn us_getaddrinfo(loop: *bun.uws.Loop, _host: ?[*:0]const u8, port: u16, socket: *?*anyopaque) callconv(.c) c_int {
        ...
        const req = if (getaddrinfo(loop, host, port, &is_cache_hit)) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    pub fn drainPendingHostNative(this: *Resolver, index: u8, globalObject: *jsc.JSGlobalObject, err: i32, result: GetAddrInfo.Result.Any) void {
        ...
        while (pending) |value| {
            ...
            pending = value.next;
            if (prev_global != new_global) {
                array = if ((result.toJS(new_global) catch .zero)) |value| {
    value
} else {
    return error.NullPointer;
}; // TODO: properly propagate exception upwards
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    pub fn getChannel(this: *Resolver) ChannelResult {
        ...
        return .{ .result = if (this.channel) |value| {
    value
} else {
    return error.NullPointer;
} };
    ...
    pub fn onDNSPollUv(watcher: [*c]bun.windows.libuv.uv_poll_t, status: c_int, events: c_int) callconv(.c) void {
        ...
        const channel = if (poll.parent.channel) |value| {
    value
} else {
    return error.NullPointer;
};
    ...
    pub fn onCloseUv(watcher: *anyopaque) callconv(.c) void {
        const poll = UvDnsPoll.fromPoll(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(watcher)));
        poll.destroy();
    }
        ...
        if (comptime Environment.isWindows) {
            ...
            if (!readable and !writable) {
                ...
                if (this.polls.fetchOrderedRemove(fd)) |entry| {
                    uv.uv_close(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&entry.value.poll), onCloseUv);
                }
                return;
            }
            ...
            if (!poll_entry.found_existing) {
                ...
                if (uv.uv_poll_init_socket(bun.uws.Loop.get().uv_loop, &poll.poll, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(fd)) < 0) {
                    poll.destroy();
    ...
    @ptrCast(&poll.poll), onCloseUv);
            ...
            // unregister).
            const loop = if (vm.event_loop_handle) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    fn getChannelServers(channel: *c_ares.Channel, globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!jsc.JSValue {
        ...
        if (r != c_ares.ARES_SUCCESS) {
            const err = if (c_ares.Error.get(r)) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        while (cur) |current| : ({
            ...
            var buf: [INET6_ADDRSTRLEN + 2 + 6 + 1]u8 = .{};
    ...
    pub fn globalLookupService(globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!jsc.JSValue {
        ...
        if (c_ares.getSockaddr(addr_s, port, @as(*std.posix.sockaddr, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&sa))) != 0) {
        ...
        const promise = request.tail.promise.value();
        channel.getNameInfo(
            @as(*std.posix.sockaddr, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&sa)),
            GetNameInfoRequest,
            request,
