...
pub fn optionsFromJS(value: jsc.JSValue, globalObject: *jsc.JSGlobalObject) FromJSError!Options {
    ...
    if (value.isObject()) {
        ...
        if (try value.get(globalObject, "flags")) |flags| {
            ...
            const filter = ~@as(u32, // safe-transpile: @bitCast requires manual review
    @bitCast(std.c.AI{ .ALL = true, .ADDRCONFIG = true, .V4MAPPED = true }));
            const int = @as(u32, // safe-transpile: @bitCast requires manual review
...
pub fn addressToJS(address: *const @import("std-net-shim").Address, globalThis: *jsc.JSGlobalObject) bun.JSError!jsc.JSValue {
...
pub fn addrInfoToJSArray(addr_info: *std.c.addrinfo, globalThis: *jsc.JSGlobalObject) bun.JSError!jsc.JSValue {
...

    {
        ...
        while (current) |this_node| : (current = (if (current) |value| {
    value
} else {
