...
pub fn getPtrSlice(globalThis: *JSGlobalObject, value: JSValue, byteOffset: ?JSValue, byteLength: ?JSValue) ValueOrError {
    ...
    var addr = @as(usize, // safe-transpile: @bitCast requires manual review
    @bitCast(num));
...
fn getCPtr(value: JSValue) ?usize {
    ...
    if (value.isNumber()) {
        ...
        const addr = @as(u64, // safe-transpile: @bitCast requires manual review
    @bitCast(value.toUInt64NoTruncate()));
        if (addr > 0) {
            return addr;
        }
    }
