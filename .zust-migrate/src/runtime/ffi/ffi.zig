...
const Offsets = extern struct {
    ...
    extern "c" var Bun__FFI__offsets: Offsets = undefined;
...
pub const FFI = struct {
    ...
    const CompileC = struct {
        ...
        const Source = union(enum) {
            ...
            pub fn deinit(this: *Source, allocator: Allocator) void {
                switch (this.*) {
                    .file => if (this.file.len > 0) // safe-transpile: free removed (memory owned by safe type),
                    .files => {
            ...
            pub fn add(this: *Source, state: *TCC.State, current_file_for_errors: *[:0]const u8) !void {
                switch (this.*) {
                    .file => {
                        current_file_for_errors[0] = this.file;
                        state.addFile(this.file) catch return error.CompilationError;
                        current_file_for_errors[0] = "";
                    },
                    .files => {
                        for (this.files.items) |file| {
                            current_file_for_errors[0] = file;
        ...
        fn getSystemRootDirOnce() void {
            if (Environment.isMac) {
                var which_buf: [bun.MAX_PATH_BYTES]u8 = .{};
                ...
                var process = bun.spawnSync(&.{
                    ...
                    // ?[*:null]?[*:0]const u8
                    //  [*:null]?[*:0]u8
                    .envp = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    ...
    @ptrCast(tcc_options)
            ...
            var pathbuf: [bun.MAX_PATH_BYTES]u8 = .{};
            ...
            for (0..this.symbols.map.values().len) |__zust_i| {
    var symbol = &this.symbols.map.values()[__zust_i];
    ...
    for (this.symbols.map.keys(), this.symbols.map.values()) |symbol, *function| {
        ...
        pub fn deinit(this: *CompileC) void {
            ...
            for (this.define.items) |define| {
                // safe-transpile: free removed (memory owned by safe type);
                if (define[1].len > 0) // safe-transpile: free removed (memory owned by safe type);
            }
            this.define.clearAndFree(bun.default_allocator);

            this.source.deinit(bun.default_allocator);
            if (this.flags.len > 0) // safe-transpile: free removed (memory owned by safe type);
            this.flags = "";
        }
    };
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    pub fn Bun__FFI__cc(globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!jsc.JSValue {
        ...
        if (try generateSymbols(globalThis, allocator, &compile_c.symbols.map, if (symbols_object.getObject()) |value| {
    value
} else {
    return error.NullPointer;
        ...
        for (0..compile_c.symbols.map.values().len) |__zust_i| {
    var function = &compile_c.symbols.map.values()[__zust_i];
        ...
        var lib = bun.handleOom(safe.Box(FFI).init(allocator, undefined));
        lib[0] = .{
            .dylib = null,
    ...
    pub fn callback(globalThis: *JSGlobalObject, interface: jsc.JSValue, js_callback: jsc.JSValue) bun.JSError!JSValue {
        ...
        var func = try safe.OffsetPtr.init(allocator, &function); defer func.deinit();
        ...
        switch (func.step) {
            ...
            .compiled => {
                const function_ = safe.Box(Function).init(allocator, undefined) catch unreachable;
                function_[0] = func.*;
    ...
    pub fn close(
        ...
        for (0..this.functions.values().len) |__zust_i| {
    var val = &this.functions.values()[__zust_i];
    ...
    pub fn print(global: *JSGlobalObject, object: jsc.JSValue, is_callback_val: ?jsc.JSValue) bun.JSError!JSValue {
        ...
        if (generateSymbols(global, bun.default_allocator, &symbols, obj) catch jsc.JSValue.zero) |val| {
            ...
            for (0..symbols.values().len) |__zust_i| {
    var function_ = &symbols.values()[__zust_i];
        ...
        for (0..symbols.values().len) |__zust_i| {
    var function = &symbols.values()[__zust_i];
            var arraylist = std.array_list.Managed(u8).init(allocator);
    ...
    pub fn generateSymbols(global: *JSGlobalObject, allocator: Allocator, symbols: *bun.StringArrayHashMapUnmanaged(Function), object: *jsc.JSObject) bun.JSError!?JSValue {
        ...
        while (try symbols_iter.next()) |prop| {
            ...
            symbols.putAssumeCapacity(bun.asByteSlice(if (function.base_name) |value| {
    value
} else {
    return error.NullPointer;
}), function);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    return error.NullPointer;
        ...
        pub fn compile(this: *Function, napiEnv: ?*napi.NapiEnv) !void {
            ...
            state.compileString(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(source_code.items)) catch {
                this.fail("Failed to compile source code");
                return;
            };

            CompilerRT.inject(state);
            state.addSymbol(if (this.base_name) |value| {
    value
} else {
    return error.NullPointer;
}, if (this.symbol_from_dynamic_library) |value| {
    value
} else {
    return error.NullPointer;
}) catch {
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    @ptrCast(source_code.items)) catch {
        ...
        pub fn printSourceCode(
            ...
            try writer.writeAll(" ");
            try writer.writeAll(bun.asByteSlice(if (this.base_name) |value| {
    value
} else {
    return error.NullPointer;
}));
    ...
    for (this.arg_types.items, 0..) |arg, i| {
            ...
            try writer.print("{s}(", .{bun.asByteSlice(if (this.base_name) |value| {
    value
} else {
    return error.NullPointer;
})});
    ...
    for (this.arg_types.items, 0..) |arg, i| {
            ...
            try writer.writeAll("  ");
            var inner_buf_: [372]u8 = .{};
            var inner_buf: safe.Slice(u8) = &.{};
...
const CompilerRT = struct {
    ...
    noinline fn memcpy(
        ...
        safe.SimdUtils.copy(dest[0..byte_count], source[0..byte_count]);
    }
