...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    pub fn boot(ctx: Command.Context, entry_path: string, loader: ?bun.options.Loader) !void {
        ...
        if (ctx.runtime_options.eval.script.len > 0) {
            const script_source = try safe.Box(logger.Source).init(allocator, undefined);
            script_source[0] = logger.Source.initPathString(entry_path, ctx.runtime_options.eval.script);
            ...
            var eval_path_buf: [bun.MAX_PATH_BYTES + trigger.len]u8 = .{};
            safe.SimdUtils.copy(eval_path_buf[0..cwd_slice.len], cwd_slice);
            safe.SimdUtils.copy(eval_path_buf[cwd_slice.len..][0..trigger.len], trigger);
            ...
            const script_source = try safe.Box(logger.Source).init(allocator, undefined);
            script_source[0] = logger.Source.initPathString(heap_entry_path, cron_script);
            vm.module_loader.eval_source = script_source;
