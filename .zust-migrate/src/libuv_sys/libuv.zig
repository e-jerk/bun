...
fn HandleMixin(comptime Type: type) type {
    ...
    @ptrCast(@alignCast(uv_handle_get_data(@ptrCast(this))));
        }
        pub fn getLoop(this: *const Type) *Loop {
            return uv_handle_get_loop(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this));
        }
        pub fn setData(handle: *Type, ptr: ?*anyopaque) void {
            uv_handle_set_data(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(handle), ptr);
        }
        pub fn close(this: *Type, cb: *const fn (*Type) callconv(.c) void) void {
            ...
            uv_close(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this), // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(cb));
        }

        pub fn hasRef(this: *const Type) bool {
            return uv_has_ref(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this)) != 0;
        }
        ...
        pub fn ref(this: *Type) void {
            ...
            uv_ref(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this));
        }
        ...
        pub fn unref(this: *Type) void {
            ...
            uv_unref(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this));
        }

        pub fn isClosing(this: *const Type) bool {
            return uv_is_closing(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this)) != 0;
        }

        pub fn isClosed(this: *const Type) bool {
            return uv_is_closed(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this));
        }

        pub fn isActive(this: *const Type) bool {
            return uv_is_active(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this)) != 0;
        }

        pub fn fd(this: *const Type) bun.FD {
            var fd_: uv_os_fd_t = windows.INVALID_HANDLE_VALUE;
            _ = uv_fileno(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this), &fd_);
...
fn ReqMixin(comptime Type: type) type {
    return struct {
        pub fn getData(this: *const Type, comptime DataType: type) ?*DataType {
            return // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(uv_req_get_data(@ptrCast(this)));
        }
    ...
    @ptrCast(this));
        ...
        pub fn setData(handle: *Type, ptr: ?*anyopaque) void {
            uv_req_set_data(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(handle), ptr);
        }
        pub fn cancel(this: *Type) void {
            _ = uv_cancel(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this));
        }
    };
}
...
pub const struct_uv_async_s = extern struct {
    ...
    pub fn init(this: *@This(), loop: *Loop, callback: uv_async_cb) void {
        ...
        if (uv_async_init(loop, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this), callback) != 0) {
        ...
        if (comptime onWrite) |callback| {
            const Wrapper = struct {
                pub fn uvWriteCb(handler: *uv_write_t, status: ReturnCode) callconv(.c) void {
                    callback(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(handler.data)), status);
                }
            };

            req.data = context;

            const rc = uv_write(req, stream, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(input), 1, &Wrapper.uvWriteCb);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn connect(this: *@This(), req: *uv_connect_t, name: []const u8, context: anytype, comptime onConnect: *const (fn (@TypeOf(context), ReturnCode) void)) Maybe(void) {
        this.data = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(context);
        const Wrapper = struct {
            pub fn uvConnectCb(handle: *uv_connect_t, status: ReturnCode) callconv(.c) void {
                onConnect(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(handle.data)), status);
            }
        };
        if (uv_pipe_connect2(req, this, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(name.ptr), name.len, UV_PIPE_NO_TRUNCATE, &Wrapper.uvConnectCb).toError(.connect2)) |err| {
            return .{ .err = err };
        }
        return .success;
    }
...
pub const Process = extern struct {
    ...
    pub fn kill(this: *@This(), signum: c_int) ReturnCode {
        return uv_process_kill(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(this)), signum);
    }

    pub fn getPid(this: *const @This()) c_int {
        return uv_process_get_pid(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(this)));
    }
};
...
fn StreamMixin(comptime Type: type) type {
    return struct {
        pub fn getWriteQueueSize(this: *Type) usize {
            return uv_stream_get_write_queue_size(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this));
        }
    ...
    @ptrCast(@alignCast(handle.data)), status);
            ...
            if (uv_listen(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this), backlog, &Wrapper.uvConnectCb).toError(.listen)) |err| {
                return .{ .err = err };
            }
            return .success;
        }
        ...
        pub fn accept(this: *Type, client: *Type) Maybe(void) {
            if (uv_accept(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this), // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(client)).toError(.accept)) |err| {
                return .{ .err = err };
            }
            return .success;
        }
    ...
    @ptrCast(context);
            const Wrapper = struct {
                pub fn uvAllocb(req: *uv_stream_t, suggested_size: usize, buffer: *uv_buf_t) callconv(.c) void {
                    const context_data: Context = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(req.data));
                    buffer[0] = uv_buf_t.init(alloc_cb(context_data, suggested_size));
                }
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            if (uv_read_start(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this), // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&Wrapper.uvAllocb), // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&Wrapper.uvReadcb)).toError(.listen)) |err| {
                return .{ .err = err };
            }
            return .success;
        }
        ...
        pub fn readStop(this: *Type) void {
            ...
            _ = uv_read_stop(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this));
        }
    ...
    @ptrCast(@alignCast(req.data));
                ...
                if (uv_write(uv_data, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this), // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(input), 1, &Wrapper.uvWriteCb).toError(.write)) |err| {
                    return .{ .err = err };
                }
                return .success;
            }
            ...
            if (uv_write(&req, this, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(input), 1, null).toError(.write)) |err| {
                return .{ .err = err };
            }
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn tryWrite(this: *Type, input: []const u8) Maybe(usize) {
            const rc = uv_try_write(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this), // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&uv_buf_t.init(input)), 1);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn tryWrite2(this: *Type, input: []const u8, send_handle: *uv_stream_t) ReturnCode {
            const rc = uv_try_write2(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this), // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&uv_buf_t.init(input)), 1, send_handle);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        pub fn isReadable(this: *Type) bool {
            return uv_is_readable(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this)) != 0;
        }

        pub fn isWritable(this: *@This()) bool {
            return uv_is_writable(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this)) != 0;
        }
    };
}
...
pub fn StreamWriterMixin(comptime Type: type, comptime pipe_field_name: std.meta.FieldEnum(Type), comptime uv_write_t_field_name: std.meta.FieldEnum(Type)) type {
    return struct {
        fn __get_pipe(this: *@This()) *uv_stream_t {
            ...
            return // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&@field(this, @tagName(@tagName(pipe_field_name))));
        }
