...
pub fn escapeHTMLForLatin1Input(allocator: std.mem.Allocator, latin1: []const u8) !Escaped(u8) {
    const Scalar = struct {
        pub const lengths: [std.math.maxInt(u8) + 1]u4 = brk: {
            var values: [std.math.maxInt(u8) + 1]u4 = .{};
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    switch (latin1.len) {
        ...
        2 => {
            const first: safe.Slice(u8) = switch (latin1[0]) {
                '"' => "&quot;",
                '&' => "&amp;",
                '\'' => "&#x27;",
                '<' => "&lt;",
            ...
            const second: safe.Slice(u8) = switch (latin1[1]) {
                '"' => "&quot;",
                '&' => "&amp;",
                '\'' => "&#x27;",
                '<' => "&lt;",
        ...
        32 => return Scalar.push(32, latin1[0..32], allocator),
            ...
            const vecs: [vec_chars.len]AsciiVector = comptime brk: {
                var _vecs: [vec_chars.len]AsciiVector = .{};
                // safe-transpile: for with index access requires manual review
    for (vec_chars, 0..) |c, i| {
                    _vecs[i] = @splat(c);
                }
                ...
                scan_and_allocate_lazily: while (ptr != end) : (ptr += 1) {
                    switch (ptr[0]) {
                        '"', '&', '\'', '<', '>' => |c| {
                            ...
                            safe.SimdUtils.copy(buf.items[0..copy_len], latin1[0..copy_len]);
                            any_needs_escape = true;
                            break :scan_and_allocate_lazily;
                        },
                        else => {},
...
pub fn escapeHTMLForUTF16Input(allocator: std.mem.Allocator, utf16: []const u16) !Escaped(u16) {
    ...
    switch (utf16.len) {
        ...
        2 => {
            ...
            var any_needs_escape = false;
            var buf: std.array_list.Managed(u16) = std.mem.zeroes(std.array_list.Managed(u16));

            if (comptime Environment.enableSIMD) {
                const vec_chars = "\"&'<>";
                const vecs: [vec_chars.len]AsciiU16Vector = brk: {
                    var _vecs: [vec_chars.len]AsciiU16Vector = .{};
                    // safe-transpile: for with index access requires manual review
    for (vec_chars, 0..) |c, i| {
                ...
                scan_and_allocate_lazily: while (remaining.len >= ascii_u16_vector_size) {
                    ...
                    if (@reduce(.Max, @as(AsciiVectorU16U1, // safe-transpile: @bitCast requires manual review
    @bitCast(vec > @as(AsciiU16Vector, @splat(@as(u16, 127))))) |
                        @as(AsciiVectorU16U1, // safe-transpile: @bitCast requires manual review
    @bitCast((vec == vecs[0]))) |
                        @as(AsciiVectorU16U1, // safe-transpile: @bitCast requires manual review
    @bitCast((vec == vecs[1]))) |
                        @as(AsciiVectorU16U1, // safe-transpile: @bitCast requires manual review
