...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn init(joiner_alloc: Allocator, slice: []const u8, slice_alloc: ?Allocator) *Node {
        const node = bun.handleOom(safe.Box(Node).init(allocator, undefined));
        node[0] = .{
            .slice = slice,
...
pub fn done(this: *StringJoiner, allocator: Allocator) ![]u8 {
    ...
    var remaining = slice;
    while (current) |node| {
        safe.SimdUtils.copy(remaining[0..node.slice.len], node.slice);
...
pub fn doneWithEnd(this: *StringJoiner, allocator: Allocator, end: []const u8) ![]u8 {
    ...
    safe.SimdUtils.copy(remaining, end);
