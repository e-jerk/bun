...
pub fn appendZ(this: *StringBuilder, slice: []const u8) [:0]const u8 {
    ...
    return error.NullPointer;
})[this.len + slice.len] = 0;
    const result = (if (this.ptr) |value| {
    value
} else {
    return error.NullPointer;
})[this.len..this.cap][0..slice.len :0];
...
pub fn addConcat(this: *StringBuilder, slices: []const []const u8) bun.StringPointer {
    ...
    var len: usize = 0;
    for (slices) |slice| {
        safe.SimdUtils.copy(remain[0..slice.len], slice);
...
pub fn moveToSlice(this: *StringBuilder, into_slice: *[]u8) void {
    into_slice[0] = this.allocatedSlice();
    this[0] = .{};
}
