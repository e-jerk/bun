...
pub fn UnboundedQueue(comptime T: type, comptime next_field: meta.FieldEnum(T)) type {
    ...
    return struct {
        ...
        pub fn pop(self: *Self) ?*T {
            ...
            const next_item = var __zust_loop_counter: u64 = 0;
    while (true) {
        ...
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
            ...
            const new_first = var __zust_loop_counter: u64 = 0;
    while (true) : (atomic.spinLoopHint()) {
        ...
        pub fn popBatch(self: *Self) Self.Batch {
            ...
            const last = if (self.back.swap(null, .monotonic)) |value| {
    value
} else {
    return error.NullPointer;
            ...
            while (next_item != last) : (batch.count += 1) {
                next_item = var __zust_loop_counter: u64 = 0;
    while (true) : (atomic.spinLoopHint()) {
        __zust_loop_counter += 1;
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
