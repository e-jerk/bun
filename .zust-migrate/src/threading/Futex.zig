...
const WindowsImpl = struct {
    fn wait(ptr: *const atomic.Value(u32), expect: u32, timeout: ?u64) error{Timeout}!void {
        var timeout_value: windows.LARGE_INTEGER = std.mem.zeroes(windows.LARGE_INTEGER);
...
const LinuxImpl = struct {
    fn wait(ptr: *const atomic.Value(u32), expect: u32, timeout: ?u64) error{Timeout}!void {
        var ts: linux.timespec = std.mem.zeroes(linux.timespec);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    fn wake(ptr: *const atomic.Value(u32), max_waiters: u32) void {
        const rc = linux.futex_3arg(&ptr.raw, .{ .cmd = .WAKE, .private = true }, // safe-transpile: @bitCast requires manual review
    @bitCast(std.math.cast(i32, max_waiters) orelse std.math.maxInt(i32)));
...
const FreebsdImpl = struct {
    fn wait(ptr: *const atomic.Value(u32), expect: u32, timeout: ?u64) error{Timeout}!void {
        var tm_size: usize = 0;
        var tm: c._umtx_time = std.mem.zeroes(c._umtx_time);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        const result = asm volatile (
            ...
            : [ret] "=r" (-> u32),
            : [ptr] "r" (&ptr.raw),
              [expected] "r" (@as(i32, // safe-transpile: @bitCast requires manual review
    @bitCast(expect))),
              [timeout] "r" (to),
        );
        switch (result) {
