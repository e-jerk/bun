...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            const msg: safe.Slice(u8) = switch (this.*) {
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    fn scanKeyword(self: *JSON5Parser, comptime keyword: []const u8) bool {
        ...
        if (!safe.SimdUtils.eql(self.source[self.pos..][0..keyword.len], keyword)) return false;
        // Check word boundary
    ...
    fn parseValue(self: *JSON5Parser) ParseError!Expr {
        ...
        switch (self.token.data) {
            ...
            .identifier => |s| {
                if (safe.SimdUtils.eql(s, "NaN")) {
                    try self.scan();
                    return Expr.init(E.Number, .{ .value = std.math.nan(f64) }, loc);
                } else if (safe.SimdUtils.eql(s, "Infinity")) {
                    try self.scan();
