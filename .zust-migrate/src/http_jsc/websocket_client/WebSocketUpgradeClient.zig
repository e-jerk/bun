...
pub fn NewHTTPUpgradeClient(comptime ssl: bool) type {
    return struct {
        ...
        pub fn connect(
            ...
            var protocol_for_subprotocols: safe.Slice(u8) = client_protocol_slice.slice();
    ...
    for (extra_headers.names(), extra_headers.values()) |name, value| {
            ...
            var connect_request: safe.Slice(u8) = &[_]u8{};
            if (using_proxy) {
...
// safe-transpile: optional unwrap requires manual review
        ...
        pub fn clearInput(this: *HTTPClient) void {
            if (this.input_body_buf.len > 0) // safe-transpile: free removed (memory owned by safe type);
            ...
            if (this.proxy) |*p| {
                if (p.getTunnel()) |tunnel| {
                    ...
                    if (!this.tcp.isClosed() and this.outgoing_websocket != null) {
                        ...
                        defer this.deref();
                        const ws = if (bun.take(&this.outgoing_websocket)) |value| {
    value
} else {
    return error.NullPointer;
};
...
const Headers8Bit = struct {
    ...
    pub fn init(
        ...
        const names_in = if (names_ptr) |value| {
    value
} else {
    return error.NullPointer;
}[0..len];
        const values_in = if (values_ptr) |value| {
    value
} else {
    return error.NullPointer;
}[0..len];
    ...
    pub fn deinit(self: Headers8Bit) void {
        for (0..self.slices.len) |__zust_i| { var s = &self.slices[__zust_i]; s.deinit() };
...
fn buildRequestBody(
    ...
    var encoded_buf: [24]u8 = .{};
    const key = blk: {
        if (user_key) |k_slice| {
            // Validate that it's a valid base64-encoded 16-byte value
            var decoded_buf: [24]u8 = .{}; // Max possible decoded size
    ...
    const extensions_line: safe.Slice(u8) = if (offer_permessage_deflate)
...
fn computeAcceptValue(key: []const u8) [28]u8 {
    ...
    var hash: bun.sha.Hashers.SHA1.Digest = std.mem.zeroes(bun.sha.Hashers.SHA1.Digest);
    hasher.final(&hash);
    var result: [28]u8 = .{};
...
pub fn parseSSLConfig(
    ...
    if (config_opt) |config| {
        ...
        const config_ptr = bun.handleOom(safe.Box(SSLConfig).init(allocator, undefined));
        config_ptr[0] = config;
        return config_ptr;
    }
