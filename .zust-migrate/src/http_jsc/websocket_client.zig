        ...
        pub fn handleHandshake(this: *WebSocket, socket: Socket, success: i32, ssl_error: uws.us_bun_verify_error_t) void {
            ...
            if (this.outgoing_websocket) |ws| {
                ...
                if (reject_unauthorized) {
                    ...
                    const ssl_ptr = @as(*BoringSSL.c.SSL, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
        ...
        pub fn consume(this: *WebSocket, data_: []const u8, left_in_fragment: usize, kind: Opcode, is_final: bool) usize {
            ...
            if (this.receiving_compressed) {
                ...
                if (data_.len > 0) {
                    ...
                    safe.SimdUtils.copy(writable[0..data_.len], data_);
        ...
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
                ...
                switch (receive_state) {
                    ...
                    .extended_payload_length_64, .extended_payload_length_16 => |rc| {
                        ...
                        safe.SimdUtils.copy(this.payload_length_frame_bytes[this.payload_length_frame_len..][0..total_received], data[0..total_received]);
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
                        ...
                        if (data.len > 0) {
                            ...
                            safe.SimdUtils.copy(slice, data[0..slice.len]);
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
                            ...
                            safe.SimdUtils.copy(this.ping_frame_bytes[6 + receive_body_remain ..][0..to_copy], data[0..to_copy]);
                            ...
                            if (this.ping_len >= 2) {
                                ...
                                var buf: [125]u8 = .{};
                                safe.SimdUtils.copy(buf[0 .. this.ping_len - 2], close_data[2..this.ping_len]);
...
// safe-transpile: optional unwrap requires manual review
                ...
                const content_to_compress: safe.Slice(u8) = switch (bytes) {
                    .utf16 => |utf16| brk: {
                        ...
                        const encode_result = strings.copyUTF16IntoUTF8(if (temp_buffer) |value| {
    value
} else {
    return error.NullPointer;
}, utf16);
                        break :brk if (temp_buffer) |value| {
    value
} else {
    return error.NullPointer;
}[0..encode_result.written];
                    },
                    .latin1 => |latin1| brk: {
                        ...
                        temp_buffer = allocator.alloc(u8, content_byte_len) catch return false;
                        const encode_result = strings.copyLatin1IntoUTF8(if (temp_buffer) |value| {
    value
} else {
    return error.NullPointer;
}, latin1);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        fn sendPong(this: *WebSocket, socket: Socket) bool {
            ...
            var header = @as(WebsocketHeader, // safe-transpile: @bitCast requires manual review
    @bitCast(@as(u16, 0)));
            header.final = true;
            header.opcode = .Pong;
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
            ...
            final_body_bytes[6..8].* = // safe-transpile: @bitCast requires manual review
    @bitCast(@byteSwap(code));
            ...
            if (body) |data| {
                if (body_len > 0) {
                    ...
                    safe.SimdUtils.copy(final_body_bytes[8..][0..body_len], body_slice);
                }
            }
        ...
        pub fn writeBinaryData(
            ...
            if (!this.hasBackpressure() and frame_size < stack_frame_size) {
                var inline_buf: [stack_frame_size]u8 = .{};
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
        ...
        pub fn init(
            ...
            const tcp = @as(*uws.us_socket_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(input_socket));
            ...
            var ws = bun.new(WebSocket, .{
                ...
                .secure = if (secure_ptr) |ptr| // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(ptr)) else null,
            ...
            const buffered_slice: safe.Slice(u8) = buffered_data[0..buffered_data_len];
    ...
    @ptrCast(ws),
        ...
        pub fn initWithTunnel(
            ...
            const tunnel: *WebSocketProxyTunnel = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(tunnel_ptr));
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn fill(globalThis: *jsc.JSGlobalObject, mask_buf: *[4]u8, output_: []u8, input_: []const u8) void {
        mask_buf[0] = globalThis.bunVM().rareData().entropySlice(4)[0..4].*;
        const mask = mask_buf.*;

        const skip_mask = @as(u32, // safe-transpile: @bitCast requires manual review
    @bitCast(mask)) == 0;
...
fn parseWebSocketHeader(
    ...
    receiving_type[0] = header.opcode;
    is_fragmented[0] = switch (header.opcode) {
        .Continue => true,
        else => false,
    } or !header.final;
    is_final[0] = header.final;
    ...
    if (header.opcode == .Text or header.opcode == .Binary) {
        need_compression[0] = header.compressed;
...
const Copy = union(enum) {
    ...
    pub fn len(this: @This(), byte_len: *usize) usize {
        switch (this) {
            .utf16 => {
                byte_len[0] = strings.elementLengthUTF16IntoUTF8(this.utf16);
                return WebsocketHeader.frameSizeIncludingMask(byte_len.*);
            },
            .latin1 => {
                byte_len[0] = strings.elementLengthLatin1IntoUTF8(this.latin1);
                return WebsocketHeader.frameSizeIncludingMask(byte_len.*);
            },
            .bytes => {
                byte_len[0] = this.bytes.len;
                return WebsocketHeader.frameSizeIncludingMask(byte_len.*);
            },
            .raw => {
                byte_len[0] = this.raw.len;
                return this.raw.len;
            },
        }
    }
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn copy(this: @This(), globalThis: *jsc.JSGlobalObject, buf: []u8, content_byte_len: usize, opcode: Opcode) void {
        if (this == .raw) {
            ...
            safe.SimdUtils.copy(buf[0..this.raw.len], this.raw);
            return;
        }
