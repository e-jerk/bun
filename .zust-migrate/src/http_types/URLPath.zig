...
threadlocal var temp_path_buf: [1024]u8 = .{};
threadlocal var big_temp_path_buf: [16384]u8 = .{};
...
pub fn parse(possibly_encoded_pathname_: string) !URLPath {
    ...
    if (strings.containsChar(decoded_pathname, '%')) {
        ...
        var possibly_encoded_pathname: safe.Slice(u8) = switch (decoded_pathname.len) {
        ...
        var fbs = @import("std-io-compat").fixedBufferStream(possibly_encoded_pathname);
