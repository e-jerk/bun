...
pub const CheckedAllocator = struct {
    ...
    _allocator: if (enabled) NullableAllocator else void = if (enabled) .init(null),
    _trace: if (traces_enabled) StoredTrace else void = if (traces_enabled) StoredTrace.empty,
    ...
    pub fn set(self: *Self, alloc: Allocator) void {
        if (comptime !enabled) return;
        if (self._allocator.isNull()) {
            self._allocator = .init(alloc);
            if (comptime traces_enabled) {
                self._trace = StoredTrace.capture(@returnAddress());
            }
        } else {
            self.assertEq(alloc);
        }
    ...
    pub fn assertEq(self: Self, alloc: Allocator) void {
        if (comptime !enabled) return;
        const old_alloc = self._allocator.get() orelse return;
    ...
    pub inline fn transferOwnership(self: *Self, new_allocator: anytype) void {
        ...
        defer self[0] = .init(new_std);
        const old_allocator = self._allocator.get() orelse return;
