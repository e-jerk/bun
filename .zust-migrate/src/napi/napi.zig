...
pub const napi_value = enum(i64) {
    ...
    pub fn set(
        ...
        self[0] = @enumFromInt(@intFromEnum(val));
    }
...
pub export fn napi_create_array_with_length(env_: napi_env, length: usize, result_: ?*napi_value) napi_status {
    ...
    const len_i64: i64 = // safe-transpile: @bitCast requires manual review
    @bitCast(length);
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    const len_i32: i32 = @truncate(len_i64);
    const len: u32 = if (len_i32 > 0) // safe-transpile: @bitCast requires manual review
    @bitCast(len_i32) else 0;
...
pub export fn napi_create_string_latin1(env_: napi_env, str: ?[*]const u8, length: usize, result_: ?*napi_value) napi_status {
    ...
    const slice: safe.Slice(u8) = brk: {
        if (str) |ptr| {
            if (NAPI_AUTO_LENGTH == length) {
                break :brk bun.sliceTo(@as([*:0]const u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(ptr)), 0);
    ...
    safe.SimdUtils.copy(bytes, slice);
...
pub export fn napi_create_string_utf16(env_: napi_env, str: ?[*]const char16_t, length: usize, result_: ?*napi_value) napi_status {
    ...
    const slice: []const u16 = brk: {
        if (str) |ptr| {
            if (NAPI_AUTO_LENGTH == length) {
                break :brk bun.sliceTo(@as([*:0]const u16, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(ptr)), 0);
            } else if (length > std.math.maxInt(i32)) {
                return env.invalidArg();
            } else {
    ...
    safe.SimdUtils.copy(chars, slice);
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    result[0] = @truncate(value.getLength(env.toJS()) catch return env.setLastError(.pending_exception));
    return env.ok();
}
pub export fn napi_strict_equals(env_: napi_env, lhs_: napi_value, rhs_: napi_value, result_: ?*bool) napi_status {
    ...
    result[0] = lhs.isStrictEqual(rhs, env.toJS()) catch return env.setLastError(.pending_exception);
    return env.ok();
}
...
pub export fn napi_open_handle_scope(env_: napi_env, result_: ?*napi_handle_scope) napi_status {
    ...
    result[0] = NapiHandleScope.open(env, false);
    return env.ok();
}
...
pub export fn napi_make_callback(env_: napi_env, _: *anyopaque, recv_: napi_value, func_: napi_value, arg_count: usize, args: ?[*]const napi_value, maybe_result: ?*napi_value) napi_status {
    ...
    const res = func.call(
        ...
        if (arg_count > 0 and args != null)
            @as([*]const jsc.JSValue, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(args.?))[0..arg_count]
        else
            &.{},
...
pub export fn napi_open_escapable_handle_scope(env_: napi_env, result_: ?*napi_escapable_handle_scope) napi_status {
    ...
    result[0] = NapiHandleScope.open(env, true);
    return env.ok();
}
...
pub export fn napi_is_arraybuffer(env_: napi_env, value_: napi_value, result_: ?*bool) napi_status {
    ...
    const value = value_.get();
    result[0] = !value.isNumber() and value.jsTypeLoose() == .ArrayBuffer;
    return env.ok();
}
...
pub export fn napi_get_arraybuffer_info(env_: napi_env, arraybuffer_: napi_value, data: ?*?[*]u8, byte_length: ?*usize) napi_status {
    ...
    if (data) |dat|
        dat[0] = array_buffer.ptr;
    if (byte_length) |len|
        len[0] = array_buffer.byte_len;
    return env.ok();
}
...
pub export fn napi_get_typedarray_info(
    ...
    if (maybe_type) |@"type"|
        @"type"[0] = array_buffer.typed_array_type.toTypedArrayType().toNapi() orelse return env.invalidArg();

    // TODO: handle detached
    if (maybe_data) |data|
        data[0] = array_buffer.ptr;

    if (maybe_length) |length|
        length[0] = array_buffer.len;
...
pub export fn napi_is_dataview(env_: napi_env, value_: napi_value, result_: ?*bool) napi_status {
    ...
    const value = value_.get();
    result[0] = !value.isEmptyOrUndefinedOrNull() and value.jsTypeLoose() == .DataView;
    return env.ok();
}
pub export fn napi_get_dataview_info(
    ...
    if (maybe_bytelength) |bytelength|
        bytelength[0] = array_buffer.byte_len;
...
pub export fn napi_create_promise(env_: napi_env, deferred_: ?*napi_deferred, promise_: ?*napi_value) napi_status {
    ...
    deferred[0] = safe.Box(jsc.JSPromise.Strong).init(allocator, undefined) catch @panic("failed to allocate napi_deferred");
...
pub export fn napi_is_promise(env_: napi_env, value_: napi_value, is_promise_: ?*bool) napi_status {
    ...
    is_promise[0] = value.asAnyPromise() != null;
    return env.ok();
}
...
pub export fn napi_is_date(env_: napi_env, value_: napi_value, is_date_: ?*bool) napi_status {
    ...
    const value = value_.get();
    is_date[0] = value.jsTypeLoose() == .JSDate;
    return env.ok();
}
...
fn napiSpan(ptr: anytype, len: usize) []const u8 {
    ...
    if (len == NAPI_AUTO_LENGTH) {
        return bun.sliceTo(if (ptr) |value| {
    value
} else {
...
pub export fn napi_create_buffer_copy(env_: napi_env, length: usize, data: [*]u8, result_data: ?*?*anyopaque, result_: ?*napi_value) napi_status {
    ...
    if (buffer.asArrayBuffer(env.toJS())) |array_buf| {
        if (length > 0) {
            safe.SimdUtils.copy(array_buf.slice()[0..length], data[0..length]);
        }
        if (result_data) |ptr| {
            ptr[0] = if (length > 0) array_buf.ptr else null;
        }
    }
...
pub export fn napi_create_async_work(
    ...
    result[0] = napi_async_work.new(env, execute, complete, data);
    return env.ok();
}
...
pub export fn napi_get_node_version(env_: napi_env, version_: ?**const napi_node_version) napi_status {
    ...
    version[0] = &napi_node_version.global;
    return env.ok();
}
...
pub export fn napi_get_uv_event_loop(env_: napi_env, loop_: ?*napi_event_loop) napi_status {
    ...
    if (bun.Environment.isWindows) {
        ...
        loop[0] = jsc.VirtualMachine.get().uvLoop();
    } else {
        // there is no uv event loop on posix, we use our event loop handle.
        loop[0] = env.toJS().bunVM().eventLoop();
    }
    return env.ok();
}
...
pub export fn napi_internal_register_cleanup_zig(env_: napi_env) void {
    ...
    env.toJS().bunVM().rareData().pushCleanupHook(env.toJS(), env, struct {
        fn callback(data: ?*anyopaque) callconv(.c) void {
            napi_internal_cleanup_env_cpp(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(data));
        }
    }.callback);
}
...
pub const NapiFinalizerTask = struct {
    ...
    pub fn init(finalizer: Finalizer) *NapiFinalizerTask {
        const finalizer_task = bun.handleOom(safe.Box(NapiFinalizerTask).init(allocator, undefined));
        finalizer_task[0] = .{
            .finalizer = finalizer,
        };
    ...
    pub fn schedule(this: *NapiFinalizerTask) void {
        ...
        if (thread_kind != .main) {
            ...
            vm.eventLoop().enqueueTaskConcurrent(safe.Box(jsc.Task.init(this)).init(allocator, undefined));
            return;
        }
    ...
    fn runAsCleanupHook(opaque_this: ?*anyopaque) callconv(.c) void {
        const this: *NapiFinalizerTask = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
