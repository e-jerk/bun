...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            const rc = std.c.kevent(loop.kqueue_fd.cast(), @as([*]const KEvent, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&change)), 1, undefined, 0, null);
            switch (bun.sys.getErrno(rc)) {
                .SUCCESS => {},
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            var updated: std.posix.timespec = std.mem.zeroes(std.posix.timespec);
...
pub const Poll = struct {
    ...
    pub const Flags = enum {
        ...
        pub fn applyKQueue(
            ...
            const filter: i16 = safe.CheckedInt(i16).init(0), const flags_: u16 = safe.CheckedInt(u16).init(0) = switch (comptime action) {
            ...
            kqueue_event[0] = std.mem.zeroes(KEvent);
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    pub fn onUpdateKQueue(
        ...
        switch (tag) {
            ...
            inline else => |t| {
                ...
                var this: *Pollable.Tag.Type(t) = // safe-transpile: @alignCast requires manual review
    @alignCast(@fieldParentPtr("io_poll", poll));
