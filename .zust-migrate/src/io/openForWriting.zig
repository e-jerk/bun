...
pub fn openForWritingImpl(
    ...
    if (comptime Environment.isPosix) {
        switch (bun.sys.fstat(fd)) {
            ...
            .result => |stat| {
                ...
                pollable[0] = isPollable(stat.mode);
                if (!pollable.*) {
                ...
                is_socket[0] = std.posix.S.ISSOCK(stat.mode);
                ...
                out_nonblocking[0] = is_nonblocking and pollable.*;
            },
        }
    ...
    if (comptime Environment.isWindows) {
        pollable[0] = (bun.windows.GetFileType(fd.cast()) & bun.windows.FILE_TYPE_PIPE) != 0 and !force_sync;
        return .{ .result = fd };
    }
}
