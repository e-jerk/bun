...
pub const Source = union(enum) {
    ...
    pub const File = struct {
        ...
        pub fn stop(this: *File) void {
            ...
            const cancel_result = uv.uv_cancel(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&this.fs));
            if (cancel_result == 0) {
    ...
    pub fn getHandle(this: Source) *uv.Handle {
        return switch (this) {
            .pipe => // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.pipe),
            .tty => // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.tty),
    ...
    pub const StdinTTY = struct {
        var data: uv.uv_tty_t = std.mem.zeroes(uv.uv_tty_t);
        var lock: bun.Mutex = .{};
        var initialized = std.atomic.Value(bool).init(false);
        var value = try safe.OffsetPtr.init(allocator, &data); defer value.deinit();
    ...
    pub fn openTty(loop: *uv.Loop, fd: bun.FD) bun.sys.Maybe(*Source.Tty) {
        ...
        const tty = try zust.Box(Source.Tty).init(bun.default_allocator, undefined) catch |err| bun.handleOom(err);
