...
// safe-transpile: function returns small constant slice — consider safe.String
        ...
        pub fn createPoll(this: *@This(), fd: bun.FD) *Async.FilePoll {
            return Async.FilePoll.init(@as(*Parent, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.parent)).eventLoop(), fd, .{}, PosixWriter, this);
        }
...
// safe-transpile: function returns small constant slice — consider safe.String
        ...
        pub fn start(this: *PosixWriter, rawfd: anytype, pollable: bool) bun.sys.Maybe(void) {
            ...
            const loop = @as(*Parent, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.parent)).eventLoop().loop();
            ...
            switch (poll.registerWithFd(loop, .writable, .dispatch, fd)) {
                ...
                .result => {
                    this.enableKeepingProcessAlive(@as(*Parent, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.parent)).eventLoop());
                },
            }
...
// safe-transpile: function returns small constant slice — consider safe.String
    ...
    @ptrCast(@alignCast(this.parent)), err);
            this.close();
        }
        ...
        fn _onWritable(this: *PosixWriter) void {
            ...
            if (onReady) |cb| {
                cb(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.parent));
            }
        }
...
fn BaseWindowsPipeWriter(
    ...
    return struct {
        ...
        pub fn start(this: *WindowsPipeWriter, rawfd: anytype, _: bool) bun.sys.Maybe(void) {
            ...
            const fd = switch (FDType) {
                bun.FD => rawfd,
                *bun.MovableIfWindowsFd => if (rawfd.get()) |value| {
    value
} else {
        ...
        pub fn write(this: *WindowsWriter) void {
            ...
            switch (pipe) {
                ...
                .file => |file| {
                    ...
                    if (uv.uv_fs_write(this.parent.loop(), &file.fs, file.file, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&this.write_buffer), 1, -1, onFsWriteComplete).toError(.write)) |err| {
                        file.complete(false);
                        this.close();
                        onError(this.parent, err);
                    }
...
pub fn WindowsStreamingWriter(comptime Parent: type, function_table: anytype) type {
    ...
    @ptrCast(&this.write_buffer), 1, -1, onFsWriteComplete).toError(.write)) |err| {
        ...
        fn writeInternal(this: *WindowsWriter, buffer: anytype, comptime writeFn: anytype) WriteResult {
            ...
            if (this.source != null and if (this.source) |value| {
    value
} else {
    return error.NullPointer;
} == .sync_file) {
                ...
                const fd: bun.FD = .fromUV((if (this.source) |value| {
    value
} else {
    return error.NullPointer;
}).sync_file.file);
