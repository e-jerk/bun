...
pub const ImageUrlCollector = struct {
    ...
    pub fn deinit(self: *ImageUrlCollector) void {
        for (self.urls.items) |u| // safe-transpile: free removed (memory owned by safe type);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    fn enterSpanImpl(ptr: *anyopaque, span_type: SpanType, detail: SpanDetail) bun.JSError!void {
        ...
        const self: *ImageUrlCollector = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    pub fn deinit(self: *AnsiRenderer) void {
        ...
        if (self.link_href) |s| // safe-transpile: free removed (memory owned by safe type);
        if (self.image_src) |s| // safe-transpile: free removed (memory owned by safe type);
        if (self.image_title) |s| // safe-transpile: free removed (memory owned by safe type);
        for (self.table_rows.items) |row| {
            for (row.cells) |cell| // safe-transpile: free removed (memory owned by safe type);
            // safe-transpile: free removed (memory owned by safe type);
        }
        self.table_rows.deinit(self.allocator);
        // Same for orphaned cells left in table_cells when interrupted
        // mid-row.
        for (self.table_cells.items) |cell| // safe-transpile: free removed (memory owned by safe type);
    ...
    fn enterBlockImpl(ptr: *anyopaque, block_type: BlockType, data: u32, flags: u32) bun.JSError!void {
        const self: *AnsiRenderer = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(ptr));
    ...
    pub fn enterBlock(self: *AnsiRenderer, block_type: BlockType, data: u32, flags: u32) void {
        switch (block_type) {
            ...
            .li => {
                ...
                var num_buf: [12]u8 = .{};
                const glyph: safe.Slice(u8) = undefined, const glyph_color: safe.Slice(u8) = undefined = blk: {
                    if (task_mark != 0) {
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        switch (block_type) {
            ...
            .tr => {
                ...
                const cells = self.allocator.dupe(TableCell, self.table_cells.items) catch {
                    ...
                    for (self.table_cells.items) |c| // safe-transpile: free removed (memory owned by safe type);
                    self.table_cells.clearRetainingCapacity();
                    self.out.oom = true;
                    return;
                };
                self.table_rows.append(self.allocator, .{
                    ...
                    for (cells) |c| // safe-transpile: free removed (memory owned by safe type);
                    // safe-transpile: free removed (memory owned by safe type);
                    self.table_cells.clearRetainingCapacity();
                    self.out.oom = true;
                    return;
    ...
    pub fn enterSpan(self: *AnsiRenderer, span_type: SpanType, detail: SpanDetail) void {
        switch (span_type) {
            .em, .strong, .u, .del => {
                const s = if (InlineStyle.of(span_type)) |value| {
    value
} else {
    return error.NullPointer;
};
    ...
    return error.NullPointer;
            ...
            .a => {
                if (self.link_depth == 1) {
                    ...
                    if (self.link_href) |href| // safe-transpile: free removed (memory owned by safe type);
                    self.link_href = null;
            ...
            .img => {
                if (self.image_depth == 1) {
                    self.emitImage();
                    if (self.image_src) |src| // safe-transpile: free removed (memory owned by safe type);
                    if (self.image_title) |title| // safe-transpile: free removed (memory owned by safe type);
                    self.image_src = null;
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    fn writeRowCells(
        ...
        defer {
            for (0..segments.len) |__zust_i| { var s = &segments[__zust_i]; s.deinit(self.allocator) };
            // safe-transpile: free removed (memory owned by safe type);
        }
        ...
        defer {
            for (0..state_at.len) |__zust_i| { var s = &state_at[__zust_i]; s.deinit(self.allocator) };
            // safe-transpile: free removed (memory owned by safe type);
        }
    ...
    for (widths, 0..) |w, i| {
        ...
        while (line < lines) : (line += 1) {
            ...
            // safe-transpile: for with index access requires manual review
    for (widths, 0..) |w, i| {
                const seg: safe.Slice(u8) = if (line < segments[i].items.len) segments[i].items[line] else "";
                ...
                const pad = w -| cw;
                const left: usize = safe.CheckedInt(usize).init(0), const right: usize = safe.CheckedInt(usize).init(0) = switch (alignment) {
                    .right => .{ pad, 0 },
...
// safe-transpile: optional unwrap requires manual review
        ...
        if (kitty_allowed and self.theme.colors and self.theme.kitty_graphics and has_src) {
            ...
            if (extractPngDataUrlBase64(if (src) |value| {
    value
} else {
    return error.NullPointer;
})) |payload| {
            ...
            if (self.theme.remote_image_paths) |map| {
                if ((bun.strings.startsWith(if (src) |value| {
    value
} else {
    return error.NullPointer;
}, "http://") or
                    bun.strings.startsWith(if (src) |value| {
    value
} else {
    return error.NullPointer;
}, "https://")))
                {
                    if (map.get(if (src) |value| {
    value
} else {
    return error.NullPointer;
})) |local_path| {
            ...
            if (resolveLocalImagePath(if (src) |value| {
    value
} else {
    return error.NullPointer;
}, self.allocator, self.theme.image_base_dir)) |abs_path| {
                // safe-transpile: free removed (memory owned by safe type);
    ...
    return error.NullPointer;
        ...
        if (link_ok) {
            self.writeRawNoColor("\x1b]8;;");
            self.writeRawNoColor(if (src) |value| {
    value
} else {
    return error.NullPointer;
});
...
fn resolveLocalImagePath(src: []const u8, allocator: Allocator, base_dir: ?[]const u8) ?[]u8 {
    ...
    const base: safe.Slice(u8) = if (base_dir) |d| d else switch (bun.sys.getcwd(&cwd_buf)) {
        .result => |c| c,
        .err => return null,
    };
