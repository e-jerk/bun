...
pub fn processAllBlocks(self: *Parser) Parser.Error!void {
    ...
    while (off < bytes.len) {
        ...
        const hdr: *const BlockHeader = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(bytes.ptr + off));
        ...
        const line_data: [*]const VerbatimLine = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(bytes.ptr + off));
