...
pub fn lookupRefDef(self: *Parser, raw_label: []const u8) ?RefDef {
    ...
    for (self.ref_defs.items) |rd| {
        if (safe.SimdUtils.eql(rd.label, normalized)) return rd;
    }
    return null;
}
...
pub fn buildRefDefHashtable(self: *Parser) error{OutOfMemory}!void {
    ...
    while (off < bytes.len) {
        ...
        const hdr: *BlockHeader = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(bytes.ptr + off));
        const hdr_off = off;
        ...
        const line_ptr: [*]VerbatimLine = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(bytes.ptr + off));
        ...
        if (lines_consumed > 0) {
            if (lines_consumed >= n_lines) {
                ...
                const line_base: [*]VerbatimLine = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(bytes.ptr + hdr_off + @sizeOf(BlockHeader)));
                var i: u32 = 0;
