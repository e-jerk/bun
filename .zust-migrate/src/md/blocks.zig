            ...
            if (effective_pivot_type == .indentedcode and n_parents == self.n_containers) {
                    ...
                    self.block_bytes.items.len > @sizeOf(BlockHeader))
                {
                    ...
                    if (top_off + @sizeOf(BlockHeader) <= self.block_bytes.items.len) {
                        const top_hdr: *const BlockHeader = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(self.block_bytes.items.ptr + (self.block_bytes.items.len - @sizeOf(BlockHeader))));
...
pub fn getBlockHeaderAt(self: *Parser, off: usize) *BlockHeader {
    ...
    @ptrCast(@alignCast(self.block_bytes.items.ptr + off));
}
