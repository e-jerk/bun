...
pub const State = opaque {
    ...
    pub fn setErrorFunc(s: *State, Context: type, errorOpaque: ?*Context, errorFunc: *const ErrorFunc(Context)) void {
        tcc_set_error_func(s, errorOpaque, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(errorFunc));
    }
    ...
    @ptrCast(tcc_get_symbol(s, name.ptr));
    }
