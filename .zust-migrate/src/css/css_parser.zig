...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    @bitCast(@as(u8, @bitCast(left)) - @as(u8, @bitCast(right)));
    }
...
pub fn DeriveParse(comptime T: type) type {
    ...
    const enum_type = if (comptime is_union_enum) @typeInfo(if (tyinfo.@"union".tag_type) |value| {
    value
} else {
    return error.NullPointer;
}) else tyinfo;
    const enum_actual_type = if (comptime is_union_enum) if (tyinfo.@"union".tag_type) |value| {
    value
} else {
    return error.NullPointer;
} else T;
    ...
    for (tyinfo.@"union".fields, 0..) |field, i| {
                    ...
                    if (first_void_index) |void_index| {
                        ...
                        if (if (first_payload_index) |value| {
    value
} else {
    return error.NullPointer;
} < void_index and void_index < if (first_payload_index) |value| {
    value
} else {
    return error.NullPointer;
} + payload_count) @compileError("Please put all the fields with data together and all the fields with no data together.");
                        if (if (first_payload_index) |value| {
    value
} else {
    return error.NullPointer;
} > void_index and if (first_payload_index) |value| {
    value
} else {
    return error.NullPointer;
} < void_index + void_count) @compileError("Please put all the fields with data together and all the fields with no data together.");
                    }
                    break :counts .{ payload_count, if (first_payload_index) |value| {
    value
} else {
    return error.NullPointer;
}, void_count, first_void_index };
                };
...
pub fn DeriveValueType(comptime T: type, comptime ValueTypeMap: anytype) type {
    const field_values: []const MediaFeatureType = field_values: {
        ...
        var mapping: [fields.len]MediaFeatureType = .{};
        // safe-transpile: for with index access requires manual review
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        pub fn onLayerRule(this: *This, layers: *const bun.css.SmallList(LayerName, 1)) void {
            ...
            for (0..layers.slice().len) |__zust_i| {
    var layer = &layers.slice()[__zust_i];
...
pub fn AtRulePrelude(comptime T: type) type {
    return union(enum) {
        ...
        page: safe.ArrayList(css_rules.page.PageSelector),
        moz_document,
            ...
            pub fn parsePrelude(this: *This, name: []const u8, input: *Parser) Result(Prelude) {
                const result: Prelude = brk: {
                    ...
                    if (Map.getASCIIICaseInsensitive(name)) |kind| switch (kind) {
                        ...
                        .page => {
                            const Fn = struct {
                                pub fn parsefn(input2: *Parser) Result(safe.ArrayList(css_rules.page.PageSelector)) {
                            ...
                            const selectors = switch (input.tryParse(Fn.parsefn, .{})) {
                                .result => |v| v,
                                .err => safe.ArrayList(css_rules.page.PageSelector).empty,
                            };
            ...
            pub fn parseBlock(this: *This, prelude: AtRuleParser.Prelude, start: *const ParserState, input: *Parser) Result(AtRuleParser.AtRule) {
                ...
                switch (prelude) {
                    .font_face => {
                        ...
                        // todo_stuff.think_mem_mgmt
                        var properties: safe.ArrayList(css_rules.font_face.FontFaceProperty) = .empty;
                    ...
                    .keyframes => {
                        ...
                        // todo_stuff.think_mem_mgmt
                        var keyframes = safe.ArrayList(css_rules.keyframes.Keyframe).empty;
                    ...
                    .layer => {
                        ...
                        if (name != null) {
                            T.CustomAtRuleParser.pushToEnclosingLayer(this.at_rule_parser, if (name) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        ...
        pub fn parseNested(this: *This, input: *Parser, is_style_rule: bool) Result(struct { DeclarationBlock, CssRuleList(T.CustomAtRuleParser.AtRule) }) {
            ...
            // TODO: think about memory management
            var errors = safe.ArrayList(ParseError(ParserError)).empty;
...
pub const ToCssResult = struct {
    ...
    dependencies: ?safe.ArrayList(Dependency),
};
...
pub fn fillPropertyBitSet(allocator: Allocator, bitset: *PropertyBitset, block: *const DeclarationBlock, custom_properties: *bun.BabyList([]const u8)) void {
    for (0..block.declarations.items.len) |__zust_i| {
    var prop = &block.declarations.items[__zust_i];
    ...
    for (0..block.important_declarations.items.len) |__zust_i| {
    var prop = &block.important_declarations.items[__zust_i];
        const tag = switch (prop.*) {
            .custom => {
                bun.handleOom(custom_properties.append(allocator, prop.custom.name.asStr()));
                continue;
...
pub fn StyleSheet(comptime AtRule: type) type {
    return struct {
        ...
        source_map_urls: safe.ArrayList(?[]const u8),
        license_comments: safe.ArrayList([]const u8),
        ...
        pub fn minify(this: *@This(), allocator: Allocator, options: MinifyOptions, extra: *const StylesheetExtra) Maybe(void, Err(MinifyErrorKind)) {
            ...
            var custom_media: ?std.StringArrayHashMapUnmanaged(css_rules.custom_media.CustomMediaRule) = if (this.options.flags.custom_media and options.targets.shouldCompileSame(.custom_media_queries)) brk: {
                ...
                for (0..this.rules.v.items.len) |__zust_i| {
    var rule = &this.rules.v.items[__zust_i];
        ...
        pub fn toCssWithWriter(
            ...
            const result = this.toCssWithWriterImpl(allocator, &printer, options) catch {
                ...
                return .{
                    .err = if (printer.error_kind) |value| {
    value
} else {
    return error.NullPointer;
},
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn parseWith(
            ...
            var license_comments = safe.ArrayList([]const u8).empty;
            ...
            var sources = safe.ArrayList([]const u8).empty;
            bun.handleOom(sources.append(allocator, options.filename));
            var source_map_urls = safe.ArrayList(?[]const u8).empty;
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn parse(allocator: Allocator, code: []const u8, options: ParserOptions, import_records: *bun.BabyList(ImportRecord), source_index: SrcIndex) Maybe(StyleAttribute, Err(ParserError)) {
        ...
        const sources = sources: {
            var s = bun.handleOom(safe.ArrayList([]const u8).initCapacity(allocator, 1));
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn addSymbolForName(this: *Parser, name: []const u8, tag: CssRef.Tag, loc: bun.logger.Loc) bun.bundle_v2.Ref {
        ...
        const extra = if (this.extra) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: function returns small constant slice — consider safe.String
    ...
    pub fn parseList(this: *Parser, comptime T: type, comptime parse_one: *const fn (*Parser) Result(T)) Result(safe.ArrayList(T)) {
    ...
    fn parseCommaSeparatedInternal(
        ...
        const alloc = sfb.get();
        var values = safe.ArrayList(T).initCapacity(alloc, 1) catch unreachable;
    ...
    pub inline fn tryParse(this: *Parser, comptime func: anytype, args_: anytype) bun.meta.ReturnOf(func) {
        ...
        const result = result: {
            const args = brk: {
                var args: std.meta.ArgsTuple(@TypeOf(func)) = std.mem.zeroes(std.meta.ArgsTuple(@TypeOf(func)));
                args[0] = this;
    ...
    pub fn expectInteger(this: *Parser) Result(i32) {
        ...
        if (tok.* == .number and tok.number.int_value != null) return .{ .result = if (tok.number.int_value) |value| {
    value
} else {
    return error.NullPointer;
} };
...
// safe-transpile: optional unwrap requires manual review
        ...
        const token = if (using_cached_token) token: {
            const cached_token = &if (this.input.cached_token) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub const nth = struct {
        ...
        switch (tok.*) {
            ...
            .ident => {
                ...
                if (bun.strings.eqlCaseInsensitiveASCIIIgnoreLength(value, "even")) {
                    ...
                    const slice, const a: i32 = safe.CheckedInt(i32).init(0) = if (bun.strings.startsWithChar(value, '-')) .{ value[1..], -1 } else .{ value, 1 };
    ...
    fn parse_b(input: *Parser, a: i32) Result(NthResult) {
        ...
        if (tok.* == .delim and tok.delim == '-') return parse_signless_b(input, a, -1);
        if (tok.* == .number and tok.number.has_sign and tok.number.int_value != null) return .{ .result = NthResult{ a, if (tok.number.int_value) |value| {
    value
} else {
    return error.NullPointer;
} } };
    ...
    fn parse_signless_b(input: *Parser, a: i32, b_sign: i32) Result(NthResult) {
        ...
        if (tok.* == .number and !tok.number.has_sign and tok.number.int_value != null) {
            const b = if (tok.number.int_value) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    fn parse_number_saturate(allocator: Allocator, string: []const u8) Maybe(i32, void) {
        ...
        const int = if (tok.* == .number and tok.number.int_value != null) if (tok.number.int_value) |value| {
    value
} else {
    return error.NullPointer;
} else {
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    pub fn consumeNumeric(this: *Tokenizer) Token {
        ...
        const has_sign: bool = undefined, const sign: f64 = undefined = brk: {
            switch (this.nextByteUnchecked()) {
                '-' => break :brk .{ true, -1.0 },
                '+' => break :brk .{ true, 1.0 },
        ...
        if (this.hasAtLeast(1) and switch (this.nextByteUnchecked()) {
            ...
            if (switch (this.byteAt(1)) {
                ...
                is_integer = false;
                this.advance(1);
                const has_sign2: bool = undefined, const sign2: f64 = undefined = brk: {
                    switch (this.nextByteUnchecked()) {
                        '-' => break :brk .{ true, -1.0 },
                        '+' => break :brk .{ true, 1.0 },
                        else => break :brk .{ false, 1.0 },
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
            ...
            .dimension => |dim| {
                ...
                const unit = dim.unit;
                if (safe.SimdUtils.eql(unit, "e") or safe.SimdUtils.eql(unit, "E") or
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    pub fn hexEscape(ascii_byte: u8, writer: anytype) !void {
        ...
        var bytes: [4]u8 = .{};
        const slice: safe.Slice(u8) = if (ascii_byte > 0x0F) slice: {
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
...
pub const signfns = struct {
    ...
    pub inline fn isSignNegative(x: f32) bool {
        ...
        return @as(u32, // safe-transpile: @bitCast requires manual review
    @bitCast(x)) & 0x8000_0000 != 0;
    }
...
pub inline fn copysign(self: f32, sign: f32) f32 {
    ...
    const self_bits = @as(u32, // safe-transpile: @bitCast requires manual review
    @bitCast(self));
    const sign_bits = @as(u32, // safe-transpile: @bitCast requires manual review
    @bitCast(sign));
    ...
    return @as(f32, // safe-transpile: @bitCast requires manual review
    @bitCast(result_bits));
}

    pub fn deepClone(comptime V: type, allocator: Allocator, list: *const safe.ArrayList(V)) safe.ArrayList(V) {
        var newlist = bun.handleOom(safe.ArrayList(V).initCapacity(allocator, list.items.len));

        for (0..list.items.len) |__zust_i| {
    var item = &list.items[__zust_i];
    ...
    pub fn deepDeinit(comptime V: type, allocator: Allocator, list: *safe.ArrayList(V)) void {
...
pub fn dtoa_short_impl(buf: *[129]u8, value: f32, comptime precision: u8) struct { []u8, Notation } {
    ...
    const buf_len = bun.fmt.FormatDouble.dtoa(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(buf[1..].ptr), @floatCast(value)).len;
