...
pub const Browsers = struct {
    ...
    for (entry, 0..) |c, i| {
                ...
                if (maybe_idx) |idx| {
                    ...
                    if (browser == null or if (browser) |value| {
    value
} else {
    return error.NullPointer;
} == .no_mapping) continue; // No mapping available
    ...
    return error.NullPointer;
                        ...
                        inline else => |browser_name| {
                            if (@field(browsers, @tagName(browser_name)) == null or
                                version < if (@field(browsers, @tagName(browser_name))) |value| {
    value
} else {
    return error.NullPointer;
})
