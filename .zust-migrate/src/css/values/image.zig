...
pub const Image = union(enum) {
    ...
    pub fn getFallbacks(this: *@This(), allocator: Allocator, targets: css.targets.Targets) css.SmallList(Image, 6) {
        ...
        if (prefixes.none) {
            ...
            if (fallbacks.lab) {
                this[0] = this.getFallback(allocator, ColorFallbackKind.LAB);
            }
...
pub const ImageSet = struct {
    ...
    options: safe.ArrayList(ImageSetOption),
    ...
    pub fn parse(input: *css.Parser) Result(ImageSet) {
        ...
        const Fn = struct {
            pub fn parseNestedBlockFn(_: void, i: *css.Parser) Result(safe.ArrayList(ImageSetOption)) {
                return i.parseCommaSeparated(ImageSetOption, ImageSetOption.parse);
            }
        };

        const options = switch (input.parseNestedBlock(safe.ArrayList(ImageSetOption), {}, Fn.parseNestedBlockFn)) {
            .result => |vv| vv,
            .err => |e| return .{ .err = e },
        };
    ...
    pub fn toCss(this: *const ImageSet, dest: *css.Printer) PrintErr!void {
        ...
        var first = true;
        for (0..this.options.items.len) |__zust_i| {
    var option = &this.options.items[__zust_i];
            if (first) {
                first = false;
            } else {
...
pub const ImageSetOption = struct {
    ...
    pub fn parse(input: *css.Parser) Result(ImageSetOption) {
        ...
        const resolution: Resolution = undefined, const file_type: ?[]const u8 = undefined = if (input.tryParse(Resolution.parse, .{}).asValue()) |res| brk: {
