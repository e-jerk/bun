...
pub const Gradient = union(enum) {
    ...
    pub fn toCss(this: *const Gradient, dest: *Printer) PrintErr!void {
        const f: safe.Slice(u8) = undefined, const prefix: ?css.VendorPrefix = undefined = switch (this.*) {
    ...
    pub fn getNecessaryFallbacks(this: *const @This(), targets: css.targets.Targets) css.ColorFallbackKind {
        ...
        switch (this.*) {
            .linear, .repeating_linear => |*linear| {
                for (0..linear.items.items.len) |__zust_i| {
    var item = &linear.items.items[__zust_i];
                    bun.bits.insert(css.ColorFallbackKind, &fallbacks, item.getNecessaryFallbacks(targets));
                }
            },
            .radial, .repeating_radial => |*radial| {
                for (0..radial.items.items.len) |__zust_i| {
    var item = &radial.items.items[__zust_i];
                    bun.bits.insert(css.ColorFallbackKind, &fallbacks, item.getNecessaryFallbacks(targets));
                }
            },
            .conic, .repeating_conic => |*conic| {
                for (0..conic.items.items.len) |__zust_i| {
    var item = &conic.items.items[__zust_i];
                    bun.bits.insert(css.ColorFallbackKind, &fallbacks, item.getNecessaryFallbacks(targets));
                }
            },
            .@"webkit-gradient" => {},
...
pub const LinearGradient = struct {
    ...
    items: safe.ArrayList(GradientItem(LengthPercentage)),
    ...
    pub fn toCss(this: *const LinearGradient, dest: *Printer, is_prefixed: bool) PrintErr!void {
        ...
        // we can flip the gradient the other direction and omit the direction.
            ...
            for (0..this.items.items.len) |__zust_i| {
    var item = &this.items.items[__zust_i];
            ...
            var flipped_items = safe.ArrayList(GradientItem(LengthPercentage)).initCapacity(
                dest.allocator,
    ...
    pub fn getFallback(this: *const @This(), allocator: std.mem.Allocator, kind: css.ColorFallbackKind) LinearGradient {
        var fallback_items = bun.handleOom(safe.ArrayList(GradientItem(LengthPercentage)).initCapacity(allocator, this.items.items.len));
        fallback_items.items.len = this.items.items.len;
        // safe-transpile: for with index access requires manual review
    for (fallback_items.items, this.items.items) |*out, *in| {
            out[0] = in.getFallback(allocator, kind);
        }
...
pub const RadialGradient = struct {
    ...
    pub fn getFallback(this: *const RadialGradient, allocator: Allocator, kind: css.ColorFallbackKind) RadialGradient {
        var items = bun.handleOom(safe.ArrayList(GradientItem(LengthPercentage)).initCapacity(allocator, this.items.items.len));
...
pub const ConicGradient = struct {
    ...
    position: Position,
    /// The color stops and transition hints for the gradient.
    items: safe.ArrayList(GradientItem(AnglePercentage)),
    ...
    pub fn getFallback(this: *const @This(), allocator: Allocator, kind: css.ColorFallbackKind) ConicGradient {
        var items = bun.handleOom(safe.ArrayList(GradientItem(AnglePercentage)).initCapacity(allocator, this.items.items.len));
        items.items.len = this.items.items.len;
        // safe-transpile: for with index access requires manual review
    for (items.items, this.items.items) |*out, *in| {
            out[0] = in.getFallback(allocator, kind);
...
pub const WebKitGradient = union(enum) {
    ...
    linear: struct {
        ...
        stops: safe.ArrayList(WebKitColorStop),
    ...
    pub fn toCss(this: *const WebKitGradient, dest: *Printer) PrintErr!void {
        switch (this.*) {
            .linear => |*linear| {
                ...
                for (0..linear.stops.items.len) |__zust_i| {
    var stop = &linear.stops.items[__zust_i];
                    try dest.delim(',', false);
                    try stop.toCss(dest);
                }
            },
            .radial => |*radial| {
                ...
                for (0..radial.stops.items.len) |__zust_i| {
    var stop = &radial.stops.items[__zust_i];
                    try dest.delim(',', false);
                    try stop.toCss(dest);
                }
            },
    ...
    pub fn getFallback(this: *const @This(), allocator: Allocator, kind: css.ColorFallbackKind) WebKitGradient {
        var stops: safe.ArrayList(WebKitColorStop) = .empty;
        switch (this.*) {
            .linear => |linear| {
                stops = bun.handleOom(safe.ArrayList(WebKitColorStop).initCapacity(allocator, linear.stops.items.len));
    ...
    for (stops.items, linear.stops.items) |*out, *in| {
            ...
            .radial => |radial| {
                stops = bun.handleOom(safe.ArrayList(WebKitColorStop).initCapacity(allocator, radial.stops.items.len));
    ...
    pub fn fromStandard(gradient: *const Gradient, allocator: Allocator) ?WebKitGradient {
        switch (gradient.*) {
            .linear => |*linear| {
                ...
                const from: struct { f32, f32 } = undefined, const to: struct { f32, f32 } = undefined = switch (linear.direction) {
...
pub fn parseItems(comptime D: type, input: *css.Parser) Result(safe.ArrayList(GradientItem(D))) {
    var items = safe.ArrayList(GradientItem(D)).empty;
    var seen_stop = false;
    ...
    while (true) {
        ...
        const Closure = struct { items: *safe.ArrayList(GradientItem(D)), seen_stop: *bool };
...
pub fn serializeItems(
    comptime D: type,
    items: *const safe.ArrayList(GradientItem(D)),
    dest: *Printer,
) PrintErr!void {
    var first = true;
...
pub fn convertStopsToWebkit(allocator: Allocator, items: *const safe.ArrayList(GradientItem(LengthPercentage))) ?safe.ArrayList(WebKitColorStop) {
    var stops: safe.ArrayList(WebKitColorStop) = bun.handleOom(safe.ArrayList(WebKitColorStop).initCapacity(allocator, items.items.len));
    // safe-transpile: for with index access requires manual review
