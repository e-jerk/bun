...
const SPACE_CHARACTERS: safe.Slice(u8) = &.{ 0x20, 0x09 };
...
pub const SyntaxString = union(enum) {
    ...
    components: safe.ArrayList(SyntaxComponent),
    ...
    pub fn toCss(this: *const This, dest: *Printer) PrintErr!void {
        ...
        switch (this.*) {
            ...
            .components => |*components| {
                var first = true;
                for (0..components.items.len) |__zust_i| {
    var component = &components.items[__zust_i];
                    if (first) {
                        first = false;
                    } else {
                        try dest.delim('|', true);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn parseString(allocator: std.mem.Allocator, input: []const u8) css.Maybe(SyntaxString, void) {
        ...
        var components = safe.ArrayList(SyntaxComponent).empty;
    ...
    while (true) {
        ...
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
            ...
            trimmed_input = @import("std-fs-compat").trimLeft(u8, trimmed_input, SPACE_CHARACTERS);
            if (trimmed_input.len == 0) {
                break;
            }
        ...
        switch (this.*) {
            ...
            .components => |components| {
                ...
                for (components.items) |component| {
                    ...
                    var parsed = safe.ArrayList(ParsedComponent).empty;
...
pub const SyntaxComponentKind = union(enum) {
    ...
    pub fn parseString(input: *[]const u8) css.Maybe(SyntaxComponentKind, void) {
        ...
        input[0] = @import("std-fs-compat").trimLeft(u8, input.*, SPACE_CHARACTERS);
        if (bun.strings.startsWithChar(input.*, '<')) {
...
pub const ParsedComponent = union(enum) {
    ...
    repeated: struct {
        ...
        components: safe.ArrayList(ParsedComponent),
    ...
    pub fn toCss(this: *const This, dest: *Printer) PrintErr!void {
        return switch (this.*) {
            ...
            .repeated => |*r| {
                var first = true;
                for (0..r.components.items.len) |__zust_i| {
    var component = &r.components.items[__zust_i];
                    if (!first) {
                        switch (r.multiplier) {
