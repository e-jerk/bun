...
// safe-transpile: function uses raw slice parameter — consider safe.String
            ...
            switch (CalcUnit.Map.getAnyCase(f) orelse return .{ .err = location.newUnexpectedTokenError(.{ .ident = f }) }) {
                ...
                .min => {
                    const Closure = struct {
                        ctx: @TypeOf(ctx),
                        pub fn parseNestedBlockFn(self: *@This(), i: *css.Parser) Result(safe.ArrayList(This)) {
                    ...
                    var closure = Closure{ .ctx = ctx };
                    var reduced = switch (input.parseNestedBlock(safe.ArrayList(This), &closure, Closure.parseNestedBlockFn)) {
                        .result => |vv| vv,
                        .err => |e| return .{ .err = e },
                    };
...
// safe-transpile: function uses raw slice parameter — consider safe.String
                    ...
                    const cmp = if (max != null and if (max) |value| {
    value
} else {
    return error.NullPointer;
} == .value and center == .value)
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        ...
        pub fn parseHypot(allocator: Allocator, args: *safe.ArrayList(This)) Result(?This) {
            if (args.items.len == 1) {
            ...
            var sum: This = first;
            for (0..args.items[i..].len) |__zust_i| {
    var arg = &args.items[i..][__zust_i];
                const Fn = struct {
        ...
        fn reduceArgs(allocator: Allocator, args: *safe.ArrayList(This), order: std.math.Order) void {
            // Reduces the arguments of a min() or max() expression, combining compatible values.
            // e.g. min(1px, 1em, 2px, 3in) => min(1px, 1em)
            var reduced = safe.ArrayList(This).empty;

            for (0..args.items.len) |__zust_i| {
    var arg = &args.items[__zust_i];
                var found: ??*This = null;
                switch (arg.*) {
                    .value => |val| {
                        for (reduced.items) |*b| {
...
pub fn MathFunction(comptime V: type) type {
    return union(enum) {
        ...
        calc: ThisCalc,
        /// The `min()` function.
        min: safe.ArrayList(ThisCalc),
        /// The `max()` function.
        max: safe.ArrayList(ThisCalc),
        /// The `clamp()` function.
        clamp: struct {
            min: ThisCalc,
            center: ThisCalc,
        ...
        pub fn isCompatible(this: *const @This(), browsers: css.targets.Browsers) bool {
            ...
            return switch (this.*) {
                ...
                .min => |*m| F.isCompatible(F.min_function, browsers) and brk: {
                    for (0..m.items.len) |__zust_i| {
    var arg = &m.items[__zust_i];
    ...
    var arg = &m.items[__zust_i];
                ...
                .hypot => |*h| F.isCompatible(F.hypot_function, browsers) and brk: {
                    for (0..h.items.len) |__zust_i| {
    var arg = &h.items[__zust_i];
                        if (!arg.isCompatible(browsers)) {
                            break :brk false;
                        }
                    }
...
fn arr2(allocator: std.mem.Allocator, a: anytype, b: anytype) safe.ArrayList(@TypeOf(a)) {
    const T = @TypeOf(a);
    if (T != @TypeOf(b)) {
        @compileError("arr2: types must match");
    }
    var arr = safe.ArrayList(T).empty;
