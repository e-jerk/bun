...
pub const CssColor = union(enum) {
    ...
    pub fn getFallback(this: *const @This(), allocator: Allocator, kind: ColorFallbackKind) CssColor {
        ...
        return switch (kind.asBits()) {
            ColorFallbackKind.RGB.asBits() => if (this.toRGB(allocator)) |value| {
    value
} else {
    return error.NullPointer;
},
            ColorFallbackKind.P3.asBits() => if (this.toP3(allocator)) |value| {
    value
} else {
    return error.NullPointer;
},
            ColorFallbackKind.LAB.asBits() => if (this.toLAB(allocator)) |value| {
    value
} else {
    return error.NullPointer;
},
    ...
    pub fn getFallbacks(this: *@This(), allocator: Allocator, targets: css.targets.Targets) css.SmallList(CssColor, 2) {
        ...
        if (fallbacks.rgb) {
            res.appendAssumeCapacity(if (this.toRGB(allocator)) |value| {
    value
} else {
    return error.NullPointer;
});
        ...
        if (fallbacks.p3) {
            res.appendAssumeCapacity(if (this.toP3(allocator)) |value| {
    value
} else {
    return error.NullPointer;
});
        ...
        if (fallbacks.lab) {
            const foo = if (this.toLAB(allocator)) |value| {
    value
} else {
    return error.NullPointer;
};
    ...
    pub fn getPossibleFallbacks(this: *const @This(), targets: css.targets.Targets) ColorFallbackKind {
        ...
        if (fallbacks.lab) {
            ...
            } else if (targets.browsers != null and css.compat.Feature.isPartiallyCompatible(&css.compat.Feature.lab_colors, if (targets.browsers) |value| {
    value
} else {
    return error.NullPointer;
})) {
        ...
        if (fallbacks.p3) {
            if (!targets.shouldCompileSame(.p3_colors)) {
                ...
                (targets.browsers == null or !css.compat.Feature.isPartiallyCompatible(&css.compat.Feature.p3_colors, if (targets.browsers) |value| {
    value
} else {
    return error.NullPointer;
})))
...
pub const ColorFallbackKind = packed struct(u8) {
    ...
    pub fn lowest(this: @This()) ColorFallbackKind {
        return bun.bits.@"and"(
            ColorFallbackKind,
            this,
            fromBitsTruncate(bun.wrappingNegation(@as(u8, // safe-transpile: @bitCast requires manual review
    @bitCast(this)))),
        );
    }
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    pub fn andBelow(this: @This()) ColorFallbackKind {
        ...
        return bun.bits.@"or"(ColorFallbackKind, this, fromBitsTruncate(@as(u8, // safe-transpile: @bitCast requires manual review
    @bitCast(this)) - 1));
    }
    ...
    pub inline fn fromBitsTruncate(b: u8) ColorFallbackKind {
        var cfk: ColorFallbackKind = // safe-transpile: @bitCast requires manual review
    @bitCast(b);
        cfk.__unused = 0;
        return cfk;
    }
...
pub fn parseColorMix(input: *css.Parser) Result(CssColor) {
    ...
    // https://drafts.csswg.org/css-color-5/#color-mix-percent-norm
    const p1: f32 = undefined, const p2: f32 = undefined = if (first_percent == null and second_percent == null) .{ @as(f32, 0.5), @as(f32, 0.5) } else brk: {
        const p2 = second_percent orelse (@as(f32, 1.0) - if (first_percent) |value| {
    value
} else {
    return error.NullPointer;
});
        const p1 = first_percent orelse (@as(f32, 1.0) - if (second_percent) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: function returns small constant slice — consider safe.String
    ...
    pub fn interpolate(
        ...
        if (this.* == .specified) {
            ...
            a[0] = @mod((@mod(a.*, 360.0) + 360.0), 360.0);
            b[0] = @mod((@mod(b.*, 360.0) + 360.0), 360.0);
        }
