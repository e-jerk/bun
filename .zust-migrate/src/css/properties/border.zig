...
const BorderShorthand = struct {
    ...
    fn toBorder(this: *const @This(), allocator: std.mem.Allocator) Border {
        return .{
            .width = if (css.generic.deepClone(@TypeOf(this.width), &this.width, allocator)) |value| {
    value
} else {
    return error.NullPointer;
},
            .style = if (css.generic.deepClone(@TypeOf(this.style), &this.style, allocator)) |value| {
    value
} else {
    return error.NullPointer;
},
            .color = if (css.generic.deepClone(@TypeOf(this.color), &this.color, allocator)) |value| {
    value
} else {
    return error.NullPointer;
},
...
const BorderProperty = packed struct(u32) {
    ...
    pub fn tryFromPropertyId(property_id: css.PropertyIdTag) ?@This() {
        ...
        inline for (comptime bun.meta.EnumFields(css.PropertyIdTag)) |field| {
...
// safe-transpile: optional unwrap requires manual review
                if (@field(@field(self, key), prop) != null and !@field(@field(self, key), prop).?.eql(val) and c.targets.browsers != null and !css.generic.isCompatible(@TypeOf(val.*), val, if (c.targets.browsers) |value| {
    value
} else {
    return error.NullPointer;
})) {
...
// safe-transpile: function uses raw slice parameter — consider safe.String
                inline fn shorthand(s: *@This(), comptime p: type, comptime prop_name: []const u8, comptime key: []const u8) void {
                    ...
                    if (has_prop) {
                        ...
                        {
                            const rect = p{
                                .top = if (bun.take(&@field(s.block_start, key))) |value| {
    value
} else {
    return error.NullPointer;
},
                                .right = if (bun.take(&@field(s.inline_end, key))) |value| {
    value
} else {
    return error.NullPointer;
},
                                .bottom = if (bun.take(&@field(s.block_end, key))) |value| {
    value
} else {
    return error.NullPointer;
},
                                .left = if (bun.take(&@field(s.inline_start, key))) |value| {
    value
} else {
    return error.NullPointer;
},
                ...
                inline fn logicalShorthand(
                    ...
                    if (has_prop) {
                        prop(s.f, prop_name, P{
                            .start = if (bun.take(&@field(start, key))) |value| {
    value
} else {
    return error.NullPointer;
},
                            .end = if (bun.take(&@field(end, key))) |value| {
    value
} else {
    return error.NullPointer;
},
...
// safe-transpile: function uses raw slice parameter — consider safe.String
                inline fn side_diff(s: *@This(), border: anytype, other: anytype, comptime prop_name: []const u8, width: anytype, style: anytype, comptime color: []const u8) void {
                    ...
                    if (eq_width and eq_style) {
                        s.f.prop(color, if (css.generic.deepClone(@TypeOf(other.color), &other.color, s.f.ctx.allocator)) |value| {
    value
} else {
    return error.NullPointer;
});
                    } else if (eq_width and eq_color) {
                        s.f.prop(style, if (css.generic.deepClone(@TypeOf(other.style), &other.style, s.f.ctx.allocator)) |value| {
    value
} else {
    return error.NullPointer;
});
                    } else if (eq_style and eq_color) {
                        s.f.prop(width, if (css.generic.deepClone(@TypeOf(other.width), &other.width, s.f.ctx.allocator)) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: function uses raw slice parameter — consider safe.String
                inline fn inlineProp(s: *@This(), comptime key: []const u8, comptime left: []const u8, comptime right: []const u8) void {
                    if (@field(s.inline_start, key) != null and css.generic.eql(@TypeOf(@field(s.inline_start, key)), &@field(s.inline_start, key), &@field(s.inline_end, key))) {
                        s.f.prop(left, if (bun.take(&@field(s.inline_start, key))) |value| {
    value
} else {
    return error.NullPointer;
});
                        s.f.prop(right, if (bun.take(&@field(s.inline_end, key))) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: optional unwrap requires manual review
    ...
    fn flushUnparsed(this: *@This(), unparsed: *const UnparsedProperty, dest: *css.DeclarationList, context: *css.PropertyHandlerContext) void {
        ...
        if (logical_supported) {
            ...
            bun.bits.insert(BorderProperty, &this.flushed_properties, if (BorderProperty.tryFromPropertyId(up.property_id)) |value| {
    value
} else {
    return error.NullPointer;
});
