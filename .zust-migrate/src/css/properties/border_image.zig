...
pub const BorderImage = struct {
    ...
    for (fallbacks.slice(), res.slice_mut()) |fallback, *out| {
            out[0] = this.deepClone(allocator);
            out.source = fallback;
        }
...
// safe-transpile: optional unwrap requires manual review
                if (@field(self, name) != null and !@field(self, name).?.eql(val) and ctx.targets.browsers != null and css.generic.isCompatible(@TypeOf(if (@field(self, name)) |value| {
    value
} else {
    return error.NullPointer;
}), val, if (ctx.targets.browsers) |value| {
    value
} else {
    return error.NullPointer;
})) {
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        ...
        switch (property.*) {
            ...
            .unparsed => |unparsed| {
                if (isBorderImageProperty(unparsed.property_id)) {
                    ...
                    bun.bits.insert(BorderImageProperty, &this.flushed_properties, if (BorderImageProperty.tryFromPropertyId(unparsed_clone.property_id)) |value| {
    value
} else {
    return error.NullPointer;
});
    ...
    fn flush(this: *@This(), dest: *css.DeclarationList, context: *css.PropertyHandlerContext) void {
        ...
        if (source != null and slice != null and width != null and outset != null and repeat != null) {
            var border_image = BorderImage{
                .source = if (source) |value| {
    value
} else {
    return error.NullPointer;
},
                .slice = if (slice) |value| {
    value
} else {
    return error.NullPointer;
},
    ...
    return error.NullPointer;
                ...
                .outset = if (outset) |value| {
    value
} else {
    return error.NullPointer;
},
                .repeat = if (repeat) |value| {
    value
} else {
    return error.NullPointer;
},
