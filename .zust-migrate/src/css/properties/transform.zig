...
pub const TransformList = struct {
    ...
    pub fn parse(input: *css.Parser) Result(@This()) {
        ...
        var results = safe.ArrayList(Transform).empty;
    ...
    var item = &this.v.items[__zust_i];
            try item.toCss(dest);
        }
    }
        ...
        switch (property.*) {
            .transform => |val| {
                ...
                if (this.transform) |*transform| {
                    transform[0] = .{ transform_val.deepClone(allocator), bun.bits.@"or"(css.VendorPrefix, transform.*[1], vp) };
                } else {
