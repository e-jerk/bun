...
// safe-transpile: function uses raw slice parameter — consider safe.String
    fn property(this: *@This(), dest: *css.DeclarationList, context: *css.PropertyHandlerContext, comptime feature: Feature, comptime prop: []const u8, val: anytype, vp: VendorPrefix) void {
        ...
        if (@field(this, prop)) |*p| {
            const v = &p.*[0];
            const prefixes = &p.*[1];
            v[0] = val.deepClone(context.allocator);
            bun.bits.insert(VendorPrefix, prefixes, vp);
            prefixes[0] = context.targets.prefixes(prefixes.*, feature);
        } else {
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    fn flush(this: *@This(), dest: *css.DeclarationList, context: *css.PropertyHandlerContext) void {
        ...
        if (_properties != null and _durations != null and _delays != null and _timing_functions != null) {
            const properties: *SmallList(PropertyId, 1) = &if (_properties) |value| {
    value
} else {
    return error.NullPointer;
}[0];
            const property_prefixes: *VendorPrefix = &if (_properties) |value| {
    value
} else {
    return error.NullPointer;
}[1];
            const durations: *SmallList(Time, 1) = &if (_durations) |value| {
    value
} else {
    return error.NullPointer;
}[0];
            const duration_prefixes: *VendorPrefix = &if (_durations) |value| {
    value
} else {
    return error.NullPointer;
}[1];
            const delays: *SmallList(Time, 1) = &if (_delays) |value| {
    value
} else {
    return error.NullPointer;
}[0];
            const delay_prefixes: *VendorPrefix = &if (_delays) |value| {
    value
} else {
    return error.NullPointer;
}[1];
            const timing_functions: *SmallList(EasingFunction, 1) = &if (_timing_functions) |value| {
    value
} else {
    return error.NullPointer;
}[0];
            const timing_prefixes: *VendorPrefix = &if (_timing_functions) |value| {
    value
} else {
    return error.NullPointer;
}[1];
    ...
    return error.NullPointer;
            ...
            const prefix: VendorPrefix = if (_properties) |value| {
    value
} else {
    return error.NullPointer;
}[1];
    ...
    return error.NullPointer;
            ...
            const prefix: VendorPrefix = if (_durations) |value| {
    value
} else {
    return error.NullPointer;
}[1];
    ...
    return error.NullPointer;
            ...
            const prefix: VendorPrefix = if (_delays) |value| {
    value
} else {
    return error.NullPointer;
}[1];
    ...
    return error.NullPointer;
            ...
            const prefix: VendorPrefix = if (_timing_functions) |value| {
    value
} else {
    return error.NullPointer;
}[1];
    ...
    inline fn getTransitions(
        ...
        for (0..properties.slice().len) |__zust_i| {
    var property_id = &properties.slice()[__zust_i];
