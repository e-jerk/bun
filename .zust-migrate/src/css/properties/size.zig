            ...
            .unparsed => |*unparsed| {
                switch (unparsed.property_id) {
                    .width, .height, .@"min-width", .@"max-width", .@"min-height", .@"max-height" => {
                        bun.bits.insert(SizeProperty, &this.flushed_properties, if (SizeProperty.tryFromPropertyIdTag(@as(PropertyIdTag, unparsed.property_id))) |value| {
    value
} else {
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    inline fn propertyHelper(
        ...
        if (@field(PropertyCategory, @tagName(category)) != this.category or (@field(this, property) != null and context.targets.browsers != null and !value.isCompatible(if (context.targets.browsers) |value| {
    value
} else {
    return error.NullPointer;
}))) {
