...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    fn flush(this: *@This(), dest: *css.DeclarationList, context: *css.PropertyHandlerContext) void {
        ...
        if (direction != null and wrap != null) {
            const dir: *FlexDirection = &if (direction) |value| {
    value
} else {
    return error.NullPointer;
}[0];
            const dir_prefix: *VendorPrefix = &if (direction) |value| {
    value
} else {
    return error.NullPointer;
}[1];
            const wrapinner: *FlexWrap = &if (wrap) |value| {
    value
} else {
    return error.NullPointer;
}[0];
            const wrap_prefix: *VendorPrefix = &if (wrap) |value| {
    value
} else {
    return error.NullPointer;
}[1];
        ...
        if (grow != null and shrink != null and basis != null) {
            const g = if (grow) |value| {
    value
} else {
    return error.NullPointer;
}[0];
            const g_prefix: *VendorPrefix = &if (grow) |value| {
    value
} else {
    return error.NullPointer;
}[1];
            const s = if (shrink) |value| {
    value
} else {
    return error.NullPointer;
}[0];
            const s_prefix: *VendorPrefix = &if (shrink) |value| {
    value
} else {
    return error.NullPointer;
}[1];
            const b = if (basis) |value| {
    value
} else {
    return error.NullPointer;
}[0];
            const b_prefix: *VendorPrefix = &if (basis) |value| {
    value
} else {
    return error.NullPointer;
}[1];
