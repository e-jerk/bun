...
pub const Property = union(PropertyIdTag) {
    ...
    comptime {
        const compile_error: safe.Slice(u8) = compile_error: {
            var compile_error: safe.Slice(u8) = "";
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    inline for (comptime bun.meta.EnumFields(PropertyId), comptime std.meta.fields(PropertyId)) |enum_field, union_field| {
    ...
    pub fn setPrefixesForTargets(this: *PropertyId, targets: Targets) void {
        switch (this.*) {
            ...
            .@"background-attachment" => {},
            .@"background-clip" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.background_clip);
            },

            .@"background-origin" => {},
            .background => {},
            .@"box-shadow" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.box_shadow);
            },
            ...
            .@"max-inline-size" => {},
            .@"box-sizing" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.box_sizing);
            },
            ...
            .@"overflow-y" => {},
            .@"text-overflow" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.text_overflow);
            },
            ...
            .@"border-top-left-radius" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.border_top_left_radius);
            },

            .@"border-top-right-radius" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.border_top_right_radius);
            },

            .@"border-bottom-left-radius" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.border_bottom_left_radius);
            },

            .@"border-bottom-right-radius" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.border_bottom_right_radius);
            },
            ...
            .@"border-end-end-radius" => {},
            .@"border-radius" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.border_radius);
            },
            ...
            .@"border-image-slice" => {},
            .@"border-image" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.border_image);
            },
            ...
            .@"outline-width" => {},
            .@"flex-direction" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.flex_direction);
            },

            .@"flex-wrap" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.flex_wrap);
            },

            .@"flex-flow" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.flex_flow);
            },

            .@"flex-grow" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.flex_grow);
            },

            .@"flex-shrink" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.flex_shrink);
            },

            .@"flex-basis" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.flex_basis);
            },

            .flex => |*x| {
                x[0] = targets.prefixes(x.*, Feature.flex);
            },

            .order => |*x| {
                x[0] = targets.prefixes(x.*, Feature.order);
            },

            .@"align-content" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.align_content);
            },

            .@"justify-content" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.justify_content);
            },

            .@"place-content" => {},
            .@"align-self" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.align_self);
            },

            .@"justify-self" => {},
            .@"place-self" => {},
            .@"align-items" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.align_items);
            },
            ...
            .@"transition-property" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.transition_property);
            },

            .@"transition-duration" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.transition_duration);
            },

            .@"transition-delay" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.transition_delay);
            },

            .@"transition-timing-function" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.transition_timing_function);
            },

            .transition => |*x| {
                x[0] = targets.prefixes(x.*, Feature.transition);
            },

            .transform => |*x| {
                x[0] = targets.prefixes(x.*, Feature.transform);
            },

            .@"transform-origin" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.transform_origin);
            },

            .@"transform-style" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.transform_style);
            },

            .@"transform-box" => {},
            .@"backface-visibility" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.backface_visibility);
            },

            .perspective => |*x| {
                x[0] = targets.prefixes(x.*, Feature.perspective);
            },

            .@"perspective-origin" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.perspective_origin);
            },
            ...
            .@"text-decoration-color" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.text_decoration_color);
            },

            .@"text-emphasis-color" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.text_emphasis_color);
            },
            ...
            .composes => {},
            .@"mask-image" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.mask_image);
            },

            .@"mask-mode" => {},
            .@"mask-repeat" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.mask_repeat);
            },

            .@"mask-position-x" => {},
            .@"mask-position-y" => {},
            .@"mask-position" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.mask_position);
            },

            .@"mask-clip" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.mask_clip);
            },

            .@"mask-origin" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.mask_origin);
            },

            .@"mask-size" => |*x| {
                x[0] = targets.prefixes(x.*, Feature.mask_size);
            },

            .@"mask-composite" => {},
            .@"mask-type" => {},
            .mask => |*x| {
                x[0] = targets.prefixes(x.*, Feature.mask);
            },
