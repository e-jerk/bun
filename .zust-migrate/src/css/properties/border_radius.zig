...
pub const BorderRadiusHandler = struct {
    ...
    fn flush(this: *@This(), dest: *css.DeclarationList, context: *css.PropertyHandlerContext) void {
        ...
        if (top_left != null and top_right != null and bottom_right != null and bottom_left != null) {
            const intersection = if (top_left) |value| {
    value
} else {
    return error.NullPointer;
}[1].bitwiseAnd(if (top_right) |value| {
    value
} else {
    return error.NullPointer;
}[1]).bitwiseAnd(if (bottom_right) |value| {
    value
} else {
    return error.NullPointer;
}[1]).bitwiseAnd(if (bottom_left) |value| {
    value
} else {
    return error.NullPointer;
}[1]);
            if (!intersection.isEmpty()) {
                ...
                dest.append(context.allocator, Property{ .@"border-radius" = .{
                    BorderRadius{
                        .top_left = if (top_left) |value| {
    value
} else {
    return error.NullPointer;
}[0].deepClone(context.allocator),
                        .top_right = if (top_right) |value| {
    value
} else {
    return error.NullPointer;
}[0].deepClone(context.allocator),
                        .bottom_right = if (bottom_right) |value| {
    value
} else {
    return error.NullPointer;
}[0].deepClone(context.allocator),
                        .bottom_left = if (bottom_left) |value| {
    value
} else {
    return error.NullPointer;
}[0].deepClone(context.allocator),
                ...
                bun.bits.remove(VendorPrefix, &if (top_left) |value| {
    value
} else {
    return error.NullPointer;
}[1], intersection);
                bun.bits.remove(VendorPrefix, &if (top_right) |value| {
    value
} else {
    return error.NullPointer;
}[1], intersection);
                bun.bits.remove(VendorPrefix, &if (bottom_right) |value| {
    value
} else {
    return error.NullPointer;
}[1], intersection);
                bun.bits.remove(VendorPrefix, &if (bottom_left) |value| {
    value
} else {
    return error.NullPointer;
}[1], intersection);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    fn maybeFlush(self: *BorderRadiusHandler, d: *css.DeclarationList, ctx: *css.PropertyHandlerContext, comptime prop: []const u8, val: anytype, vp: css.VendorPrefix) void {
        ...
        if (@field(self, prop) != null and ctx.targets.browsers != null and !css.generic.isCompatible(Size2D(LengthPercentage), val, if (ctx.targets.browsers) |value| {
    value
} else {
    return error.NullPointer;
})) {
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    fn propertyHelper(self: *BorderRadiusHandler, d: *css.DeclarationList, ctx: *css.PropertyHandlerContext, comptime prop: []const u8, val: *const Size2D(LengthPercentage), vp: css.VendorPrefix) void {
        ...
        if (@field(self, prop)) |*existing| {
            existing[0] = .{ val.deepClone(ctx.allocator), vp };
        } else {
