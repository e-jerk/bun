...
pub const BoxShadowHandler = struct {
    ...
    pub fn handleProperty(this: *@This(), property: *const Property, dest: *css.DeclarationList, context: *css.PropertyHandlerContext) bool {
        switch (property.*) {
            .@"box-shadow" => |*b| {
                ...
                if (this.box_shadows != null and context.targets.browsers != null and !box_shadows.isCompatible(if (context.targets.browsers) |value| {
    value
} else {
    return error.NullPointer;
                ...
                if (this.box_shadows) |*bxs| {
                    ...
                    if (!val.eql(box_shadows) and !bun.bits.contains(VendorPrefix, prefixes.*, prefix)) {
                        ...
                        val[0] = box_shadows.deepClone(context.allocator);
    ...
    pub fn flush(this: *@This(), dest: *css.DeclarationList, context: *css.PropertyHandlerContext) void {
        ...
        const box_shadows: SmallList(BoxShadow, 1) = undefined, const prefixes2: VendorPrefix = undefined = bun.take(&this.box_shadows) orelse {
            this.flushed = true;
            return;
        };
        ...
        if (!this.flushed) {
            ...
            for (0..box_shadows.slice().len) |__zust_i| {
    var shadow = &box_shadows.slice()[__zust_i];
    ...
    for (box_shadows.slice(), rgb.slice_mut()) |*input, *output| {
                    ...
                    inline for (fields) |field| {
                        if (comptime safe.SimdUtils.eql(field.name, "color")) continue;
