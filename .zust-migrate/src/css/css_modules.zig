...
pub const CssModule = struct {
    config: *const Config,
    sources: *const safe.ArrayList([]const u8),
    hashes: safe.ArrayList([]const u8),
    exports_by_source_index: safe.ArrayList(CssModuleExports),
    ...
    pub fn new(
        ...
        const hashes = hashes: {
            var hashes = bun.handleOom(safe.ArrayList([]const u8).initCapacity(allocator, sources.items.len));
        ...
        const exports_by_source_index = exports_by_source_index: {
            var exports_by_source_index = bun.handleOom(safe.ArrayList(CssModuleExports).initCapacity(allocator, sources.items.len));
    ...
    pub fn getReference(this: *CssModule, allocator: Allocator, name: []const u8, source_index: u32) void {
        ...
        if (gop.found_existing) {
            ...
            gop.value_ptr.* = CssModuleExport{
                .name = this.config.pattern.writeToString(allocator, safe.ArrayList(u8).empty, this.hashes.items[source_index], this.sources.items[source_index], name),
                .composes = .empty,
                .is_referenced = true,
            };
        }
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn referenceDashed(
        ...
        const reference, const key = if (from.*) |specifier| switch (specifier) {
            ...
            if (gop.found_existing) {
                ...
                var res = safe.ArrayList(u8).empty;
    ...
    var sel = &selectors.v.slice()[__zust_i];
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        ...
        for (0..this.segments.slice().len) |__zust_i| {
    var segment = &this.segments.slice()[__zust_i];
            switch (segment.*) {
                .literal => |s| {
...
// safe-transpile: function returns small constant slice — consider safe.String
    pub fn writeToStringWithPrefix(
        ...
        const Closure = struct { res: safe.ArrayList(u8), allocator: Allocator };
        var closure = Closure{ .res = safe.ArrayList(u8).empty, .allocator = allocator };
        this.write(
            hash_,
            path,
            local,
...
// safe-transpile: function uses raw slice parameter — consider safe.String
                pub fn writefn(self: *Closure, slice: []const u8, replace_dots: bool) void {
                    ...
                    if (replace_dots) {
                        ...
                        for (0..self.res.items[start..end].len) |__zust_i| {
    var c = &self.res.items[start..end][__zust_i];
                            if (c.* == '.') {
                                c.* = '-';
                            }
                        }
...
pub const CssModuleExport = struct {
    ...
    composes: safe.ArrayList(CssModuleReference),
