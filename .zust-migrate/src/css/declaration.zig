...
pub const DeclarationList = safe.ArrayList(css.Property);
...
pub const DeclarationBlock = struct {
    ...
    important_declarations: safe.ArrayList(css.Property) = .empty,
    /// A list of normal declarations in the block.
    declarations: safe.ArrayList(css.Property) = .empty,
    ...
    pub fn toCss(this: *const This, dest: *Printer) PrintErr!void {
        ...
        inline for (DECLS) |decl_field_name| {
            ...
            const is_important = comptime safe.SimdUtils.eql(decl_field_name, "important_declarations");
...
// safe-transpile: function uses raw slice parameter — consider safe.String
            inline fn handle(
                ...
                for (0..@field(self, decl_field).items.len) |__zust_i| {
    var prop = &@field(self, decl_field).items[__zust_i];
    ...
    pub fn hashPropertyIds(this: *const @This(), hasher: *std.hash.Wyhash) void {
        for (0..this.declarations.items.len) |__zust_i| {
    var decl = &this.declarations.items[__zust_i];
            decl.propertyId().hash(hasher);
        }

        for (0..this.important_declarations.items.len) |__zust_i| {
    var decl = &this.important_declarations.items[__zust_i];
            decl.propertyId().hash(hasher);
        }
    }
