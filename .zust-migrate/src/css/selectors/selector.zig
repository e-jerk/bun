...
pub fn downlevelSelectors(allocator: Allocator, selectors: []Selector, targets: css.targets.Targets) css.VendorPrefix {
    ...
    for (0..selectors.len) |__zust_i| {
    var selector = &selectors[__zust_i];
...
pub fn downlevelComponent(allocator: Allocator, component: *Component, targets: css.targets.Targets) css.VendorPrefix {
            ...
            return switch (pc.*) {
                .dir => |*d| {
                    if (targets.shouldCompileSame(.dir_selector)) {
                        component[0] = downlevelDir(allocator, d.direction, targets);
                ...
                .lang => |l| {
                    ...
                    if (l.languages.items.len > 1 and targets.shouldCompileSame(.lang_selector_list)) {
                        component[0] = .{ .is = langListToSelectors(allocator, l.languages.items) };
                        return downlevelComponent(allocator, component, targets);
                    }
                    return css.VendorPrefix{};
                },
    ...
    for (new_selectors, selectors) |*new, *sel| {
                        new[0] = sel.deepClone(allocator);
                    }
                    break :selectors new_selectors;
                } });
                var list = bun.handleOom(safe.ArrayList(Selector).initCapacity(allocator, 1));
                list.appendAssumeCapacity(is);
                component[0] = .{ .negation = list.items };
    ...
    if (!targets.shouldCompileSame(.lang_selector_list)) {
        const c = Component{
            .non_ts_pseudo_class = PseudoClass{
                .lang = .{ .languages = lang: {
                    var list = bun.handleOom(safe.ArrayList([]const u8).initCapacity(allocator, RTL_LANGS.len));
...
fn langListToSelectors(allocator: Allocator, langs: []const []const u8) []Selector {
    ...
    for (langs, selectors[0..]) |lang, *sel| {
        sel[0] = Selector.fromComponent(allocator, Component{
            .non_ts_pseudo_class = PseudoClass{
                .lang = .{ .languages = langs: {
                    var list = bun.handleOom(safe.ArrayList([]const u8).initCapacity(allocator, 1));
...
pub fn getPrefix(selectors: *const SelectorList) css.VendorPrefix {
    ...
    for (0..selectors.v.slice().len) |__zust_i| {
    var selector = &selectors.v.slice()[__zust_i];
...
fn isSelectorUnused(
    ...
    for (0..selector.components.items.len) |__zust_i| {
    var component = &selector.components.items[__zust_i];
        switch (component.*) {
            .class, .id => |ident| {
...
pub const serialize = struct {
    ...
    pub fn serializeSelector(
        ...
        if (comptime bun.Environment.isDebug) {
            ...
            for (0..selector.components.items.len) |__zust_i| {
    var comp = &selector.components.items[__zust_i];
                debug(" {f}\n", .{comp});
            }
    ...
    var simple = &slice[__zust_i];
            ...
            if (perform_step_2) {
                ...
                if (i < compound.len) {
                    for (0..iter[i..].len) |__zust_i| {
    var simple = &iter[i..][__zust_i];
...
pub const tocss_servo = struct {
    pub fn toCss_SelectorList(
        ...
        if (selectors.len > 1) {
            for (0..selectors[1..].len) |__zust_i| {
    var selector = &selectors[1..][__zust_i];
                try dest.writeStr(", ");
    ...
    pub fn toCss_Selector(
        ...
        while (compound_selectors.next()) |compound| {
            ...
            // If we are in this case, after we have serialized the universal
            // selector, we skip Step 2 and continue with the algorithm.
            const can_elide_namespace, const first_non_namespace: usize = safe.CheckedInt(usize).init(0) = if (0 >= compound.len)
                .{ true, 0 }
            else switch (compound[0]) {
            ...
            if (first_non_namespace == compound.len - 1) {
                ...
                if (next_combinator == .pseudo_element and compound[first_non_namespace].asCombinator() == .slot_assignment) {
                    ...
                    // Iterate over everything so we serialize the namespace
                    // too.
                    for (0..compound.len) |__zust_i| {
    var simple = &compound[__zust_i];
