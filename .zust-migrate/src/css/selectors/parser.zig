...
pub const SELECTOR_WHITESPACE: safe.Slice(u8) = &[_]u8{ ' ', '\t', '\n', '\r', 0x0C };
...
fn compute_complex_selector_specificity(comptime Impl: type, iter: []const GenericComponent(Impl)) Specificity {
    ...
    for (0..iter.len) |__zust_i| {
    var simple_selector = &iter[__zust_i];
...
fn compute_simple_selector_specificity(
    ...
    switch (simple_selector.*) {
        ...
        .nth_of => |nth_of_data| {
            ...
            specificity.class_like_selectors += 1;
            var max: u32 = 0;
            for (0..nth_of_data.selectors.len) |__zust_i| {
    var selector = &nth_of_data.selectors[__zust_i];
...
fn parse_selector(
    ...
    outer_loop: var __zust_loop_counter: u64 = 0;
    while (true) {
...
pub const PseudoClass = union(enum) {
    ...
    lang: struct {
        ...
        languages: safe.ArrayList([]const u8),
    ...
    pub fn getNecessaryPrefixes(this: *PseudoClass, targets: css.targets.Targets) css.VendorPrefix {
        ...
        const p: *css.VendorPrefix = undefined, const feature: F = undefined = switch (this.*) {
        ...
        p[0] = targets.prefixes(p.*, feature);
        return p.*;
    }
    ...
    pub fn parseNonTsFunctionalPseudoClass(
        ...
        const pseudo_class = pseudo_class: {
            if (bun.strings.eqlCaseInsensitiveASCIIICheckLength(name, "lang")) {
                ...
                var args = safe.ArrayList(css.css_properties.custom.TokenOrValue).empty;
...
pub fn GenericSelectorList(comptime Impl: type) type {
    ...
    for (this.this.v.slice(), 0..) |*sel, i| {
        ...
        pub fn anyHasPseudoElement(this: *const This) bool {
            for (0..this.v.slice().len) |__zust_i| {
    var sel = &this.v.slice()[__zust_i];
        ...
        pub fn specifitiesAllEqual(this: *const This) bool {
            ...
            for (0..this.v.slice()[1..].len) |__zust_i| {
    var sel = &this.v.slice()[1..][__zust_i];
...
pub fn GenericSelector(comptime Impl: type) type {
    ...
    return struct {
        ...
        components: safe.ArrayList(GenericComponent(Impl)),
    ...
    for (this.components.items, 0..) |*comp, i| {
        ...
        pub fn hasCombinator(this: *const This) bool {
            for (0..this.components.items.len) |__zust_i| {
    var c = &this.components.items[__zust_i];
...
pub fn parse_one_simple_selector(
    ...
    switch (token) {
        ...
        .colon => {
            ...
            const is_single_colon: bool = undefined, const next_token: css.Token = undefined = switch ((switch (input.nextIncludingWhitespace()) {
                .err => |e| return .{ .err = e },
                .result => |v| v,
            }).*) {
            ...
            const name: safe.Slice(u8) = undefined, const is_functional = switch (next_token) {
                    ...
                    if (parser.parsePart() and bun.strings.eqlCaseInsensitiveASCIIICheckLength(name, "part")) {
                        ...
                        const Closure = struct {
                            ...
                            pub fn parsefn(self: *const @This(), input2: *css.Parser) Result([]Impl.SelectorImpl.Identifier) {
                                ...
                                var result = safe.ArrayList(Impl.SelectorImpl.Identifier).initCapacity(
...
pub fn parse_attribute_selector(comptime Impl: type, parser: *SelectorParser, input: *css.Parser) Result(GenericComponent(Impl)) {
    ...
    const namespace: ?N = undefined, const local_name: safe.Slice(u8) = undefined = brk: {
        input.skipWhitespace();
    ...
    const value_str: safe.Slice(u8) = switch (input.expectIdentOrString()) {
        .result => |v| v,
        .err => |e| {
    ...
    const local_name_lower: Impl.SelectorImpl.LocalName = undefined, const local_name_is_ascii_lowercase: bool = undefined = brk: {
        if (a: {
            // safe-transpile: for with index access requires manual review
