...
pub fn SelectorBuilder(comptime Impl: type) type {
    ...
    return struct {
        ...
        pub fn buildWithSpecificityAndFlags(this: *This, spec: SpecificityAndFlags) BuildResult {
            ...
            const rest: []const T = undefined, const current: []const T = undefined = splitFromEnd(T, this.simple_selectors.slice(), this.current_len);
            ...
            var components = safe.ArrayList(T).empty;
    ...
    while (true) {
        ...
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
                ...
                if (current_simple_selectors_i < current_simple_selectors.len) {
                    ...
                    if (combinator_i >= 0) {
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                        const combo: Combinator = undefined, const len: usize = safe.CheckedInt(usize).init(0) = combinators[@intCast(combinator_i)];
