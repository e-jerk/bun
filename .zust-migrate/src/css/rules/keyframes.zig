...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    pub const QualifiedRuleParser = struct {
        pub const Prelude = safe.ArrayList(KeyframeSelector);
...
pub const Keyframe = struct {
    ...
    selectors: safe.ArrayList(KeyframeSelector),
...
pub const KeyframesRule = struct {
    ...
    keyframes: safe.ArrayList(Keyframe),
    ...
    pub fn toCss(this: *const This, dest: *Printer) PrintErr!void {
        ...
        inline for (.{ "webkit", "moz", "ms", "o", "none" }) |prefix_name| {
            if (@field(this.vendor_prefix, prefix_name)) {
                ...
                var first = true;
                for (0..this.keyframes.items.len) |__zust_i| {
    var keyframe = &this.keyframes.items[__zust_i];
                    if (first) {
                        first = false;
                    } else if (!dest.minify) {
                        try dest.writeChar('\n'); // no indent
