...
pub const FontFaceProperty = union(enum) {
    ...
    unicode_range: safe.ArrayList(UnicodeRange),
...
pub const UrlSource = struct {
    ...
    tech: safe.ArrayList(FontTechnology),
    ...
    pub fn parse(input: *css.Parser) Result(UrlSource) {
        ...
        const tech = if (input.tryParse(css.Parser.expectFunctionMatching, .{"tech"}).isOk()) tech: {
            const Fn = struct {
                pub fn parseNestedBlockFn(_: void, i: *css.Parser) Result(safe.ArrayList(FontTechnology)) {
                    return i.parseList(FontTechnology, FontTechnology.parse);
                }
            };
            break :tech switch (input.parseNestedBlock(safe.ArrayList(FontTechnology), {}, Fn.parseNestedBlockFn)) {
                .result => |vv| vv,
                .err => |e| return .{ .err = e },
            };
        } else safe.ArrayList(FontTechnology).empty;
...
pub const FontFaceRule = struct {
    ...
    properties: safe.ArrayList(FontFaceProperty),
