...
pub const SupportsCondition = union(enum) {
    ...
    /// An `and` expression.
    @"and": safe.ArrayList(SupportsCondition),

    /// An `or` expression.
    @"or": safe.ArrayList(SupportsCondition),
    ...
    pub fn parse(input: *css.Parser) Result(SupportsCondition) {
        ...
        var conditions = safe.ArrayList(SupportsCondition).empty;
    ...
    while (true) {
        ...
        if (conditions.items.len == 1) {
            const ret = if (conditions.pop()) |value| {
    value
} else {
    ...
    pub fn toCss(this: *const SupportsCondition, dest: *css.Printer) css.PrintErr!void {
        switch (this.*) {
            ...
            .@"and" => |conditions| {
                var first = true;
                for (0..conditions.items.len) |__zust_i| {
    var cond = &conditions.items[__zust_i];
                    if (first) {
                        first = false;
                    } else {
