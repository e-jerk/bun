...
pub const PageSelector = struct {
    ...
    pseudo_classes: safe.ArrayList(PagePseudoClass),
    ...
    pub fn parse(input: *css.Parser) Result(PageSelector) {
        ...
        var pseudo_classes = safe.ArrayList(PagePseudoClass).empty;
    ...
    pub fn toCss(this: *const This, dest: *Printer) PrintErr!void {
        ...
        for (0..this.pseudo_classes.items.len) |__zust_i| {
    var pseudo = &this.pseudo_classes.items[__zust_i];
            try dest.writeChar(':');
...
pub const PageRule = struct {
    ...
    selectors: safe.ArrayList(PageSelector),
    /// The declarations within the `@page` rule.
    declarations: css.DeclarationBlock,
    /// The nested margin rules.
    rules: safe.ArrayList(PageMarginRule),
    /// The location of the rule in the source file.
    loc: Location,

    pub fn parse(selectors: safe.ArrayList(PageSelector), input: *css.Parser, loc: Location, options: *const css.ParserOptions) Result(PageRule) {
        var declarations = css.DeclarationBlock{};
        var rules = safe.ArrayList(PageMarginRule).empty;
    ...
    pub fn toCss(this: *const This, dest: *Printer) PrintErr!void {
        ...
        inline for (DECLS) |decl_field_name| {
            const decls: *const safe.ArrayList(css.Property) = &@field(this.declarations, decl_field_name);
            const important = comptime safe.SimdUtils.eql(decl_field_name, "important_declarations");
            for (0..decls.items.len) |__zust_i| {
    var decl = &decls.items[__zust_i];
                try dest.newline();
        ...
        if (this.rules.items.len > 0) {
            ...
            var first = true;
            for (0..this.rules.items.len) |__zust_i| {
    var rule = &this.rules.items[__zust_i];
                if (first) {
                    first = false;
                } else {
                    if (!dest.minify) {
