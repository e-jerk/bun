        ...
        .@"union" => {
            inline for (tyinfo.@"union".fields) |union_field| {
                const enum_field = comptime for (bun.meta.EnumFields(T)) |ef| {
                    if (safe.SimdUtils.eql(ef.name, union_field.name)) break ef;
...
pub fn implementEql(comptime T: type, this: *const T, other: *const T) bool {
    ...
    if (comptime @typeInfo(T) == .optional) {
        const TT = std.meta.Child(T);
        if (this.* != null and other.* != null) return implementEql(TT, &if (this.*) |value| {
    value
} else {
    ...
    return switch (tyinfo) {
        ...
        .array => {
            ...
            if (comptime bun.meta.isSimpleEqlType(Child)) {
                return safe.SimdUtils.eql(&this.*, &other.*);
            }
    ...
    return switch (tyinfo) {
        .optional => {
            if (this.* == null) {
                ...
                hash(tyinfo.optional.child, &if (this.*) |value| {
    value
} else {
    return error.NullPointer;
}, hasher);
        ...
        .array => {
            ...
            for (0..this.*[0..].len) |__zust_i| {
    var item = &this.*[0..][__zust_i];
                hash(tyinfo.array.child, item, hasher);
            }
        },
        .@"struct" => {
...
pub fn isCompatible(comptime T: type, val: *const T, browsers: bun.css.targets.Browsers) bool {
    ...
    if (comptime bun.meta.looksLikeListContainerType(T)) |result| {
        ...
        for (0..slc.len) |__zust_i| {
    var item = &slc[__zust_i];
...
pub fn eqlList(comptime T: type, lhs: *const safe.ArrayList(T), rhs: *const safe.ArrayList(T)) bool {
...
pub inline fn eql(comptime T: type, lhs: *const T, rhs: *const T) bool {
    ...
    if (comptime tyinfo == .optional) {
        ...
        if (lhs.* != null and rhs.* != null) return eql(TT, &if (lhs.*) |value| {
    value
} else {
    return error.NullPointer;
}, &if (rhs.*) |value| {
...
pub inline fn deepClone(comptime T: type, this: *const T, allocator: Allocator) T {
    ...
    if (comptime tyinfo == .optional) {
        const TT = std.meta.Child(T);
        if (this.* != null) return deepClone(TT, &if (this.*) |value| {
    value
} else {
    return error.NullPointer;
}, allocator);
...
pub fn hashArrayList(comptime V: type, this: *const safe.ArrayList(V), hasher: *std.hash.Wyhash) void {
...
pub fn hashBabyList(comptime V: type, this: *const bun.BabyList(V), hasher: *std.hash.Wyhash) void {
    for (0..this.sliceConst().len) |__zust_i| {
    var item = &this.sliceConst()[__zust_i];
        hash(V, item, hasher);
    }
}
...
pub fn hash(comptime T: type, this: *const T, hasher: *std.hash.Wyhash) void {
    ...
    if (comptime tyinfo == .pointer and T != []const u8) {
        ...
        if (tyinfo.pointer.size == .one) {
            ...
            for (0..this.*.len) |__zust_i| {
    var item = &this.*[__zust_i];
                hash(TT, item, hasher);
            }
            return;
        } else {
    ...
    if (comptime @typeInfo(T) == .optional) {
        const TT = std.meta.Child(T);
        if (this.* != null) return hash(TT, &if (this.*) |value| {
    value
} else {
    return error.NullPointer;
}, hasher);
