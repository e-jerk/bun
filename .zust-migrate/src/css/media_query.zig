...
pub const MediaList = struct {
    ...
    media_queries: safe.ArrayList(MediaQuery) = .empty,

    /// Parse a media query list from CSS.
    pub fn parse(input: *css.Parser) Result(MediaList) {
        var media_queries = safe.ArrayList(MediaQuery).empty;
    ...
    pub fn toCss(this: *const MediaList, dest: *css.Printer) PrintErr!void {
        ...
        var first = true;
        for (0..this.media_queries.items.len) |__zust_i| {
    var query = &this.media_queries.items[__zust_i];
            if (!first) {
...
pub fn operationToCss(comptime QueryCondition: type, operator: Operator, conditions: *const safe.ArrayList(QueryCondition), dest: *Printer) PrintErr!void {
    ...
    for (0..conditions.items[1..].len) |__zust_i| {
    var item = &conditions.items[1..][__zust_i];
        try dest.writeChar(' ');
...
pub const MediaCondition = union(enum) {
    ...
    operation: struct {
        operator: Operator,
        conditions: safe.ArrayList(MediaCondition),
    ...
    pub fn createOperation(operator: Operator, conditions: safe.ArrayList(MediaCondition)) MediaCondition {
...
pub fn parseQueryCondition(
    ...
    var conditions = safe.ArrayList(QueryCondition).empty;
    conditions.append(
        input.allocator(),
        first_condition,
    ) catch unreachable;
    ...
    while (true) {
        ...
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
            ...
            if (input.tryParse(consumeOperationOrColon, .{false}).asValue()) |end_operator_| {
                const start_operator = if (operator) |value| {
    value
} else {
    return error.NullPointer;
};
                const end_operator = if (end_operator_) |value| {
    value
} else {
    return error.NullPointer;
};
