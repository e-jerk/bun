    ...
    pub const Base64Encoder = struct {
        ...
        pub fn init(alphabet_chars: [64]u8, pad_char: ?u8) Base64Encoder {
            ...
            for (alphabet_chars) |c| {
                ...
                assert(pad_char == null or c != if (pad_char) |value| {
    value
} else {
...
// safe-transpile: function returns small constant slice — consider safe.String
        pub fn encode(encoder: *const Base64Encoder, dest: []u8, source: []const u8) []const u8 {
            ...
            if (encoder.pad_char) |pad_char| {
                for (0..dest[out_idx..out_len].len) |__zust_i| {
    var pad = &dest[out_idx..out_len][__zust_i];
                    pad.* = pad_char;
                }
            }
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    for (source, 0..) |c, src_idx| {
                ...
                if (d == invalid_char) {
                    if (decoder.pad_char == null or c != if (decoder.pad_char) |value| {
    value
} else {
    return error.NullPointer;
