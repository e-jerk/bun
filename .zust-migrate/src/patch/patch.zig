const WHITESPACE: safe.Slice(u8) = " \t\n\r";
...
pub const PatchFile = struct {
    ...
    pub fn deinit(this: *PatchFile, allocator: Allocator) void {
        for (0..this.parts.items.len) |__zust_i| { var part = &this.parts.items[__zust_i]; part.deinit(allocator) };
        ...
        fn patchDirAbsPath(state: *@This(), fd: bun.FD) bun.sys.Maybe([:0]const u8) {
            ...
            return switch (bun.sys.getFdPath(fd, &state.pathbuf)) {
                .result => |p| {
                    ...
                    return .{ .result = if (state.patch_dir_abs_path) |value| {
    value
} else {
    ...
    pub fn apply(this: *const PatchFile, allocator: Allocator, patch_dir: bun.FD) ?bun.sys.Error {
        ...
        for (0..this.parts.items.len) |__zust_i| {
    var part = &this.parts.items[__zust_i];
    ...
    fn applyPatch(
        ...
        const lines_count = brk: {
            ...
            for (0..patch.hunks.items.len) |__zust_i| {
    var hunk = &patch.hunks.items[__zust_i];
...
const FileDeets = struct {
    ...
    fn deinit(this: *FileDeets, allocator: Allocator) void {
        for (0..this.hunks.items.len) |__zust_i| {
    var hunk = &this.hunks.items[__zust_i];
...
pub const FilePatch = struct {
    ...
    pub fn deinit(this: *FilePatch, allocator: Allocator) void {
        for (0..this.hunks.items.len) |__zust_i| { var hunk = &this.hunks.items[__zust_i]; hunk.deinit(allocator) };
        this.hunks.deinit(allocator);
        bun.destroy(this);
    }
};
...
fn patchFileSecondPass(files: []FileDeets) ParseErr!PatchFile {
    ...
    for (0..files.len) |__zust_i| {
    var file = &files[__zust_i];
...
const PatchLinesParser = struct {
    ...
    fn deinit(this: *PatchLinesParser, allocator: Allocator, comptime clear_result_retaining_capacity: bool) void {
        ...
        for (0..this.result.items.len) |__zust_i| { var file_deet = &this.result.items[__zust_i]; file_deet.deinit(allocator) };
...
// safe-transpile: optional unwrap requires manual review
                                this.current_hunk.?.parts.append(bun.default_allocator, if (this.current_hunk_mutation_part) |value| {
    value
} else {
    return error.NullPointer;
}) catch unreachable;
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    fn parseHunkHeaderLineImpl(text_: []const u8) ParseErr!struct { line_nr: u32, line_count: u32, rest: []const u8 } {
        ...
        var line_nr_count: safe.Slice(u8) = "1";
...
pub fn spawnOpts(
    ...
    const envp: [:null]?[*:0]const u8 = brk: {
        ...
        if (PATH) |p| {
            envp_buf[envp_buf.len - 1] = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(p.ptr);
        }
        break :brk envp_buf;
    };
...
pub fn gitDiffPreprocessPaths(
    ...
    const old_folder = if (comptime bun.Environment.isWindows) brk: {
        ...
        safe.SimdUtils.copy(cpy[0..old_folder_.len], old_folder_);
    ...
    const new_folder = if (comptime bun.Environment.isWindows) brk: {
        ...
        safe.SimdUtils.copy(cpy[0..new_folder_.len], new_folder_);
        std.mem.replaceScalar(u8, cpy, '\\', '/');
        if (sentinel) {
...
fn gitDiffPostprocess(stdout: *std.array_list.Managed(u8), old_folder: []const u8, new_folder: []const u8) !void {
    ...
    var old_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
    var new_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);

    const @"a/$old_folder/", const @"b/$new_folder/" = brk: {
        old_buf[0] = 'a';
        old_buf[1] = '/';
        safe.SimdUtils.copy(old_buf[2..][0..old_folder_trimmed.len], old_folder_trimmed);
        old_buf[2 + old_folder_trimmed.len] = '/';

        new_buf[0] = 'b';
        new_buf[1] = '/';
        safe.SimdUtils.copy(new_buf[2..][0..new_folder_trimmed.len], new_folder_trimmed);
...
// zust: use safe.String or safe.GuardedSlice for slice operations
            if (std.mem.indexOf(u8, line, @"a/$old_folder/")) |idx| {
                ...
                const line_start = if (line_iter.index) |value| {
    value
} else {
    return error.NullPointer;
} - 1 - line.len;
                if (line_iter.index) |value| {
    value
} else {
    return error.NullPointer;
} -= 1 + line.len;
...
// zust: use safe.String or safe.GuardedSlice for slice operations
    ...
    return error.NullPointer;
} -= new_folder_trimmed.len + 1;
                saw_b_folder = line_idx;
                continue;
            }
            if (saw_a_folder == null or if (saw_a_folder) |value| {
    value
} else {
    return error.NullPointer;
} != line_idx) {
// zust: use safe.String or safe.GuardedSlice for slice operations
    ...
    return error.NullPointer;
            ...
            if (saw_b_folder == null or if (saw_b_folder) |value| {
    value
} else {
    return error.NullPointer;
} != line_idx) {
...
fn shouldSkipLine(line: []const u8) bool {
    return line.len == 0 or
        (switch (line[0]) {
            ...
            (!(line.len >= 4 and (safe.SimdUtils.eql(line[0..4], "--- ") or safe.SimdUtils.eql(line[0..4], "+++ ")))));
}
