...
pub fn ThreadlocalBuffers(comptime T: type) type {
    return struct {
        ...
        noinline fn alloc() *T {
            @branchHint(.cold);
            const s = safe.Box(Storage).init(allocator, undefined) catch outOfMemory();
        ...
        fn free(node: *ThreadlocalBuffersNode) void {
            instance = null;
            const s: *Storage = // safe-transpile: @alignCast requires manual review
    ...
    return switch (@typeInfo(@TypeOf(value))) {
        ...
        .pointer => |info| switch (info.size) {
            ...
            .many => {
                ...
                const sentinel = @as(*align(1) const info.child, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(sentinel_ptr)).*;
...
pub fn span(pointer: anytype) Span(@TypeOf(pointer)) {
    ...
    if (ptr_info.sentinel_ptr) |s_ptr| {
        const s = @as(*align(1) const ptr_info.child, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(s_ptr)).*;
...
pub fn copy(comptime Type: type, dest: []Type, src: []const Type) void {
    const input: safe.Slice(u8) = std.mem.sliceAsBytes(src);
    const output: safe.Slice(u8) = std.mem.sliceAsBytes(dest);
...
pub fn clone(item: anytype, allocator: std.mem.Allocator) !@TypeOf(item) {
    ...
    for (slice, 0..) |*val, i| {
                val[0] = try item[i].clone(allocator);
            }
            return slice;
        }
...
pub fn getenvTruthy(key: [:0]const u8) bool {
    if (getenvZ(key)) |value| return safe.SimdUtils.eql(value, "true") or safe.SimdUtils.eql(value, "1");
    return false;
}
...
pub fn ComptimeEnumMap(comptime T: type) type {
    var entries: [std.enums.values(T).len]struct { [:0]const u8, T } = .{};
    // safe-transpile: for with index access requires manual review
    for (std.enums.values(T), &entries) |value, *entry| {
        entry[0] = .{ .@"0" = @tagName(value), .@"1" = value };
    }
...
pub fn zero(comptime Type: type) Type {
    var out: [@sizeOf(Type)]u8 align(@alignOf(Type)) = .{};
    @memset(@as([*]u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&out))[0..out.len], 0);
    return @as(Type, // safe-transpile: @bitCast requires manual review
    @bitCast(out));
}
            ...
            .one => switch (@typeInfo(ptr_info.child)) {
                .array => |array_info| {
                    if (array_info.sentinel_ptr) |sentinel_ptr| {
                        const sentinel = @as(*align(1) const array_info.child, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(sentinel_ptr)).*;
                        if (sentinel == end) {
            ...
            .many => if (ptr_info.sentinel_ptr) |sentinel_ptr| {
                const sentinel = @as(*align(1) const ptr_info.child, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(sentinel_ptr)).*;
...
pub fn reloadProcess(
    ...
    for (bun.argv, dupe_argv) |src, *dest| {
        dest[0] = (allocator.dupeZ(u8, src) catch unreachable).ptr;
    }
    ...
    const environ = allocator.allocSentinel(?[*:0]const u8, environ_count, null) catch unreachable;
    {
        ...
        for (0..environ_count) |i| {
            ...
            if (src == null) {
                ...
                dest[0] = (allocator.dupeZ(u8, sliceTo(if (src) |value| {
    value
} else {
    return error.NullPointer;
}, 0)) catch unreachable).ptr;
            }
        }
    }
    ...
    const newargv = @as([*:null]?[*:0]const u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(dupe_argv.ptr));

    // we clone envp so that the memory address of environment variables isn't the same as the libc one
    const envp = @as([*:null]?[*:0]const u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(environ.ptr));
    ...
    if (comptime Environment.isMac) {
        ...
        switch (spawn.spawnZ(exec_path, actions, attrs, @as([*:null]?[*:0]const u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(newargv)), @as([*:null]?[*:0]const u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(envp)))) {
            .err => |err| {
                if (may_return) {
        ...
        if (is_option) {
            ...
            const arg = switch (ArgType) {
                ...
                [:0]const u8 => arg: {
                    ...
                    safe.SimdUtils.copy(arg, env[start..j]);
                    break :arg arg;
                },
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    for (argvu16, out_argv) |argraw, *out| {
            ...
            out[0] = string_builder.append16(arg, default_allocator) orelse @panic("Failed to allocate memory for argv");
        }
    ...
    return struct {
        ...
        pub fn get(self: *@This()) bool {
            if (self.value == .unknown) {
                const parent: *Parent = // safe-transpile: @alignCast requires manual review
    @alignCast(@fieldParentPtr(field, self));
...
pub fn serializable(input: anytype) @TypeOf(input) {
    ...
    const result: *T = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&zeroed);
    ...
    while (true) {
        ...
        @import("std-fs-compat").makeDir(dir, component.path) catch |err| switch (err) {
            error.PathAlreadyExists => {
                var path_buf2: [MAX_PATH_BYTES * 2]u8 = .{};
...
pub inline fn pathLiteral(comptime literal: anytype) *const [literal.len:0]u8 {
    if (!Environment.isWindows) return // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(literal);
    return comptime {
        var buf: [literal.len:0]u8 = .{};
        // safe-transpile: for with index access requires manual review
...
pub inline fn OSPathLiteral(comptime literal: anytype) *const [literal.len:0]OSPathChar {
    ...
    @ptrCast(literal);
    return comptime {
        var buf: [literal.len:0]OSPathChar = .{};
        // safe-transpile: for with index access requires manual review
    for (literal, 0..) |char, i| {
            buf[i] = if (char == '/') '\\' else char;
            assert(buf[i] != 0 and buf[i] < 128);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        var status: w.IO_STATUS_BLOCK = std.mem.zeroes(w.IO_STATUS_BLOCK);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    fn alloc(ctx: *anyopaque, n: usize, alignment: std.mem.Alignment, ra: usize) ?[*]u8 {
        const self: *StackFallbackAllocator = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    @ptrCast(@alignCast(ctx));
        if (self.fixed.ownsPtr(buf.ptr)) {
            return // safe-transpile: free removed (memory owned by safe type);
        }
...
pub fn create(allocator: std.mem.Allocator, comptime T: type, t: T) *T {
    const pointer = safe.Box(T).init(allocator, undefined) catch outOfMemory();
    pointer[0] = t;
    return pointer;
}
...
pub inline fn tryNew(comptime T: type, init: T) OOM!*T {
    const pointer = if (heap_breakdown.enabled)
        ...
        const pointer = try safe.Box(T).init(allocator, undefined);
        pointer[0] = init;
...
pub fn reinterpretSlice(comptime T: type, slice: anytype) ReinterpretSliceType(T, @TypeOf(slice)) {
    ...
    const new_ptr = @as(if (is_const) [*]const T else [*]T, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
        ...
        fn load() [:0]const u8 {
            const bytes = switch (bun.sys.File.readFrom(bun.FD.cwd(), abs_path, default_allocator)) {
                .result => |b| {
                    ...
                    safe.SimdUtils.copy(buf[0..b.len], b);
                    buf[b.len] = 0;
                    // safe-transpile: free removed (memory owned by safe type);
        ...
        pub fn load() ![:0]u8 {
            const init = blk: {
                if (Environment.isMac) {
                    var symlink_buf: [std.posix.PATH_MAX + 1:0]u8 = .{};
                    ...
                    var resolved: [std.posix.PATH_MAX + 1:0]u8 = .{};
                    const r = std.c.realpath(exe_path.ptr, &resolved);
                    if (r) |rp| {
                        const resolved_slice = std.mem.sliceTo(rp, 0);
                        safe.SimdUtils.copy(value[0..resolved_slice.len], resolved_slice);
                        break :blk value[0..resolved_slice.len];
                    } else {
                        safe.SimdUtils.copy(value[0..exe_path.len], exe_path);
...
pub fn Once(comptime f: anytype) type {
    return struct {
        const Return = if (@typeInfo(@TypeOf(f)).@"fn".return_type) |value| {
    value
} else {
    return error.NullPointer;
};
