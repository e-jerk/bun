...
pub fn exit(code: u32) noreturn {
    ...
    if (Environment.isDebug) {
        bun.assert((if (bun.debug_allocator_data.backing) |value| {
    value
} else {
    ...
    switch (Environment.os) {
        .mac => std.c.exit(// safe-transpile: @bitCast requires manual review
    @bitCast(code)),
        .windows => {
            Bun__onExit();
            ...
            if (Environment.enable_asan) {
                std.c.exit(// safe-transpile: @bitCast requires manual review
    @bitCast(code));
                std.c.abort(); // exit should be noreturn
            }
            bun.c.quick_exit(// safe-transpile: @bitCast requires manual review
    @bitCast(code));
