...
var stderr_stream: Source.StreamType = std.mem.zeroes(Source.StreamType);
var stdout_stream: Source.StreamType = std.mem.zeroes(Source.StreamType);
...
pub const Source = struct {
    ...
    pub const Stdio = struct {
        extern "c" var bun_is_stdio_null: [3]i32 = undefined;
...
pub fn Scoped(comptime tag: anytype, comptime visibility: Visibility) type {
    const tagname = comptime if (!Environment.enable_logs) .{} else brk: {
        const input = switch (@typeInfo(@TypeOf(tag))) {
            .enum_literal => @tagName(tag),
            else => tag,
        };
        var ascii_slice: [input.len]u8 = .{};
        // safe-transpile: for with index access requires manual review
    for (input, &ascii_slice) |in, *out| {
            out[0] = std.ascii.toLower(in);
        }
...
fn ScopedLogger(comptime tagname: []const u8, comptime visibility: Visibility) type {
    ...
    return struct {
        var buffer: [4096]u8 = .{};
        var buffered_writer: File.QuietWriter.Adapter = std.mem.zeroes(File.QuietWriter.Adapter);
        var out: *std.Io.Writer = std.mem.zeroes(*std.Io.Writer);
        var out_set = false;
...
pub fn prettyFmt(comptime fmt: string, comptime is_enabled: bool) [:0]const u8 {
    comptime var new_fmt: [fmt.len * 4]u8 = .{};
...
pub const ScopedDebugWriter = struct {
    pub var scoped_file_writer: File.QuietWriter = std.mem.zeroes(File.QuietWriter);
