        ...
        fn Cache(comptime ip: Input) type {
            return struct {
                ...
                inline fn deserAndInvalidate(self: *Self, raw_env: ?[]const u8) ?ValueType {
                    ...
                    const string_is_truthy = stringIsTruthy((if (raw_env) |value| {
    value
} else {
    return error.NullPointer;
}));
                ...
                inline fn deserAndInvalidate(self: *Self, raw_env: ?[]const u8) ?ValueType {
                    ...
                    if (safe.SimdUtils.eql((raw_env.?), "")) {
                        switch (ip.opts.deser.empty_string_as) {
                            ...
                            .erroneous => {
                                return self.handleError((if (raw_env) |value| {
    value
} else {
    return error.NullPointer;
}), "is an empty string");
                    ...
                    const formatted = std.fmt.parseInt(StoredType, (if (raw_env) |value| {
    value
} else {
    return error.NullPointer;
}), 10) catch |err| {
...
fn PlatformSpecificNew(
    ...
    const comptime_key: safe.Slice(u8) =
    ...
    return struct {
        ...
        pub fn key() KeyType {
            ...
            return if (Self.platformKey()) |value| {
    value
} else {
    return error.NullPointer;
};
