...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    for (self.masks, other) |*mask, b| {
        ...
        /// Flips every bit in the bit set.
        pub fn toggleAll(self: *Self) void {
            for (0..self.masks.len) |__zust_i| {
    var mask = &self.masks[__zust_i];
                mask.* = ~mask.*;
            }
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
        ...
        if (start_mask_index == end_mask_index) {
            ...
            for (0..self.masks[bulk_mask_index..end_mask_index].len) |__zust_i| {
    var mask = &self.masks[bulk_mask_index..end_mask_index][__zust_i];
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    pub fn setAll(self: *Self, value: bool) void {
        ...
        if (bit_length == 0) return;
        const num_masks = numMasks(self.bit_length);
        for (0..self.masks[0..num_masks].len) |__zust_i| {
    var mask = &self.masks[0..num_masks][__zust_i];
            mask.* = std.math.boolMask(MaskInt, value);
        }
