    ...
    while (true) {
        ...
        pub fn remove(list: *Self, node: *Node) void {
            if (list.first == node) {
                ...
                var current_elm = (if (list.first) |value| {
    value
} else {
    return error.NullPointer;
});
                while (current_elm.next != node) {
                    current_elm = (if (current_elm.next) |value| {
    value
} else {
    return error.NullPointer;
});
        ...
        pub fn release(node: *LinkedList.Node) void {
            if (comptime max_count > 0) {
                if (data().count >= max_count) {
                    if (comptime log_allocations) @import("std-fs-compat").File.stderr().writeAll(comptime std.fmt.comptimePrint("Free {s} - {d} bytes\n", .{ @typeName(Type), @sizeOf(Type) })) catch {};
                    destroyNode(node);
                    return;
                }
            }
