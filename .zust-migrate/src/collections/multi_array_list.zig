...
pub fn MultiArrayList(comptime T: type) type {
    return struct {
        ...
        const Elem = switch (@typeInfo(T)) {
            ...
            .@"union" => |u| struct {
                pub const Bare = blk: {
                    var bare_fields: [u.fields.len]std.builtin.Type.UnionField = .{};
    ...
    inline for (fields, 0..) |field_info, i| {
            ...
            pub fn toMultiArrayList(self: Slice) Self {
                ...
                const aligned_ptr: [*]align(@alignOf(Elem)) u8 = // safe-transpile: @alignCast requires manual review
    @alignCast(unaligned_ptr);
                return .{
                    .bytes = aligned_ptr,
                    .len = self.len,
    ...
    inline for (fields, 0..) |field_info, i| {
                if (@sizeOf(field_info.type) != 0) {
                    ...
                    safe.SimdUtils.copy(this_slice.items(field)[offset..], other_slice.items(field));
                }
            }
        }
    ...
    inline for (fields, 0..) |field_info, i| {
                if (@sizeOf(field_info.type) != 0) {
                    const field = @as(Field, @enumFromInt(i));
                    safe.SimdUtils.copy(other_slice.items(field), self_slice.items(field));
                }
            }
            // safe-transpile: free removed (memory owned by safe type);
            self[0] = other;
    ...
    inline for (fields, 0..) |field_info, i| {
                if (@sizeOf(field_info.type) != 0) {
                    const field = @as(Field, @enumFromInt(i));
                    safe.SimdUtils.copy(result_slice.items(field), self_slice.items(field));
                }
            }
            return result;
        }
    ...
    inline for (fields, 0..) |field_info, i| {
        ...
        const Entry = entry: {
            var entry_fields: [fields.len]std.builtin.Type.StructField = .{};
            for (0..fields.len) |i| {
                entry_fields[i] = .{
