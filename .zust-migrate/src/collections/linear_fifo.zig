...
pub fn LinearFifo(
    ...
    return struct {
        ...
        const Self = @This();
        pub const Reader = @import("std-io-compat").MakeGenericReader(*Self, error{}, readFn);
        pub const Writer = @import("std-io-compat").MakeGenericWriter(*Self, error{OutOfMemory}, appendWrite);
        ...
        pub fn deinit(self: Self) void {
            if (buffer_type == .Dynamic) // safe-transpile: free removed (memory owned by safe type);
        }
        ...
        pub fn realign(self: *Self) void {
            if (self.buf.len - self.head >= self.count) {
                ...
                var tmp: [std.heap.page_size_min / 2 / @sizeOf(T)]T = .{};
        ...
        pub fn ensureTotalCapacity(self: *Self, size: usize) !void {
            ...
            if (buffer_type == .Dynamic) {
                ...
                if (self.count > 0) {
                    ...
                    safe.SimdUtils.copy(new_bytes[0..old_bytes.len], old_bytes);
                }
...
test "LinearFifo(u8, .Dynamic)" {
    ...
    {
        try testing.expectEqual(@as(u8, 'H'), if (fifo.readItem()) |value| {
    value
} else {
    return error.NullPointer;
});
        try testing.expectEqual(@as(u8, 'E'), if (fifo.readItem()) |value| {
    value
} else {
    return error.NullPointer;
});
        try testing.expectEqual(@as(u8, 'L'), if (fifo.readItem()) |value| {
    value
} else {
    return error.NullPointer;
});
    ...
    return error.NullPointer;
        ...
        try testing.expectEqual(@as(u8, 'O'), if (fifo.readItem()) |value| {
    value
} else {
    return error.NullPointer;
});
    ...
    {
        try fifo.print("{s}, {s}!", .{ "Hello", "World" });
        var result: [30]u8 = .{};
...

    {
        ...
        var result: [30]u8 = .{};
        try testing.expectEqualSlices(u8, "This", if ((try fifo.reader().readUntilDelimiterOrEof(&result, ' '))) |value| {
    value
} else {
    return error.NullPointer;
});
        try testing.expectEqualSlices(u8, "is", if ((try fifo.reader().readUntilDelimiterOrEof(&result, ' '))) |value| {
    value
} else {
    return error.NullPointer;
});
        try testing.expectEqualSlices(u8, "a", if ((try fifo.reader().readUntilDelimiterOrEof(&result, ' '))) |value| {
    value
} else {
    return error.NullPointer;
});
        try testing.expectEqualSlices(u8, "test", if ((try fifo.reader().readUntilDelimiterOrEof(&result, ' '))) |value| {
    value
} else {
    return error.NullPointer;
});
...

    {
        ...
        var in_fbs = @import("std-io-compat").fixedBufferStream("pump test");
        var out_buf: [50]u8 = .{};
        var out_fbs = @import("std-io-compat").fixedBufferStream(&out_buf);
...
test "LinearFifo" {
    inline for ([_]type{ u1, u8, u16, u64 }) |T| {
        inline for ([_]LinearFifoBufferType{ LinearFifoBufferType{ .Static = 32 }, .Slice, .Dynamic }) |bt| {
            ...
            var buf: if (bt == .Slice) [32]T else void = std.mem.zeroes(if (bt == .Slice) [32]T else void);
            var fifo = switch (bt) {
            ...
            {
                try testing.expectEqual(@as(T, 0), if (fifo.readItem()) |value| {
    value
} else {
    return error.NullPointer;
});
                try testing.expectEqual(@as(T, 1), if (fifo.readItem()) |value| {
    value
} else {
    return error.NullPointer;
});
