...
pub fn HiveArray(comptime T: type, comptime capacity: u16) type {
    return struct {
        ...
        pub fn get(self: *Self) ?*T {
            ...
            bun.asan.unpoison(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(ret), @sizeOf(T));
            return ret;
        }
        ...
        pub fn at(self: *Self, index: u16) *T {
            ...
            const ret = &self.buffer[index];
            bun.asan.assertUnpoisoned(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(ret));
            return ret;
        }
    ...
    @ptrCast(value));
            ...
            const end = @as([*]const T, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(start)) + capacity;
    ...
    @ptrCast(start)) + capacity;
        ...
        pub const Fallback = struct {
            ...
            fn getImpl(self: *This) *T {
                ...
                return bun.handleOom(safe.Box(T).init(allocator, undefined));
            }
            ...
            pub fn tryGet(self: *This) OOM!*T {
                ...
                return try safe.Box(T).init(allocator, undefined);
            }
...
test "HiveArray" {
    ...
    {
        const b = if (a.get()) |value| {
    value
} else {
    return error.NullPointer;
};
        try testing.expect(if (a.get()) |value| {
    value
} else {
    return error.NullPointer;
} != b);
