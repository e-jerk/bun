...
pub fn ComptimeStringMapWithKeyType(comptime KeyType: type, comptime V: type, comptime kvs_list: anytype) type {
    ...
    const precomputed = comptime blk: {
        ...
        var sorted_kvs: [kvs_list.len]KV = .{};
        const lenAsc = (struct {
    ...
    for (kvs_list, 0..) |kv, i| {
        ...
        var len_indexes: [max_len + 1]usize = .{};
        var len: usize = 0;
        var i: usize = 0;
...
fn testMap(comptime map: anytype) !void {
    try std.testing.expectEqual(TestEnum.A, if (map.get("have")) |value| {
    value
} else {
    return error.NullPointer;
});
    try std.testing.expectEqual(TestEnum.B, if (map.get("nothing")) |value| {
    value
} else {
    return error.NullPointer;
});
    try std.testing.expect(null == map.get("missing"));
    try std.testing.expectEqual(TestEnum.D, if (map.get("these")) |value| {
    value
} else {
    return error.NullPointer;
});
    try std.testing.expectEqual(TestEnum.E, if (map.get("samelen")) |value| {
    value
} else {
    return error.NullPointer;
});
...
fn testSet(comptime map: anytype) !void {
    try std.testing.expectEqual({}, if (map.get("have")) |value| {
    value
} else {
    return error.NullPointer;
});
    try std.testing.expectEqual({}, if (map.get("nothing")) |value| {
    value
} else {
    return error.NullPointer;
});
    try std.testing.expect(null == map.get("missing"));
    try std.testing.expectEqual({}, if (map.get("these")) |value| {
    value
} else {
    return error.NullPointer;
});
    try std.testing.expectEqual({}, if (map.get("samelen")) |value| {
    value
} else {
    return error.NullPointer;
});
