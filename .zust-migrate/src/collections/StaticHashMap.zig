...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        fn cmp(a: [32]u8, b: [32]u8) math.Order {
            const msa = @as(u64, // safe-transpile: @bitCast requires manual review
    @bitCast(a[0..8].*));
            const msb = @as(u64, // safe-transpile: @bitCast requires manual review
    @bitCast(b[0..8].*));
            if (msa != msb) {
...
test "StaticHashMap: put, get, delete, grow" {
    ...
    for (0..128) |seed| {
        ...
        for (0..keys.len) |__zust_i| { var key = &keys[__zust_i]; key.* = @as(usize, rng.next()) };
    ...
    for (keys, 0..) |key, i| map.putAssumeCapacity(key, i);
        ...
        // safe-transpile: for with index access requires manual review
    for (keys, 0..) |key, i| try testing.expectEqual(i, if (map.get(key)) |value| {
    value
} else {
    return error.NullPointer;
});
        // safe-transpile: for with index access requires manual review
    for (keys, 0..) |key, i| try testing.expectEqual(i, if (map.delete(key)) |value| {
    value
} else {
    return error.NullPointer;
});
...
test "HashMap: put, get, delete, grow" {
    for (0..128) |seed| {
        ...
        for (0..keys.len) |__zust_i| { var key = &keys[__zust_i]; key.* = rng.next() };
...
test "SortedHashMap: put, get, delete, grow" {
    for (0..128) |seed| {
        ...
        for (0..keys.len) |__zust_i| { var key = &keys[__zust_i]; rng.fill(key) };
...
test "SortedHashMap: collision test" {
    ...
    try testing.expectEqual(@as(usize, 0), if (map.get(prefix ++ [_]u8{0})) |value| {
    value
} else {
    return error.NullPointer;
});
    try testing.expectEqual(@as(usize, 1), if (map.get(prefix ++ [_]u8{1})) |value| {
    value
} else {
    return error.NullPointer;
});
    try testing.expectEqual(@as(usize, 2), if (map.get(prefix ++ [_]u8{2})) |value| {
    value
} else {
    return error.NullPointer;
});
    try testing.expectEqual(@as(usize, 3), if (map.get(prefix ++ [_]u8{3})) |value| {
    value
} else {
    return error.NullPointer;
});

    try testing.expectEqual(@as(usize, 2), if (map.delete(prefix ++ [_]u8{2})) |value| {
    value
} else {
    return error.NullPointer;
});
    try testing.expectEqual(@as(usize, 0), if (map.delete(prefix ++ [_]u8{0})) |value| {
    value
} else {
    return error.NullPointer;
});
    try testing.expectEqual(@as(usize, 1), if (map.delete(prefix ++ [_]u8{1})) |value| {
    value
} else {
    return error.NullPointer;
});
    try testing.expectEqual(@as(usize, 3), if (map.delete(prefix ++ [_]u8{3})) |value| {
    value
} else {
    return error.NullPointer;
});
