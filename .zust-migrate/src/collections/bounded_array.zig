...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        pub fn fromSlice(m: []const T) error{Overflow}!Self {
            ...
            safe.SimdUtils.copy(list.slice(), m);
            return list;
        }
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            safe.SimdUtils.copy(self.slice()[i..][0..items.len], items);
        }
        ...
        pub fn replaceRange(
            ...
            if (range.len == new_items.len) {
                safe.SimdUtils.copy(range[0..new_items.len], new_items);
            } else if (range.len < new_items.len) {
                const first = new_items[0..range.len];
                const rest = new_items[range.len..];
                safe.SimdUtils.copy(range[0..first.len], first);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        pub fn orderedRemove(self: *Self, i: usize) T {
            ...
            if (newlen == i) return if (self.pop()) |value| {
    value
} else {
    return error.NullPointer;
            ...
            // safe-transpile: for with index access requires manual review
    for (self.slice()[i..newlen], 0..) |*b, j| b[0] = self.get(i + 1 + j);
        ...
        pub fn swapRemove(self: *Self, i: usize) T {
            if (self.len - 1 == i) return if (self.pop()) |value| {
    value
} else {
    return error.NullPointer;
};
            const old_item = self.get(i);
            self.set(i, if (self.pop()) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            safe.SimdUtils.copy(self.slice()[old_len..][0..items.len], items);
        }
