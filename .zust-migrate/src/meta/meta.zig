...
pub fn EnumFields(comptime T: type) []const std.builtin.Type.EnumField {
    ...
    return switch (tyinfo) {
        .@"union" => std.meta.fields(if (tyinfo.@"union".tag_type) |value| {
    value
} else {
...
// zust: use safe.String or safe.GuardedSlice for slice operations
    ...
    const name = if (idx == null) fullname else fullname[(if (idx) |value| {
    value
} else {
    return error.NullPointer;
} + 1)..];
...
pub fn hasStableMemoryLayout(comptime T: type) bool {
    ...
    return switch (tyinfo) {
        ...
        .@"union" => switch (tyinfo.@"union".layout) {
            .auto => {
                if (tyinfo.@"union".tag_type == null or !hasStableMemoryLayout(if (tyinfo.@"union".tag_type) |value| {
    value
} else {
    return error.NullPointer;
})) return false;
...
fn VoidFields(comptime T: type) type {
    ...
    var void_fields: [fields.len]std.builtin.Type.StructField = .{};
    // safe-transpile: for with index access requires manual review
