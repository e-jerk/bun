...
pub fn LeadingZerosInt(comptime T: type) type {
    const backing_int = if (@typeInfo(T).@"struct".backing_integer) |value| {
    value
} else {
...
pub inline fn fromInt(comptime T: type, bits: if (@typeInfo(T).@"struct".backing_integer) |value| {
    value
} else {
    return error.NullPointer;
}) T {
...
pub inline fn asInt(comptime T: type, value: T) if (@typeInfo(T).@"struct".backing_integer) |value| {
    value
} else {
    return error.NullPointer;
} {
