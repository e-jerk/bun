...
pub const MacroContext = struct {
    pub const MacroMap = @import("array-hash-map-compat").Auto(i32, Macro);
...
pub const Runner = struct {
    ...
    threadlocal var args_buf: [3]js.JSObjectRef = .{};
    threadlocal var exception_holder: jsc.ZigException.Holder = std.mem.zeroes(jsc.ZigException.Holder);
    ...
    pub const Run = struct {
        ...
        pub fn runAsync(
            ...
            const result = js.JSObjectCallAsFunctionReturnValueHoldingAPILock(
                ...
                null,
                args.len,
                @as([*]js.JSObjectRef, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(args.ptr)),
            );
