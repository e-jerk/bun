...
pub fn notifyServerRoutesUpdated(this: *const HTTPServerAgent, server: jsc.API.AnyServer) !void {
    if (this.agent) |agent| {
        ...
        defer {
            for (0..routes.items.len) |__zust_i| {
    var route = &routes.items[__zust_i];
                route.deinit();
            }
            routes.deinit();
        }
        ...
        switch (server.userRoutes()) {
            inline else => |user_routes| {
                for (0..user_routes.len) |__zust_i| {
    var user_route = &user_routes[__zust_i];
        ...
        for (0..config.static_routes.items.len) |__zust_i| {
    var route = &config.static_routes.items[__zust_i];
            try routes.append(.{
...
pub const Route = extern struct {
    ...
    var param_name = &this.params()[__zust_i];
            param_name.deref();
        }
...
pub const InspectorHTTPServerAgent = opaque {
    ...
    pub fn notifyServerRoutesUpdated(agent: *InspectorHTTPServerAgent, serverId: ServerId, hotReloadId: HotReloadId, routes: []Route) void {
        bun.cpp.Bun__HTTPServerAgent__notifyServerRoutesUpdated(agent, serverId, hotReloadId, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(routes.ptr), routes.len);
    }
};
