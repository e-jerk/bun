...
// safe-transpile: optional unwrap requires manual review
                return if (this.weak.get().?.asPromise()) |value| {
    value
} else {
    ...
    return error.NullPointer;
            ...
            pub fn swap(this: *WeakType) *jsc.JSPromise {
                const prom = if (this.weak.swap().asPromise()) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
            (this.strong.get() orelse return).asPromise().?.reject(globalThis, val catch if (globalThis.tryTakeException()) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: optional unwrap requires manual review
        ...
        pub fn reject(this: *Strong, globalThis: *jsc.JSGlobalObject, val: JSError!jsc.JSValue) bun.JSTerminated!void {
            try this.swap().reject(globalThis, val catch if (globalThis.tryTakeException()) |value| {
    value
} else {
    return error.NullPointer;
});
        ...
        pub fn init(globalThis: *jsc.JSGlobalObject) Strong {
            return Strong{
                .strong = .create(
                    safe.Box(globalThis).init(allocator, undefined).toJS(),
                    globalThis,
                ),
            };
        }
...
// safe-transpile: optional unwrap requires manual review
            return if (this.strong.get().?.asPromise()) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        pub fn value(this: *const Strong) JSValue {
            return if (this.strong.get()) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        pub fn swap(this: *Strong) *jsc.JSPromise {
            const prom = if (this.strong.swap().asPromise()) |value| {
    value
} else {
    return error.NullPointer;
};
    ...
    pub fn wrap(
        ...
        const promise = JSC__JSPromise__wrap(globalObject, &ctx, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&Wrapper.call));
