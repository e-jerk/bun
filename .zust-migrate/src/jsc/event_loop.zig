...
fn updateCounts(this: *EventLoop) void {
    ...
    const loop = if (this.virtual_machine.event_loop_handle) |value| {
    value
} else {
...
pub fn usocketsLoop(this: *const EventLoop) *uws.Loop {
    ...
    return if (this.virtual_machine.event_loop_handle) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub fn autoTickActive(this: *EventLoop) void {
    ...
    if (loop.isActive()) {
        this.processGCTimer();
        var timespec: bun.timespec = std.mem.zeroes(bun.timespec);
...
pub fn enqueueTaskConcurrentBatch(this: *EventLoop, batch: ConcurrentTask.Queue.Batch) void {
    ...
    this.concurrent_tasks.pushBatch(if (batch.front) |value| {
    value
} else {
    return error.NullPointer;
}, if (batch.last) |value| {
