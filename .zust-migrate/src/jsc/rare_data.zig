...
pub const PathBuf = struct {
    ...
    pub fn get(self: *PathBuf, min_len: usize, fallback: std.mem.Allocator) bun.StackFallbackAllocator {
        const buf: safe.Slice(u8) = if (min_len <= 2 * S)
            (self.small orelse blk: {
                self.small = bun.handleOom(safe.Box(SmallBuf).init(allocator, undefined));
                break :blk if (self.small) |value| {
    value
} else {
    return error.NullPointer;
            ...
            (self.medium orelse blk: {
                self.medium = bun.handleOom(safe.Box(MediumBuf).init(allocator, undefined));
                break :blk if (self.medium) |value| {
    value
} else {
    return error.NullPointer;
};
            ...
            (self.large orelse blk: {
                self.large = bun.handleOom(safe.Box(LargeBuf).init(allocator, undefined));
                break :blk if (self.large) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub fn pipeReadBuffer(this: *RareData) *PipeReadBuffer {
    return this.temp_pipe_read_buffer orelse {
        this.temp_pipe_read_buffer = bun.handleOom(safe.Box(PipeReadBuffer).init(allocator, undefined));
        return if (this.temp_pipe_read_buffer) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub fn filePolls(this: *RareData, vm: *jsc.VirtualMachine) *Async.FilePoll.Store {
    return this.file_polls_ orelse {
        this.file_polls_ = safe.Box(Async.FilePoll.Store).init(allocator, undefined) catch unreachable;
// safe-transpile: optional unwrap requires manual review
        this.file_polls_.?.* = Async.FilePoll.Store.init();
        return if (this.file_polls_) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub fn nextUUID(this: *RareData) UUID {
    if (this.entropy_cache == null) {
        this.entropy_cache = safe.Box(EntropyCache).init(allocator, undefined) catch unreachable;
// safe-transpile: optional unwrap requires manual review
...
pub fn boringEngine(rare: *RareData) *BoringSSL.ENGINE {
    return rare.boring_ssl_engine orelse brk: {
        ...
        break :brk if (rare.boring_ssl_engine) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub fn defaultClientSslCtx(rare: *RareData) *uws.SslCtx {
    ...
    return if (rare.default_client_ssl_ctx) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub fn nodeFSStatWatcherScheduler(rare: *RareData, vm: *jsc.VirtualMachine) bun.ptr.RefPtr(StatWatcherScheduler) {
    return (rare.node_fs_stat_watcher_scheduler orelse init: {
        ...
        break :init if (rare.node_fs_stat_watcher_scheduler) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub fn websocketDeflate(this: *RareData) *WebSocketDeflate.RareData {
    return this.websocket_deflate orelse brk: {
        ...
        break :brk if (this.websocket_deflate) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub fn spawnSyncEventLoop(this: *RareData, vm: *jsc.VirtualMachine) *SpawnSyncEventLoop {
    return this._spawn_sync_event_loop.get() orelse brk: {
        ...
        const ptr: *SpawnSyncEventLoop = if (this._spawn_sync_event_loop.get()) |value| {
    value
} else {
    return error.NullPointer;
};
