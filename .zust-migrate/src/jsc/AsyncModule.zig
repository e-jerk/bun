...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    @alignCast(@fieldParentPtr("modules", this));
        }
...
// safe-transpile: optional unwrap requires manual review
        opts.promise_ptr.?.* = if (this_promise.asInternalPromise()) |value| {
    value
} else {
    ...
    pub fn done(this: *AsyncModule, jsc_vm: *VirtualMachine) void {
        var clone = safe.Box(AsyncModule).init(allocator, undefined) catch unreachable;
        clone[0] = this.*;
    ...
    pub fn onDone(this: *AsyncModule) void {
        ...
        defer log.deinit();
        var errorable: jsc.ErrorableResolvedSource = std.mem.zeroes(jsc.ErrorableResolvedSource);
    ...
    pub fn resolveError(this: *AsyncModule, vm: *VirtualMachine, import_record_id: u32, result: PackageResolveError) !void {
        ...
        const msg: safe.Slice(u8) = try switch (result.err) {
            ...
            error.DistTagNotFound, error.NoMatchingVersion => brk: {
                const prefix: safe.Slice(u8) = if (result.err == error.NoMatchingVersion and result.version.tag == .npm and result.version.value.npm.version.isExact())
                    "Version not found"
        ...
        const name: safe.Slice(u8) = switch (result.err) {
        ...
        const location = if (logger.rangeData(&this.parse_result.source, this.parse_result.ast.import_records.at(import_record_id).range, "").location) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        var promise = if (promise_value.asInternalPromise()) |value| {
    value
} else {
    return error.NullPointer;
};
