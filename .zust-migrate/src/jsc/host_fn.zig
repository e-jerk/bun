...
pub fn toJSHostCall(
    ...
    var scope: jsc.ExceptionValidationScope = std.mem.zeroes(jsc.ExceptionValidationScope);
...
pub fn fromJSHostCallGeneric(
    ...
    comptime function: anytype,
    args: std.meta.ArgsTuple(@TypeOf(function)),
) error{JSError}!if (@typeInfo(@TypeOf(function)).@"fn".return_type) |value| {
    value
} else {
...
pub fn wrap1(comptime func: anytype) @"return": {
    ...
    break :@"return" fn (if (p[0].type) |value| {
    value
} else {
    return error.NullPointer;
}) callconv(.c) JSValue;
    ...
    return struct {
        pub fn wrapped(arg0: if (p[0].type) |value| {
    value
} else {
    return error.NullPointer;
}) callconv(.c) JSValue {
...
fn checkWrapParams(comptime func: anytype, comptime N: u8) []const std.builtin.Type.Fn.Param {
    ...
    } else if (if (params[0].type) |value| {
    value
} else {
    return error.NullPointer;
} != *JSGlobalObject) {
...
pub fn wrapInstanceMethod(
    ...
    inline for (FunctionTypeInfo.params, 0..) |param, i| {
                const ArgType = if (param.type) |value| {
    value
} else {
    return error.NullPointer;
};
