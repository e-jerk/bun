...
pub fn NewHotReloader(comptime Ctx: type, comptime EventLoopType: type, comptime reload_immediately: bool) type {
    return struct {
        ...
        pub fn init(ctx: *Ctx, fs: *bun.fs.FileSystem, verbose: bool, clear_screen_flag: bool) *Watcher {
            const reloader = bun.handleOom(safe.Box(Reloader).init(allocator, undefined));
            reloader[0] = .{
                .ctx = ctx,
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        ...
        pub fn enableHotModuleReloading(this: *Ctx, entry_path: ?[]const u8) void {
            ...
            var reloader = bun.handleOom(safe.Box(Reloader).init(allocator, undefined));
            reloader[0] = .{
                .ctx = this,
            ...
            if (comptime @TypeOf(this.bun_watcher) == ImportWatcher) {
                ...
                this.transpiler.resolver.watcher = bun.resolver.ResolveWatcher(*Watcher, Watcher.onMaybeWatchDirectory).init(if (this.bun_watcher) |value| {
    value
} else {
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        ...
        pub fn getContext(this: *@This()) *Watcher {
            if (comptime @TypeOf(this.ctx.bun_watcher) == ImportWatcher) {
                ...
                return if (this.ctx.bun_watcher) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        pub noinline fn onFileUpdate(
            ...
            var _on_file_update_path_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
    ...
    for (parents, 0..) |parent_hash, entry_id| {
                                        if (parent_hash == current_hash) {
                                            ...
                                            var path_buf: [std.fs.max_path_bytes]u8 = .{};
                                            safe.SimdUtils.copy(path_buf[0..affected_path.len], affected_path);
                                            path_buf[affected_path.len] = 0;
                                            const was_deleted = std.c.faccessat(std.c.AT.FDCWD, @as([*:0]u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&path_buf)), std.c.F_OK, 0) != 0;
                        ...
                        if (entries_option) |dir_ent| {
                            ...
                            for (affected) |changed_name_| {
                                const changed_name: safe.Slice(u8) = if (comptime Environment.isKqueue)
                                    changed_name_
                                else
                                    bun.asByteSlice(if (changed_name_) |value| {
    value
} else {
    return error.NullPointer;
});
                                ...
                                if (loader != .file) {
                                    var path_string: bun.PathString = std.mem.zeroes(bun.PathString);
                                    var file_hash: Watcher.HashType = last_file_hash;
                                    const abs_path: string = brk: {
                                        if (dir_ent.entries.get(@as([]const u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(changed_name)))) |file_ent| {
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
                                            ...
                                            safe.SimdUtils.copy(_on_file_update_path_buf[0..file_path_without_trailing_slash.len], file_path_without_trailing_slash);
                                            _on_file_update_path_buf[file_path_without_trailing_slash.len] = std.fs.path.sep;

                                            safe.SimdUtils.copy(_on_file_update_path_buf[file_path_without_trailing_slash.len..][0..changed_name.len], changed_name);
