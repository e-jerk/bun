...
pub const ZigException = extern struct {
    ...
    pub fn deinit(this: *ZigException) void {
        ...
        for (0..this.stack.source_lines_ptr[0..this.stack.source_lines_len].len) |__zust_i| {
    var line = &this.stack.source_lines_ptr[0..this.stack.source_lines_len][__zust_i];
            line.deref();
        }

        for (0..this.stack.frames_ptr[0..this.stack.frames_len].len) |__zust_i| {
    var frame = &this.stack.frames_ptr[0..this.stack.frames_len][__zust_i];
            frame.deinit();
        }
    ...
    pub const Holder = extern struct {
        ...
        pub const Zero: Holder = Holder{
            .frames = brk: {
                var _frames: [frame_count]ZigStackFrame = .{};
                @memset(&_frames, ZigStackFrame.Zero);
                break :brk _frames;
            },
            .source_line_numbers = brk: {
                var lines: [source_lines_count]i32 = .{};
                @memset(&lines, -1);
                break :brk lines;
            },

            .source_lines = brk: {
                var lines: [source_lines_count]String = .{};
