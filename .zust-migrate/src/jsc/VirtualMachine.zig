...
macros: MacroMap,
macro_entry_points: @import("array-hash-map-compat").Auto(i32, *MacroEntryPoint),
...
origin_timer: @import("std-fs-compat").Timer = undefined,
...
pub fn uwsLoop(this: *const VirtualMachine) *uws.Loop {
    if (comptime Environment.isPosix) {
        ...
        return if (this.event_loop_handle) |value| {
    value
} else {
...
fn ensureSourceCodePrinter(this: *VirtualMachine) void {
    if (source_code_printer == null) {
        ...
        source_code_printer = safe.Box(js_printer.BufferPrinter).init(allocator, undefined) catch unreachable;
// safe-transpile: optional unwrap requires manual review
...
fn wrapUnhandledRejectionErrorForUncaughtException(globalObject: *JSGlobalObject, reason: JSValue) JSValue {
    ...
    const reasonStr = blk: {
        var scope: jsc.TopExceptionScope = std.mem.zeroes(jsc.TopExceptionScope);
    ...
    switch (this.unhandledRejectionsMode()) {
        ...
        .warn => {
            ...
            jsc.fromJSHostCallGeneric(globalObject, @src(), Bun__promises__emitUnhandledRejectionWarning, .{ globalObject, reason, promise }) catch |err| {
                _ = globalObject.reportUncaughtException(if (globalObject.takeException(err).asException(globalObject.vm())) |value| {
    value
} else {
    return error.NullPointer;
});
...
pub inline fn nodeFS(this: *VirtualMachine) *Node.fs.NodeFS {
    return this.node_fs orelse brk: {
        this.node_fs = safe.Box(Node.fs.NodeFS).init(allocator, undefined) catch unreachable;
// safe-transpile: optional unwrap requires manual review
        ...
        break :brk if (this.node_fs) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub inline fn rareData(this: *VirtualMachine) *jsc.RareData {
    return this.rare_data orelse brk: {
        this.rare_data = safe.Box(jsc.RareData).init(allocator, undefined) catch unreachable;
// safe-transpile: optional unwrap requires manual review
        this.rare_data.?.* = .{};
        ...
        bun.asan.registerRootRegion(if (this.rare_data) |value| {
    value
} else {
    return error.NullPointer;
}, @sizeOf(jsc.RareData));
        break :brk if (this.rare_data) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub fn enterUWSLoop(this: *VirtualMachine) void {
    var loop = if (this.event_loop_handle) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub const MacroMap = @import("array-hash-map-compat").Auto(i32, jsc.C.JSObjectRef);
...
pub fn initWithModuleGraph(
    ...
    vm[0] = VirtualMachine{
        ...
        .origin_timer = @import("std-fs-compat").Timer.start() catch @panic("Timers are not supported on this system."),
        .origin_timestamp = getOriginTimestamp(),
        .ref_strings = jsc.RefString.Map.init(allocator),
        .ref_strings_mutex = .{},
        .standalone_module_graph = if (opts.graph) |value| {
    value
} else {
    return error.NullPointer;
},
    ...
    vm.transpiler.resolver.standalone_module_graph = if (opts.graph) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub fn init(opts: Options) !*VirtualMachine {
    jsc.markBinding(@src());
    const allocator = opts.allocator;
    var log: *logger.Log = std.mem.zeroes(*logger.Log);
    if (opts.log) |__log| {
        log = __log;
    } else {
        log = try safe.Box(logger.Log).init(allocator, undefined);
        log[0] = logger.Log.init(allocator);
    }
    ...
    vm[0] = VirtualMachine{
        ...
        .macros = MacroMap.init(allocator),
        .macro_entry_points = @TypeOf(vm.macro_entry_points).init(allocator),
        .origin_timer = @import("std-fs-compat").Timer.start() catch @panic("Please don't mess with timers."),
        .origin_timestamp = getOriginTimestamp(),
        .ref_strings = jsc.RefString.Map.init(allocator),
        .ref_strings_mutex = .{},
        .debug_thread_id = if (Environment.allow_assert) std.Thread.getCurrentId(),
...
fn refCountedStringWithWasNew(this: *VirtualMachine, new: *bool, input_: []const u8, hash_: ?u32, comptime dupe: bool) *jsc.RefString {
    ...
    if (!entry.found_existing) {
        ...
        const ref = safe.Box(jsc.RefString).init(allocator, undefined) catch unreachable;
        ref[0] = jsc.RefString{
...
pub fn fetchWithoutOnLoadPlugins(
    ...
    return try ModuleLoader.transpileSourceCode(
        ...
        lr.virtual_source,
        null,
        if (VirtualMachine.source_code_printer) |value| {
    value
} else {
    return error.NullPointer;
},
...
fn _resolve(
    ...
    var query_string: safe.Slice(u8) = "";
...
pub fn resolveMaybeNeedsTrailingSlash(
    ...
    if (is_a_file_path and specifier.length() > comptime @as(u32, @intFromFloat(@trunc(@as(f64, @floatFromInt(bun.MAX_PATH_BYTES)) * 1.5)))) {
        ...
        res[0] = ErrorableString.err(error.NameTooLong, (try bun.api.ResolveMessage.create(global, VirtualMachine.get().allocator, msg, source_utf8.slice())));
        return;
    }
    ...
    jsc_vm._resolve(&result, specifier_utf8.slice(), normalizeSource(source_utf8.slice()), is_esm, is_a_file_path) catch |err_| {
        ...
        {
            res[0] = ErrorableString.err(err, (try bun.api.ResolveMessage.create(global, VirtualMachine.get().allocator, msg, source_utf8.slice())));
        }
    ...
    if (query_string) |query| {
        ...
        query[0] = if (result.query_string.len > 0)
    ...
    res[0] = ErrorableString.ok(bun.String.cloneUTF8(result.path));
}
...
pub fn processFetchLog(globalThis: *JSGlobalObject, specifier: bun.String, referrer: bun.String, log: *logger.Log, ret: *ErrorableResolvedSource, err: anyerror) void {
    switch (log.msgs.items.len) {
        0 => {
            ...
            {
                ret[0] = ErrorableResolvedSource.err(err, (bun.api.BuildMessage.create(globalThis, globalThis.allocator(), msg) catch |e| globalThis.takeException(e)));
            }
            return;
        },
        ...
        1 => {
            ...
            ret[0] = ErrorableResolvedSource.err(err, switch (msg.metadata) {
    ...
    for (logs, errors) |msg, *current| {
            ...
            ret[0] = ErrorableResolvedSource.err(
                err,
...
pub fn loadEntryPointForWebWorker(this: *VirtualMachine, entry_path: string) anyerror!*JSInternalPromise {
    ...
    return if (this.pending_internal_promise) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
    ...
    // GC cell.
    if (this.rare_data) |rare| {
        for (0..rare.cleanup_hooks.items.len) |__zust_i| {
    var hook = &rare.cleanup_hooks.items[__zust_i];
...
pub fn loadMacroEntryPoint(this: *VirtualMachine, entry_path: string, function_name: string, specifier: string, hash: i32) !*JSInternalPromise {
    ...
    if (!entry_point_entry.found_existing) {
        var macro_entry_pointer: *MacroEntryPoint = safe.Box(MacroEntryPoint).init(allocator, undefined) catch unreachable;
    ...
    if (value.isAggregateError(this.global)) {
        const AggregateErrorIterator = struct {
            ...
            pub fn iteratorWithColor(vm: *VM, globalObject: *JSGlobalObject, ctx: ?*anyopaque, nextValue: JSValue) callconv(.c) void {
                iterator(vm, globalObject, nextValue, if (ctx) |value| {
    value
} else {
    return error.NullPointer;
}, true);
...
pub fn remapStackFramePositions(this: *VirtualMachine, frames: [*]jsc.ZigStackFrame, frames_count: usize) void {
    ...
    for (0..frames[0..frames_count].len) |__zust_i| {
    var frame = &frames[0..frames_count][__zust_i];
...
pub fn remapZigException(
    ...
    try error_instance.toZigException(this.global, exception);
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    if (this.hide_bun_stackframes) {
        for (0..frames.len) |__zust_i| {
    var frame = &frames[__zust_i];
...
// safe-transpile: optional unwrap requires manual review
            ...
            must_reset_parser_arena_later[0] = true;
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    for (lines, source_lines[0..lines.len], source_line_numbers[0..lines.len]) |line, *line_dest, *line_number| {
                ...
                line_dest[0] = String.init(line);
                line_number[0] = current_line_number;
...
fn printErrorInstance(
    ...
    if (source_lines.next()) |source| brk: {
        ...
        if (this.hide_bun_stackframes) {
            for (0..exception.stack.frames().len) |__zust_i| {
    var frame = &exception.stack.frames()[__zust_i];
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    if (is_error_instance) {
        ...
        const error_obj = if (error_instance.getObject()) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
    if (if (this.ipc) |value| {
    value
} else {
    return error.NullPointer;
} != .waiting) return this.ipc.?.initialized;
// safe-transpile: optional unwrap requires manual review
...
pub const ExitHandler = struct {
    ...
    pub fn dispatchOnExit(this: *ExitHandler) void {
        jsc.markBinding(@src());
        const vm: *VirtualMachine = // safe-transpile: @alignCast requires manual review
    @alignCast(@fieldParentPtr("exit_handler", this));
