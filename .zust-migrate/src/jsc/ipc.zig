...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    pub fn serialize(writer: *bun.io.StreamBuffer, global: *jsc.JSGlobalObject, value: JSValue, is_internal: IsInternal) !usize {
        var out: bun.String = std.mem.zeroes(bun.String);
...
pub const SendQueue = struct {
    ...
    pub fn deinit(self: *@This()) void {
        ...
        for (0..self.queue.items.len) |__zust_i| { var item = &self.queue.items[__zust_i]; item.deinit() };
        self.queue.deinit();
    ...
    fn _socketClosed(this: *SendQueue) void {
        ...
        if (was_open and this.after_close_task == null) {
            ...
            this.getGlobalThis().bunVM().enqueueTask(if (this.after_close_task) |value| {
    value
} else {
    ...
    pub fn closeSocketNextTick(this: *SendQueue, nextTick: bool) void {
        ...
        jsc.VirtualMachine.get().enqueueTask(if (this.close_next_tick) |value| {
    value
} else {
    return error.NullPointer;
});
    ...
    pub fn onAckNack(this: *SendQueue, global: *JSGlobalObject, ack_nack: enum { ack, nack }) void {
        ...
        const item = &if (this.waiting_for_ack) |value| {
    value
} else {
    return error.NullPointer;
};
    ...
    if (internal_command) |icmd| {
        switch (icmd) {
            .handle => |msg_data| {
                ...
                const fd: bun.FD = if (bun.take(&send_queue.incoming_fd)) |value| {
    value
} else {
    return error.NullPointer;
};
