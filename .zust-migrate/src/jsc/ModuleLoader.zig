    ...
    switch (loader) {
        .js, .jsx, .ts, .tsx, .json, .jsonc, .toml, .yaml, .json5, .text, .md => {
            ...
            var arena_: ?*bun.ArenaAllocator = brk: {
                ...
                const arena = try safe.Box(bun.ArenaAllocator).init(allocator, undefined);
                arena[0] = bun.ArenaAllocator.init(bun.default_allocator);
                break :brk arena;
            };
...
// safe-transpile: optional unwrap requires manual review
                        arena_.?.deinit();
                        jsc_vm.allocator.destroy(if (arena_) |value| {
    value
} else {
    ...
    return error.NullPointer;
            ...
            var fallback_source: logger.Source = std.mem.zeroes(logger.Source);
...
// safe-transpile: optional unwrap requires manual review
    ...
    return error.NullPointer;
            ...
            printer.ctx.reset();
            defer source_code_printer[0] = printer;
            _ = brk: {
            ...
            const module_info_deserialized: ?*anyopaque = if (module_info) |mi| // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(mi.asDeserialized()) else null;
                ...
                if (virtual_source) |source| {
                    if (globalObject) |globalThis| {
                        ...
                        const decoded: jsc.DecodedJSValue = .{
                            .u = .{ .ptr = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(globalThis) },
                        };
        ...
        .html => {
            ...
            const html_bundle = try jsc.API.HTMLBundle.init(if (globalObject) |value| {
    value
} else {
    return error.NullPointer;
}, path.text);
            return ResolvedSource{
                ...
                .jsvalue_for_export = html_bundle.toJS(if (globalObject) |value| {
    value
} else {
    return error.NullPointer;
}),
            ...
            const value = brk: {
                if (!jsc_vm.origin.isEmpty()) {
                    ...
                    break :brk try bun.String.createUTF8ForJS(if (globalObject) |value| {
    value
} else {
    return error.NullPointer;
}, buf.slice());
...
pub export fn Bun__fetchBuiltinModule(
    ...
    if (ModuleLoader.fetchBuiltinModule(
        ...
        ret[0] = jsc.ErrorableResolvedSource.ok(builtin);
        return true;
    } else {
        return false;
    }
...
pub export fn Bun__transpileFile(
    ...
    var lr = options.getLoaderAndVirtualSource(_specifier.slice(), jsc_vm, &virtual_source_to_use, &blob_to_deinit, type_attribute_str) catch {
        ret[0] = jsc.ErrorableResolvedSource.err(error.JSErrorObject, globalObject.ERR(.MODULE_NOT_FOUND, "Blob not found", .{}).toJS());
        return null;
    };
    ...
    if (force_loader_type.unwrap()) |loader_type| {
        ...
        if (node_module_module.findLongestRegisteredExtension(jsc_vm, _specifier.slice())) |entry| {
            switch (entry) {
                ...
                .custom => |strong| {
                    ret[0] = jsc.ErrorableResolvedSource.ok(ResolvedSource{
                        .allocator = null,
                        .source_code = bun.String.empty,
                        .specifier = .empty,
                        .source_url = .empty,
    ...
    ret[0] = jsc.ErrorableResolvedSource.ok(
    ...
    return error.NullPointer;
            ...
            switch (err) {
                ...
                error.JSError => {
                    ret[0] = jsc.ErrorableResolvedSource.err(error.JSError, globalObject.takeError(error.JSError));
                    return null;
                },
                else => {
...
export fn Bun__resolveEmbeddedNodeFile(vm: *VirtualMachine, in_out_str: *bun.String) bool {
    ...
    in_out_str[0] = bun.String.cloneUTF8(result);
    return true;
}
