pub const AnyPromise = union(enum) {
    ...
    pub fn asJSPromise(this: AnyPromise) *JSPromise {
        return switch (this) {
            .normal => |p| p,
            .internal => |p| // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(p),
        };
    }
    ...
    pub fn wrap(
        ...
        JSC__AnyPromise__wrap(globalObject, this.asValue(), &ctx, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
