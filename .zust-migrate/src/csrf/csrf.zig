...
pub fn generate(
    ...
    const timestamp_u64: u64 = // safe-transpile: @bitCast requires manual review
    @bitCast(@as(i64, timestamp));
    ...
    safe.SimdUtils.copy(payload_buf[0..8], &timestamp_bytes);
    safe.SimdUtils.copy(payload_buf[8..24], &nonce);
    safe.SimdUtils.copy(payload_buf[24..32], &expires_in_bytes);
    ...
    safe.SimdUtils.copy(out_buffer[0..8], &timestamp_bytes);
    safe.SimdUtils.copy(out_buffer[8..24], &nonce);
    safe.SimdUtils.copy(out_buffer[24..32], &expires_in_bytes);
    safe.SimdUtils.copy(out_buffer[32 .. 32 + digest.len], digest);
...
pub fn verify(options: VerifyOptions) bool {
    ...
    const decoded: safe.Slice(u8) = brk: switch (encoding) {
    ...
    const current_time = @as(u64, // safe-transpile: @bitCast requires manual review
    @bitCast(@import("std-fs-compat").milliTimestamp()));
