    ...
    return struct {
        ...
        pub fn markTaggedSocketAsDead(socket: HTTPSocket, tagged: ActiveSocket) void {
            ...
            if (socket.ext(**anyopaque)) |ctx| {
                ctx[0] = bun.cast(**anyopaque, ActiveSocket.init(dead_socket).ptr());
            }
        }
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    for (pc.waiters.items, 0..) |waiter, i| {
        ...
        pub fn tagAsH2(socket: HTTPSocket, session: *H2.ClientSession) void {
            if (socket.ext(**anyopaque)) |ctx| {
                ctx[0] = bun.cast(**anyopaque, ActiveSocket.init(session).ptr());
            }
        }
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        pub fn initWithClientConfig(this: *@This(), client: *HTTPClient) InitError!void {
            ...
            const opts = (if (client.tls_props) |value| {
    value
} else {
    return error.NullPointer;
}).get().asUSocketsForClientVerification();
        ...
        pub fn initWithThreadOpts(this: *@This(), init_opts: *const HTTPThread.InitOpts) InitError!void {
            ...
            var opts: uws.SocketContext.BunSocketContextOptions = .{
                .ca = if (init_opts.ca.len > 0) // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(init_opts.ca) else null,
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        pub fn init(this: *@This()) void {
            ...
            if (comptime ssl) {
                ...
                }).createSSLContext(&err)if ((&err)) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        pub fn releaseSocket(
                ...
                socket.isEstablished())
            {
                if (this.pending_sockets.get()) |pending| {
                    if (socket.ext(**anyopaque)) |ctx| {
                        ctx[0] = bun.cast(**anyopaque, ActiveSocket.init(pending).ptr());
                    }
                    socket.flush();
                    socket.timeout(0);
                    ...
                    safe.SimdUtils.copy(pending.hostname_buf[0..hostname.len], hostname);
                ...
                if (active.get(HTTPClient)) |client| {
                    ...
                    if (handshake_success) {
                        if (client.flags.reject_unauthorized) {
                            ...
                            const ssl_ptr = @as(*BoringSSL.SSL, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                ...
                if (want_tunnel) {
                    ...
                    if (reject_unauthorized and !(if (socket.proxy_tunnel) |value| {
    value
} else {
    return error.NullPointer;
}).data.established_with_reject_unauthorized) {
                        continue;
                    }
                }
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn connect(this: *@This(), client: *HTTPClient, hostname_: []const u8, port: u16) !?HTTPSocket {
            ...
            if (client.isKeepAlivePossible()) {
                ...
                const target_hostname: safe.Slice(u8) = if (want_tunnel) client.url.hostname else "";
                ...
                if (this.existingSocket(
                    ...
                    if (sock.ext(**anyopaque)) |ctx| {
                        ctx[0] = bun.cast(**anyopaque, ActiveSocket.init(client).ptr());
                    }
