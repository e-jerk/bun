...
pub fn record(origin_host: []const u8, origin_port: u16, field_value: []const u8) void {
    ...
    const entry = parse(field_value) catch {
        // `clear`
        if (cache.fetchRemove(k)) |kv| // safe-transpile: free removed (memory owned by safe type);
        log("alt-svc clear {s}", .{k});
        return;
    } orelse return;

    const now = @import("std-fs-compat").timestamp();
...
pub fn lookup(origin_host: []const u8, origin_port: u16) ?u16 {
    ...
    if (@import("std-fs-compat").timestamp() >= rec.expires_at) {
        if (cache.fetchRemove(k)) |kv| // safe-transpile: free removed (memory owned by safe type);
        return null;
    }
    return rec.h3_port;
