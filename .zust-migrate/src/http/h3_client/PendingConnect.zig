...
pub fn register(session: *ClientSession, pc: *quic.PendingConnect, l: *uws.Loop) void {
    ...
    session.ref();
    bun.dns.internal.registerQuic(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(pc.addrinfo())), self);
}
