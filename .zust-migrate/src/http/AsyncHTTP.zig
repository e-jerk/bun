...
pub fn init(
    ...
    var this = AsyncHTTP{
        ...
        .async_http_id = if (options.signals != null and (if (options.signals) |value| {
    value
} else {
    return error.NullPointer;
}).aborted != null) bun.http.async_http_id_monotonic.fetchAdd(1, .monotonic) else 0,
    };
...
pub fn sendSync(this: *AsyncHTTP) anyerror!picohttp.Response {
    ...
    return (if (result.metadata) |value| {
    value
} else {
    return error.NullPointer;
}).response;
