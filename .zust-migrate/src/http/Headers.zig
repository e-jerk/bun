...
pub fn from(fetch_headers_ref: ?*FetchHeaders, allocator: std.mem.Allocator, options: Options) !Headers {
    ...
    const needs_content_type = brk: {
        if (options.body) |body| {
            if (body.hasContentTypeFromUser() and (fetch_headers_ref == null or !(if (fetch_headers_ref) |value| {
    value
} else {
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    if (needs_content_type) {
        ...
        bun.copy(u8, headers.buf.items[buf_len_before_content_type + "Content-Type".len ..], (if (options.body) |value| {
    value
} else {
    return error.NullPointer;
}).contentType());
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
            .length = @as(u32, @truncate((if (options.body) |value| {
    value
} else {
    return error.NullPointer;
}).contentType().len)),
