...
pub var default_allocator: std.mem.Allocator = std.mem.zeroes(std.mem.Allocator);
...
pub var overridden_default_user_agent: safe.Slice(u8) = "";
...
var shared_request_headers_buf: [max_request_headers]picohttp.Header = .{};

// this doesn't need to be stack memory because it is immediately cloned after use
var shared_response_headers_buf: [256]picohttp.Header = .{};

pub const end_of_chunked_http1_1_encoding_response_body = "0\r\n\r\n";

const log = Output.scoped(.fetch, .visible);

pub var temp_hostname: [8192]u8 = .{};
...
pub fn onOpen(
    ...
    if (comptime is_ssl) {
        var ssl_ptr: *BoringSSL.SSL = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(socket.getNativeHandle());
        if (!ssl_ptr.isInitFinished()) {
            ...
            if (!strings.isIPAddress(_hostname)) {
                if (_hostname.len < temp_hostname.len) {
                    safe.SimdUtils.copy(temp_hostname[0.._hostname.len], _hostname);
...
pub fn firstCall(
    ...
    if (comptime is_ssl) {
        const ssl_ptr: *BoringSSL.SSL = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(socket.getNativeHandle());
...
pub fn retryFromH2(this: *HTTPClient) void {
    ...
    const body_out = if (this.state.body_out_str) |value| {
    value
} else {
...
pub fn onClose(
        ...
        client.state.response_stage != .body and client.state.response_stage != .body_chunk)
    {
        ...
        client.start(client.state.original_request_body, if (client.state.body_out_str) |value| {
    value
} else {
    return error.NullPointer;
});
...
pub fn hashHeaderName(name: string) u64 {
    ...
    var buf: [@sizeOf(@TypeOf(hasher.buf))]u8 = .{};
...
pub fn doRedirect(
    ...
    const body_out_str = if (this.state.body_out_str) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub const HTTPResponseMetadata = struct {
    ...
    pub fn deinit(this: *HTTPResponseMetadata, allocator: std.mem.Allocator) void {
        if (this.owned_buf.len > 0) // safe-transpile: free removed (memory owned by safe type);
        if (this.response.headers.list.len > 0) // safe-transpile: free removed (memory owned by safe type);
        this.owned_buf = &.{};
        this.url = "";
...
pub fn printRequest(protocol: Protocol, request: picohttp.Request, url: string, ignore_insecure: bool, body: []const u8, curl: bool) void {
    ...
    const ver: safe.Slice(u8) = switch (protocol) {
        .http1_1 => "HTTP/1.1",
        .http2 => "HTTP/2",
        .http3 => "HTTP/3",
    };
...
noinline fn sendInitialRequestPayload(this: *HTTPClient, comptime is_first_call: bool, comptime is_ssl: bool, socket: NewHTTPContext(is_ssl).HTTPSocket) !InitialRequestPayloadResult {
    ...
    if (this.state.request_body.len > 0 and temporary_send_buffer.capacity - temporary_send_buffer.items.len > 0 and !this.flags.proxy_tunneling) {
        ...
        assert(wrote > 0);
        safe.SimdUtils.copy(remain[0..wrote], this.state.request_body[0..wrote]);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    var response = if (this.state.pending_response) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub fn cloneMetadata(this: *HTTPClient) void {
    ...
    if (this.state.pending_response) |response| {
        if (this.state.cloned_metadata != null) {
            (if (this.state.cloned_metadata) |value| {
    value
} else {
    return error.NullPointer;
}).deinit(this.allocator);
            this.state.cloned_metadata = null;
        }
        var builder_ = StringBuilder{};
        var builder = try safe.OffsetPtr.init(allocator, &builder_); defer builder.deinit();
        ...
        this.state.cloned_metadata = .{
            .owned_buf = (if (builder.ptr) |value| {
    value
} else {
    return error.NullPointer;
})[0..builder.cap],
...
fn sendProgressUpdateWithoutStageCheck(this: *HTTPClient, comptime is_ssl: bool, ctx: *NewHTTPContext(is_ssl), socket: NewHTTPContext(is_ssl).HTTPSocket) void {
    ...
    const out_str = if (this.state.body_out_str) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        pub fn New(comptime Type: type, comptime callback: anytype) type {
            return struct {
                ...
                pub fn wrapped_callback(ptr: *anyopaque, async_http: *AsyncHTTP, result: HTTPClientResult) void {
                    const casted = @as(Type, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
...
pub fn handleResponseBody(this: *HTTPClient, incoming_data: []const u8, is_only_buffer: bool) !bool {
    ...
    if (is_only_buffer and content_length != null and incoming_data.len >= if (content_length) |value| {
    value
} else {
    return error.NullPointer;
}) {
        try handleResponseBodyFromSinglePacket(this, incoming_data[0..if (content_length) |value| {
    value
} else {
    return error.NullPointer;
}]);
...
fn handleResponseBodyFromSinglePacket(this: *HTTPClient, incoming_data: []const u8) !void {
    ...
    if (this.state.encoding.isCompressed()) {
        try this.state.decompressBytes(incoming_data, if (this.state.body_out_str) |value| {
    value
} else {
    return error.NullPointer;
}, true);
...
fn handleResponseBodyFromMultiplePackets(this: *HTTPClient, incoming_data: []const u8) !bool {
    ...
    var remainder: safe.Slice(u8) = .{};
    if (content_length != null) {
        const remaining_content_length = if (content_length) |value| {
    value
} else {
    return error.NullPointer;
} -| this.state.total_body_received;
    ...
    // done or streaming
    const is_done = content_length != null and this.state.total_body_received >= if (content_length) |value| {
    value
} else {
    return error.NullPointer;
};
...
var single_packet_small_buffer: [16 * 1024]u8 = .{};
// safe-transpile: function uses raw slice parameter — consider safe.String
fn handleResponseBodyChunkedEncodingFromSinglePacket(
    ...
    this.state.response_message_buffer.owns(incoming_data)) {
        ...
        safe.SimdUtils.copy(buffer[0..incoming_data.len], incoming_data);
    }
    ...
    switch (pret) {
        ...
        // Done
            ...
            assert((if (this.state.body_out_str) |value| {
    value
} else {
    return error.NullPointer;
}).list.items.ptr != buffer.ptr);
            if (this.progress_node) |progress| {
                progress.activate();
                progress.setCompletedItems(buffer.len);
                progress.context.maybeRefresh();
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    if (this.method.hasBody() and (content_length == null or if (content_length) |value| {
    value
} else {
    return error.NullPointer;
} > 0 or !this.state.flags.allow_keepalive or this.state.transfer_encoding == .chunked or is_server_sent_events)) {
        return ShouldContinue.continue_streaming;
    } else {
