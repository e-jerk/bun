...
pub const Address = extern union {
    ...
    pub fn initIp4(ip: [4]u8, port: u16) Address {
        const result: Address = .{ .in = .{
            ...
            .addr = @as(u32, // safe-transpile: @bitCast requires manual review
    @bitCast(ip)),
            .zero = [_]u8{0} ** 8,
        }};
        return result;
    ...
    pub fn initPosix(addr: *const std.posix.sockaddr) Address {
        return switch (addr.family) {
            std.posix.AF.INET => .{ .in = @as(*const std.posix.sockaddr.in, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(addr))).* },
            std.posix.AF.INET6 => .{ .in6 = @as(*const std.posix.sockaddr.in6, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(addr))).* },
