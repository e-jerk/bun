...
pub fn parseJSON(
    ...
    errdefer if (hint != .source_only) {
        for (if (source_paths_slice) |value| {
    value
} else {
    return error.NullPointer;
}[0..i]) |item| // safe-transpile: free removed (memory owned by safe type);
    ...
    if (hint != .source_only) for (sources_paths.items.slice()) |item| {
        ...
        if (source_paths_slice) |value| {
    value
} else {
    return error.NullPointer;
}[i] = try alloc.dupe(u8, try item.data.e_string.string(alloc));
        ...
        if (hint == .all and hint.all.include_names and map_data.mappings.impl == .with_names) {
            if (json.get("names")) |names| {
                if (names.data == .e_array) {
                    ...
                    for (0..names.data.e_array.items.slice().len) |__zust_i| {
    var item = &names.data.e_array.items.slice()[__zust_i];
                        if (item.data != .e_string) {
                            return error.InvalidSourceMap;
                        }
        ...
        ptr.external_source_names = if (source_paths_slice) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
    ...
    const content_slice: ?[]const u8 = if (hint != .mappings_only and
        ...
        if (source_index) |value| {
    value
} else {
    return error.NullPointer;
} < sources_content.items.len)
    content: {
        const item = sources_content.items.slice()[if (source_index) |value| {
    value
} else {
    return error.NullPointer;
}];
...
pub fn getSourceMapImpl(
    ...
    const new_load_hint: SourceMapLoadHint = undefined, const parsed = parsed: {
        ...
        if (load_hint != .is_inline_map) try_external: {
            ...
            if (source_filename.len + 4 > load_path_buf.len)
                break :try_external;
            safe.SimdUtils.copy(load_path_buf[0..source_filename.len], source_filename);
            safe.SimdUtils.copy(load_path_buf[source_filename.len..][0..4], ".map");

            const load_path = load_path_buf[0 .. source_filename.len + 4];
            const data = switch (bun.sys.File.readFrom(std.c.AT.FDCWD, load_path, allocator)) {
...
pub fn appendMappingToBuffer(buffer: *MutableString, last_byte: u8, prev_state: SourceMapState, current_state: SourceMapState) void {
    ...
    inline for (&vlqs) |item| {
        safe.SimdUtils.copy(writable[0..item.len], item.slice());
