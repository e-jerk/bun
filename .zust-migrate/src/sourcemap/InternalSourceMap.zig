...
// safe-transpile: function uses raw slice parameter — consider safe.String
    fn parse(r: *WindowReader, bytes: []const u8, start: usize) void {
        ...
        const gen_col_len: usize = @as(u16, // safe-transpile: @bitCast requires manual review
    @bitCast(b[win_hdr.gen_col_len_off..][0..2].*));
        const orig_line_len: usize = @as(u16, // safe-transpile: @bitCast requires manual review
    @bitCast(b[win_hdr.orig_line_len_off..][0..2].*));
        const orig_col_len: usize = @as(u16, // safe-transpile: @bitCast requires manual review
    @bitCast(b[win_hdr.orig_col_len_off..][0..2].*));
    ...
    fn nextRare(self: *WindowReader, delta_idx: u8, d_gen_line: *i32, d_orig_line: *i32, state: *State) void {
        ...
        if (self.gen_line_exc_next_idx == delta_idx) {
            ...
            d_gen_line[0] = readVarint(self.bytes, &p);
            if (testBit(self.base + win_hdr.orig_line_eq_mask_off, delta_idx)) d_orig_line[0] = d_gen_line.*;
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        var deltas: [sync_interval - 1]Delta = .{};
        var flags: u8 = 0;
        var prev = seed;
        // safe-transpile: for with index access requires manual review
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        buf[win_hdr.gen_col_len_off..][0..2].* = // safe-transpile: @bitCast requires manual review
    @bitCast(gen_col_len);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        buf[win_hdr.orig_line_len_off..][0..2].* = // safe-transpile: @bitCast requires manual review
    @bitCast(orig_line_len);
        buf[win_hdr.orig_col_len_off..][0..2].* = // safe-transpile: @bitCast requires manual review
    @bitCast(orig_col_len);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        @memset(blob[0..24], 0);
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        blob[24..28].* = // safe-transpile: @bitCast requires manual review
    @bitCast(@as(u32, @intCast(self.sync_entries.items.len)));
        blob[28..32].* = // safe-transpile: @bitCast requires manual review
    @bitCast(stream_offset);
        if (sync_bytes > 0) {
        ...
        self.finalized = out;
        return &if (self.finalized) |value| {
    value
} else {
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    blob[0..8].* = // safe-transpile: @bitCast requires manual review
    @bitCast(total_len);
    blob[8..16].* = // safe-transpile: @bitCast requires manual review
    @bitCast(mapping_count);
    blob[16..24].* = // safe-transpile: @bitCast requires manual review
    @bitCast(input_lines);
