...
pub const List = struct {
    ...
    fn ensureWithNames(this: *List, allocator: std.mem.Allocator) !void {
        ...
        safe.SimdUtils.copy(new_slices.items(.generated), old_slices.items(.generated));
        safe.SimdUtils.copy(new_slices.items(.original), old_slices.items(.original));
        safe.SimdUtils.copy(new_slices.items(.source_index), old_slices.items(.source_index));
