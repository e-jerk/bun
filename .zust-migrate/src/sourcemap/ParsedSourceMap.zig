...
const AnySourceProvider = union(enum) {
    ...
    pub fn ptr(this: AnySourceProvider) *anyopaque {
        return switch (this) {
            .zig => // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.zig),
            .bake => // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.bake),
            .dev_server => // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this.dev_server),
        };
    }
