...
pub fn fromCallbackAutoDeinit(ptr: anytype, comptime fieldName: [:0]const u8) *AnyTaskWithExtraContext {
    ...
    const Wrapper = struct {
        ...
        pub fn function(this: *anyopaque, extra: *anyopaque) void {
            const that: *@This() = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    ...
    const task = bun.handleOom(safe.Box(Wrapper).init(allocator, undefined));
    task[0] = Wrapper{
        .any_task = AnyTaskWithExtraContext{
...
pub fn New(comptime Type: type, comptime ContextType: type, comptime Callback: anytype) type {
    ...
    @ptrCast(@alignCast(this.?))),
                    @as(*ContextType, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(extra.?))),
                },
            );
        }
    };
