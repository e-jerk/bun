...
pub const PackedNextPtr = enum(usize) {
    ...
    pub inline fn setPtr(self: *PackedNextPtr, ptr: ?*ConcurrentTask) void {
        ...
        const ptr_bits = if (ptr) |p| @intFromPtr(p) else 0;
        self[0] = @enumFromInt(ptr_bits | auto_del);
    }
    ...
    pub inline fn atomicLoadPtr(self: *const PackedNextPtr, ordering: std.builtin.AtomicOrder) ?*ConcurrentTask {
        const value = @atomicLoad(usize, @as(*const usize, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(self)), ordering);
    ...
    pub inline fn atomicStorePtr(self: *PackedNextPtr, ptr: ?*ConcurrentTask, ordering: std.builtin.AtomicOrder) void {
        ...
        const self_ptr: *usize = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(self);
...
pub fn createFrom(task: anytype) *ConcurrentTask {
    jsc.markBinding(@src());
    return safe.Box(Task.init(task)).init(allocator, undefined);
}

pub fn fromCallback(ptr: anytype, comptime callback: anytype) *ConcurrentTask {
    jsc.markBinding(@src());

    return safe.Box(ManagedTask.New(std.meta.Child(@TypeOf(ptr)), callback).init(ptr)).init(allocator, undefined);
}
