...
pub fn NewSocketHandler(comptime is_ssl: bool) type {
        ...
        pub fn ssl(this: ThisSocket) ?*BoringSSL.SSL {
            if (comptime is_ssl) {
                if (this.getNativeHandle()) |handle| {
                    return @as(*BoringSSL.SSL, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(handle));
                }
                return null;
            }
    ...
    @ptrCast(switch (this.socket) {
                .connected => |socket| socket.getNativeHandle(),
                .connecting => |socket| socket.getNativeHandle(),
                .detached => null,
                .upgradedDuplex => |socket| if (is_ssl) @as(*anyopaque, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(socket.ssl() orelse return null)) else null,
                .pipe => |socket| if (is_ssl and Environment.isWindows) @as(*anyopaque, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(socket.ssl() orelse return null)) else null,
            } orelse return null);
        }
