...
pub const Timer = opaque {
    ...
    pub fn set(this: *Timer, ptr: anytype, cb: ?*const fn (*Timer) callconv(.c) void, ms: i32, repeat_ms: i32) void {
        ...
        @as(*@TypeOf(ptr), // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(value_ptr))).* = ptr;
    }
    ...
    @ptrCast(@alignCast(c.us_timer_ext(this).*.?)));
    }

    pub fn as(this: *Timer, comptime Type: type) Type {
        @setRuntimeSafety(false);
        return if (@as(*?Type, @ptrCast(@alignCast(c.us_timer_ext(this)))).*) |value| {
    value
} else {
