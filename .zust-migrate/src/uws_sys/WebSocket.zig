pub fn NewWebSocket(comptime ssl_flag: c_int) type {
    return opaque {
        ...
        pub fn raw(this: *WebSocket) *RawWebSocket {
            return @as(*RawWebSocket, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this));
        }
    ...
    @ptrCast(@alignCast(c.uws_ws_get_user_data(ssl_flag, this.raw()))));
        }
        ...
        pub fn cork(this: *WebSocket, ctx: anytype, comptime callback: anytype) void {
            ...
            const Wrapper = struct {
                pub fn wrap(user_data: ?*anyopaque) callconv(.c) void {
                    @call(bun.callmod_inline, callback, .{bun.cast(ContextType, if (user_data) |value| {
    value
} else {
...
pub const RawWebSocket = opaque {
    ...
    pub fn asSocket(this: *RawWebSocket) *uws.Socket {
        return @as(*uws.Socket, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this));
    }
};
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn publishWithOptions(ssl: bool, app: *anyopaque, topic: []const u8, message: []const u8, opcode: Opcode, compress: bool) bool {
        return switch (ssl) {
            inline else => |tls| uws.NewApp(tls).publishWithOptions(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app), topic, message, opcode, compress),
        };
    }
    ...
    pub fn Wrap(
        ...
        return extern struct {
            ...
            pub fn onOpen(raw_ws: *RawWebSocket) callconv(.c) void {
                const ws = @unionInit(AnyWebSocket, active_field_name, @as(*WebSocket, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(raw_ws)));
                const this = if (ws.as(Type)) |value| {
    value
} else {
    return error.NullPointer;
};
    ...
    return error.NullPointer;
            ...
            pub fn onUpgrade(ptr: *anyopaque, res: *uws_res, req: *Request, context: *uws.WebSocketUpgradeContext, id: usize) callconv(.c) void {
                @call(bun.callmod_inline, Server.onWebSocketUpgrade, .{
                    bun.cast(*Server, ptr),
                    @as(*NewApp(is_ssl).Response, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(res)),
                    req,
                    context,
                    id,
