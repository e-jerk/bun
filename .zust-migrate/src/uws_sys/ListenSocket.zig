...
// safe-transpile: function returns small constant slice — consider safe.String
    ...
    pub fn ext(this: *ListenSocket, comptime T: type) *T {
        return // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(c.us_listen_socket_ext(this)));
    }
    ...
    pub fn addServerName(this: *ListenSocket, hostname: [*:0]const u8, ssl_ctx: *uws.SslCtx, user: anytype) bool {
        const U = @TypeOf(user);
        const erased: ?*anyopaque = if (U == @TypeOf(null)) null else // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@constCast(user));
    ...
    pub fn findServerNameUserdata(this: *ListenSocket, comptime T: type, hostname: [*:0]const u8) ?*T {
        return // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(c.us_listen_socket_find_server_name_userdata(this, hostname)));
    }
