...
pub const SocketGroup = extern struct {
    ...
    pub fn init(self: *SocketGroup, loop_: *Loop, vt: ?*const VTable, owner_ptr: anytype) void {
        ...
        const erased: ?*anyopaque = if (P == @TypeOf(null)) null else switch (@typeInfo(P)) {
            .pointer => |p| if (p.size == .one) // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@constCast(owner_ptr)) else @compileError("SocketGroup.init owner must be a single-item pointer"),
            .optional => if (owner_ptr) |o| // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@constCast(o)) else null,
    ...
    pub fn connect(
        ...
        return if (has_dns_resolved != 0)
            .{ .socket = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(ptr)) }
        else
            .{ .connecting = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(ptr)) };
    }
