pub fn NewApp(comptime ssl: bool) type {
    ...
    return struct {
        ...
        pub fn close(this: *ThisApp) void {
            return c.uws_app_close(ssl_flag, @as(*uws_app_s, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this)));
        }

        pub fn closeIdleConnections(this: *ThisApp) void {
            return c.uws_app_close_idle(ssl_flag, @as(*uws_app_s, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this)));
        }
    ...
    @ptrCast(c.uws_create_app(ssl_flag, opts));
        }

        pub fn destroy(app: *ThisApp) void {
            return c.uws_app_destroy(ssl_flag, @as(*uws_app_s, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)));
        }

        pub fn setFlags(this: *ThisApp, require_host_header: bool, use_strict_method_validation: bool) void {
            return c.uws_app_set_flags(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this)), require_host_header, use_strict_method_validation);
        }

        pub fn setMaxHTTPHeaderSize(this: *ThisApp, max_header_size: u64) void {
            return c.uws_app_set_max_http_header_size(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this)), max_header_size);
        }

        pub fn clearRoutes(app: *ThisApp) void {
            return c.uws_app_clear_routes(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)));
        }
...
// safe-transpile: function uses raw slice parameter — consider safe.String
                    ...
                    if (comptime UserDataType == void) {
                        return @call(
                            ...
                            .{
                                {},
                                req,
                                @as(*Response, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(res))),
                        ...
                        return @call(
                            ...
                            handler,
                            .{
                                @as(UserDataType, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    ...
    @ptrCast(@alignCast(res))),
        ...
        pub const ListenSocket = opaque {
            pub inline fn close(this: *ThisApp.ListenSocket) void {
                return @as(*uws.ListenSocket, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this)).close();
            }
            pub inline fn getLocalPort(this: *ThisApp.ListenSocket) i32 {
                return @as(*uws.ListenSocket, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this)).getLocalPort();
            }

            pub fn socket(this: *ThisApp.ListenSocket) uws.NewSocketHandler(ssl) {
                return uws.NewSocketHandler(ssl).from(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(this));
            }
        };
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn get(
            ...
            c.uws_app_get(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), pattern.ptr, pattern.len, RouteHandler(UserDataType, handler).handle, if (UserDataType == void) null else user_data);
        }
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn post(
            app: *ThisApp,
            ...
            c.uws_app_post(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), pattern.ptr, pattern.len, RouteHandler(UserDataType, handler).handle, if (UserDataType == void) null else user_data);
        }
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn options(
            ...
            c.uws_app_options(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), pattern.ptr, pattern.len, RouteHandler(UserDataType, handler).handle, if (UserDataType == void) null else user_data);
        }
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn delete(
            ...
            c.uws_app_delete(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), pattern.ptr, pattern.len, RouteHandler(UserDataType, handler).handle, if (UserDataType == void) null else user_data);
        }
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn patch(
            ...
            c.uws_app_patch(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), pattern.ptr, pattern.len, RouteHandler(UserDataType, handler).handle, if (UserDataType == void) null else user_data);
        }
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn put(
            ...
            c.uws_app_put(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), pattern.ptr, pattern.len, RouteHandler(UserDataType, handler).handle, if (UserDataType == void) null else user_data);
        }
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn head(
            ...
            c.uws_app_head(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), pattern.ptr, pattern.len, RouteHandler(UserDataType, handler).handle, if (UserDataType == void) null else user_data);
        }
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn connect(
            ...
            c.uws_app_connect(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), pattern.ptr, pattern.len, RouteHandler(UserDataType, handler).handle, if (UserDataType == void) null else user_data);
        }
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn trace(
            ...
            c.uws_app_trace(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), pattern.ptr, pattern.len, RouteHandler(UserDataType, handler).handle, if (UserDataType == void) null else user_data);
        }
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn method(
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn any(
            ...
            c.uws_app_any(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), pattern.ptr, pattern.len, RouteHandler(UserDataType, handler).handle, if (UserDataType == void) null else user_data);
        }
        pub fn domain(app: *ThisApp, pattern: [:0]const u8) void {
            c.uws_app_domain(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), pattern);
        }
        pub fn run(app: *ThisApp) void {
            return c.uws_app_run(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)));
        }
        pub fn listen(
            app: *ThisApp,
            ...
            const Wrapper = struct {
                pub fn handle(socket: ?*uws.ListenSocket, conf: c.uws_app_listen_config_t, data: ?*anyopaque) callconv(.c) void {
                    if (comptime UserData == void) {
                        @call(bun.callmod_inline, handler, .{ {}, @as(?*ThisApp.ListenSocket, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(socket)), conf });
                    } else {
                        @call(bun.callmod_inline, handler, .{
                            @as(UserData, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(data.?))),
                            @as(?*ThisApp.ListenSocket, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(socket)),
                            conf,
                        });
                    }
            ...
            return c.uws_app_listen(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), port, Wrapper.handle, user_data);
        }
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    @ptrCast(@alignCast(data))),
            ...
            return c.uws_app_set_on_clienterror(ssl_flag, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app), Wrapper.handle, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(user_data));
        }
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            return c.uws_app_listen_with_config(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), config.host, @as(u16, @intCast(config.port)), config.options, Wrapper.handle, user_data);
        }
    ...
    @ptrCast(socket)),
            ...
            return c.uws_app_listen_domain_with_options(
                ssl_flag,
                @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)),
                domain_name.ptr,
                domain_name.len,
                flags,
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn numSubscribers(app: *ThisApp, topic: []const u8) u32 {
            return c.uws_num_subscribers(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), topic.ptr, topic.len);
        }
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn publish(app: *ThisApp, topic: []const u8, message: []const u8, opcode: Opcode, compress: bool) bool {
            return c.uws_publish(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), topic.ptr, topic.len, message.ptr, message.len, opcode, compress);
        }
        ...
        pub fn removeServerName(app: *ThisApp, hostname_pattern: [*:0]const u8) void {
            return c.uws_remove_server_name(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), hostname_pattern);
        }
        pub fn addServerName(app: *ThisApp, hostname_pattern: [*:0]const u8) void {
            return c.uws_add_server_name(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), hostname_pattern);
        }
        pub fn addServerNameWithOptions(app: *ThisApp, hostname_pattern: [*:0]const u8, opts: BunSocketContextOptions) !void {
            if (c.uws_add_server_name_with_options(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), hostname_pattern, opts) != 0) {
        ...
        pub fn missingServerName(app: *ThisApp, handler: c.uws_missing_server_handler, user_data: ?*anyopaque) void {
            return c.uws_missing_server_name(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), handler, user_data);
        }
        pub fn filter(app: *ThisApp, handler: c.uws_filter_handler, user_data: ?*anyopaque) void {
            return c.uws_filter(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), handler, user_data);
        }
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn ws(app: *ThisApp, pattern: []const u8, ctx: *anyopaque, id: usize, behavior_: WebSocketBehavior) void {
            var behavior = behavior_;
            uws_ws(ssl_flag, @as(*uws_app_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(app)), ctx, pattern.ptr, pattern.len, id, &behavior);
        }
