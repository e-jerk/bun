...
pub fn NewResponse(ssl_flag: i32) type {
    return opaque {
        ...
        pub inline fn castRes(res: *c.uws_res) *Response {
            return @as(*Response, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(res)));
        }

        pub inline fn downcast(res: *Response) *c.uws_res {
            return @as(*c.uws_res, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(res)));
        }

        pub inline fn downcastSocket(res: *Response) *bun.uws.us_socket_t {
            return @as(*bun.uws.us_socket_t, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(res)));
        }
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        ...
        pub fn state(res: *const Response) State {
            return c.uws_res_state(ssl_flag, @as(*const c.uws_res, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(res))));
        }
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        pub fn getNativeHandle(res: *Response) bun.FD {
            if (comptime Environment.isWindows) {
                ...
                return .fromNative(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(c.uws_res_get_native_handle(ssl_flag, res.downcast())));
            }
            ...
            const Wrapper = struct {
                pub fn handle(this: *c.uws_res, user_data: ?*anyopaque) callconv(.c) void {
                    if (comptime UserDataType == void) {
                        ...
                        // null should always be treated as a no-op, there's no case where it should have any effect.
                        @call(bun.callmod_inline, handler, .{ @as(UserDataType, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(user_data_ptr))), castRes(this) });
                    }
                }
            };
        ...
        pub fn corked(
            ...
            const Wrapper = struct {
                ...
                pub fn handle(user_data: ?*anyopaque) callconv(.c) void {
                    const args: Args = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(user_data.?));
                    @call(bun.callmod_inline, handler_fn, args.*);
                }
            };

            c.uws_res_cork(ssl_flag, res.downcast(), // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@constCast(&args_tuple)), Wrapper.handle);
        }
