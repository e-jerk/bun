...
fn ptrWithoutPanic(buf: []const u8) [*]const u8 {
    const null_terminated_ptr = struct {
        ...
        const null_terminated_ptr: safe.Slice(u8) = &[_]u8{0};
    }.null_terminated_ptr;
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        ...
        pub fn addDocumentContentHandlers(
            ...
            builder.lol_html_rewriter_builder_add_document_content_handlers(
                if (doctype_handler_data != null)
                    DirectiveHandler(DocType, DocTypeHandler, if (doctype_handler) |value| {
    value
} else {
    return error.NullPointer;
                ...
                if (comment_handler_data != null)
                    DirectiveHandler(Comment, CommentHandler, if (comment_handler) |value| {
    value
} else {
    return error.NullPointer;
})
                ...
                if (text_chunk_handler_data != null)
                    DirectiveHandler(TextChunk, TextChunkHandler, if (text_chunk_handler) |value| {
    value
} else {
    return error.NullPointer;
})
                ...
                if (end_tag_handler_data != null)
                    DirectiveHandler(DocEnd, DocEndHandler, if (end_tag_handler) |value| {
    value
} else {
    return error.NullPointer;
})
        ...
        pub fn addElementContentHandlers(
            ...
            return switch (builder.lol_html_rewriter_builder_add_element_content_handlers(
                ...
                if (element_handler != null and element_handler_data != null)
                    DirectiveHandler(Element, ElementHandler, if (element_handler) |value| {
    value
} else {
    return error.NullPointer;
})
...
// safe-transpile: function uses raw slice parameter — consider safe.String
            ...
            return struct {
                fn writeChunk(ptr: [*]const u8, len: usize, user_data: *anyopaque) callconv(.c) void {
                    ...
                    const this = @as(*OutputSinkType, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    pub fn getUserData(this: *const TextChunk, comptime Type: type) ?*Type {
        auto_disable();
        return @as(?*Type, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(this.lol_html_text_chunk_user_data_get())));
    }
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    pub fn getUserData(element: *const Element, comptime Type: type) ?*Type {
        auto_disable();
        return @as(?*Element, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(lol_html_element_user_data_get(element))));
    }
...
// safe-transpile: function returns small constant slice — consider safe.String
    ...
    fn deinit_external(_: [*]u8, ptr: *anyopaque, len: u32) callconv(.c) void {
        auto_disable();
        lol_html_str_free(.{ .ptr = @as([*]const u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(ptr)), .len = len });
    }
