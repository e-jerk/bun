...
pub fn convertEnvToWTF8() bun.OOM!void {
    ...
    while (true) {
        ...
        const str_len = if (std.mem.indexOfScalar(u8, wtf8_buf[len..], 0)) |value| {
    value
} else {
    return error.NullPointer;
        ...
        if (str_len == 0) break; // array ends with empty null-terminated string
        const str_ptr: [*:0]u8 = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(wtf8_buf[len..].ptr);
        try envp.append(str_ptr);
    }
    try envp.append(null);

    const envp_slice: []?[*:0]u8 = try envp.toOwnedSlice();
    const envp_nonnull_slice: [][*:0]u8 = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(envp_slice[0 .. envp_slice.len - 1]);
