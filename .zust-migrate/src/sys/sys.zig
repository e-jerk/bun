...
pub fn getcwdZ(buf: *bun.PathBuffer) Maybe([:0]const u8) {
    ...
    const rc: ?[*:0]u8 = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(std.c.getcwd(buf, bun.MAX_PATH_BYTES));
    return if (rc != null)
        Result{ .result = if (rc) |value| {
    value
} else {
    return error.NullPointer;
}[0..std.mem.len(if (rc) |value| {
    value
} else {
    return error.NullPointer;
}) :0] }
    else
        if (Result.errnoSysP(@as(c_int, 0), .getcwd, buf)) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        if (@as(isize, // safe-transpile: @bitCast requires manual review
    @bitCast(rc)) > 0) {
...
pub fn fcntl(fd: bun.FD, cmd: i32, arg: anytype) Maybe(fnctl_int) {
    ...
    while (true) {
        ...
        const result = switch (@TypeOf(arg)) {
            ...
            i64 => fcntl_symbol(fd.cast(), cmd, @as(c_long, // safe-transpile: @bitCast requires manual review
    @bitCast(arg))),
...
pub fn normalizePathWindows(
    ...
    if (std.fs.path.isAbsoluteWindowsWTF16(path)) {
        ...
        if (path_.len >= 4 and path.len >= 4) {
            ...
            {
                safe.SimdUtils.copy(buf[0..bun.strings.w("\\??\\NUL").len], bun.strings.w("\\??\\NUL"));
                ...
                (path[3] == '/' or path[3] == '\\'))
            {
                ...
                if (path[2] == '.') {
                    ...
                    buf[0..4].* = .{ '\\', '\\', '.', '\\' };
                    const rest = path[4..];
                    safe.SimdUtils.copy(buf[4..][0..rest.len], rest);
                    buf[path.len] = 0;
    ...
    safe.SimdUtils.copy(buf1[0..base_path.len], base_path);
    buf1[base_path.len] = '\\';
    safe.SimdUtils.copy(buf1[base_path.len + 1 .. joined_len], path);
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    var io: w.IO_STATUS_BLOCK = std.mem.zeroes(w.IO_STATUS_BLOCK);
...
pub fn openFileAtWindowsNtPath(
    ...
    var result: windows.HANDLE = std.mem.zeroes(windows.HANDLE);
    ...
    var io: windows.IO_STATUS_BLOCK = std.mem.zeroes(windows.IO_STATUS_BLOCK);
...
pub fn openatOSPath(dirfd: bun.FD, file_path: bun.OSPathSliceZ, flags: i32, perm: bun.Mode) Maybe(bun.FD) {
    if (comptime Environment.isMac) {
        ...
        const rc = darwin_nocancel.@"openat$NOCANCEL"(dirfd.cast(), file_path.ptr, // safe-transpile: @bitCast requires manual review
    @bitCast(bun.O.toPacked(flags)), perm);
        if (comptime Environment.allow_assert)
    ...
    while (true) {
        ...
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
            ...
            const rc = std.c.openat(dirfd.cast(), file_path, // safe-transpile: @bitCast requires manual review
    @bitCast(bun.O.toPacked(flags)), perm);
            if (comptime Environment.allow_assert)
                log("openat({f}, {s}, {d}) = {d}", .{ dirfd, bun.sliceTo(file_path, 0), flags, rc });
...
pub fn access(path: bun.OSPathSliceZ, mode: i32) Maybe(void) {
    ...
    return Maybe(void).errnoSysP(syscall.access(path, // safe-transpile: @bitCast requires manual review
    @bitCast(mode)), .access, path) orelse .success;
}
...
pub fn writev(fd: bun.FD, buffers: []std.posix.iovec) Maybe(usize) {
    if (comptime Environment.isMac) {
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        const rc = writev_sym(fd.cast(), @as([*]std.posix.iovec_const, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(buffers.ptr)), @as(i32, @intCast(buffers.len)));
        if (comptime Environment.allow_assert)
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            const rc = writev_sym(fd.cast(), @as([*]std.posix.iovec_const, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(buffers.ptr)), @intCast(buffers.len));
            if (comptime Environment.allow_assert)
                log("writev({f}, {d}) = {d}", .{ fd, veclen(buffers), rc });
...
pub fn pread(fd: bun.FD, buf: []u8, offset: i64) Maybe(usize) {
    ...
    const ioffset = @as(i64, // safe-transpile: @bitCast requires manual review
    @bitCast(offset)); // the OS treats this as unsigned
    var __zust_loop_counter: u64 = 0;
    while (true) {
        __zust_loop_counter += 1;
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
...
pub fn ftruncate(fd: bun.FD, size: isize) Maybe(void) {
    if (comptime Environment.isWindows) {
        var io_status_block: std.os.windows.IO_STATUS_BLOCK = std.mem.zeroes(std.os.windows.IO_STATUS_BLOCK);
        var eof_info = std.os.windows.FILE_END_OF_FILE_INFORMATION{
            .EndOfFile = // safe-transpile: @bitCast requires manual review
    @bitCast(size),
        };
    ...
    return var __zust_loop_counter: u64 = 0;
    while (true) {
        __zust_loop_counter += 1;
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
...
fn getFdPathFreeBSDLinuxulator(fd: bun.FD, out_buffer: *bun.PathBuffer) Maybe([]u8) {
    var buf: ["/dev/fd/-2147483648".len + 1:0]u8 = .{};
...
pub fn getFdPath(fd: bun.FD, out_buffer: *bun.PathBuffer) Maybe([]u8) {
    switch (Environment.os) {
        .windows => {
            var wide_buf: [windows.PATH_MAX_WIDE]u16 = .{};
        ...
        .linux => {
            ...
            var buf: ["/proc/self/fd/-2147483648".len + 1:0]u8 = .{};
        ...
        .freebsd => {
            ...
            var info: c.struct_kinfo_file = std.mem.zeroes(c.struct_kinfo_file);
            ...
            safe.SimdUtils.copy(out_buffer[0..path.len], path);
...
pub fn mmap(
    ...
    if (rc == fail) {
        return .initErr(.{
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
            .errno = @as(sys.Error.Int, @truncate(@intFromEnum(getErrno(@as(i64, // safe-transpile: @bitCast requires manual review
    @bitCast(@intFromPtr(fail))))))),
            .syscall = .mmap,
        });
    }

    return .initResult(@as([*]align(page_size_min) u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(rc)))[0..length]);
}
...
pub fn getFileAttributes(path: anytype) ?WindowsFileAttributes {
    ...
    if (T == u16) {
        ...
        const attributes: WindowsFileAttributes = // safe-transpile: @bitCast requires manual review
    @bitCast(dword);
        return attributes;
    } else {
        const wbuf = bun.w_path_buffer_pool.get();
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        switch (getErrno(rc)) {
            .INTR => continue,
            else => return if (Maybe(void).errnoSysFd(rc, .futimens, fd)) |value| {
    value
} else {
    return error.NullPointer;
},
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        switch (getErrno(rc)) {
            .INTR => continue,
            else => return if (Maybe(void).errnoSysP(rc, .utimensat, path)) |value| {
    value
} else {
    return error.NullPointer;
},
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
        ...
        var basic_info: w.FILE_BASIC_INFORMATION = std.mem.zeroes(w.FILE_BASIC_INFORMATION);
    ...
    if (std.meta.sentinel(@TypeOf(subpath)) == null) {
        ...
        safe.SimdUtils.copy(path_buf[0..subpath.len], subpath);
        path_buf[subpath.len] = 0;
        const slice: [:0]const u8 = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(path_buf);
...
pub fn isExecutableFilePath(path: anytype) bool {
    ...
    if (comptime Environment.isWindows) {
        var buf: [(bun.MAX_PATH_BYTES / 2) + 1]u16 = .{};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        var plarge_integer: i64 = // safe-transpile: @bitCast requires manual review
    @bitCast(offset_high);
        const rc = kernel32.SetFilePointerEx(
            fd.cast(),
            @as(windows.LARGE_INTEGER, // safe-transpile: @bitCast requires manual review
    @bitCast(offset_low)),
            &plarge_integer,
            windows.FILE_BEGIN,
        );
...
pub fn setFileOffsetToEndWindows(fd: bun.FD) Maybe(usize) {
    if (comptime Environment.isWindows) {
        var new_ptr: std.os.windows.LARGE_INTEGER = std.mem.zeroes(std.os.windows.LARGE_INTEGER);
...
pub fn dupWithFlags(fd: bun.FD, _: i32) Maybe(bun.FD) {
    if (comptime Environment.isWindows) {
        var target: windows.HANDLE = std.mem.zeroes(windows.HANDLE);
...
pub fn linkatTmpfile(tmpfd: bun.FD, dirfd: bun.FD, name: [:0]const u8) Maybe(void) {
    ...
    while (true) {
        ...
        const rc = if (current_status != -1) std.os.linux.linkat(
            ...
            //        AT_SYMLINK_FOLLOW);
            //
            var procfs_buf: ["/proc/self/fd/-2147483648".len + 1:0]u8 = .{};
...
pub fn getFileSize(fd: bun.FD) Maybe(usize) {
    if (Environment.isWindows) {
        var size: windows.LARGE_INTEGER = std.mem.zeroes(windows.LARGE_INTEGER);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        .size = @as(u64, // safe-transpile: @bitCast requires manual review
    @bitCast(st.size)),
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
...
pub fn copyFileZSlowWithHandle(in_handle: bun.FD, to_dir: bun.FD, destination: [:0]const u8) Maybe(void) {
    if (comptime Environment.isWindows) {
        var buf0: bun.WPathBuffer = std.mem.zeroes(bun.WPathBuffer);
        var buf1: bun.WPathBuffer = std.mem.zeroes(bun.WPathBuffer);
