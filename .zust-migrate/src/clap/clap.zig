...
fn parseLongNames(comptime param_str: []const u8) Names {
    comptime {
        ...
        const aliases = blk: {
            var result: [alias_count][]const u8 = .{};
            var idx: usize = 0;
...
fn expectParam(expect: Param(Help), actual: Param(Help)) void {
    ...
    if (expect.names.long) |long| {
        testing.expectEqualStrings(long, if (actual.names.long) |value| {
    value
} else {
...
pub fn usageFull(
    ...
    for (params) |param| {
        ...
        const name = if (param.names.short) |*s| @as([*]const u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(s))[0..1] else param.names.long orelse {
            positional = param;
            continue;
        };
        if (cos.bytes_written != 0)
