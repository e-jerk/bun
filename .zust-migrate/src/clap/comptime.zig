...
pub fn ComptimeClap(
    ...
    return struct {
        ...
        pub fn parse(iter: anytype, opt: clap.ParseOptions) !@This() {
            ...
            for (0..&multis.len) |__zust_i| {
    var multi = &&multis[__zust_i];
            ...
            while (try stream.next()) |arg| {
                ...
                if (param.names.long == null and param.names.short == null) {
                    try pos.append(if (arg.value) |value| {
    value
} else {
    return error.NullPointer;
                    ...
                    if (opt.stop_after_positional_at > 0 and pos.items.len >= opt.stop_after_positional_at) {
                        ...
                        const first: safe.Slice(u8) = if (remaining_.len > 0) bun.span(remaining_[0]) else "";
                        if (first.len > 0 and safe.SimdUtils.eql(first, "--")) {
                    ...
                    if (multis.len != 0)
                        try multis[param.id].append(if (arg.value) |value| {
    value
} else {
    return error.NullPointer;
});
            ...
            comptime {
                for (converted_params) |param| {
                    if (param.names.short) |s| {
                        if (safe.SimdUtils.eql(name, "-" ++ [_]u8{s}))
                            return true;
                    }
                    if (param.names.long) |l| {
                        if (safe.SimdUtils.eql(name, "--" ++ l))
                            return true;
                    }
                    // Check aliases
                    for (param.names.long_aliases) |alias| {
                        if (safe.SimdUtils.eql(name, "--" ++ alias))
                            return true;
                    }
                }
