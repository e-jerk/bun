...
const Token = union(enum) {
    ...
    close,
    eof,
    const Tag = if (@typeInfo(Token).@"union".tag_type) |value| {
    value
} else {
    ...
    if (root.atoms == .single) {
        if (start > 0) {
            if (root.bubble_up) |bubble_up| {
                return expandNested(bubble_up, out, out_key, out_key_counter, if (root.bubble_up_next) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    if (root.bubble_up) |bubble_up| {
        return try expandNested(bubble_up, out, out_key, out_key_counter, if (root.bubble_up_next) |value| {
    value
} else {
    return error.NullPointer;
});
...
pub fn calculateExpandedAmount(tokens: []const Token) u32 {
    ...
    for (tokens) |tok| {
        switch (tok) {
            ...
            .comma => {
                const top = if (nested_brace_stack.lastMut()) |value| {
    value
} else {
    return error.NullPointer;
};
            ...
            .close => {
                const entry = if (nested_brace_stack.pop()) |value| {
    value
} else {
    return error.NullPointer;
};
                ...
                if (nested_brace_stack.len() > 0) {
                    const parent = if (nested_brace_stack.lastMut()) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            .close => {
                var top = if (brace_stack.pop()) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            .comma => {
                var top = if (brace_stack.lastMut()) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            while (brace_stack.len() > 0) {
                const top_idx = if (brace_stack.pop()) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        fn appendChar(self: *@This(), char: Chars.CodepointType) Allocator.Error!void {
            if (self.tokens.items.len > 0) {
                ...
                if (last.* == .text) {
                    ...
                    const len = bun.strings.encodeWTF8Rune(&buf, // safe-transpile: @bitCast requires manual review
    @bitCast(char));
