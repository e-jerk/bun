...
pub fn TracyAllocator(comptime name: ?[:0]const u8) type {
    return struct {
        ...
        fn allocFn(ptr: *anyopaque, len: usize, ptr_align: u8, ret_addr: usize) ?[*]u8 {
            const self = @as(*Self, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
...
fn ___tracy_connected() c_int {
    const Fn = if (dlsym(Tracy.connected, "___tracy_connected")) |value| {
    value
} else {
...
fn ___tracy_set_thread_name(name: [*:0]const u8) void {
    const Fn = if (dlsym(Tracy.set_thread_name, "___tracy_set_thread_name")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_frame_mark_start(name: [*:0]const u8) void {
    const Fn = if (dlsym(Tracy.emit_frame_mark_start, "___tracy_emit_frame_mark_start")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_frame_mark_end(name: [*:0]const u8) void {
    const Fn = if (dlsym(Tracy.emit_frame_mark_end, "___tracy_emit_frame_mark_end")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_zone_begin(
    ...
    const Fn = if (dlsym(Tracy.emit_zone_begin, "___tracy_emit_zone_begin")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_zone_begin_callstack(
    ...
    const Fn = if (dlsym(Tracy.emit_zone_begin_callstack, "___tracy_emit_zone_begin_callstack")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_zone_text(ctx: ___tracy_c_zone_context, txt: [*]const u8, size: usize) void {
    const Fn = if (dlsym(Tracy.emit_zone_text, "___tracy_emit_zone_text")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_zone_name(ctx: ___tracy_c_zone_context, txt: [*]const u8, size: usize) void {
    const Fn = if (dlsym(Tracy.emit_zone_name, "___tracy_emit_zone_name")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_zone_color(ctx: ___tracy_c_zone_context, color: u32) void {
    const Fn = if (dlsym(Tracy.emit_zone_color, "___tracy_emit_zone_color")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_zone_value(ctx: ___tracy_c_zone_context, value: u64) void {
    const Fn = if (dlsym(Tracy.emit_zone_value, "___tracy_emit_zone_value")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_zone_end(ctx: ___tracy_c_zone_context) void {
    const Fn = if (dlsym(Tracy.emit_zone_end, "___tracy_emit_zone_end")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_memory_alloc(ptr: *const anyopaque, size: usize, secure: c_int) void {
    const Fn = if (dlsym(Tracy.emit_memory_alloc, "___tracy_emit_memory_alloc")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_memory_alloc_callstack(ptr: *const anyopaque, size: usize, depth: c_int, secure: c_int) void {
    const Fn = if (dlsym(Tracy.emit_memory_alloc_callstack, "___tracy_emit_memory_alloc_callstack")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_memory_free(ptr: *const anyopaque, secure: c_int) void {
    const Fn = if (dlsym(Tracy.emit_memory_free, "___tracy_emit_memory_free")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_memory_free_callstack(ptr: *const anyopaque, depth: c_int, secure: c_int) void {
    const Fn = if (dlsym(Tracy.emit_memory_free_callstack, "___tracy_emit_memory_free_callstack")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_memory_alloc_named(ptr: *const anyopaque, size: usize, secure: c_int, name: [*:0]const u8) void {
    const Fn = if (dlsym(Tracy.emit_memory_alloc_named, "___tracy_emit_memory_alloc_named")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_memory_alloc_callstack_named(ptr: *const anyopaque, size: usize, depth: c_int, secure: c_int, name: [*:0]const u8) void {
    const Fn = if (dlsym(Tracy.emit_memory_alloc_callstack_named, "___tracy_emit_memory_alloc_callstack_named")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_memory_free_named(ptr: *const anyopaque, secure: c_int, name: [*:0]const u8) void {
    const Fn = if (dlsym(Tracy.emit_memory_free_named, "___tracy_emit_memory_free_named")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_memory_free_callstack_named(ptr: *const anyopaque, depth: c_int, secure: c_int, name: [*:0]const u8) void {
    const Fn = if (dlsym(Tracy.emit_memory_free_callstack_named, "___tracy_emit_memory_free_callstack_named")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_message(txt: [*]const u8, size: usize, callstack: c_int) void {
    const Fn = if (dlsym(Tracy.emit_message, "___tracy_emit_message")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_messageL(txt: [*:0]const u8, callstack: c_int) void {
    const Fn = if (dlsym(Tracy.emit_messageL, "___tracy_emit_messageL")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_messageC(txt: [*]const u8, size: usize, color: u32, callstack: c_int) void {
    const Fn = if (dlsym(Tracy.emit_messageC, "___tracy_emit_messageC")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_messageLC(txt: [*:0]const u8, color: u32, callstack: c_int) void {
    const Fn = if (dlsym(Tracy.emit_messageLC, "___tracy_emit_messageLC")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn ___tracy_emit_frame_mark(name: ?[*:0]const u8) void {
    const Fn = if (dlsym(Tracy.emit_frame_mark, "___tracy_emit_frame_mark")) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn dlsym(comptime Type: type, comptime symbol: [:0]const u8) ?Type {
    ...
    get: {
        if (Handle.handle == null) {
            ...
            const RLTD: std.c.RTLD = if (bun.Environment.isMac) // safe-transpile: @bitCast requires manual review
    @bitCast(@as(i32, -2)) else if (bun.Environment.isLinux) .{} else {};
