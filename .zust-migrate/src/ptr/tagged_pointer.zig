...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    pub inline fn from(val: anytype) TaggedPointer {
        ...
        return switch (ValueType) {
            f64, i64, u64 => // safe-transpile: @bitCast requires manual review
    @bitCast(val),
            ?*anyopaque, *anyopaque => // safe-transpile: @bitCast requires manual review
    @bitCast(@intFromPtr(val)),
            else => @compileError("Unsupported type: " ++ @typeName(ValueType)),
        };
    }

    pub inline fn to(this: TaggedPointer) *anyopaque {
        return @ptrFromInt(@as(u64, // safe-transpile: @bitCast requires manual review
    @bitCast(this)));
    }
};
...
pub fn TagTypeEnumWithTypeMap(comptime Types: anytype) struct {
    ...
    var enum_names: [Types.len][:0]const u8 = .{};
    var enum_values: [Types.len]TaggedPointer.Tag = .{};
    ...
    return .{
        .tag_type = @Type(.{
            .@"enum" = .{
                ...
                .fields = blk: {
                    var enum_fields: [Types.len]std.builtin.Type.EnumField = .{};
                    for (0..Types.len) |i| {
                        enum_fields[i] = .{
                            .name = enum_names[i],
                            .value = enum_values[i],
        ...
        pub inline fn call(this: This, comptime fn_name: []const u8, args_without_this: anytype, comptime Ret: type) Ret {
            inline for (type_map) |entry| {
                if (this.repr.data == entry.value) {
                    ...
                    const args = brk: {
                        var args: std.meta.ArgsTuple(@TypeOf(@field(entry.ty, fn_name))) = std.mem.zeroes(std.meta.ArgsTuple(@TypeOf(@field(entry.ty, fn_name))));
                        args[0] = pointer;
