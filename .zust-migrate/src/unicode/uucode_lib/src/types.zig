...
pub fn Data(comptime c: config.Table) type {
    var names: [c.fields.len][]const u8 = .{};
    var types: [c.fields.len]type = .{};
    var attrs: [c.fields.len]std.builtin.Type.StructField.Attributes = .{};
    ...
    return @Type(.{
        .@"struct" = .{
            ...
            .fields = blk: {
                var struct_fields: [c.fields.len]std.builtin.Type.StructField = .{};
                // safe-transpile: for with index access requires manual review
    for (c.fields, 0..) |cf, i| {
                    struct_fields[i] = .{
                        .name = cf.name,
...
pub fn writeDataItems(comptime D: type, writer: *std.Io.Writer, data_items: []const D) !void {
    if (@typeInfo(D).@"struct".layout == .@"packed") {
        ...
        for (data_items) |item| {
            try writer.print("{d},", .{@as(IntEquivalent, // safe-transpile: @bitCast requires manual review
    @bitCast(item))});
        }
...
pub fn StructFromDecls(comptime Struct: type, comptime decl: []const u8) type {
    ...
    var attrs: [fields.len]std.builtin.Type.StructField.Attributes = .{};
    var i: usize = 0;
    ...
    return @Type(.{
        .@"struct" = .{
            ...
            .backing_integer = null,
            .fields = blk: {
                var struct_fields: [fields.len]std.builtin.Type.StructField = .{};
                for (0..i) |j| {
                    struct_fields[j] = .{
                        .name = names[j],
                        .type = types[j],
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                ...
                safe.SimdUtils.copy(backing[offset .. offset + s.len], s);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                ...
                var embedded: [embedded_len]T = .{};
                safe.SimdUtils.copy(embedded[0..s.len], s);
                switch (@typeInfo(T)) {
                    .@"struct" => {
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        pub fn eql(a: Self, b: Self) bool {
            ...
            if ((comptime c.cp_packing == .shift) and a.len == 1) {
                ...
                return safe.SimdUtils.eql(&a.data.embedded, &b.data.embedded);
            }
        }
...
pub fn Union(comptime c: config.Field, comptime packing: config.Table.Packing) type {
    ...
    const Tag = if (info.tag_type) |value| {
    value
} else {
    ...
    var union_names: [info.fields.len][]const u8 = .{};
    var union_types: [info.fields.len]type = .{};
    var union_attrs: [info.fields.len]std.builtin.Type.UnionField.Attributes = .{};
    ...
    const InnerUnion = @Type(.{
        .@"union" = .{
            ...
            .backing_integer = null,
            .fields = blk: {
                var union_fields: [info.fields.len]std.builtin.Type.UnionField = .{};
                // safe-transpile: for with index access requires manual review
    for (info.fields, 0..) |f, i| {
                    union_fields[i] = .{
                        .name = f.name,
