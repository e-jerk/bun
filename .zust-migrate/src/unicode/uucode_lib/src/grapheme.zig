...
pub fn CustomIterator(
    ...
    return struct {
        ...
        pub fn nextCodePoint(self: *Self) ?IteratorResult {
            ...
            const cp1 = (if (self.next_cp) |value| {
    value
} else {
...
test "Iterator nextCodePoint/peekCodePoint" {
    ...
    try std.testing.expect(it.i == 0);
    try std.testing.expect((if (result) |value| {
    value
} else {
    return error.NullPointer;
}).code_point == 0x1F469); // 👩
    ...
    return error.NullPointer;
}).code_point == 0x1F1E8); // Regional Indicator "C"
    try std.testing.expect((if (result) |value| {
    value
} else {
    return error.NullPointer;
    ...
    return error.NullPointer;
}).code_point == 0x1F1ED); // Regional Indicator "H"
    try std.testing.expect((if (result) |value| {
    value
} else {
    return error.NullPointer;
...
pub fn computeGraphemeBreak(
    ...
    if (gb1 == .indic_conjunct_break_consonant) {
        ...
        if (isIndicConjunctBreakExtend(gb2)) {
            state[0] = .indic_conjunct_break_consonant;
            return false;
        } else if (gb2 == .indic_conjunct_break_linker) {
            // jump straight to linker state
            state[0] = .indic_conjunct_break_linker;
            return false;
        }
...
fn testGraphemeBreak(getActualIsBreak: fn (cp1: u21, cp2: u21, state: *BreakState) bool) !void {
    ...
    const file = try std.c.AT.FDCWD.openFile(file_path, .{});
    defer file.close();
    ...
    while (lines.next()) |line| : (line_num += 1) {
        ...
        const start = if (parts.next()) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        var cp1 = try parseCp(if (parts.next()) |value| {
    value
} else {
    return error.NullPointer;
});
        var gb1 = get(.grapheme_break, cp1);
        var expected_str = if (parts.next()) |value| {
    value
} else {
    return error.NullPointer;
};
        var cp2 = try parseCp(if (parts.next()) |value| {
    value
} else {
    return error.NullPointer;
});
        var gb2 = get(.grapheme_break, cp2);
        var next_expected_str = if (parts.next()) |value| {
    value
} else {
    return error.NullPointer;
};
    ...
    while (true) {
        ...
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
            ...
            cp2 = try parseCp(if (parts.next()) |value| {
    value
} else {
    return error.NullPointer;
});
            gb2 = get(.grapheme_break, cp2);
            next_expected_str = if (parts.next()) |value| {
    value
} else {
    return error.NullPointer;
};
...
test "long emoji zwj sequences with emoji modifiers" {
    ...
    return error.NullPointer;
}).code_point == 0x1F3FB); // Light Skin Tone
    try std.testing.expect(!(if (result) |value| {
    value
} else {
    return error.NullPointer;
    ...
    return error.NullPointer;
}).code_point == 0x1F3FF); // Dark Skin Tone
    try std.testing.expect((if (result) |value| {
    value
} else {
    return error.NullPointer;
