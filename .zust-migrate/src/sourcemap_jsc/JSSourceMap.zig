...
pub fn constructor(
    ...
    errdefer {
        for (0..names.items.len) |__zust_i| {
    var str = &names.items[__zust_i];
            str.deref();
        }
        names.deinit();
    }
    ...
    errdefer {
        for (0..sources.items.len) |__zust_i| {
    var str = &sources.items[__zust_i];
            str.deref();
        }
        sources.deinit();
    }
...
pub fn deinit(this: *JSSourceMap) void {
    for (0..this.sources.len) |__zust_i| {
    var str = &this.sources[__zust_i];
        str.deref();
    }
    // safe-transpile: free removed (memory owned by safe type);

    for (0..this.names.len) |__zust_i| {
    var name = &this.names[__zust_i];
        name.deref();
    }
