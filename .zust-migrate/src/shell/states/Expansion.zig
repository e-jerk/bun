...
pub const Result = union(enum) {
    ...
    pub fn pushResultSliceOwned(this: *Result, buf: [:0]const u8) PushAction {
        ...
        switch (this.*) {
            ...
            .array_of_ptr => {
                bun.handleOom(this.array_of_ptr.append(@as([*:0]const u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(buf.ptr))));
                return .moved;
            },
            .single => {
...
pub fn expandVarAndCmdSubst(this: *Expansion, start_word_idx: u32) ?Yield {
    switch (this.node.*) {
        ...
        .compound => |cmp| {
            ...
            for (0..cmp.atoms[start_word_idx + starting_offset ..].len) |__zust_i| {
    var simple_atom = &cmp.atoms[start_word_idx + starting_offset ..][__zust_i];
...
fn expansionSizeHint(this: *const Expansion, atom: *const ast.Atom, has_unknown: *bool) usize {
    return switch (@as(ast.Atom.Tag, atom.*)) {
        ...
        .compound => {
            ...
            var out: usize = 0;
            for (0..atom.compound.atoms.len) |__zust_i| {
    var simple = &atom.compound.atoms[__zust_i];
...
pub const ShellGlobTask = struct {
    ...
    pub fn createOnMainThread(walker: *GlobWalker, expansion: *Expansion) *This {
        ...
        var this = bun.handleOom(safe.Box(This).init(allocator, undefined));
        this[0] = .{
