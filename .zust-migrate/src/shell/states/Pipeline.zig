...
pub fn init(
    ...
    const pipeline = safe.Box(Pipeline).init(allocator, undefined);
    pipeline[0] = .{
...
fn setupCommands(this: *Pipeline) ?Yield {
    const cmd_count = brk: {
        var i: u32 = 0;
        for (0..this.node.items.len) |__zust_i| {
    var item = &this.node.items[__zust_i];
            if (switch (item.*) {
                .assigns => false,
                else => true,
            }) i += 1;
    ...
    for (0..this.cmds.?.len) |__zust_i| c.* = .{
    var c = &this.cmds.?[__zust_i]; .result = 0 };
    ...
    if (cmd_count > 1) {
        ...
        if (Pipeline.initializePipes(pipes, &pipes_set).asErr()) |err| {
            for (0..pipes[0..pipes_set].len) |__zust_i| {
    var pipe = &pipes[0..pipes_set][__zust_i];
                closefd(pipe[0]);
                closefd(pipe[1]);
            }
...
// safe-transpile: optional unwrap requires manual review
    ...
    const idx = brk: {
        const ptr_value: u64 = // safe-transpile: @bitCast requires manual review
    @bitCast(child.ptr.repr);
        _ = ptr_value;
        // safe-transpile: for with index access requires manual review
    for (if (this.cmds) |value| {
    value
} else {
    return error.NullPointer;
}, 0..) |cmd_or_result, i| {
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        while (i > 0) : (i -= 1) {
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            const cmd_or_result = if (this.cmds) |value| {
    value
} else {
    return error.NullPointer;
}[@intCast(i)];
...
pub fn deinit(this: *Pipeline) void {
    if (this.cmds) |cmds| {
        for (0..cmds.len) |__zust_i| {
    var cmd_or_result = &cmds[__zust_i];
            if (cmd_or_result.* == .cmd) {
...
fn initializePipes(pipes: []Pipe, set_count: *u32) Maybe(void) {
    for (0..pipes.len) |__zust_i| {
    var pipe = &pipes[__zust_i];
