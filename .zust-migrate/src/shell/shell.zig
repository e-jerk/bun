...
pub const ShellErr = union(enum) {
    ...
    pub fn deinit(this: *const @This(), allocator: Allocator) void {
        switch (this.*) {
            ...
            .custom => // safe-transpile: free removed (memory owned by safe type),
            .invalid_arguments => {},
            .todo => // safe-transpile: free removed (memory owned by safe type),
        }
    }
};
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    pub inline fn enqueueTaskConcurrentWaitPid(this: @This(), task: anytype) void {
        this.globalThis.bunVMConcurrently().enqueueTaskConcurrent(safe.Box(jsc.Task.init(task)).init(allocator, undefined));
    }
...
// safe-transpile: optional unwrap requires manual review
    ...
    pub inline fn enqueueTaskConcurrentWaitPid(this: @This(), task: anytype) void {
        var anytask = bun.handleOom(safe.Box(jsc.AnyTaskWithExtraContext).init(allocator, undefined));
...
pub const AST = struct {
    pub const Script = struct {
        ...
        pub fn memoryCost(this: *const @This()) usize {
            var cost: usize = 0;
            for (0..this.stmts.len) |__zust_i| {
    var stmt = &this.stmts[__zust_i];
    ...
    pub const Stmt = struct {
        ...
        pub fn memoryCost(this: *const @This()) usize {
            var cost: usize = 0;
            for (0..this.exprs.len) |__zust_i| {
    var expr = &this.exprs[__zust_i];
    ...
    pub const Expr = union(Expr.Tag) {
        ...
        pub fn memoryCost(this: *const @This()) usize {
            return switch (this.*) {
                .assign => |assign| brk: {
                    var cost: usize = 0;
                    for (0..assign.len) |__zust_i| {
    var expr = &assign[__zust_i];
                        cost += expr.memoryCost();
                    }
                    break :brk cost;
                },
    ...
    pub const CondExpr = struct {
        ...
        pub const Op = enum {
            ...
            const SINGLE_ARG_OPS: []const std.builtin.Type.EnumField = brk: {
                ...
                var ret: [count]std.builtin.Type.EnumField = .{};
                var len: usize = 0;
                for (fields) |f| {
                    if (f.name[0] == '-' and f.name.len == 2) {
                        ret[len] = f;
        ...
        pub fn to_expr(this: CondExpr, alloc: Allocator) !Expr {
            const condexpr = try safe.Box(CondExpr).init(allocator, undefined);
            condexpr[0] = this;
            return .{
                .condexpr = condexpr,
            };
    ...
    pub const If = struct {
        ...
        pub fn to_expr(this: If, alloc: Allocator) !Expr {
            const @"if" = try safe.Box(If).init(allocator, undefined);
            @"if"[0] = this;
            return .{
                .@"if" = @"if",
            };
    ...
    pub const Pipeline = struct {
        ...
        pub fn memoryCost(this: *const @This()) usize {
            var cost: usize = 0;
            for (0..this.items.len) |__zust_i| {
    var item = &this.items[__zust_i];
    ...
    pub const PipelineItem = union(enum) {
        ...
        pub fn memoryCost(this: *const @This()) usize {
            ...
            switch (this.*) {
                ...
                .assigns => |assigns| {
                    for (0..assigns.len) |__zust_i| {
    var assign = &assigns[__zust_i];
    ...
    pub const CmdOrAssigns = union(CmdOrAssigns.Tag) {
        ...
        pub fn to_pipeline_item(this: CmdOrAssigns, alloc: Allocator) PipelineItem {
            switch (this) {
                .cmd => |cmd| {
                    const cmd_ptr = try safe.Box(Cmd).init(allocator, undefined);
                    cmd_ptr[0] = cmd;
                    return .{ .cmd = cmd_ptr };
                },
                .assigns => |assigns| {
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    pub const Cmd = struct {
        ...
        pub fn memoryCost(this: *const @This()) usize {
            ...
            for (0..this.assigns.len) |__zust_i| {
    var assign = &this.assigns[__zust_i];
                cost += assign.memoryCost();
            }
            for (0..this.name_and_args.len) |__zust_i| {
    var atom = &this.name_and_args[__zust_i];
    ...
    pub const RedirectFlags = packed struct(u8) {
        ...
        pub inline fn isEmpty(this: RedirectFlags) bool {
            return @as(u8, // safe-transpile: @bitCast requires manual review
    @bitCast(this)) == 0;
        }
        ...
        pub fn merge(a: RedirectFlags, b: RedirectFlags) RedirectFlags {
            const anum: u8 = // safe-transpile: @bitCast requires manual review
    @bitCast(a);
            const bnum: u8 = // safe-transpile: @bitCast requires manual review
    @bitCast(b);
    ...
    pub const Atom = union(Atom.Tag) {
        ...
        pub fn merge(this: Atom, right: Atom, allocator: Allocator) !Atom {
            ...
            if (this == .compound and right == .compound) {
                ...
                safe.SimdUtils.copy(atoms[0..this.compound.atoms.len], this.compound.atoms);
                safe.SimdUtils.copy(atoms[this.compound.atoms.len .. this.compound.atoms.len + right.compound.atoms.len], right.compound.atoms);
                return .{ .compound = .{
                    .atoms = atoms,
            ...
            if (this == .simple) {
                ...
                atoms[0] = this.simple;
                safe.SimdUtils.copy(atoms[1 .. right.compound.atoms.len + 1], right.compound.atoms);
                return .{ .compound = .{
                    .atoms = atoms,
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    pub const CompoundAtom = struct {
        ...
        fn _atomsMemoryCost(this: *const @This()) usize {
            var cost: usize = 0;
            for (0..this.atoms.len) |__zust_i| {
    var atom = &this.atoms[__zust_i];
                cost += atom.memoryCost();
            }
            return cost;
        }
...
pub const Parser = struct {
    ...
    pub fn parse_impl(self: *Parser) !AST.Script {
        var stmts = safe.ArrayList(AST.Stmt).init(self.alloc);
...
// safe-transpile: function returns small constant slice — consider safe.String
    ...
    fn expectIfClauseTextToken(self: *Parser, comptime if_clause_token: @TypeOf(.enum_literal)) Token {
        ...
        if (self.peek() == .Text and
            ...
            safe.SimdUtils.eql(self.text(self.peek().Text), tagname))
        {
...
// safe-transpile: optional unwrap requires manual review
    ...
    fn allocate(self: *const Parser, comptime T: type, val: T) !*T {
        const heap = try safe.Box(T).init(allocator, undefined);
        heap[0] = val;
        return heap;
    }
...
// safe-transpile: function returns small constant slice — consider safe.String
    pub fn combineErrors(self: *Parser) []const u8 {
        ...
        const str = str: {
            ...
            var i: usize = 0;
            for (errors) |e| {
                safe.SimdUtils.copy(buf[i .. i + e.msg.len], e.msg);
                i += e.msg.len;
            }
            break :str buf;
        };
...
// safe-transpile: function returns small constant slice — consider safe.String
    pub fn combineErrors(this: *const LexResult, arena: Allocator) []const u8 {
        ...
        const str = str: {
            ...
            var i: usize = 0;
            for (errors) |e| {
                safe.SimdUtils.copy(buf[i .. i + e.msg.len()], e.msg.slice(this.strpool));
                i += e.msg.len();
            }
            break :str buf;
        };
...
pub fn NewLexer(comptime encoding: StringEncoding) type {
    ...
    return struct {
        ...
        strpool: safe.ArrayList(u8),
        tokens: safe.ArrayList(Token),
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn new(alloc: Allocator, src: []const u8, strings_to_escape: []bun.String, jsobjs_len: u32) @This() {
            return .{
                ...
                .tokens = safe.ArrayList(Token).init(alloc),
                .strpool = safe.ArrayList(u8).init(alloc),
                .errors = safe.ArrayList(LexError).init(alloc),
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        fn eat_literal(self: *@This(), comptime CodepointType: type, comptime literal: []const CodepointType) bool {
            ...
            if (safe.SimdUtils.eql(&slice, literal_skip_first))
                return true;
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        fn looksLikeJSObjRef(self: *@This()) bool {
            ...
            return safe.SimdUtils.eql(bytes[0 .. LEX_JS_OBJREF_PREFIX.len - 1], LEX_JS_OBJREF_PREFIX[1..]);
        }

        fn looksLikeJSStringRef(self: *@This()) bool {
            const bytes = self.chars.srcBytesAtCursor();
            if (LEX_JS_STRING_PREFIX.len - 1 >= bytes.len) return false;
            return safe.SimdUtils.eql(bytes[0 .. LEX_JS_STRING_PREFIX.len - 1], LEX_JS_STRING_PREFIX[1..]);
        }
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        fn matchesAsciiLiteral(self: *@This(), literal: []const u8) bool {
            ...
            return safe.SimdUtils.eql(bytes[0..literal.len], literal[0..]);
        }
                    ...
                    switch (bytes[i]) {
                        '0'...'9' => {
                            if (digit_buf_count >= digit_buf.len) {
                                ...
                                var error_buf: [ERROR_STR.len + digit_buf.len + 1]u8 = .{};
            ...
            switch (this.*) {
                .inlined => |*inlined| {
                    if (comptime bun.trait.isContainer(T) and @hasDecl(T, "memoryCost")) {
                        for (0..inlined.slice().len) |__zust_i| {
    var item = &inlined.slice()[__zust_i];
                            cost += item.memoryCost();
                        }
                    } else {
                ...
                .heap => {
                    if (comptime bun.trait.isContainer(T) and @hasDecl(T, "memoryCost")) {
                        for (0..this.heap.slice().len) |__zust_i| {
    var item = &this.heap.slice()[__zust_i];
                            cost += item.memoryCost();
                        }
        ...
        pub fn initWithSlice(vals: []const T) @This() {
            ...
            if (vals.len <= INLINED_MAX) {
                var this: @This() = @This().zeroes;
                safe.SimdUtils.copy(this.inlined.items[0..vals.len], vals);
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    fn shellParseImpl(
        ...
        const script_ast = Interpreter.parse(arena.allocator(), script.items[0..], jsobjs.items[0..], jsstrings.items[0..], &out_parser, &out_lex_result) catch |err| {
            if (err == ParseError.Lex) {
                ...
                const str = (if (out_lex_result) |value| {
    value
} else {
    return error.NullPointer;
}).combineErrors(arena.allocator());
                return globalThis.throwPretty("{s}", .{str});
            }
