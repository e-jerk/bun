    ...
    if (this.writer.start(&this.fd, this.flags.pollable).asErr()) |e_| {
        ...
        if (bun.Environment.isWindows) {
            ...
            if (e.getErrno() == .BADF) {
                ...
                this.flags.nonblocking = false;
                this.flags.is_socket = false;
                return this.writer.startWithFile(if (this.fd.get()) |value| {
    value
} else {
...
pub fn cancelChunks(this: *IOWriter, ptr_: anytype) void {
    ...
    for (0..slice[idx..].len) |__zust_i| {
    var w = &slice[idx..][__zust_i];
...
pub fn skipDead(this: *IOWriter) void {
    ...
    for (0..slice[this.writer_idx..].len) |__zust_i| {
    var w = &slice[this.writer_idx..][__zust_i];
        if (w.isDead()) {
            this.writer_idx += 1;
...
pub fn brokenPipeForWriters(this: *IOWriter) void {
    ...
    for (0..writers.len) |__zust_i| {
    var w = &writers[__zust_i];
        if (w.isDead()) {
            offset += w.len;
            continue;
        }
...
pub fn drainBufferedData(parent: *IOWriter, buf: []const u8, max_write_size: usize, received_hup: bool) bun.io.WriteResult {
    ...
    while (drained < trimmed.len) {
        const attempt = tryWriteWithWriteFn(if (parent.fd.get()) |value| {
    value
} else {
    return error.NullPointer;
}, buf, bun.sys.write);
