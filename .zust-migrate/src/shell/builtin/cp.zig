...
pub fn start(this: *Cp) Yield {
    ...
    if (maybe_filepath_args == null or (if (maybe_filepath_args) |value| {
    value
} else {
...
pub fn ignoreEbusyErrorIfPossible(this: *Cp) Yield {
    ...
    outer_loop: for (this.state.ebusy.state.tasks.items[this.state.ebusy.idx..], 0..) |task_, i| {
            ...
            const failure_src = if (task.src_absolute) |value| {
    value
} else {
    return error.NullPointer;
};
            const failure_tgt = if (task.tgt_absolute) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub inline fn bltn(this: *@This()) *Builtin {
    ...
    @alignCast(@fieldParentPtr("cp", this));
        ...
        if (task.err) |*err| {
            if (err.* == .sys and
                ...
                (task.tgt_absolute != null and
                    err.sys.path.eqlUTF8(if (task.tgt_absolute) |value| {
    value
} else {
    return error.NullPointer;
})) or
                (task.src_absolute != null and
                    err.sys.path.eqlUTF8(if (task.src_absolute) |value| {
    value
} else {
    return error.NullPointer;
})))
            ...
            if (bun.take(&task.tgt_absolute)) |tgt| {
                ...
                if (gop.found_existing) // safe-transpile: free removed (memory owned by safe type);
            }
...
pub const ShellCpTask = struct {
    ...
    verbose_output: safe.ArrayList(u8) = safe.ArrayList(u8).init(bun.default_allocator),
    ...
    fn takeOutput(this: *ShellCpTask) safe.ArrayList(u8) {
        const out = this.verbose_output;
        this.verbose_output = safe.ArrayList(u8).init(bun.default_allocator);
        return out;
    }
    ...
    fn runFromThreadPoolImpl(this: *ShellCpTask) ?bun.shell.ShellErr {
        ...
        const tgt_is_dir: bool = undefined, const tgt_exists: bool = undefined = switch (this.isDir(tgt)) {
        ...
        const args = jsc.Node.fs.Arguments.Cp{
            .src = jsc.Node.PathLike{ .string = bun.PathString.init(if (this.src_absolute) |value| {
    value
} else {
    return error.NullPointer;
}) },
            .dest = jsc.Node.PathLike{ .string = bun.PathString.init(if (this.tgt_absolute) |value| {
    value
} else {
    return error.NullPointer;
}) },
        ...
        debug("Scheduling {s} -> {s}", .{ if (this.src_absolute) |value| {
    value
} else {
    return error.NullPointer;
}, if (this.tgt_absolute) |value| {
    value
} else {
    return error.NullPointer;
} });
