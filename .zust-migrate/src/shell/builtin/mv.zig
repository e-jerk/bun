...
pub const ShellMvBatchedTask = struct {
    ...
    pub fn moveInDir(this: *@This(), src: [:0]const u8, buf: *bun.PathBuffer) bool {
        ...
        switch (Syscall.renameat(this.cwd, src, if (this.target_fd) |value| {
    value
} else {
    ...
    while (!(this.state == .done or this.state == .err)) {
        switch (this.state) {
            ...
            .check_target => {
                ...
                const maybe_fd: ?bun.FD = switch (if (check_target.task.result) |value| {
    value
} else {
    return error.NullPointer;
}) {
                ...
                for (0..this.state.executing.tasks.len) |__zust_i| {
    var t = &this.state.executing.tasks[__zust_i];
...
pub fn batchedMoveTaskDone(this: *Mv, task: *ShellMvBatchedTask) void {
    ...
    if (exec.tasks_done >= exec.task_count) {
        if (exec.err) |err| {
            ...
            if (exec.err_path_owned) // safe-transpile: free removed (memory owned by safe type);
            exec.err = null;
...
pub fn deinit(this: *Mv) void {
    ...
    if (this.state == .executing) {
        if (this.state.executing.err) |err| {
            if (this.state.executing.err_path_owned) // safe-transpile: free removed (memory owned by safe type);
        }
    }
}
...
pub inline fn bltn(this: *Mv) *Builtin {
    ...
    @alignCast(@fieldParentPtr("mv", this));
