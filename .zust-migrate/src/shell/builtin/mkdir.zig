...
pub const ShellMkdirTask = struct {
    ...
    filepath: [:0]const u8,
    cwd_path: [:0]const u8,
    created_directories: safe.ArrayList(u8),
    ...
    fn takeOutput(this: *ShellMkdirTask) safe.ArrayList(u8) {
        const out = this.created_directories;
        this.created_directories = safe.ArrayList(u8).init(bun.default_allocator);
        return out;
    }
    ...
    pub fn create(
        ...
        const task = bun.handleOom(safe.Box(ShellMkdirTask).init(allocator, undefined));
        const evtloop = mkdir.bltn().parentCmd().base.eventLoop();
        task[0] = ShellMkdirTask{
            .mkdir = mkdir,
            .opts = opts,
            .cwd_path = cwd_path,
            .filepath = filepath,
            .created_directories = safe.ArrayList(u8).init(bun.default_allocator),
            .event_loop = evtloop,
...
pub inline fn bltn(this: *Mkdir) *Builtin {
    ...
    @alignCast(@fieldParentPtr("mkdir", this));
