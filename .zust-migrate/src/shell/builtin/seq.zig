...
pub fn start(this: *@This()) Yield {
    ...
    while (iter.next()) |item| {
        ...
        if (safe.SimdUtils.eql(arg, "-s") or safe.SimdUtils.eql(arg, "--separator")) {
        ...
        if (safe.SimdUtils.eql(arg, "-t") or safe.SimdUtils.eql(arg, "--terminator")) {
        ...
        if (safe.SimdUtils.eql(arg, "-w") or safe.SimdUtils.eql(arg, "--fixed-width")) {
    ...
    const int2 = std.fmt.parseFloat(f32, bun.sliceTo((if (maybe2) |value| {
    value
} else {
    return error.NullPointer;
}), 0)) catch return this.fail("seq: invalid argument\n");
    ...
    const int3 = std.fmt.parseFloat(f32, bun.sliceTo((if (maybe3) |value| {
    value
} else {
    return error.NullPointer;
}), 0)) catch return this.fail("seq: invalid argument\n");
...
pub inline fn bltn(this: *@This()) *Builtin {
    ...
    @alignCast(@fieldParentPtr("seq", this));
