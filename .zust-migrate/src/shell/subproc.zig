    ...
    const Writable = union(enum) {
        ...
        pub fn init(
            ...
            switch (stdio) {
                ...
                .fd => {
                    return Writable{ .fd = (if (result) |value| {
    value
} else {
    ...
    pub const Readable = union(enum) {
        ...
        pub fn init(out_type: bun.shell.Subprocess.OutKind, stdio: Stdio, shellio: ?*sh.IOWriter, event_loop: jsc.EventLoopHandle, process: *ShellSubprocess, result: StdioResult, allocator: std.mem.Allocator, max_size: u32, is_sync: bool) Readable {
            ...
            return switch (stdio) {
                ...
                .ipc, .dup2, .ignore => Readable{ .ignore = {} },
                .path => Readable{ .ignore = {} },
                .fd => Readable{ .fd = (if (result) |value| {
    value
} else {
    return error.NullPointer;
}) },
...
// safe-transpile: function uses raw slice parameter — consider safe.String
            ...
            pub fn next(this: *@This()) !?@This().Entry {
                ...
                safe.SimdUtils.copy(value[0..entry.value_ptr.value.len], entry.value_ptr.value);
    ...
    fn spawnMaybeSyncImpl(
        ...
        defer if (!stdio_consumed) {
            for (0..&spawn_args.stdio.len) |__zust_i| { var s = &&spawn_args.stdio[__zust_i]; s.deinit() };
        };
    ...
    @ptrCast(spawn_args.env_array.items.ptr),
        ...
        var subprocess = bun.handleOom(safe.Box(Subprocess).init(allocator, undefined));
        out_subproc[0] = subprocess;
        ...
        switch (subprocess.process.watch()) {
            .result => {},
            .err => {
                notify_caller_process_already_exited[0] = true;
        ...
        const exit_code: ?u8 = brk: {
            ...
            if (status == .signaled) {
                if (status.signalCode()) |code| {
                    break :brk if (code.toExitCode()) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        pub fn append(this: *BufferedOutput, bytes: []const u8) void {
            switch (this.*) {
                ...
                .array_buffer => {
                    ...
                    safe.SimdUtils.copy(array_buf_slice[this.array_buffer.i .. this.array_buffer.i + length], bytes[0..length]);
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
...
// safe-transpile: function returns small constant slice — consider safe.String
    ...
    pub fn start(this: *PipeReader, process: *ShellSubprocess, event_loop: jsc.EventLoopHandle) bun.sys.Maybe(void) {
        ...
        switch (this.reader.start(if (this.stdio_result) |value| {
    value
} else {
    return error.NullPointer;
}, true)) {
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn onReadChunk(ptr: *anyopaque, chunk: []const u8, has_more: bun.io.ReadState) bool {
        var this: *PipeReader = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    ...
    pub fn trySignalDoneToCmd(
        ...
        if (this.process) |proc| {
            ...
            if (this.captured_writer.err) |e| {
                ...
                if (this.state == .done) // safe-transpile: free removed (memory owned by safe type);
                if (this.state != .err) {
                    this.state = .{ .err = e };
                } else {
                    e.deref();
    ...
    pub fn toReadableStream(this: *PipeReader, globalObject: *jsc.JSGlobalObject) jsc.JSValue {
        ...
        switch (this.state) {
            ...
            .err => |err| {
                ...
                jsc.WebCore.ReadableStream.cancel(&if (jsc.WebCore.ReadableStream.fromJS(empty, globalObject)) |value| {
    value
} else {
    return error.NullPointer;
}, globalObject);
...
extern "C" const BUN_DEFAULT_PATH_FOR_SPAWN: [*:0]const u8 = undefined;
