...
pub const CowFd = struct {
    ...
    pub fn init(fd: bun.FD) *CowFd {
        const this = bun.handleOom(safe.Box(CowFd).init(allocator, undefined));
        this[0] = .{
            .__fd = fd,
        };
...
pub const Interpreter = struct {
    ...
    pub fn takeErr(this: *ThisInterpreter) Syscall.Error {
        const e = if (this.last_err) |value| {
    value
} else {
    ...
    pub fn deinit(this: *ThisInterpreter) void {
...
// safe-transpile: function returns small constant slice — consider safe.String
        ...
        pub fn dupeForSubshell(
            ...
            const duped = bun.handleOom(safe.Box(ShellExecEnv).init(allocator, undefined));
            ...
            const stdout: Bufio = switch (io.stdout) {
                .fd => brk: {
                    if (io.stdout.fd.captured != null) break :brk .{ .borrowed = if (io.stdout.fd.captured) |value| {
    value
} else {
    return error.NullPointer;
} };
            ...
            const stderr: Bufio = switch (io.stderr) {
                .fd => brk: {
                    if (io.stderr.fd.captured != null) break :brk .{ .borrowed = if (io.stderr.fd.captured) |value| {
    value
} else {
    return error.NullPointer;
} };
        ...
        pub fn changeCwdImpl(this: *ShellExecEnv, _: *ThisInterpreter, new_cwd_: anytype, comptime in_init: bool) Maybe(void) {
            ...
            const new_cwd: [:0]const u8 = brk: {
                if (is_abs) {
                    if (is_sentinel) {
                        safe.SimdUtils.copy(ResolvePath.join_buf[0..new_cwd_.len], new_cwd_[0..new_cwd_.len]);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        out_parser[0] = try bun.shell.Parser.new(arena_allocator, lex_result, jsobjs);
...
// safe-transpile: optional unwrap requires manual review
    ...
    fn initImpl(
        ...
        const interpreter = bun.handleOom(safe.Box(ThisInterpreter).init(allocator, undefined));
        interpreter[0] = .{
            .command_ctx = ctx,
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    return error.NullPointer;
}).combineErrors(shargs.arena_allocator());
        ...
        const code = if (interp.exit_code) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    pub fn asyncCmdDone(this: *ThisInterpreter, async: *Async) void {
        ...
        if (this.async_commands_executing == 0 and this.exit_code != null) {
            this.finish(if (this.exit_code) |value| {
    value
} else {
    return error.NullPointer;
}).run();
    ...
    return struct {
        ...
        pub fn create(this: @This(), comptime Ty: type) *Ty {
            if (comptime bun.Environment.enableAllocScopes) {
                return bun.handleOom(safe.Box(Ty).init(allocator, undefined));
            }
            return bun.handleOom(safe.Box(Ty).init(allocator, undefined));
        }
...
// safe-transpile: function returns small constant slice — consider safe.String
        pub fn tagName(this: @This()) []const u8 {
            return if (Ptr.typeNameFromTag(this.tagInt())) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub const ShellSyscall = struct {
    ...
    pub fn getPath(dirfd: anytype, to: [:0]const u8, buf: *bun.PathBuffer) Maybe([:0]const u8) {
        ...
        if (ResolvePath.Platform.posix.isAbsolute(to[0..to.len])) {
            ...
            safe.SimdUtils.copy(buf[source_root.len..][0 .. to.len - 1], to[1..]);
        ...
        const dirpath = brk: {
            ...
            safe.SimdUtils.copy(buf[0..dirfd.len], dirfd[0..dirfd.len]);
...
pub fn isPollable(fd: bun.FD, mode: bun.Mode) bool {
    return switch (bun.Environment.os) {
        .windows, .wasm => false,
        .linux, .freebsd => posix.S.ISFIFO(mode) or posix.S.ISSOCK(mode) or @import("std-fs-compat").isatty(fd.native()),
        ...
        .mac => if (posix.S.ISREG(mode)) false else posix.S.ISFIFO(mode) or posix.S.ISSOCK(mode) or @import("std-fs-compat").isatty(fd.native()),
    };
}
