...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    for (filters, buf) |filter, *converted| {
                converted[0] = try WorkspaceFilter.init(allocator, filter, original_cwd, &path_buf);
            }
...
// safe-transpile: function returns small constant slice — consider safe.String
    fn truncateWithEllipsis(allocator: std.mem.Allocator, text: []const u8, max_width: usize, only_end: bool) ![]const u8 {
        ...
        safe.SimdUtils.copy(result[0..start_chars], text[0..start_chars]);
        safe.SimdUtils.copy(result[start_chars .. start_chars + ellipsis.len], ellipsis);
        safe.SimdUtils.copy(result[start_chars + ellipsis.len ..], text[text.len - end_chars ..]);
...
var __zust_loop_counter: u64 = 0;
    ...
    if (__loop_limit_1 > 1_000_000) break;
            ...
            // The render body
            {
                ...
                for (state.viewport_start..viewport_end) |i| {
                    ...
                    const needs_header = current_dep_type == null or !strings.eql((if (current_dep_type) |value| {
    value
} else {
    ...
    for (state.packages, state.selected) |p, sel| {
                    ...
                    var checkbox_color: safe.Slice(u8) = "green"; // default
            ...
            switch (byte) {
                ...
                'a', 'A' => {
                    ...
                    for (0..state.packages.len) |__zust_i| {
    var pkg = &state.packages[__zust_i];
                ...
                'i', 'I' => {
                    // Invert selection
                    for (0..state.selected.len) |__zust_i| {
    var sel = &state.selected[__zust_i];
                        sel.* = !sel.*;
                    }
                    state.toggle_all = false; // Reset toggle_all mode
                },
