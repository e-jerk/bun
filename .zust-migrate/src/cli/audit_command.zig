...
const AuditResult = struct {
    ...
    pub fn deinit(self: *AuditResult) void {
        ...
        while (iter.next()) |entry| {
            ...
            for (0..entry.value_ptr.dependents.items.len) |__zust_i| {
    var dependent = &entry.value_ptr.dependents.items[__zust_i];
...
fn collectPackagesForAudit(allocator: std.mem.Allocator, pm: *PackageManager, prod_only: bool) bun.OOM!struct { audit_body: []u8, skipped_packages: std.array_list.Managed([]const u8) } {
    ...
    if (prod_only) {
        ...
        try buildProductionPackageSet(allocator, pm, &(if (prod_packages) |value| {
    value
} else {
    ...
    return error.NullPointer;
        ...
        for (0..packages_list.items.len) |__zust_i| {
    var item = &packages_list.items[__zust_i];
        ...
        for ((if (found_package) |value| {
    value
} else {
    return error.NullPointer;
}).versions.items) |existing_ver| {
            if (safe.SimdUtils.eql(existing_ver, ver_str)) {
                version_exists = true;
                break;
            }
        }
                ...
                if (key.data == .e_string) {
                    ...
                    if (prop.value) |value| {
                        if (value.data == .e_string) {
                            ...
                            if (safe.SimdUtils.eql(field_name, "severity")) {
                                vulnerability.severity = field_value;
                            } else if (safe.SimdUtils.eql(field_name, "title")) {
                                vulnerability.title = field_value;
                            } else if (safe.SimdUtils.eql(field_name, "url")) {
                                vulnerability.url = field_value;
                            } else if (safe.SimdUtils.eql(field_name, "vulnerable_versions")) {
                                vulnerability.vulnerable_versions = field_value;
                            } else if (safe.SimdUtils.eql(field_name, "id")) {
                                vulnerability.id = field_value;
                            }
                        } else if (value.data == .e_number) {
                            if (safe.SimdUtils.eql(field_name, "id")) {
...
fn findDependencyPaths(
    ...
    for (dep_slice) |dependency| {
        ...
        if (safe.SimdUtils.eql(dep_name, target_package)) {
    ...
    while (queue.readItem()) |*current| {
        ...
        for (0..dep_slice.len) |__zust_i| {
    var dependency = &dep_slice[__zust_i];
            const dep_name = dependency.name.slice(buf);
    ...
    for (pkg_resolutions, pkg_deps, pkg_names) |resolution, workspace_deps, pkg_name| {
            ...
            const workspace_dep_slice = workspace_deps.get(dependencies);
            for (0..workspace_dep_slice.len) |__zust_i| {
    var dependency = &workspace_dep_slice[__zust_i];
                const dep_name = dependency.name.slice(buf);
                if (bun.strings.eql(dep_name, current.*)) {
                        ...
                        if (value.data == .e_array) {
                            ...
                            for (vulns) |vuln| {
                                if (vuln.data == .e_object) {
                                    ...
                                    if (safe.SimdUtils.eql(vulnerability.severity, "low")) {
                                        vuln_counts.low += 1;
                                    } else if (safe.SimdUtils.eql(vulnerability.severity, "moderate")) {
                                        vuln_counts.moderate += 1;
                                    } else if (safe.SimdUtils.eql(vulnerability.severity, "high")) {
                                        vuln_counts.high += 1;
                                    } else if (safe.SimdUtils.eql(vulnerability.severity, "critical")) {
        ...
        while (package_iter.next()) |entry| {
            ...
            if (package_info.vulnerabilities.items.len > 0) {
                ...
                for (package_info.vulnerabilities.items) |vuln| {
                    if (vuln.title.len > 0) {
                        if (safe.SimdUtils.eql(vuln.severity, "critical")) {
                            Output.prettyln("  <red>critical<d>:<r> {s} - <d>{s}<r>", .{ vuln.title, vuln.url });
                        } else if (safe.SimdUtils.eql(vuln.severity, "high")) {
                            Output.prettyln("  <red>high<d>:<r> {s} - <d>{s}<r>", .{ vuln.title, vuln.url });
                        } else if (safe.SimdUtils.eql(vuln.severity, "moderate")) {
