...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn open(
        ...
        var spawned = try safe.Box(SpawnedEditorContext).init(allocator, undefined);
        spawned[0] = .{};
        ...
        if (editor == .vscode and line != null and (if (line) |value| {
    value
} else {
    return error.NullPointer;
}).len > 0) {
