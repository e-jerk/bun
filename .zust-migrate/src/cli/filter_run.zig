...
// safe-transpile: function uses raw slice parameter — consider safe.String
    fn elide(data_: []const u8, max_lines: ?usize) ElideResult {
        ...
        if (max_lines == null or (if (max_lines) |value| {
    value
} else {
    return error.NullPointer;
}) == 0) return .{ .content = data, .elided_count = 0 };
        var i: usize = data.len;
        var lines: usize = 0;
        while (i > 0) : (i -= 1) {
            if (data[i - 1] == '\n') {
                lines += 1;
                if (lines >= (if (max_lines) |value| {
    value
} else {
    return error.NullPointer;
})) {
    ...
    fn redraw(this: *This, is_abort: bool) !void {
        ...
        for (0..this.handles.len) |__zust_i| {
    var handle = &this.handles[__zust_i];
...
pub fn runScriptsWithFilter(ctx: Command.Context) !noreturn {
    ...
    safe.SimdUtils.copy(pre_script_name[0..3], "pre");
    safe.SimdUtils.copy(pre_script_name[3..], script_name);

    const post_script_name = try ctx.allocator.alloc(u8, script_name.len + 4);
    safe.SimdUtils.copy(post_script_name[0..4], "post");
    safe.SimdUtils.copy(post_script_name[4..], script_name);
    ...
    var root_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
    ...
    for (0..state.handles.len) |__zust_i| {
    var handle = &state.handles[__zust_i];
