...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    pub fn publish(
        ...
        var req = http.AsyncHTTP.initSync(
            ...
            if (publish_headers.content.ptr) |value| {
    value
} else {
    return error.NullPointer;
}[0..publish_headers.content.len],
            &response_buf,
            publish_req_body,
            null,
            null,
        ...
        switch (res.status_code) {
            400...std.math.maxInt(@TypeOf(res.status_code)) => {
                ...
                var otp_req = http.AsyncHTTP.initSync(
                    ...
                    publish_url,
                    otp_headers.entries,
                    if (otp_headers.content.ptr) |value| {
    value
} else {
    return error.NullPointer;
}[0..otp_headers.content.len],
                    &response_buf,
                    publish_req_body,
                    null,
                    null,
...
var __zust_loop_counter: u64 = 0;
    ...
    if (__loop_limit_1 > 1_000_000) break;
                ...
                var req = http.AsyncHTTP.initSync(
                    ...
                    if (auth_headers.content.ptr) |value| {
    value
} else {
    return error.NullPointer;
}[0..auth_headers.content.len],
                    response_buf,
                    "",
                    null,
                    null,
