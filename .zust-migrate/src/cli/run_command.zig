...
var path_buf2: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
...
pub const RunCommand = struct {
    ...
    pub fn findShell(PATH: string, cwd: string) ?stringZ {
        const Once = struct {
            var shell_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
                ...
                pub fn run(PATH_: string, cwd_: string) ?stringZ {
                    if (findShellImpl(PATH_, cwd_)) |found| {
                        if (found.len < shell_buf.len) {
                            safe.SimdUtils.copy(shell_buf[0..found.len], found);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn runBinary(
        ...
        if (Environment.isWindows and bun.FeatureFlags.windows_bunx_fast_path and bun.strings.hasSuffixComptime(executable, ".exe")) {
            ...
            safe.SimdUtils.copy(wpath[wpath.len - "bunx".len ..], comptime bun.strings.w("bunx"));
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    fn runBinaryWithoutBunxPath(
        ...
        var argv = try safe.OffsetPtr.init(allocator, &argv_); defer argv.deinit();
                        ...
                        if (code != 0) {
                            if (!silent) {
                                const is_probably_trying_to_run_a_pkg_script =
                                    ...
                                    ((code == 1 and bun.strings.eqlComptime((if (original_script_for_bun_run) |value| {
    value
} else {
    return error.NullPointer;
}), "test")) or
                                        (code == 2 and bun.strings.eqlAnyComptime((if (original_script_for_bun_run) |value| {
    value
} else {
    return error.NullPointer;
}), &.{
                                ...
                                if (is_probably_trying_to_run_a_pkg_script) {
                                    ...
                                    Output.note("a package.json script \"{s}\" was not found", .{(if (original_script_for_bun_run) |value| {
    value
} else {
    return error.NullPointer;
})});
    ...
    pub fn bunNodeFileUtf8(allocator: std.mem.Allocator) ![:0]const u8 {
        ...
        var temp_path_buffer: bun.WPathBuffer = std.mem.zeroes(bun.WPathBuffer);
        var target_path_buffer: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
    ...
    @ptrCast(&temp_path_buffer),
        ...
        safe.SimdUtils.copy(target_path_buffer[converted.len..][0..file_name.len], file_name);
    ...
    pub fn createFakeTemporaryNodeExecutable(
        ...
        if (Environment.isPosix) {
            var argv0 = @as([*:0]const u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(optional_bun_path.ptr));
            ...
            if (bun.argv[0][0] == '/') {
                optional_bun_path[0] = bun.argv[0];
                argv0 = bun.argv[0];
...
var __zust_loop_counter: u64 = 0;
    ...
    if (__loop_limit_1 > 1_000_000) break;
            ...
            var target_path_buffer: bun.WPathBuffer = std.mem.zeroes(bun.WPathBuffer);
    ...
    @ptrCast(&target_path_buffer[prefix.len]),
            ...
            safe.SimdUtils.copy(target_path_buffer[0..prefix.len], prefix);
            ...
            const dir_name = "bun-node" ++ if (Environment.isDebug)
                ...
                "";
            safe.SimdUtils.copy(target_path_buffer[prefix.len..][len..].ptr, comptime bun.strings.w(dir_name));
            ...
            inline for (.{ "node.exe", "bun.exe" }) |name| {
                ...
                safe.SimdUtils.copy(target_path_buffer[len + prefix.len ..][0..file_name.len], comptime bun.strings.w(file_name));

                const file_slice = target_path_buffer[0 .. prefix.len + len + file_name.len - "\x00".len];

                if (bun.windows.CreateHardLinkW(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(file_slice.ptr), image_path.ptr, null) == 0) {
    ...
    pub fn configureEnvForRun(
        ...
        const args = ctx.args;
        this_transpiler[0] = try transpiler.Transpiler.init(ctx.allocator, ctx.log, args, env);
        this_transpiler.options.env.behavior = api.DotEnvBehavior.load_all;
                    ...
                    loop: while (key_i < keys.len) : (key_i += 1) {
                        ...
                        if (filter == Filter.script_exclude) {
                            for (reject_list) |default| {
                                if (safe.SimdUtils.eql(default, key)) {
                                    continue :loop;
                                }
                            }
                        }
                        ...
                        if (filter == Filter.script_and_descriptions and max_description_len > 0) {
                            var description = if (scripts.get(key)) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    fn prefetchRemoteImages(
        ...
        for (remote_urls.items) |raw_url| {
            const d = safe.Box(RemoteImageDownload).init(allocator, undefined) catch continue;
            d[0] = .{
        ...
        for (downloads.items) |d| {
            ...
            const ext: safe.Slice(u8) = if (bun.strings.endsWith(d.url, ".png")) ".png" else if (bun.strings.endsWith(d.url, ".jpg") or bun.strings.endsWith(d.url, ".jpeg")) ".jpg" else if (bun.strings.endsWith(d.url, ".gif")) ".gif" else if (bun.strings.endsWith(d.url, ".webp")) ".webp" else ".bin";
            var name_buf: [64]u8 = .{};
        ...
        const columns: u16 = brk: {
            ...
            if (comptime bun.Environment.isPosix) {
                ...
                if (windows.GetStdHandle(windows.STD_OUTPUT_HANDLE) catch null) |handle| {
                    var csbi: windows.CONSOLE_SCREEN_BUFFER_INFO = std.mem.zeroes(windows.CONSOLE_SCREEN_BUFFER_INFO);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        var base_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
        var cwd_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
        const abs_md_path: safe.Slice(u8) = blk: {
    ...
    fn maybeOpenWithBunJS(ctx: Command.Context) bool {
        if (ctx.args.entry_points.len == 0)
            return false;
        var script_name_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
            ...
            const file = bun.FD.fromStdFile((brk: {
                if (std.fs.path.isAbsolute(script_name_to_search)) {
                    ...
                    const file_pathZ = brk2: {
                        safe.SimdUtils.copy(script_name_buf[0..file_path.len], file_path);
                    ...
                    var path_buf_2: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            absolute_script_path = brk: {
                ...
                var fd_path_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
                break :brk bun.getFdPath(file, &fd_path_buf) catch return false;
            };
        }

        _ = _bootAndHandleError(ctx, (if (absolute_script_path) |value| {
    value
} else {
    return error.NullPointer;
}), null);
    ...
    pub fn exec(
        ...
        if (target_name.len > 0 and target_name[0] == '.') {
            ...
            if (default_loader != null and ((if (default_loader) |value| {
    value
} else {
    return error.NullPointer;
}).canBeRunByBun() or (if (default_loader) |value| {
    value
} else {
    return error.NullPointer;
}) == .md)) {
        ...
        if (target_name.len == 1 and target_name[0] == '-') {
            ...
            var entry_point_buf: [bun.MAX_PATH_BYTES + trigger.len]u8 = .{};
            var cwd_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
            const cwd = try bun.sys.getcwd(&cwd_buf).unwrap();
            safe.SimdUtils.copy(entry_point_buf[0..cwd.len], cwd);
            safe.SimdUtils.copy(entry_point_buf[cwd.len..][0..trigger.len], trigger);
...
// safe-transpile: optional unwrap requires manual review
        ...
        if (resolution) |resolved| {
            ...
            const path = if (resolved_mutable.path()) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        if (Environment.isWindows and bun.FeatureFlags.windows_bunx_fast_path) try_bunx_file: {
            ...
            safe.SimdUtils.copy(ptr[0..root.len], root);
            ptr = ptr[4..];
        ...
        if (ctx.filters.len == 0 and !ctx.workspaces and CLI.Cli.cmd != null and if (CLI.Cli.cmd) |value| {
    value
} else {
    return error.NullPointer;
} == .AutoCommand) {
    ...
    return error.NullPointer;
}).isJavaScriptLikeOrJSON() or target_name.len > 0 and (target_name[0] == '.' or target_name[0] == '/' or std.fs.path.isAbsolute(target_name))) {
    ...
    pub fn execAsIfNode(ctx: Command.Context) !void {
        ...
        if (ctx.runtime_options.eval.script.len > 0) {
            ...
            var entry_point_buf: [bun.MAX_PATH_BYTES + trigger.len]u8 = .{};
            var eval_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
    ...
    fn @"bun feedback"(ctx: Command.Context) !noreturn {
        const trigger = bun.pathLiteral("/[eval]");
        var entry_point_buf: [bun.MAX_PATH_BYTES + trigger.len]u8 = .{};
        var path_buf3: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
...
pub const BunXFastPath = struct {
    ...
    var direct_launch_buffer: bun.WPathBuffer = std.mem.zeroes(bun.WPathBuffer);
    var environment_buffer: bun.WPathBuffer = std.mem.zeroes(bun.WPathBuffer);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    fn appendWindowsArgument(buffer: []u16, arg: []const u8) usize {
        ...
        if (!needs_quote) {
            ...
            safe.SimdUtils.copy(buffer[0..len], source);
            return len;
        }
        ...
        if (!has_quote_or_backslash) {
            ...
            buffer[0] = '"';
            safe.SimdUtils.copy(buffer[1 .. 1 + len], source);
            buffer[len + 1] = '"';
            return len + 2;
        }
