...
pub fn generateFiles(allocator: std.mem.Allocator, entry_point: string, dependencies: []const []const u8, dev_dependencies: []const []const u8, template: Template, react_component_export: []const u8) !void {
    ...
    var normalized_name: safe.Slice(u8) = if (std.fs.path.isAbsolute(entry_point))
    ...
    switch (@as(Template.Tag, template)) {
        inline else => |active| {
            ...
            var filenames: [files.len]string = .{};
    ...
    for (reachable_files) |file| {
        switch (loaders[file.get()]) {
            .tsx, .jsx => {
                ...
                var source_code: safe.Slice(u8) = source.contents;
...
// zust: use safe.String or safe.GuardedSlice for slice operations
            ...
            .html => {
                const source: *const bun.logger.Source = &sources[file.get()];
                const source_code: safe.Slice(u8) = source.contents;
...
fn getShadcnComponents(bundler: *BundleV2, reachable_files: []const js_ast.Index) !bun.StringSet {
    ...
    var import_record = &import_records.slice()[__zust_i];
        ...
        if (loader == .jsx or loader == .tsx) {
            ...
            if (filename[0] >= 'a' and filename[0] <= 'z') {
...

                {
                    ...
                    // - "My-App" -> "Myapp"
                    if (output_index > 1) {
                        for (0..duped[1..output_index].len) |__zust_i| {
    var c = &duped[1..output_index][__zust_i];
                            switch (c.*) {
                                'A'...'Z' => {
                                    c.* = c.* + 32;
                                },
