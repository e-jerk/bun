...
fn loadGlobalBunfig(allocator: std.mem.Allocator, ctx: Command.Context, comptime cmd: Command.Tag) !void {
    ...
    var config_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
...
pub fn loadConfig(allocator: std.mem.Allocator, user_config_path_: ?string, ctx: Command.Context, comptime cmd: Command.Tag) OOM!void {
    ...
    var config_path_: safe.Slice(u8) = user_config_path_ orelse "";
    ...
    var config_path: [:0]u8 = .{};
    if (config_path_[0] == '/') {
        safe.SimdUtils.copy(config_buf[0..config_path_.len], config_path_);
        config_buf[config_path_.len] = 0;
        config_path = config_buf[0..config_path_.len :0];
    } else {
        if (ctx.args.absolute_working_dir == null) {
            var secondbuf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
            const cwd = bun.getcwd(&secondbuf) catch return;

            ctx.args.absolute_working_dir = try allocator.dupeZ(u8, cwd);
        }

        var parts = [_]string{ if (ctx.args.absolute_working_dir) |value| {
    value
} else {
    return error.NullPointer;
}, config_path_ };
        config_path_ = resolve_path.joinAbsStringBuf(
            if (ctx.args.absolute_working_dir) |value| {
    value
} else {
    return error.NullPointer;
},
...
pub fn parse(allocator: std.mem.Allocator, ctx: Command.Context, comptime cmd: Command.Tag) !Api.TransformOptions {
    ...
    if (cmd == .TestCommand) {
        ...
        if (args.option("--test-name-pattern")) |namePattern| {
            ...
            ctx.test_options.test_filter_regex = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(regex);
        }
    ...
    if (opts.port != null and opts.origin == null) {
        opts.origin = try std.fmt.allocPrint(allocator, "http://localhost:{d}/", .{if (opts.port) |value| {
    value
} else {
    return error.NullPointer;
}});
    ...
    for (raw, 0..) |val, i| {
        ...
        if (args.option("--target")) |_target| brk: {
            ...
            if (if (opts.target) |value| {
    value
} else {
    return error.NullPointer;
} == .bun) {
                ctx.debug.run_in_bun = if (opts.target) |value| {
    value
} else {
    return error.NullPointer;
} == .bun;
                ...
                if (ctx.bundler_options.bytecode) {
                    Output.errGeneric("target must be 'bun' when bytecode is true. Received: {s}", .{@tagName(if (opts.target) |value| {
    value
} else {
    return error.NullPointer;
})});
                ...
                if (ctx.bundler_options.bake) {
                    Output.errGeneric("target must be 'bun' when using --app. Received: {s}", .{@tagName(if (opts.target) |value| {
    value
} else {
    return error.NullPointer;
})});
...
// safe-transpile: optional unwrap requires manual review
    ...
    if (output_file != null)
        ctx.debug.output_file = (if (output_file) |value| {
    value
} else {
    return error.NullPointer;
});
