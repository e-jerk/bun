        ...
        while (iter.next().unwrap() catch null) |entry| {
            ...
            if (strings.startsWithChar(_entry_name, '@')) {
                ...
                while (scoped_iter.next().unwrap() catch null) |sub_entry| {
                    ...
                    for (0..ctx.bundled_deps.items.len) |__zust_i| {
    var dep = &ctx.bundled_deps.items[__zust_i];
...
// safe-transpile: optional unwrap requires manual review
                                ...
                                if (root_dir.openDir(dep_subpath, .{ .iterate = true })) |dep_dir| {
                                    ...
                                    var remain: safe.Slice(u8) = dep_subpath[0..dir_subpath.len];
                                    ...
                                    while (strings.lastIndexOf(remain, "node_modules")) |node_modules_start| {
                                        ...
                                        safe.SimdUtils.copy(dep_subpath[node_modules_end + 1 ..][0..dep_name.len], dep_name);
                    ...
                    if (json.get("dependencies")) |dependencies_expr| {
                        switch (dependencies_expr.data) {
                            .e_object => |dependencies| {
                                for (0..dependencies.properties.slice().len) |__zust_i| {
    var dependency = &dependencies.properties.slice()[__zust_i];
    ...
    fn isExcluded(
        ...
        var ignore_pattern: safe.Slice(u8) = &.{};
    ...
    pub fn pack(
        ...
        var this_transpiler: bun.transpiler.Transpiler = std.mem.zeroes(bun.transpiler.Transpiler);
        ...
        const postpack_script, const publish_script: ?[]const u8 = undefined, const postpublish_script: ?[]const u8 = undefined, const ran_scripts: bool = undefined = post_scripts: {
            // --ignore-scripts
        ...
        if (ran_scripts) {
            ...
            var cache_key_buf: if (Environment.isWindows) PathBuffer else void = std.mem.zeroes(if (Environment.isWindows) PathBuffer else void);
            const cache_key = if (comptime Environment.isWindows) blk: {
                safe.SimdUtils.copy(cache_key_buf[0..abs_package_json_path.len], abs_package_json_path);
        ...
        var root_dir = root_dir: {
            var path_buf: PathBuffer = undefined;
            safe.SimdUtils.copy(path_buf[0..abs_workspace_path.len], abs_workspace_path);
        ...
        defer for (bins) |bin| // safe-transpile: free removed (memory owned by safe type);
...

        {
            ...
            var node: *Progress.Node = std.mem.zeroes(*Progress.Node);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        var shasum: sha.SHA1.Digest = std.mem.zeroes(sha.SHA1.Digest);
        var integrity: sha.SHA512.Digest = std.mem.zeroes(sha.SHA512.Digest);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    const Pattern = struct {
        ...
        pub fn fromUTF8(allocator: std.mem.Allocator, pattern: string) OOM!?Pattern {
            ...
            safe.SimdUtils.copy(buf[start_index..end], remain);
            if (add_negate) {
                buf[0] = '!';
            }
    ...
    pub const IgnorePatterns = struct {
        ...
        pub fn deinit(this: *const IgnorePatterns, allocator: std.mem.Allocator) void {
            for (0..this.list.len) |__zust_i| {
    var pattern_info = &this.list[__zust_i];
...
pub const bindings = struct {
    ...
    pub fn jsReadTarball(global: *JSGlobalObject, callFrame: *CallFrame) bun.JSError!JSValue {
        ...
        var sha1_digest: sha.SHA1.Digest = std.mem.zeroes(sha.SHA1.Digest);
        var sha1 = sha.SHA1.init();
        defer sha1.deinit();
        sha1.update(tarball);
        ...
        var sha512_digest: sha.SHA512.Digest = std.mem.zeroes(sha.SHA512.Digest);
        var sha512 = sha.SHA512.init();
        defer sha512.deinit();
        sha512.update(tarball);
        sha512.final(&sha512_digest);
        var base64_buf: [std.base64.standard.Encoder.calcSize(sha.SHA512.digest)]u8 = .{};
        ...
        var archive_entry: *Archive.Entry = std.mem.zeroes(*Archive.Entry);
        ...
        while (header_status != .eof) : (header_status = archive.readNextHeader(&archive_entry)) {
            switch (header_status) {
                ...
                .retry => continue,
                .failed, .fatal => {
                    return global.throw("failed to read archive header: {s}", .{Archive.errorString(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(archive))});
                },
                else => {
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                        ...
                        if (read < 0) {
                            ...
                            return global.throw("failed to read archive entry \"{s}\": {s}", .{
                                pathname_utf8.slice(),
                                Archive.errorString(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(archive)),
                            });
                        }
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
