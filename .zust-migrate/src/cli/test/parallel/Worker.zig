...
/// `@import("std-fs-compat").milliTimestamp()` at the most recent dispatch; drives lazy
/// scale-up.
dispatched_at: i64 = 0,
...
pub fn start(this: *Worker) !void {
    ...
    const process = if (this.process) |value| {
    value
} else {
...
pub fn dispatch(this: *Worker, file_idx: u32, file: []const u8) void {
    ...
    this.dispatched_at = @import("std-fs-compat").milliTimestamp();
}
