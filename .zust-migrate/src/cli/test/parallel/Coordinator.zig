...
pub const Coordinator = struct {
    ...
    fn hasUndispatchedFiles(this: *const Coordinator) bool {
        for (0..this.workers.len) |__zust_i| { var w = &this.workers[__zust_i]; if (!w.range.isEmpty()) return true };
        return false;
    }
    ...
    fn findStealVictim(this: *Coordinator) ?*Worker {
        ...
        var most: u32 = 0;
        for (0..this.workers.len) |__zust_i| {
    var v = &this.workers[__zust_i];
            if (v.range.len() > most) {
                most = v.range.len();
                victim = v;
            }
    ...
    fn abortAll(this: *Coordinator) noreturn {
        ...
        for (0..this.workers[0..this.spawned_count].len) |__zust_i| {
    var w = &this.workers[0..this.spawned_count][__zust_i];
            if (w.process) |p| {
    ...
    fn bailOut(this: *Coordinator) void {
        ...
        Output.flush();
        for (0..this.workers[0..this.spawned_count].len) |__zust_i| {
    var other = &this.workers[0..this.spawned_count][__zust_i];
...
// safe-transpile: function returns small constant slice — consider safe.String
    ...
    pub fn onFrame(this: *Coordinator, w: *Worker, kind: Frame.Kind, rd: *Frame.Reader) void {
        switch (kind) {
            ...
            .file_done => {
                var nums: [9]u32 = .{};
                for (0..&nums.len) |__zust_i| { var n = &&nums[__zust_i]; n.* = rd.u32_() };
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    pub const AbortHandler = struct {
        ...
        var prev_int: if (Environment.isPosix) bun.sys.Sigaction else void = std.mem.zeroes(if (Environment.isPosix) bun.sys.Sigaction else void);
        var prev_term: if (Environment.isPosix) bun.sys.Sigaction else void = std.mem.zeroes(if (Environment.isPosix) bun.sys.Sigaction else void);
