...
pub const Bunfig = struct {
    ...
    pub const Parser = struct {
        ...
        fn parseRegistryObject(this: *Parser, obj: *js_ast.E.Object) !api.NpmRegistry {
            ...
            if (obj.get("url")) |url| {
                ...
                const href = if (url.asString(this.allocator)) |value| {
    value
} else {
    return error.NullPointer;
            ...
            if (obj.get("username")) |username| {
                ...
                registry.username = if (username.asString(this.allocator)) |value| {
    value
} else {
    return error.NullPointer;
};
            ...
            if (obj.get("password")) |password| {
                ...
                registry.password = if (password.asString(this.allocator)) |value| {
    value
} else {
    return error.NullPointer;
};
            ...
            if (obj.get("token")) |token| {
                ...
                registry.token = if (token.asString(this.allocator)) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        fn loadLogLevel(this: *Parser, expr: js_ast.Expr) !void {
            ...
            this.bunfig.log_level = switch (Matcher.match(if (expr.asString(this.allocator)) |value| {
    value
} else {
    return error.NullPointer;
})) {
            ...
            if (comptime cmd.isNPMRelated() or cmd == .RunCommand or cmd == .AutoCommand or cmd == .TestCommand) {
                if (json.getObject("install")) |install_obj| {
                    var install: *api.BunInstall = this.ctx.install orelse brk: {
                        const install_box = try zust.Box(api.BunInstall).init(this.allocator, undefined);
                        const install = install_box.ptr;
                        install[0] = std.mem.zeroes(api.BunInstall);
                    ...
                    if (install_obj.get("auto")) |auto_install_expr| {
                        if (auto_install_expr.data == .e_string) {
                            ...
                            this.ctx.debug.global_cache = if (if (auto_install_expr.asBool()) |value| {
    value
} else {
    return error.NullPointer;
})
...
// safe-transpile: optional unwrap requires manual review
                    ...
                    if (install_obj.get("concurrentScripts")) |jobs| {
                        if (jobs.data == .e_number) {
                            ...
                            if (if (install.concurrent_scripts) |value| {
    value
} else {
    return error.NullPointer;
} == 0) install.concurrent_scripts = null;
                        }
                    }
...
// safe-transpile: optional unwrap requires manual review
            ...
            if (this.bunfig.jsx == null) {
                ...
                var jsx: *api.Jsx = &if (this.bunfig.jsx) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
    ...
    return error.NullPointer;
                    ...
                    const loader = options.Loader.fromString(if (value.asString(allocator)) |value| {
    value
} else {
    return error.NullPointer;
}) orelse {
