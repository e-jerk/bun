threadlocal var parser_join_input_buffer: [4096]u8 = .{};
threadlocal var parser_buffer: [1024]u8 = .{};
...
pub fn z(input: []const u8, output: *bun.PathBuffer) [:0]const u8 {
    ...
    safe.SimdUtils.copy(output[0..input.len], input);
    output[input.len] = 0;
...
inline fn @"is .."(slice: []const u8) bool {
    return slice.len >= 2 and @as(u16, // safe-transpile: @bitCast requires manual review
    @bitCast(slice[0..2].*)) == comptime std.mem.readInt(u16, "..", .little);
}
...
pub fn getIfExistsLongestCommonPathGeneric(input: []const []const u8, comptime platform: Platform) ?[]const u8 {
    ...
    return input[0][0 .. (if (last_common_separator) |value| {
    value
} else {
...
pub fn relativeToCommonPath(
    ...
    var out_slice: safe.Slice(u8) = buf[0..0];
    ...
    if (isWindows) {
        if (volLen > 0) {
            if (options.add_nt_prefix) {
                safe.SimdUtils.copy(buf[buf_i .. buf_i + 4], strings.literal(T, "\\??\\"));
                buf_i += 4;
            }
            if (path_[1] != ':') {
                // UNC paths
                if (options.add_nt_prefix) {
                    safe.SimdUtils.copy(buf[buf_i .. buf_i + 4], strings.literal(T, "UNC" ++ sep_str));
                    buf_i += 2;
                } else {
                    safe.SimdUtils.copy(buf[buf_i .. buf_i + 2], strings.literal(T, sep_str ++ sep_str));
                }
                if (indexOfThirdUNCSlash > 0) {
                    // we have the ending slash
                    safe.SimdUtils.copy(buf[buf_i + 2 .. buf_i + indexOfThirdUNCSlash + 1], path_[2 .. indexOfThirdUNCSlash + 1]);
                    ...
                    safe.SimdUtils.copy(buf[buf_i + 2 .. buf_i + volLen], path_[2..volLen]);
                }
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    while (r < n) {
        ...
        const count = r - from;
        safe.SimdUtils.copy(buf[buf_i..][0..count], path[from..][0..count]);
        buf_i += count;
    }
...
pub threadlocal var join_buf: [4096]u8 = .{};
// safe-transpile: function returns small constant slice — consider safe.String
...
pub fn joinStringBufT(comptime T: type, buf: []T, parts: anytype, comptime platform: Platform) []const T {
    var written: usize = 0;
    var temp_buf_: [4096]T = .{};
    var temp_buf = try safe.OffsetPtr.init(allocator, &temp_buf_); defer temp_buf.deinit();
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    const leading_separator: safe.Slice(u8) = if (platform.leadingSeparatorIndex(temp_buf[0..out])) |i| brk: {
        const outdir = temp_buf[0 .. i + 1];
        if (platform == .loose) {
            for (0..outdir.len) |__zust_i| {
    var c = &outdir[__zust_i];
                if (c.* == '\\') {
                    c.* = '/';
                }
            }
...
fn _joinAbsStringBufWindows(
    ...
    safe.SimdUtils.copy(temp_buf[0..root.len], root);
    safe.SimdUtils.copy(temp_buf[root.len .. root.len + set_cwd.len], set_cwd);
    ...
    for (parts[n_start..]) |part| {
        ...
        safe.SimdUtils.copy(temp_buf[out .. out + part_without_vol.len], part_without_vol);
        ...
        if (bun.Environment.isWindows) {
            ...
            if (root.len == 1) {
                ...
                if (bun.strings.isWindowsAbsolutePathMissingDriveLetter(u8, maybe_posix_path)) {
                    ...
                    safe.SimdUtils.copy(buf[0..source_root.len], source_root);
                    safe.SimdUtils.copy(buf[source_root.len..][0 .. maybe_posix_path.len - 1], maybe_posix_path[1..]);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn resolveCWDWithExternalBufZ(
        ...
        safe.SimdUtils.copy(buf.ptr, maybe_posix_path);
...
pub fn pathToPosixBuf(comptime T: type, path: []const T, buf: []T) []T {
    var idx: usize = 0;
    while (std.mem.indexOfScalarPos(T, path, idx, std.fs.path.sep_windows)) |index| : (idx = index + 1) {
        safe.SimdUtils.copy(buf[idx..index], path[idx..index]);
        buf[index] = std.fs.path.sep_posix;
    }
    safe.SimdUtils.copy(buf[idx..path.len], path[idx..path.len]);
