...
pub const Location = struct {
    ...
    pub fn clone(this: Location, allocator: std.mem.Allocator) !Location {
        return Location{
            ...
            .column = this.column,
            .length = this.length,
            .line_text = if (this.line_text != null) try allocator.dupe(u8, if (this.line_text) |value| {
    value
} else {
    ...
    pub fn cloneWithBuilder(this: Location, string_builder: *StringBuilder) Location {
        return Location{
            ...
            .column = this.column,
            .length = this.length,
            .line_text = if (this.line_text != null) string_builder.append(if (this.line_text) |value| {
    value
} else {
    return error.NullPointer;
}) else null,
...
// safe-transpile: optional unwrap requires manual review
        const new_line_text = try allocator.dupe(u8, if (this.location.?.line_text) |value| {
    value
} else {
    return error.NullPointer;
});
        var new_location = if (this.location) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
    ...
    pub fn writeFormat(
        ...
        const message_color = switch (kind) {
            .err => comptime if (Output.color_map.get("b")) |value| {
    value
} else {
    return error.NullPointer;
},
            .note => comptime if (Output.color_map.get("blue")) |value| {
    value
} else {
    return error.NullPointer;
},
            else => comptime if (Output.color_map.get("d")) |value| {
    value
} else {
    return error.NullPointer;
} ++ if (Output.color_map.get("b")) |value| {
    value
} else {
    return error.NullPointer;
},
        ...
        const color_name: string = switch (kind) {
            .err => comptime if (Output.color_map.get("red")) |value| {
    value
} else {
    return error.NullPointer;
},
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                    ...
                    if ((comptime enable_ansi_colors) and message_color.len > 0) {
                        ...
                        // always bold the ^
                        try to.writeAll(comptime if (Output.color_map.get("b")) |value| {
    value
} else {
    return error.NullPointer;
});
...
pub const Msg = struct {
    ...
    pub fn memoryCost(this: *const Msg) usize {
        ...
        for (0..this.notes.len) |__zust_i| {
    var note = &this.notes[__zust_i];
    ...
    pub fn deinit(msg: *Msg, allocator: std.mem.Allocator) void {
        ...
        for (0..msg.notes.len) |__zust_i| {
    var note = &msg.notes[__zust_i];
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        if (notes_count > 0) {
            ...
            var note_i: usize = 0;
            for (0..self.msgs.items.len) |__zust_i| {
    var msg = &self.msgs.items[__zust_i];
...
pub const Source = struct {
    ...
    pub fn isWebAssembly(this: *const Source) bool {
        ...
        const bytes = @as(u32, // safe-transpile: @bitCast requires manual review
    @bitCast(this.contents[0..4].*));
