...
// safe-transpile: optional unwrap requires manual review
        ...
        var path: safe.Slice(u8) = full_path;
        var bucket: safe.Slice(u8) = this.bucket;
        ...
        var normalized_path_buffer: [1024 + 63 + 2]u8 = .{}; // 1024 max key size and 63 max bucket name
        var path_buffer: [1024]u8 = .{};
        var bucket_buffer: [63]u8 = .{};
        ...
        var tmp_buffer: [4096]u8 = .{};

        const authorization = brk: {
            // we hash the hash so we need 2 buffers
            var hmac_sig_service: [bun.BoringSSL.c.EVP_MAX_MD_SIZE]u8 = .{};
            var hmac_sig_service2: [bun.BoringSSL.c.EVP_MAX_MD_SIZE]u8 = .{};
            ...
            if (signQuery) {
                var token_encoded_buffer: [2048]u8 = .{}; // token is normaly like 600-700 but can be up to 2k
                ...
                var content_md5_encoded_buffer: [128]u8 = .{}; // MD5 as base64 (which is required for AWS SigV4) is always 44, when encoded its always 46 (44 + ==)
                ...
                var content_disposition_encoded_buffer: [512]u8 = .{};
                ...
                var content_type_encoded_buffer: [256]u8 = .{};
...
// safe-transpile: function returns small constant slice — consider safe.String
    ...
    const table = init: {
        ...
        for (0..128) |i| {
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            t[i] = generate(// safe-transpile: @bitCast requires manual review
    @bitCast(@as(u7, @intCast(i))));
        }
        break :init t;
    };
...
// safe-transpile: function returns small constant slice — consider safe.String
    pub fn get(key: Key) []const u8 {
        return table[@as(u7, // safe-transpile: @bitCast requires manual review
    @bitCast(key))];
    }
};
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    inline fn formatForKey(
        ...
        return std.fmt.bufPrint(buf, fmtString(key), .{ method, path, query } ++
            (if (key.content_disposition) .{if (content_disposition) |value| {
    value
} else {
    return error.NullPointer;
}} else .{}) ++
            (if (key.content_encoding) .{if (content_encoding) |value| {
    value
} else {
    return error.NullPointer;
}} else .{}) ++
            (if (key.content_md5) .{if (content_md5) |value| {
    value
} else {
    return error.NullPointer;
}} else .{}) ++
            .{host} ++
            (if (key.acl) .{if (acl) |value| {
    value
} else {
    return error.NullPointer;
}} else .{}) ++
            .{ hash, date } ++
            (if (key.session_token) .{if (session_token) |value| {
    value
} else {
    return error.NullPointer;
}} else .{}) ++
            (if (key.storage_class) .{if (storage_class) |value| {
    value
} else {
    return error.NullPointer;
}} else .{}) ++
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn format(
        ...
        return switch (@as(u7, // safe-transpile: @bitCast requires manual review
    @bitCast(key))) {
