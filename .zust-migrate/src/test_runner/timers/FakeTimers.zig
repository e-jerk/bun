...
pub var current_time: struct {
    ...
    offset_lock: @import("std-fs-compat").Mutex = .{},
    ...
    pub fn set(this: *@This(), globalObject: *jsc.JSGlobalObject, v: struct {
        ...
        vm.overridden_performance_now = // safe-transpile: @bitCast requires manual review
    @bitCast(v.offset.ns());
    }
...
fn fire(this: *FakeTimers, globalObject: *jsc.JSGlobalObject, next: *bun.api.Timer.EventLoopTimer) void {
    ...
    if (bun.Environment.ci_assert) {
        ...
        bun.assert(next.next.eql(&(if (prev) |value| {
    value
} else {
    return error.NullPointer;
})) or next.next.greater(&(if (prev) |value| {
    value
} else {
    return error.NullPointer;
})));
