...
pub fn deinit(this: *Collection) void {
    ...
    for (0..this.describe_callback_queue.items.len) |__zust_i| {
    var item = &this.describe_callback_queue.items[__zust_i];
        item.deinit();
    }
    this.describe_callback_queue.deinit();
    for (0..this.current_scope_callback_queue.items.len) |__zust_i| {
    var item = &this.current_scope_callback_queue.items[__zust_i];
        item.deinit();
    }
...
pub fn step(buntest_strong: bun_test.BunTestPtr, globalThis: *jsc.JSGlobalObject, data: bun_test.BunTest.RefDataValue) bun.JSError!bun_test.StepResult {
    ...
    while (this.describe_callback_queue.items.len > 0) {
        ...
        var first = if (this.describe_callback_queue.pop()) |value| {
    value
} else {
