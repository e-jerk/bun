...
pub fn printDiffMain(arena: std.mem.Allocator, not: bool, received_slice: []const u8, expected_slice: []const u8, writer: anytype, config: DiffConfig) std.Io.Writer.Error!void {
    ...
    if (diff_segments.items.len > 0) for (0..diff_segments.items[0 .. diff_segments.items.len - 1].len) |__zust_i| {
    var diff_segment = &diff_segments.items[0 .. diff_segments.items.len - 1][__zust_i];
    ...
    for (diff_segments.items, 0..) |segment, i| {
            if (segment.mode != .equal) {
                ...
                for (0..diff_segments.items[i..end].len) |__zust_i| {
    var seg = &diff_segments.items[i..end][__zust_i];
                    seg.skip = false;
                }
            }
        }

        {
            ...
            while (i > 0) {
                ...
                if (segment.mode != .equal) {
                    ...
                    for (0..diff_segments.items[start .. i + 1].len) |__zust_i| {
    var seg = &diff_segments.items[start .. i + 1][__zust_i];
                        seg.skip = false;
                    }
                }
            }
    ...
    for (0..diff_segments.items.len) |__zust_i| {
    var segment = &diff_segments.items[__zust_i];
...
fn printSegment(
    ...
    try printTruncatedLine(if (lines.next()) |value| {
    value
} else {
...
fn printModifiedSegment(
    ...
    for (0..char_diff.items.len) |__zust_i| {
    var item = &char_diff.items[__zust_i];
        switch (item.operation) {
            .delete => {
