...
pub const JestPrettyFormat = struct {
    ...
    pub fn format(
        ...
        var fmt: JestPrettyFormat.Formatter = std.mem.zeroes(JestPrettyFormat.Formatter);
        defer {
        ...
        var tag: JestPrettyFormat.Formatter.Tag.Result = std.mem.zeroes(JestPrettyFormat.Formatter.Tag.Result);
...
var __zust_loop_counter: u64 = 0;
    ...
    pub const Formatter = struct {
        ...
        threadlocal var name_buf: [512]u8 = .{};
        ...
        pub fn PropertyIterator(comptime Writer: type, comptime enable_ansi_colors_: bool) type {
            return struct {
                ...
                pub fn forEach(
                    ...
                    const key = if ((@as(?[*c]ZigString, @ptrCast(key_)))) |value| {
    value
} else {
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
                ...
                .Private => {
                    if (value.as(jsc.WebCore.Response)) |response| {
                        ...
                        const toJSONFunction = if ((try value.get(this.globalThis, "toJSON"))) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                ...
                .JSON => {
                    ...
                    if (jsType == .JSDate) {
                        ...
                        var iso_string_buf: [36]u8 = .{};
                        var out_buf: safe.Slice(u8) = std.fmt.bufPrint(&iso_string_buf, "{f}", .{str}) catch "";
                        if (out_buf.len > 2) {
                            // trim the quotes
                ...
                .JSX => {
                    ...
                    if (try value.get(this.globalThis, "props")) |props| {
                        ...
                        const props_obj = if (props.getObject()) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                ...
                .TypedArray => {
                    const arrayBuffer = if (value.asArrayBuffer(this.globalThis)) |value| {
    value
} else {
    return error.NullPointer;
};
                    ...
                    if (slice.len > 0) {
                        switch (jsType) {
                            .Int8Array => {
                                const slice_with_type: []align(std.meta.alignment([]i8)) i8 = // safe-transpile: @alignCast requires manual review
    @alignCast(std.mem.bytesAsSlice(i8, slice));
                            ...
                            .Int16Array => {
                                const slice_with_type: []align(std.meta.alignment([]i16)) i16 = // safe-transpile: @alignCast requires manual review
    @alignCast(std.mem.bytesAsSlice(i16, slice));
                            ...
                            .Uint16Array => {
                                const slice_with_type: []align(std.meta.alignment([]u16)) u16 = // safe-transpile: @alignCast requires manual review
    @alignCast(std.mem.bytesAsSlice(u16, slice));
                            ...
                            .Int32Array => {
                                const slice_with_type: []align(std.meta.alignment([]i32)) i32 = // safe-transpile: @alignCast requires manual review
    @alignCast(std.mem.bytesAsSlice(i32, slice));
                            ...
                            .Uint32Array => {
                                const slice_with_type: []align(std.meta.alignment([]u32)) u32 = // safe-transpile: @alignCast requires manual review
    @alignCast(std.mem.bytesAsSlice(u32, slice));
                            ...
                            .Float16Array => {
                                const slice_with_type: []align(std.meta.alignment([]f16)) f16 = // safe-transpile: @alignCast requires manual review
    @alignCast(std.mem.bytesAsSlice(f16, slice));
                            ...
                            .Float32Array => {
                                const slice_with_type: []align(std.meta.alignment([]f32)) f32 = // safe-transpile: @alignCast requires manual review
    @alignCast(std.mem.bytesAsSlice(f32, slice));
                            ...
                            .Float64Array => {
                                const slice_with_type: []align(std.meta.alignment([]f64)) f64 = // safe-transpile: @alignCast requires manual review
    @alignCast(std.mem.bytesAsSlice(f64, slice));
                            ...
                            .BigInt64Array => {
                                const slice_with_type: []align(std.meta.alignment([]i64)) i64 = // safe-transpile: @alignCast requires manual review
    @alignCast(std.mem.bytesAsSlice(i64, slice));
                            ...
                            .BigUint64Array => {
                                const slice_with_type: []align(std.meta.alignment([]u64)) u64 = // safe-transpile: @alignCast requires manual review
    @alignCast(std.mem.bytesAsSlice(u64, slice));
                                this.indent += 1;
                                defer this.indent -|= 1;
                                for (slice_with_type) |el| {
                                    writer.writeAll("\n");
                            ...
                            // Uint8Array, Uint8ClampedArray, DataView, ArrayBuffer
                                ...
                                const slice_with_type: []align(std.meta.alignment([]u8)) u8 = // safe-transpile: @alignCast requires manual review
    @alignCast(std.mem.bytesAsSlice(u8, slice));
                                this.indent += 1;
                                defer this.indent -|= 1;
                                for (slice_with_type) |el| {
                                    writer.writeAll("\n");
