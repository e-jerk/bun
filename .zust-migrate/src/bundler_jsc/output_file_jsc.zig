...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn toJS(
        ...
        var blob = safe.Box(jsc.WebCore.Blob).init(allocator, undefined) catch unreachable;
        blob[0] = jsc.WebCore.Blob.initWithStore(store, globalThis);
        if (mime_type) |mime| {
...
pub fn toJS(
    ...
    return switch (this.value) {
        ...
        .saved => brk: {
            var build_output = safe.Box(jsc.API.BuildArtifact).init(allocator, undefined) catch @panic("Unable to allocate Artifact");
            ...
            build_output[0] = jsc.API.BuildArtifact{
                .blob = jsc.WebCore.Blob.initWithStore(file_blob, globalObject),
                .hash = this.hash,
                .loader = this.input_loader,
                .output_kind = this.output_kind,
