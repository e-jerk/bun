...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            const replacement: safe.Slice(u8) = inline for (interp_map) |entry| {
            ...
            safe.SimdUtils.copy(interp_region[0..replacement.len], replacement);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        safe.SimdUtils.copy(self.data.items[new_file_offset + header_size ..][0..payload.len], payload);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            safe.SimdUtils.copy(shdr_bytes, std.mem.asBytes(&shdr));
        }
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            safe.SimdUtils.copy(self.data.items[phdr_offset..][0..phdr_size], std.mem.asBytes(&extended));
        }
    }
