    ...
    const LoadCommandIterator = struct {
        ...
        pub const Entry = struct {
            ...
            pub fn cast(entry: Entry, comptime Cmd: type) ?Cmd {
                ...
                const ptr: *align(1) const Cmd = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(entry.data.ptr);
                var cmd = ptr.*;
            ...
            pub fn getSections(entry: Entry) []align(1) const macho.section_64 {
                const segment_lc = if (entry.cast(macho.segment_command_64)) |value| {
    value
} else {
    return error.NullPointer;
};
                const sects_ptr: [*]align(1) const macho.section_64 = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(entry.data[@sizeOf(macho.segment_command_64)..]);
                return sects_ptr[0..segment_lc.nsects];
            }

// safe-transpile: function returns small constant slice — consider safe.String
            pub fn getDylibPathName(entry: Entry) []const u8 {
                const dylib_lc = if (entry.cast(macho.dylib_command)) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: function returns small constant slice — consider safe.String
            pub fn getRpathPathName(entry: Entry) []const u8 {
                const rpath_lc = if (entry.cast(macho.rpath_command)) |value| {
    value
} else {
    return error.NullPointer;
};
            ...
            pub fn getBuildVersionTools(entry: Entry) []align(1) const macho.build_tool_version {
                const build_lc = if (entry.cast(macho.build_version_command)) |value| {
    value
} else {
    return error.NullPointer;
};
                const tools_ptr: [*]align(1) const macho.build_tool_version = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(entry.data[@sizeOf(macho.build_version_command)..]);
        ...
        pub fn next(it: *LoadCommandIterator) ?Entry {
            ...
            const hdr: *align(1) const macho.load_command = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(hdr_bytes.ptr);
            var cmd = hdr.*;
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn init(allocator: Allocator, obj_file: []const u8, blob_to_embed_length: usize) !*MachoFile {
        ...
        const header: *const macho.mach_header_64 = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(data.items.ptr));
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn writeSection(self: *MachoFile, data: []const u8) !void {
        ...
        while (iter.next()) |entry| {
            ...
            switch (cmd.cmd) {
                .SEGMENT_64 => {
                    const command = if (entry.cast(macho.segment_command_64)) |value| {
    value
} else {
    return error.NullPointer;
};
                    if (strings.eqlComptime(command.segName(), "__BUN")) {
                        if (command.nsects > 0) {
                            ...
                            const sections = @as([*]macho.section_64, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(&self.data.items[section_offset + @sizeOf(macho.segment_command_64)])))[0..command.nsects];
                            for (0..sections.len) |__zust_i| {
    var sect = &sections[__zust_i];
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        const code_sign_cmd: ?*align(1) macho.linkedit_data_command =
            if (code_sign_cmd_idx) |idx|
                @as(*align(1) macho.linkedit_data_command, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(@constCast(&self.data.items[idx]))))
            else
                null;
        const linkedit_seg: *align(1) macho.segment_command_64 =
            if (linkedit_seg_idx) |idx|
                @as(*align(1) macho.segment_command_64, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(@constCast(&self.data.items[idx]))))
            else
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        safe.SimdUtils.copy(self.data.items[original_fileoff + 8 ..][0..data.len], data);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    fn updateLoadCommandOffsets(self: *MachoFile, previous_fileoff: u64, size_diff: u64, new_linkedit_fileoff: u64, new_linkedit_filesize: u64, sig_size: usize) !void {
        ...
        while (iter.next()) |entry| {
            ...
            switch (cmd.cmd) {
                .SYMTAB => {
                    const symtab: *align(1) macho.symtab_command = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(cmd_ptr));
                ...
                .DYSYMTAB => {
                    const dysymtab: *align(1) macho.dysymtab_command = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(cmd_ptr));
                ...
                .DYLD_EXPORTS_TRIE,
                => {
                    const linkedit_cmd: *align(1) macho.linkedit_data_command = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(cmd_ptr));
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                ...
                .DYLD_INFO, .DYLD_INFO_ONLY => {
                    const dyld_info: *align(1) macho.dyld_info_command = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(cmd_ptr));
    ...
    fn validateSegments(self: *MachoFile) !void {
        ...
        while (iter.next()) |entry| {
            ...
            if (cmd.cmd == .SEGMENT_64) {
                const seg = if (entry.cast(macho.segment_command_64)) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn init(allocator: Allocator, obj: []const u8) !*MachoSigner {
            ...
            const header = @as(*align(1) const macho.mach_header_64, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(obj.ptr)).*;
            ...
            while (it.next()) |cmd| {
                if (cmd.hdr.cmd == .SEGMENT_64) {
                    const seg = if (cmd.cast(macho.segment_command_64)) |value| {
    value
} else {
    return error.NullPointer;
};
            ...
            while (it.next()) |cmd| {
                switch (cmd.hdr.cmd) {
                    .CODE_SIGNATURE => {
                        const cs = if (cmd.cast(macho.linkedit_data_command)) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
            ...
            safe.SimdUtils.copy(self.data.items[offset..][0..@sizeOf(@TypeOf(super_blob))], mem.asBytes(&super_blob));
            safe.SimdUtils.copy(self.data.items[offset + @sizeOf(@TypeOf(super_blob))..][0..@sizeOf(@TypeOf(blob_index))], mem.asBytes(&blob_index));
            safe.SimdUtils.copy(self.data.items[offset + @sizeOf(@TypeOf(super_blob)) + @sizeOf(@TypeOf(blob_index))..][0..@sizeOf(@TypeOf(code_dir))], mem.asBytes(&code_dir));
            safe.SimdUtils.copy(self.data.items[offset + @sizeOf(@TypeOf(super_blob)) + @sizeOf(@TypeOf(blob_index)) + @sizeOf(@TypeOf(code_dir))..][0..id.len], id);
            ...
            while (remaining.len >= PAGE_SIZE) {
                ...
                var digest: bun.sha.SHA256.Digest = std.mem.zeroes(bun.sha.SHA256.Digest);
                bun.sha.SHA256.hash(page, &digest, null);
                safe.SimdUtils.copy(self.data.items[write_offset..][0..digest.len], &digest);
            ...
            if (remaining.len > 0) {
                ...
                safe.SimdUtils.copy(last_page[0..remaining.len], remaining);
                var digest: bun.sha.SHA256.Digest = std.mem.zeroes(bun.sha.SHA256.Digest);
