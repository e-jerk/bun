...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    fn assertOwned(self: Self, ptr: anytype) void {
        const cast_ptr: [*]const u8 = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(switch (@typeInfo(@TypeOf(ptr)).pointer.size) {
            .c, .one, .many => ptr,
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    fn trackExternalFree(self: *Self, slice: anytype, ret_addr: ?usize) FreeError!void {
        ...
        const ptr: safe.Slice(u8) = switch (@typeInfo(@TypeOf(slice))) {
    ...
    fn setPointerExtra(self: *Self, ptr: *anyopaque, extra: Extra) void {
        const locked = self.lock();
        defer self.unlock();
        const allocation = locked.history.allocations.getPtr(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(ptr)) orelse
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        ...
        fn downcastImpl(
            ...
            const state = if (comptime enabled) blk: {
                ...
                const state: *State = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(std_alloc.ptr));
                break :blk state;
            };
...
fn vtable_alloc(ctx: *anyopaque, len: usize, alignment: std.mem.Alignment, ret_addr: usize) ?[*]u8 {
    const raw_state: *State = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
