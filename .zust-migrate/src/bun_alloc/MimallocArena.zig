...
pub const Borrowed = struct {
    ...
    fn fromOpaque(ptr: *anyopaque) Borrowed {
        return .{ ._heap = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    ...
    fn alignedAlloc(self: Borrowed, len: usize, alignment: Alignment) ?[*]u8 {
        ...
        return if (ptr) |p|
            @as([*]u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(p))
        else
            null;
    }
...
fn global_vtable_alloc(_: *anyopaque, len: usize, alignment: Alignment, _: usize) ?[*]u8 {
    ...
    return if (ptr) |p| // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(p) else null;
}
