pub const ContentsOrFd = union(enum) {
    ...
    const Tag = if (@typeInfo(ContentsOrFd).@"union".tag_type) |value| {
    value
} else {
...
fn getEmptyCSSAST(
    ...
    var ast = JSAst.init(if ((try js_parser.newLazyExportAST(allocator, transpiler.options.define, opts, log, root, source, ""))) |value| {
    value
} else {
    return error.NullPointer;
});
...
fn getEmptyAST(log: *Logger.Log, transpiler: *Transpiler, opts: js_parser.Parser.Options, allocator: std.mem.Allocator, source: *const Logger.Source, comptime RootType: type) !JSAst {
    ...
    return JSAst.init(if ((try js_parser.newLazyExportAST(allocator, transpiler.options.define, opts, log, root, source, ""))) |value| {
    value
} else {
    return error.NullPointer;
});
...
fn getAST(
    ...
    return error.NullPointer;
        ...
        .toml => {
            ...
            return JSAst.init(if ((try js_parser.newLazyExportAST(allocator, transpiler.options.define, opts, &temp_log, root, source, ""))) |value| {
    value
} else {
    return error.NullPointer;
});
        ...
        .sqlite_embedded, .sqlite => {
            ...
            const path_to_use = brk: {
                ...
                if (loader == .sqlite_embedded) {
                    ...
                    unique_key_for_additional_file[0] = .{
                        .key = embedded_path,
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        .css => {
            ...
            var ast = JSAst.init(if ((try js_parser.newLazyExportASTImpl(allocator, transpiler.options.define, opts, &temp_log, root, source, "", extra.symbols))) |value| {
    value
} else {
    return error.NullPointer;
});
        ...
        .file, .wasm => {
            ...
            const unique_key: safe.Slice(u8) = if (transpiler.options.dev_server != null)
...
fn getCodeForParseTask(
    ...
    return try ctx.run(if (task.ctx.plugins) |value| {
    value
} else {
    return error.NullPointer;
}, from_plugin);
...
const OnBeforeParsePlugin = struct {
    ...
    pub export fn OnBeforeParsePlugin__isDone(this: *OnBeforeParsePlugin) i32 {
        ...
        if (result.source_ptr != null) {
            const wrapper: *OnBeforeParseResultWrapper = result.getWrapper();
            return @intFromBool(if (result.source_ptr) |value| {
    value
} else {
    return error.NullPointer;
} != if (wrapper.original_source) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: optional unwrap requires manual review
    ...
    opts.macro_context = &if (transpiler.macro_context) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
    ...
    ast.target = target;
    if (ast.parts.len <= 1 and ast.css == null and (task.loader == null or if (task.loader) |value| {
    value
} else {
    return error.NullPointer;
} != .html)) {
...
pub fn runFromThreadPool(this: *ParseTask) void {
    ...
    const result = bun.handleOom(safe.Box(Result).init(allocator, undefined));
