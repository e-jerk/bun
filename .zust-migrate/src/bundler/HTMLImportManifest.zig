...
fn writeEntryItem(
    ...
    if (hash > 0) {
        var base64_buf: [bun.base64.encodeLenFromSize(@sizeOf(@TypeOf(hash))) + 2]u8 = .{};
...
pub fn writeEscapedJSON(index: u32, graph: *const Graph, linker_graph: *const LinkerGraph, chunks: []const Chunk, writer: anytype) !void {
    ...
    defer bytes.deinit();
    try write(index, graph, linker_graph, chunks, @import("std-io-compat").writer(bytes));
...
pub fn write(index: u32, graph: *const Graph, linker_graph: *const LinkerGraph, chunks: []const Chunk, writer: anytype) !void {
    ...
    const bv2: *const BundleV2 = // safe-transpile: @alignCast requires manual review
    @alignCast(@fieldParentPtr("graph", graph));
