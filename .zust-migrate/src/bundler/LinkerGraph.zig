...
pub fn load(
    ...
    for (entry_points, path_strings, source_indices) |i, *path_string, *source_index| {
            ...
            if (entry_point_original_names.get(i.get())) |original_name| {
                path_string[0] = bun.PathString.init(original_name);
            } else {
                path_string[0] = bun.PathString.init(source.path.text);
            }

            source_index[0] = source.index.get();
        }
    ...
    for (scb.list.items(.use_directive), scb.list.items(.source_index), scb.list.items(.reference_source_index)) |use, original_id, ref_id| {
            ...
            for (this.reachable_files) |source_id| {
                for (0..import_records_list[source_id.get()].slice().len) |__zust_i| {
    var import_record = &import_records_list[source_id.get()].slice()[__zust_i];
    ...
    for (this.reachable_files, 0..) |source_index, i| {
        ...
        this.stable_source_indices = @as([]const u32, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(stable_source_indices));
    }
    ...
    for (symbols.slice(), input_symbols.symbols_for_source.slice()) |*dest, src| {
            dest[0] = bun.handleOom(src.clone(this.allocator));
        }
...
pub fn takeAstOwnership(this: *LinkerGraph) void {
    ...
    for (0..ast.items(.import_records).len) |__zust_i| {
    var import_records = &ast.items(.import_records)[__zust_i];
        import_records.transferOwnership(heap);
    }
    for (0..ast.items(.parts).len) |__zust_i| {
    var parts = &ast.items(.parts)[__zust_i];
        parts.transferOwnership(heap);
        for (parts.slice()) |*part| {
            part.dependencies.transferOwnership(heap);
        }
    }
    for (0..ast.items(.symbols).len) |__zust_i| {
    var symbols = &ast.items(.symbols)[__zust_i];
...
pub fn propagateAsyncDependencies(this: *LinkerGraph) !void {
    const State = struct {
        ...
        fn visit(self: *@This(), index: usize) void {
            ...
            for (0..self.import_records[index].sliceConst().len) |__zust_i| {
    var import_record = &self.import_records[index].sliceConst()[__zust_i];
