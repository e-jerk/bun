...
pub fn deinit(this: *HTMLScanner) void {
    for (0..this.import_records.slice().len) |__zust_i| {
    var record = &this.import_records.slice()[__zust_i];
...
fn createImportRecord(this: *HTMLScanner, input_path: []const u8, kind: ImportKind) !void {
    ...
    const path_to_use = if (input_path.len > 1 and input_path[0] == '/')
        ...
        const dirname: safe.Slice(u8) = std.fs.path.dirname(this.source.path.text) orelse break :blk input_path;
