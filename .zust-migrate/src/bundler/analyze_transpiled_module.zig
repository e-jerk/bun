...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn create(source: []const u8, gpa: std.mem.Allocator) !*ModuleInfoDeserialized {
        ...
        // safe-transpile: free removed (memory owned by safe type);
        var rem: safe.Slice(u8) = duped;
        const res = try safe.Box(ModuleInfoDeserialized).init(allocator, undefined);
        defer _ = res.deinit();
        ...
        const flags: Flags = // safe-transpile: @bitCast requires manual review
    @bitCast((try eatC(&rem, 1))[0]);
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
        ...
        try writer.writeByte(// safe-transpile: @bitCast requires manual review
    @bitCast(self.flags));
...
pub const ModuleInfo = struct {
    ...
    pub fn create(gpa: std.mem.Allocator, is_typescript: bool) !*ModuleInfo {
        const res = try safe.Box(ModuleInfo).init(allocator, undefined);
        res[0] = ModuleInfo.init(gpa, is_typescript);
        return res;
    }
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    pub fn replaceStringID(self: *ModuleInfo, old_id: StringID, new_id: StringID) void {
        ...
        for (0..self.buffer.items.len) |__zust_i| {
    var item = &self.buffer.items[__zust_i];
    ...
    for (self.record_kinds.items, 0..) |k, idx| {
        ...
        {
            var i: usize = 0;
            for (0..self.record_kinds.items.len) |__zust_i| {
    var k = &self.record_kinds.items[__zust_i];
