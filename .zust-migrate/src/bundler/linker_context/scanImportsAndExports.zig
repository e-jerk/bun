...
pub fn scanImportsAndExports(this: *LinkerContext) ScanImportsAndExportsError!void {
    ...
    const output_format = this.options.output_format;
    {
        ...
        for (reachable) |source_index_| {
            ...
            // Is it CSS?
            if (css_asts[id] != null) {
                const css_ast = if (css_asts[id]) |value| {
    value
} else {
    return error.NullPointer;
                ...
                for (0..css_ast.composes.values().len) |__zust_i| {
    var composes = &css_ast.composes.values()[__zust_i];
...
// safe-transpile: optional unwrap requires manual review
    ...
    for (aliases, copies) |alias, *copy| {
                    ...
                    copy[0] = this.graph.generateNewSymbol(source_index, .other, original_name);
                }
...
// safe-transpile: optional unwrap requires manual review
    ...
    for (imports_to_bind.keys(), imports_to_bind.values()) |ref_untyped, import_untyped| {
            ...
            if (is_entry_point) {
                ...
                for (aliases) |alias| {
                    const exp = if (resolved_exports_list.get(alias)) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
        ...
        fn visit(v: *@This(), idx: Index.Int, ast: *bun.css.BundlerStyleSheet, ref: Ref) void {
            ...
            if (ast.composes.getPtr(ref)) |composes| {
                for (0..composes.composes.sliceConst().len) |__zust_i| {
    var compose = &composes.composes.sliceConst()[__zust_i];
                    // is an import
