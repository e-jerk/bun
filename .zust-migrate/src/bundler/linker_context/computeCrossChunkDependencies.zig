pub fn computeCrossChunkDependencies(c: *LinkerContext, chunks: []Chunk) bun.OOM!void {
    ...
    for (0..chunk_metas.len) |__zust_i| {
    var meta = &chunk_metas[__zust_i];
    ...
    {
        const cross_chunk_dependencies = safe.Box(CrossChunkDependencies).init(allocator, undefined) catch unreachable;
        defer _ = cross_chunk_dependencies.deinit();
            ...
            if (chunk.entry_point.is_entry_point) {
                ...
                if (flags.wrap != .cjs) {
                    ...
                    for (sorted_and_filtered_export_aliases) |alias| {
                        const export_ = if (resolved_exports.get(alias)) |value| {
    value
} else {
...
fn computeCrossChunkDependenciesWithChunkMetas(c: *LinkerContext, chunks: []Chunk, chunk_metas: []ChunkMeta) !void {
    ...
    for (chunks, chunk_metas, 0..) |*chunk, *chunk_meta, chunk_index| {
        ...
        for (chunk_meta.imports.keys()) |import_ref| {
            const symbol = if (c.graph.symbols.getConst(import_ref)) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
                    ...
                    if (clause_items.len > 0) {
                        ...
                        const export_clause = safe.Box(js_ast.S.ExportClause).init(allocator, undefined) catch unreachable;
                        export_clause[0] = .{
