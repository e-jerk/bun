pub fn generateChunksInParallel(
...

    {
        ...
        if (c.parse_graph.css_file_count > 0) {
            const total_count = total_count: {
                ...
                for (0..chunks.len) |__zust_i| {
    var chunk = &chunks[__zust_i];
    ...
    for (chunks, chunk_contexts) |*chunk, *chunk_ctx| {
                switch (chunk.content) {
                    .javascript => {
                        chunk_ctx[0] = .{ .c = c, .chunks = chunks, .chunk = chunk };
    ...
    for (chunks, 0..) |*chunk, chunk_index_in_chunks_list| {
            ...
            const _code_result = if (is_standalone and chunk.content == .html)
                chunk.intermediate_output.codeStandalone(
                    ...
                    false,
                    false,
                    if (standalone_chunk_contents) |value| {
    value
} else {
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
            ...
            const source_map_index: ?u32 = if (sourcemap_output_file != null)
                try output_files.insertForSourcemapOrBytecode(if (sourcemap_output_file) |value| {
    value
} else {
    return error.NullPointer;
})
    ...
    return error.NullPointer;
            ...
            const module_info_index: ?u32 = if (module_info_output_file != null)
                try output_files.insertForSourcemapOrBytecode(if (module_info_output_file) |value| {
    value
} else {
    return error.NullPointer;
})
...
// safe-transpile: optional unwrap requires manual review
    ...
    if (is_standalone) {
        ...
        for (0..result.items.len) |__zust_i| {
    var item = &result.items[__zust_i];
            if (item.loader == .html) {
