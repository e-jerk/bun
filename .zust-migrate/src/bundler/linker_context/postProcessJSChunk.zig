...
pub fn postProcessJSChunk(ctx: GenerateChunkCtx, worker: *ThreadPool.Worker, chunk: *Chunk, chunk_index: usize) !void {
    ...
    var cross_chunk_prefix: js_printer.PrintResult = std.mem.zeroes(js_printer.PrintResult);
    var cross_chunk_suffix: js_printer.PrintResult = std.mem.zeroes(js_printer.PrintResult);
...
// safe-transpile: optional unwrap requires manual review
    ...
    const hashbang, const banner = if (chunk.isEntryPoint()) brk: {
        ...
        if (c.options.banner.len > 0 and strings.hasPrefixComptime(c.options.banner, "#!")) {
            ...
            break :brk .{ banner_hashbang, @import("std-fs-compat").trimLeft(u8, c.options.banner[newline_pos..], "\r\n") };
        }
...
pub fn generateEntryPointTailJS(
    ...
    for (sorted_and_filtered_export_aliases, 0..) |alias, i| {
                            var resolved_export = if (resolved_exports.get(alias)) |value| {
    value
} else {
                                ...
                                fn_body[0] = Stmt.alloc(
                                    ...
                                    S.Return{
                                        .value = Expr.init(
                                            E.Identifier,
                                            E.Identifier{
                                                .ref = if (export_item.name.ref) |value| {
    value
} else {
    return error.NullPointer;
},
