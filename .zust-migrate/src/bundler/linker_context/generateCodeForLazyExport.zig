pub fn generateCodeForLazyExport(this: *LinkerContext, source_index: Index.Int) bun.OOM!void {
    ...
    if (maybe_css_ast) |css_ast| {
        ...
        if (css_ast.local_scope.count() > 0) out: {
            ...
            var composes_visited = bun.handleOom(std.array_hash_map.Auto(bun.bundle_v2.Ref, void).init(this.allocator()));
...
// safe-transpile: optional unwrap requires manual review
                ...
                fn visitComposes(visitor: *@This(), ast: *bun.css.BundlerStyleSheet, css_ref: bun.css.CssRef, idx: Index.Int) void {
                    ...
                    if (ast.composes.count() > 0) {
                        ...
                        for (0..composes.composes.slice().len) |__zust_i| {
    var compose = &composes.composes.slice()[__zust_i];
                            // it is imported
