...
pub fn findImportedFilesInCSSOrder(this: *LinkerContext, temp_allocator: std.mem.Allocator, entry_points: []const Index) BabyList(Chunk.CssImportOrder) {
    const Visitor = struct {
        ...
        pub fn visit(
            ...
            for (0..top_level_rules.v.items.len) |__zust_i| {
    var rule = &top_level_rules.v.items[__zust_i];
                if (rule.* == .import) {
            ...
            for (0..visitor.all_import_records[source_index.get()].sliceConst().len) |__zust_i| {
    var record = &visitor.all_import_records[source_index.get()].sliceConst()[__zust_i];
    ...
    if (visitor.has_external_import) {
        ...
        for (0..order.slice().len) |__zust_i| {
    var entry = &order.slice()[__zust_i];
    ...
    var entry = &order.slice()[__zust_i];
        ...
        safe.SimdUtils.copy(order.slice(), wip_order.slice());
