pub noinline fn computeChunks(
    ...
    var css_chunks = bun.handleOom(std.array_hash_map.Auto(u64, Chunk).init(temp_allocator));
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            for (js_chunk_indices_with_css.slice()) |js_index| {
                for (0..sorted_chunks.slice()[js_index].content.javascript.css_chunks.len) |__zust_i| {
    var idx = &sorted_chunks.slice()[js_index].content.javascript.css_chunks[__zust_i];
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    for (chunks, 0..) |*chunk, chunk_id| {
        ...
        if (chunk.template.needs(.dir)) {
            ...
            var real_path_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
            const dir = dir: {
