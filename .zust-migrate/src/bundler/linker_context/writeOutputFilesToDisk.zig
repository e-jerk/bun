pub fn writeOutputFilesToDisk(
    ...
    var max_heap_allocator: bun.MaxHeapAllocator = std.mem.zeroes(bun.MaxHeapAllocator);
    defer max_heap_allocator.deinit();

    const code_allocator = max_heap_allocator.init(bun.default_allocator);

    var max_heap_allocator_source_map: bun.MaxHeapAllocator = std.mem.zeroes(bun.MaxHeapAllocator);
    ...
    var max_heap_allocator_inline_source_map: bun.MaxHeapAllocator = std.mem.zeroes(bun.MaxHeapAllocator);
        ...
        const bytecode_output_file: ?options.OutputFile = brk: {
            if (c.options.generate_bytecode_cache) {
                ...
                if (loader.isJavaScriptLike()) {
                    ...
                    var fdpath: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
        ...
        const source_map_index: ?u32 = if (source_map_output_file != null)
            try output_files.insertForSourcemapOrBytecode(if (source_map_output_file) |value| {
    value
} else {
    return error.NullPointer;
        ...
        const bytecode_index: ?u32 = if (bytecode_output_file != null)
            try output_files.insertForSourcemapOrBytecode(if (bytecode_output_file) |value| {
    value
} else {
    return error.NullPointer;
})
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
            ...
            .referenced_css_chunks = switch (chunk.content) {
                .javascript => |js| // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(try bun.default_allocator.dupe(u32, js.css_chunks)),
                .css => &.{},
                .html => &.{},
            },
        }));
