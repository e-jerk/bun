...
pub fn BundleThread(CompletionStruct: type) type {
    return struct {
        ...
        ready_event: @import("std-fs-compat").ResetEvent,
        ...
        pub const singleton = struct {
            ...
            fn loadOnceImpl() void {
                const bundle_thread = bun.handleOom(zust.Box(Self).init(bun.default_allocator, undefined)).ptr;
                bundle_thread[0] = uninitialized;
    ...
    return error.NullPointer;
        ...
        fn threadMain(instance: *Self) void {
            ...
            var timer: bun.windows.libuv.Timer = std.mem.zeroes(bun.windows.libuv.Timer);
    ...
    while (true) {
        ...
        fn generateInNewThread(completion: *CompletionStruct, generation: bun.Generation) !void {
            ...
            var ast_memory_allocator = (try zust.Box(js_ast.ASTMemoryAllocator).init(allocator, undefined)).ptr;
            ast_memory_allocator[0] = .{ .allocator = allocator };
            ast_memory_allocator.reset();
            ast_memory_allocator.push();

            const transpiler = (try zust.Box(bun.Transpiler).init(allocator, undefined)).ptr;
