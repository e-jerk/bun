...
// safe-transpile: function uses raw slice parameter — consider safe.String
        fn memcpyEscapingClosingTags(dest: []u8, content: []const u8, close_tag: []const u8) usize {
            ...
            var dst: usize = 0;
            while (strings.indexOf(remaining, "</")) |idx| {
                safe.SimdUtils.copy(dest[dst..][0..idx], remaining[0..idx]);
                dst += idx;
                remaining = remaining[idx + 2 ..];
            ...
            safe.SimdUtils.copy(dest[dst..][0..remaining.len], remaining);
            dst += remaining.len;
            return dst;
        }
...
// safe-transpile: optional unwrap requires manual review
                    ...
                    for (pieces.slice()) |piece| {
                        ...
                        if (data.len > 0)
                            safe.SimdUtils.copy(remain[0..data.len], data);
                                ...
                                if (standalone_chunk_contents) |scc| {
                                    ...
                                    if (inline_content) |content| {
                                        ...
                                        if (piece.query.kind == .chunk) {
                                            ...
                                            safe.SimdUtils.copy(remain[0..content.len], content);
...
// safe-transpile: optional unwrap requires manual review
                                    ...
                                    .html_import => {
                                        var fixed_buffer_stream = @import("std-io-compat").fixedBufferStream(remain);
                                ...
                                if (cheap_normalizer[0].len > 0) {
                                    safe.SimdUtils.copy(remain[0..cheap_normalizer[0].len], cheap_normalizer[0]);
                                ...
                                if (cheap_normalizer[1].len > 0) {
                                    safe.SimdUtils.copy(remain[0..cheap_normalizer[1].len], cheap_normalizer[1]);
