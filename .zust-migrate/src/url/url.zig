...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        while (url.pathname.len > 1 and @as(u16, // safe-transpile: @bitCast requires manual review
    @bitCast(url.pathname[0..2].*)) == comptime std.mem.readInt(u16, "//", .little)) {
...
pub const QueryStringMap = struct {
    ...
    threadlocal var _name_count: [8]string = .{};
