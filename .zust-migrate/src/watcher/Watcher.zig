...
pub fn init(comptime T: type, ctx: *T, fs: *bun.fs.FileSystem, allocator: std.mem.Allocator) !*Watcher {
    const wrapped = struct {
        fn onFileUpdateWrapped(ctx_opaque: *anyopaque, events: []WatchEvent, changed_files: []?[:0]u8, watchlist: WatchList) void {
            T.onFileUpdate(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(ctx_opaque)), events, changed_files, watchlist);
        }
        fn onErrorWrapped(ctx_opaque: *anyopaque, err: bun.sys.Error) void {
            if (@hasDecl(T, "onWatchError")) {
                T.onWatchError(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(ctx_opaque)), err);
            } else {
                T.onError(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(ctx_opaque)), err);
            }
        }
    };
    ...
    pub const Op = packed struct(u8) {
        ...
        pub fn format(op: Op, w: *std.Io.Writer) !void {
            ...
            inline for (comptime std.meta.fieldNames(Op)) |name| {
                if (comptime safe.SimdUtils.eql(name, "_padding")) continue;
                if (@field(op, name)) {
                    if (!first) {
                        try w.writeAll(",");
                    }
...
// safe-transpile: optional unwrap requires manual review
        ...
        const path: [:0]const u8 = if (clone_file_path and file_path_.len > 0 and file_path_[file_path_.len - 1] == 0)
            ...
            safe.SimdUtils.copy(buf[0..trailing_slash.len], trailing_slash);
