...
pub fn writePreQuotedString(text_in: []const u8, comptime Writer: type, writer: Writer, comptime quote_char: u8, comptime ascii_only: bool, comptime json: bool, comptime encoding: strings.Encoding) !void {
    const text = if (comptime encoding == .utf16) @as([]const u16, // safe-transpile: @alignCast requires manual review
    @alignCast(std.mem.bytesAsSlice(u16, text_in))) else text_in;
    ...
    pub fn For(comptime Type: type, comptime handler: (fn (t: *Type, chunk: SourceMap.Chunk, source: *const logger.Source) anyerror!void)) type {
        return struct {
            pub fn onChunk(self: *anyopaque, chunk: SourceMap.Chunk, source: *const logger.Source) anyerror!void {
                try handler(@as(*Type, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(self))), chunk, source);
            }
...
pub const RequireOrImportMeta = struct {
    ...
    pub const Callback = struct {
        ...
        pub fn call(self: Callback, id: u32, was_unwrapped_require: bool) RequireOrImportMeta {
            return self.callback(if (self.ctx) |value| {
    value
} else {
    return error.NullPointer;
}, id, was_unwrapped_require);
        }
        ...
        pub fn init(
            ...
            return Callback{
                ...
                .callback = @as(*const Fn, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&callback)),
            };
        }
    };
        ...
        pub const BinaryExpressionVisitor = struct {
            ...
            pub fn checkAndPrepare(v: *BinaryExpressionVisitor, p: *Printer) bool {
                ...
                if (e.op.isRightAssociative()) {
                    left_level[0] = e_level;
                }

                if (e.op.isLeftAssociative()) {
                    right_level[0] = e_level;
                }
...
// safe-transpile: optional unwrap requires manual review
        ...
        fn printInternalBunImport(p: *Printer, import: S.Import, comptime Statement: type, statement: Statement) void {
            ...
            if (import.default_name) |default| {
                ...
                p.print("var ");
                p.printSymbol(if (default.ref) |value| {
    value
} else {
    return error.NullPointer;
});
    ...
    for (import.items, 0..) |item, i| {
                ...
                if (import.star_name_loc == null and import.default_name == null) {
                    ...
                    p.printSymbol(if (name.ref) |value| {
    value
} else {
    return error.NullPointer;
});
            ...
            if (may_have_module_info) {
                if (p.moduleInfo()) |mi| {
                    ...
                    if (import.default_name) |default| {
                        const name = p.renamer.nameForSymbol(if (default.ref) |value| {
    value
} else {
    return error.NullPointer;
});
                    ...
                    for (import.items) |item| {
                        const name = p.renamer.nameForSymbol(if (item.name.ref) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: optional unwrap requires manual review
            if (close_brace_loc != null and close_brace_loc.?.start > loc.start) {
                p.addSourceMapping(if (close_brace_loc) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: optional unwrap requires manual review
            ...
            for (0..decls[1..].len) |__zust_i| {
    var decl = &decls[1..][__zust_i];
                p.print(",");
                p.printSpace();
...
// safe-transpile: optional unwrap requires manual review
                        if (value.loc_ref.ref.?.eql(id.ref)) {
                            if (p.options.commonjs_named_exports_deoptimized or value.needs_decl) {
                                ...
                                p.printSymbol(if (value.loc_ref.ref) |value| {
    value
} else {
    return error.NullPointer;
});
    ...
    var part = &e.parts[__zust_i];
                ...
                .e_import_identifier => |e| {
                    ...
                    const ref = if (p.options.module_type != .internal_bake_dev)
                        ...
                        e.ref;
                    const symbol = if (p.symbols().get(ref)) |value| {
    value
} else {
    return error.NullPointer;
};
    ...
    return error.NullPointer;
                    ...
                    while (p.binary_expression_stack.items.len > stack_bottom) {
                        var last = if (p.binary_expression_stack.pop()) |value| {
    value
} else {
    return error.NullPointer;
};
            ...
            if (comptime !is_json) {
                if (item.kind == .spread) {
                    if (comptime is_json and Environment.allow_assert)
                        unreachable;
                    p.print("...");
                    p.printExpr(if (item.value) |value| {
    value
} else {
    return error.NullPointer;
}, .comma, ExprFlag.None());
        ...
        pub fn printStmt(p: *Printer, stmt: Stmt, tlmtlo: TopLevel) !void {
            ...
            switch (stmt.data) {
                ...
                .s_class => |s| {
                    ...
                    p.addSourceMapping(stmt.loc);
// safe-transpile: optional unwrap requires manual review
                    const nameRef = if (s.class.class_name.?.ref) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
                        ...
                        .stmt => |s2| {
                            switch (s2.data) {
                                .s_function => |func| {
                                    ...
                                    const func_name: ?[]const u8 = if (func.func.name) |name| p.renamer.nameForSymbol(if (name.ref) |value| {
    value
} else {
    return error.NullPointer;
}) else null;
                                ...
                                .s_class => |class| {
                                    ...
                                    if (class.class.class_name) |name| {
                                        p.print("class ");
                                        p.printIdentifier(p.renamer.nameForSymbol(if (name.ref) |value| {
    value
} else {
    return error.NullPointer;
}));
    ...
    for (s.items, 0..) |item, i| {
                                    const symbol = if (p.symbols().getWithLink(item.name.ref.?)) |value| {
    value
} else {
    return error.NullPointer;
};
                    ...
                    var array = std.ArrayListUnmanaged(js_ast.ClauseItem){ .items = s.items, .capacity = s.items.len };
                    {
                        ...
                        while (i < array.items.len) {
                            ...
                            if (item.original_name.len > 0) {
                                if (p.symbols().get(if (item.name.ref) |value| {
    value
} else {
    return error.NullPointer;
})) |symbol| {
                                    if (symbol.namespace_alias) |namespace| {
                                        ...
                                        if (namespace.was_originally_property_access) {
                                            p.print("var ");
                                            p.printSymbol(if (item.name.ref) |value| {
    value
} else {
    return error.NullPointer;
});
                    ...
                    if (record.path.is_disabled) {
                        ...
                        if (s.items.len > 0 or s.default_name != null) {
                            ...
                            if (s.default_name) |default_name| {
                                ...
                                p.print("default:");
                                p.printSpace();
                                p.printSymbol(if (default_name.ref) |value| {
    value
} else {
    return error.NullPointer;
});
    ...
    for (s.items, 0..) |item, i| {
                    ...
                    if (may_have_module_info) {
                        if (p.moduleInfo()) |mi| {
                            ...
                            if (s.default_name) |name| {
                                const local_name = p.renamer.nameForSymbol(if (name.ref) |value| {
    value
} else {
    return error.NullPointer;
});
                            ...
                            for (s.items) |item| {
                                const local_name = p.renamer.nameForSymbol(if (item.name.ref) |value| {
    value
} else {
    return error.NullPointer;
});
                ...
                .s_break => |s| {
                    ...
                    if (s.label) |label| {
                        p.print(" ");
                        p.printSymbol(if (label.ref) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: function returns small constant slice — consider safe.String
    ...
    @ptrCast(&ctx.buffer.list.items.ptr[ctx.buffer.list.items.len]));
    }
...
pub fn printAst(
    ...
    var renamer: rename.Renamer = std.mem.zeroes(rename.Renamer);
    var no_op_renamer: rename.NoOpRenamer = std.mem.zeroes(rename.NoOpRenamer);
...
// safe-transpile: optional unwrap requires manual review
    ...
    if (comptime generate_source_map) {
        if (opts.source_map_handler) |handler| {
            try handler.onSourceMapChunk(if (source_maps_chunk) |value| {
    value
} else {
    return error.NullPointer;
}, source);
