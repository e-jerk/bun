...
pub fn initClient() *boring.SSL {
    if (ctx_store != null) _ = boring.SSL_CTX_up_ref(if (ctx_store) |value| {
    value
} else {
    ...
    const ctx = ctx_store orelse brk: {
        ctx_store = if (boring.SSL_CTX.init()) |value| {
    value
} else {
    return error.NullPointer;
};
        break :brk if (ctx_store) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub fn canonicalizeIP(addr_str: []const u8, outIP: *[INET6_ADDRSTRLEN + 1]u8) ?[]const u8 {
    ...
    var ip_std_text: [INET6_ADDRSTRLEN + 1]u8 = .{};
    ...
    if (index >= 0) {
        ...
        if (boring.X509_get_ext(x509, index)) |ext| {
            ...
            if (host_is_ip) {
                ...
                var canonicalIPBuf: [INET6_ADDRSTRLEN + 1]u8 = .{};
                var certIPBuf: [INET6_ADDRSTRLEN + 1]u8 = .{};
