...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    pub fn deinit(self: *YarnLock) void {
        for (0..self.entries.items.len) |__zust_i| {
    var entry = &self.entries.items[__zust_i];
                ...
                if (current_dep_type) |dep_type| {
                    if (strings.indexOf(trimmed, " ")) |space_idx| {
                        ...
                        const map = switch (dep_type) {
                            .production => &if (current_deps) |value| {
    value
} else {
    return error.NullPointer;
},
                            .optional => &if (current_optional_deps) |value| {
    value
} else {
    return error.NullPointer;
},
                            .peer => &if (current_peer_deps) |value| {
    value
} else {
    return error.NullPointer;
},
                            .development => &if (current_dev_deps) |value| {
    value
} else {
    return error.NullPointer;
},
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    fn consolidateAndAppendEntry(self: *YarnLock, new_entry: Entry) !void {
        ...
        for (0..self.entries.items.len) |__zust_i| {
    var existing_entry = &self.entries.items[__zust_i];
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    for (yarn_lock.entries.items, 0..) |entry, yarn_idx| {
        ...
        const name = if (is_npm_alias and entry.resolved != null)
            YarnLock.Entry.getPackageNameFromResolvedUrl(if (entry.resolved) |value| {
    value
} else {
    return error.NullPointer;
}) orelse YarnLock.Entry.getNameFromSpec(entry.specs[0])
    ...
    return error.NullPointer;
        ...
        const name_to_use = blk: {
            if (entry.commit != null and entry.git_repo_name != null) {
                break :blk if (entry.git_repo_name) |value| {
    value
} else {
    return error.NullPointer;
};
            ...
            .resolution = blk: {
                if (entry.workspace) {
                    ...
                    if (entry.resolved) |resolved| {
                        var owner_str: safe.Slice(u8) = "";
                        var repo_str: safe.Slice(u8) = resolved;
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    defer {
        for (0..package_dependents.len) |__zust_i| {
    var list = &package_dependents[__zust_i];
            list.deinit();
        }
        // safe-transpile: free removed (memory owned by safe type);
    }
