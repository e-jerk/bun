...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn fromUrl(
        ...
        const project_part: safe.Slice(u8) = blk: {
...
// safe-transpile: function returns small constant slice — consider safe.String
    pub fn toStringWithColon(self: Self, buf: *StringWithColonBuffer) []const u8 {
        ...
        const protocol_str = if (strings.getKey(self)) |value| {
    value
} else {
    return error.NullPointer;
        ...
        safe.SimdUtils.copy(buf[0..protocol_str.len], protocol_str);
...
// safe-transpile: function returns small constant slice — consider safe.String
    ...
    fn toUrl(self: *const UrlProtocolPair, allocator: std.mem.Allocator) ?*jsc.URL {
        ...
        var protocol_buf: WellDefinedProtocol.StringWithColonBuffer = std.mem.zeroes(WellDefinedProtocol.StringWithColonBuffer);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
                fn default(
                    ...
                    requiresUser(user);
                    const cmsh: safe.Slice(u8) = if (committish) |c| c else "";
                    const cmsh_sep = if (cmsh.len > 0) "#" else "";

                    return std.fmt.allocPrint(
                        alloc,
                        "git@{s}:{s}/{s}.git{s}{s}",
                        .{ self.domain(), if (user) |value| {
    value
} else {
    return error.NullPointer;
}, project, cmsh_sep, cmsh },
                    );
                }
...
// safe-transpile: function uses raw slice parameter — consider safe.String
                fn default(
                    ...
                    return std.fmt.allocPrint(
                        ...
                        .{ auth_str, auth_sep, self.domain(), if (user) |value| {
    value
} else {
    return error.NullPointer;
}, project, cmsh_sep, cmsh },
            ...
            const extract = struct {
                const Result = struct {
                    ...
                    fn move(self: *Result) struct {
                        ...
                        const buffer = if (self._owned_buffer) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
                    ...
                    const project = bun.strings.trimSuffixComptime(if (project_part) |value| {
    value
} else {
    return error.NullPointer;
}, ".git");
                ...
                fn sourcehut(
                    ...
                    if (aux) |a| {
                        if (safe.SimdUtils.eql(a, "archive")) {
                            return null;
                        }
                    }
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    fn fromDomain(domain_str: []const u8) ?HostProvider {
        inline for (std.meta.fields(Self)) |field| {
            ...
            if (safe.SimdUtils.eql(provider.domain(), domain_str)) {
                return provider;
            }
        }
    ...
    fn fromUrlDomain(url: *jsc.URL) ?HostProvider {
        ...
        var fba_mem: [max_hostname_len]u8 = .{};
