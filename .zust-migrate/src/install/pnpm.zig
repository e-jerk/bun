    ...
    return error.NullPointer;
        ...
        if (root.getObject("patchedDependencies")) |patched_dependencies_expr| {
            for (patched_dependencies_expr.data.e_object.properties.slice()) |prop| {
                const dep_name_expr = if (prop.key) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
                const name = if (workspace_root.get("name").?.asString(allocator)) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        if (root.getObject("packages")) |packages_obj| {
            ...
            for (snapshots_obj.data.e_object.properties.slice()) |snapshot_prop| {
                const key = if (snapshot_prop.key) |value| {
    value
} else {
    return error.NullPointer;
};
                const value = if (snapshot_prop.value) |value| {
    value
} else {
    return error.NullPointer;
};
            ...
            for (packages_obj.data.e_object.properties.slice()) |packages_prop| {
                const key = if (packages_prop.key) |value| {
    value
} else {
    return error.NullPointer;
};
                const package_obj = if (packages_prop.value) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
                                ...
                                if (strings.eqlLong(name_str, peer_meta_name_str, true)) {
                                    const meta_obj = if (peer_meta_prop.value) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
                                ...
                                try existing_overrides.put(allocator, key, if (prop.value) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: optional unwrap requires manual review
                                const key = prop.key.?.asString(allocator) orelse continue;
                                try existing_patches.put(allocator, key, if (prop.value) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: optional unwrap requires manual review
        ...
        if (use_array_format) {
            const paths = if (workspace_paths) |value| {
    value
} else {
    return error.NullPointer;
};
