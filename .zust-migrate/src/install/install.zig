...
pub fn buntaghashbuf_make(buf: *BuntagHashBuf, patch_hash: u64) [:0]u8 {
    safe.SimdUtils.copy(buf[0..bun_hash_tag.len], bun_hash_tag);
...
pub fn initializeMiniStore() void {
    ...
    if (MiniStore.instance == null) {
        var mini_store = bun.handleOom(zust.Box(MiniStore).init(bun.default_allocator, undefined)).ptr;
        mini_store[0] = .{
        ...
        var mini_store = if (MiniStore.instance) |value| {
    value
} else {
