...
fn extract(this: *const ExtractTarball, log: *logger.Log, tgz_bytes: []const u8) !Install.ExtractData {
    ...
    var tmpname_buf: if (Environment.isWindows) bun.WPathBuffer else bun.PathBuffer = std.mem.zeroes(if (Environment.isWindows) bun.WPathBuffer else bun.PathBuffer);
    ...
    const tmpname = try FileSystem.tmpname(basename[0..@min(basename.len, 32)], std.mem.asBytes(&tmpname_buf), bun.fastRandom());
    {
...

        {
            ...
            if (tgz_bytes.len > 16) {
                ...
                const last_4_bytes: u32 = // safe-transpile: @bitCast requires manual review
    @bitCast(tgz_bytes[tgz_bytes.len - 4 ..][0..4].*);
...
pub fn moveToCacheDirectory(
    ...
    if (comptime Environment.isWindows) {
        var did_retry = false;
        var path2_buf: bun.WPathBuffer = std.mem.zeroes(bun.WPathBuffer);
                    ...
                    if (!did_retry) {
                        switch (err.getErrno()) {
                            .NOTEMPTY, .PERM, .BUSY, .EXIST => {
                                ...
                                var tempdest_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
                                safe.SimdUtils.copy(tempdest_buf[0..tmpname.len], tmpname);
    ...
    var json_path: safe.Slice(u8) = "";
    var json_buf: safe.Slice(u8) = "";
