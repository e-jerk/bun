...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    for (old_workspace_dependencies, old_workspace_resolutions) |dependency, *resolution| {
            if (!dependency.behavior.isEnabled(features) and resolution.* < end) {
                resolution[0] = invalid_package_id;
                any_changes = true;
            }
        }
    }
...
fn preprocessUpdateRequests(old: *Lockfile, manager: *PackageManager, updates: []PackageManager.UpdateRequest, exact_versions: bool) !void {
    ...
    for (root_deps, old_resolutions) |dep, old_resolution| {
...

        {
            ...
            for (0..updates.len) |__zust_i| {
    var update = &updates[__zust_i];
                if (update.package_id == invalid_package_id) {
                    for (root_deps, old_resolutions) |*dep, old_resolution| {
                        if (dep.name_hash == String.Builder.stringHash(update.name)) {
                            if (old_resolution >= old.packages.len) continue;
...
pub fn clean(
    ...
    defer {
        for (0..log.msgs.items.len) |__zust_i| {
    var item = &log.msgs.items[__zust_i];
...
pub fn cleanWithLogger(
    ...
    var timer: @import("std-fs-compat").Timer = std.mem.zeroes(@import("std-fs-compat").Timer);
    ...
    var new: *Lockfile = try safe.Box(Lockfile).init(allocator, undefined);
    new.initEmpty(
        old.allocator,
    );
    ...
    if (old.workspace_paths.count() > 0 or old.workspace_versions.count() > 0) {
        ...
        for (0..old.workspace_paths.values().len) |__zust_i| {
    var path = &old.workspace_paths.values()[__zust_i];
    ...
    for (old.workspace_paths.values(), new.workspace_paths.values()) |*src, *dest| {
            dest[0] = workspace_paths_builder.append(String, old.str(src));
        }
        @memcpy(
    ...
    for (versions, new.workspace_versions.values()) |src, *dest| {
            dest[0] = src.append(old.buffers.string_bytes.items, @TypeOf(&workspace_paths_builder), &workspace_paths_builder);
        }
...
pub const MetaHashFormatter = struct {
    ...
    pub fn format(this: MetaHashFormatter, writer: *std.Io.Writer) !void {
        var remain: safe.Slice(u8) = this.meta_hash[0..];
    ...
    for (pkg_names, pkg_name_hashes, pkg_resolutions, pkg_bins, pkg_metas) |pkg_name, pkg_name_hash, pkg_res, *pkg_bin, *pkg_meta| {
            switch (pkg_res.tag) {
                .npm => {
                    ...
                    pkg_bin[0] = pkg.package.bin.clone(manifest.string_buf, manifest.extern_strings_bin_entries, extern_strings_list.items, extern_strings, @TypeOf(&builder), &builder);
...
pub const Printer = struct {
    ...
    pub fn print(
        ...
        var lockfile_path_buf1: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
        var lockfile_path_buf2: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
        ...
        var lockfile = try safe.Box(Lockfile).init(allocator, undefined);
    ...
    pub fn printWithLockfile(
        ...
        var env_loader: *DotEnv.Loader = brk: {
            ...
            map[0] = DotEnv.Map.init(allocator);

            const loader = try safe.Box(DotEnv.Loader).init(allocator, undefined);
            loader[0] = DotEnv.Loader.init(map, allocator);
            loader.quiet = true;
            break :brk loader;
        };
...
pub fn saveToDisk(this: *Lockfile, load_result: *const LoadResult, options: *const PackageManager.Options) void {
    ...
    const bytes = bytes: {
        ...
        if (bytes.items.len >= end_pos)
            bytes.items[end_pos..][0..@sizeOf(usize)].* = // safe-transpile: @bitCast requires manual review
    @bitCast(total_size);
...
// safe-transpile: function returns small constant slice — consider safe.String
    ...
    pub fn appendWithoutPool(this: *StringBuilder, comptime Type: type, slice: string, hash: u64) Type {
        ...
        bun.copy(u8, if (this.ptr) |value| {
    value
} else {
    return error.NullPointer;
}[this.len..this.cap], slice);
        const final_slice = if (this.ptr) |value| {
    value
} else {
    return error.NullPointer;
}[this.len..this.cap][0..slice.len];
        this.len += slice.len;
...
pub fn eql(l: *const Lockfile, r: *const Lockfile, cut_off_pkg_id: usize, allocator: std.mem.Allocator) OOM!bool {
    ...
    var depth_buf: Tree.DepthBuf = std.mem.zeroes(Tree.DepthBuf);
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    if (string_builder.ptr) |value| {
    value
} else {
    return error.NullPointer;
}[0..hash_prefix.len].* = hash_prefix.*;
    ...
    return error.NullPointer;
}[string_builder.len..string_builder.cap][0..hash_suffix.len].* = hash_suffix.*;
    string_builder.len += hash_suffix.len;

    const alphabetized_name_version_string = if (string_builder.ptr) |value| {
    value
} else {
    return error.NullPointer;
...
pub const default_trusted_dependencies_list: []const []const u8 = brk: {
    ...
    var buf: [max_default_trusted_dependencies][]const u8 = .{};
    var i: usize = 0;
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    var names: [i][]const u8 = .{};
    safe.SimdUtils.copy(names[0..i], buf[0..i]);
    const final = names;
    break :brk &final;
};
