...
// safe-transpile: optional unwrap requires manual review
            builder.count(if (entry.key.?.asString(lockfile.allocator)) |value| {
    value
} else {
...
pub fn parseFromOverrides(
    ...
    return error.NullPointer;
        ...
        const k = if (key.asString(lockfile.allocator)) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        const value = value: {
            ...
            const value_expr = if (prop.value) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub fn parseFromResolutions(
    ...
    return error.NullPointer;
        ...
        var k = if (key.asString(lockfile.allocator)) |value| {
    value
} else {
    return error.NullPointer;
};
