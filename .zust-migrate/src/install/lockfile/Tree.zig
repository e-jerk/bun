...
pub fn toExternal(this: Tree) External {
    var out = External{};
    out[0..4].* = @as(Id, // safe-transpile: @bitCast requires manual review
    @bitCast(this.id));
    out[4..8].* = @as(Id, // safe-transpile: @bitCast requires manual review
    @bitCast(this.dependency_id));
    out[8..12].* = @as(Id, // safe-transpile: @bitCast requires manual review
    @bitCast(this.parent));
    out[12..16].* = @as(u32, // safe-transpile: @bitCast requires manual review
    @bitCast(this.dependencies.off));
    out[16..20].* = @as(u32, // safe-transpile: @bitCast requires manual review
    @bitCast(this.dependencies.len));
...
pub fn toTree(out: External) Tree {
    return .{
        .id = // safe-transpile: @bitCast requires manual review
    @bitCast(out[0..4].*),
        .dependency_id = // safe-transpile: @bitCast requires manual review
    @bitCast(out[4..8].*),
        .parent = // safe-transpile: @bitCast requires manual review
    @bitCast(out[8..12].*),
        .dependencies = .{
            .off = // safe-transpile: @bitCast requires manual review
    @bitCast(out[12..16].*),
            .len = // safe-transpile: @bitCast requires manual review
    @bitCast(out[16..20].*),
        },
    };
}
    ...
    return struct {
        ...
        pub fn init(lockfile: *const Lockfile) @This() {
            ...
            if (comptime path_style == .node_modules) {
                safe.SimdUtils.copy(iter.path_buf[0.."node_modules".len], "node_modules");
            }
            return iter;
        }
...
pub fn relativePathAndDepth(
    ...
    if (tree.id > 0) {
        ...
        while (depth_buf_len > 0) : (depth_buf_len -= 1) {
            ...
            safe.SimdUtils.copy(path_buf[path_written..][0..name.len], name);
            path_written += name.len;

            if (comptime path_style == .node_modules) {
                safe.SimdUtils.copy(path_buf[path_written..][0.."/node_modules".len], std.fs.path.sep_str ++ "node_modules");
...
// safe-transpile: function returns small constant slice — consider safe.String
        ...
        pub fn clean(this: *@This()) OOM!CleanResult {
            ...
            for (0..trees.len) |__zust_i| {
    var tree = &trees[__zust_i];
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            if (@intFromPtr(trees.ptr) != @intFromPtr(list_ptr)) {
                var new: [*]Tree = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(list_ptr);
    ...
    for (builder.sort_buf.items) |dep_id| {
        ...
        switch (hoisted) {
            ...
            .resolve_replace => |replace| {
                ...
                for (0..dependency_lists[replace.id].items.len) |__zust_i| {
    var placed_dep_id = &dependency_lists[replace.id].items[__zust_i];
