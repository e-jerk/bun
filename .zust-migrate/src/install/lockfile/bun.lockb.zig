...
const has_patched_dependencies_tag: u64 = // safe-transpile: @bitCast requires manual review
    @bitCast(@as([8]u8, "pAtChEdD".*));
const has_workspace_package_ids_tag: u64 = // safe-transpile: @bitCast requires manual review
    @bitCast(@as([8]u8, "wOrKsPaC".*));
const has_trusted_dependencies_tag: u64 = // safe-transpile: @bitCast requires manual review
    @bitCast(@as([8]u8, "tRuStEDd".*));
const has_empty_trusted_dependencies_tag: u64 = // safe-transpile: @bitCast requires manual review
    @bitCast(@as([8]u8, "eMpTrUsT".*));
const has_overrides_tag: u64 = // safe-transpile: @bitCast requires manual review
    @bitCast(@as([8]u8, "oVeRriDs".*));
const has_catalogs_tag: u64 = // safe-transpile: @bitCast requires manual review
    @bitCast(@as([8]u8, "cAtAlOgS".*));
const has_config_version_tag: u64 = // safe-transpile: @bitCast requires manual review
    @bitCast(@as([8]u8, "cNfGvRsN".*));
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    total_size[0] = try stream.getPos();
...
pub fn load(
    ...
    var header_buf_: [header_bytes.len]u8 = .{};
            ...
            if (next_num == has_workspace_package_ids_tag) {
                {
                    ...
                    safe.SimdUtils.copy(lockfile.workspace_versions.keys(), workspace_package_name_hashes.items);
                    safe.SimdUtils.copy(lockfile.workspace_versions.values(), workspace_versions_list.items);
...

                {
                    ...
                    safe.SimdUtils.copy(lockfile.workspace_paths.keys(), workspace_paths_hashes.items);
                    safe.SimdUtils.copy(lockfile.workspace_paths.values(), workspace_paths_strings.items);
...
// safe-transpile: optional unwrap requires manual review
                safe.SimdUtils.copy(lockfile.trusted_dependencies.?.keys(), trusted_dependencies_hashes.items);
// safe-transpile: optional unwrap requires manual review
