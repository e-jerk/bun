...
pub fn readArray(stream: *Stream, allocator: Allocator, comptime ArrayList: type) !ArrayList {
    ...
    return ArrayList{
        .items = try allocator.dupe(PointerType, @as([*]PointerType, // safe-transpile: @alignCast requires manual review
    @alignCast(misaligned.ptr))[0..misaligned.len]),
...
pub fn load(stream: *Stream, allocator: Allocator, log: *logger.Log, pm_: ?*PackageManager) !Buffers {
    ...
    if (this.trees.items.len > 0 and this.trees.items[0].dependency_id != Tree.root_dep_id) {
        ...
        for (0..this.trees.items.len) |__zust_i| {
    var tree = &this.trees.items[__zust_i];
        ...
        for (0..this.hoisted_dependencies.items.len) |__zust_i| {
    var package_id = &this.hoisted_dependencies.items[__zust_i];
