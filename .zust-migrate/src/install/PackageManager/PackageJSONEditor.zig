...
pub fn editTrustedDependencies(allocator: std.mem.Allocator, package_json: *Expr, names_to_add: []string) !void {
    ...
    const new_trusted_deps: JSAst.ExprNodeList = brk: {
        ...
        safe.SimdUtils.copy(deps[0..trusted_dependencies.len], trusted_dependencies);
    ...
    if (package_json.data != .e_object or package_json.data.e_object.properties.len == 0) {
        ...
        safe.SimdUtils.copy(root_properties[0..package_json.data.e_object.properties.len], package_json.data.e_object.properties.slice());
        ...
        if (current_package_json.asProperty(group_str)) |root| {
            if (root.expr.data == .e_object) {
                if (options.before_install) {
                    ...
                    for (0..root.expr.data.e_object.properties.slice().len) |__zust_i| {
    var dep = &root.expr.data.e_object.properties.slice()[__zust_i];
...
pub fn edit(
    ...
    var request = &updates.*[__zust_i];
        ...
        if (current_package_json.data != .e_object or current_package_json.data.e_object.properties.len == 0) {
            ...
            current_package_json[0] = JSAst.Expr.allocate(allocator, JSAst.E.Object, .{
                .properties = JSAst.G.Property.List.fromOwnedSlice(root_properties),
            }, logger.Loc.Empty);
        } else {
            if (needs_new_dependency_list and needs_new_trusted_dependencies_list) {
                ...
                safe.SimdUtils.copy(root_properties[0..current_package_json.data.e_object.properties.len], current_package_json.data.e_object.properties.slice());
                ...
                current_package_json[0] = Expr.allocate(allocator, E.Object, .{
