    ...
    switch (manager.getPreinstallState(pkg.meta.id)) {
        .unknown => {
            ...
            const patch_hash: ?u64 = brk: {
                ...
                defer out_name_and_version_hash[0] = name_and_version_hash;
                ...
                out_patchfile_hash[0] = if (patched_dep.patchfileHash()) |value| {
    value
} else {
    return error.NullPointer;
};
                break :brk if (patched_dep.patchfileHash()) |value| {
    value
} else {
    return error.NullPointer;
};
