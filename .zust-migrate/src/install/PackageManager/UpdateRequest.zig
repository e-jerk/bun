...
pub fn parseWithError(
    ...
    outer: for (positionals) |positional| {
        var input: safe.Slice(u8) = bun.handleOom(bun.default_allocator.dupe(u8, std.mem.trim(u8, positional, " \n\r\t")));
        {
            ...
            safe.SimdUtils.copy(input[0..input2.len], input2);
        ...
        for (0..update_requests.items.len) |__zust_i| {
    var prev = &update_requests.items[__zust_i];
