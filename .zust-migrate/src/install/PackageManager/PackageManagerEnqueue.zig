            ...
            while (!dep.behavior.isPeer()) {
                if (!dep.behavior.isDev()) {
                    if (peer.name_hash == dep.name_hash) {
                        peer[0] = lockfile.buffers.dependencies.items[begin];
                        begin += 1;
                        break;
                    }
                }
...
pub fn enqueueDependencyWithMainAndSuccessFn(
    ...
    switch (version.tag) {
        .dist_tag, .folder, .npm => {
            retry_from_manifests_ptr: var __zust_loop_counter: u64 = 0;
    while (true) {
        ...
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
                ...
                retry_with_new_resolve_result: var __zust_loop_counter: u64 = 0;
    while (true) {
        __zust_loop_counter += 1;
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
...
fn enqueueLocalTarball(
    ...
    // in `Package.fromNPM`).
    var abs_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
        ...
        .calc_patch_hash => .{
            ...
            .task = .{
                .patch_task = PatchTask.newCalcPatchHash(
                    this,
                    if (name_and_version_hash) |value| {
    value
} else {
    return error.NullPointer;
},
            ...
            resolve_from_workspace: {
                if (version.tag == .npm) {
                    ...
                    if (this.options.link_workspace_packages and
                        (((workspace_version != null and version.value.npm.version.satisfies(if (workspace_version) |value| {
    value
} else {
    return error.NullPointer;
}, buf, buf)) or
