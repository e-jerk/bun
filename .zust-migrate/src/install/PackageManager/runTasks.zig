            ...
            .package_manifest => |*manifest_req| {
                ...
                if (log_level.showProgress()) {
                    if (!has_updated_this_run) {
                        manager.setNodeName(if (manager.downloads_node) |value| {
    value
} else {
    return error.NullPointer;
}, name.slice(), ProgressStrings.download_emoji, true);
                ...
                const metadata = task.response.metadata orelse {
                    ...
                    if (@TypeOf(callbacks.onPackageManifestError) != void) {
                        ...
                        if (manager.subcommand != .remove) {
                            for (0..manager.update_requests.len) |__zust_i| {
    var request = &manager.update_requests[__zust_i];
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
                        ...
                        entry.value_ptr.manifest.pkg.public_max_age = if (timestamp_this_tick) |value| {
    value
} else {
    return error.NullPointer;
};
                        ...
                        const dependency_list_entry = if (manager.task_queue.getEntry(task.task_id)) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
    ...
    return error.NullPointer;
}, extract.name.slice(), ProgressStrings.extract_emoji, true);
                        has_updated_this_run = true;
                    }
                }
    ...
    while (resolve_tasks_iter.next()) |task| {
        ...
        switch (task.tag) {
            .package_manifest => {
                ...
                const dependency_list_entry = if (manager.task_queue.getEntry(task.id)) |value| {
    value
} else {
    return error.NullPointer;
};
    ...
    return error.NullPointer;
}, manifest.name(), ProgressStrings.download_emoji, true);
                        has_updated_this_run = true;
                    }
                }
            },
    ...
    return error.NullPointer;
}, alias, ProgressStrings.extract_emoji, true);
                        has_updated_this_run = true;
                    }
                }
            },
    ...
    return error.NullPointer;
}, name, ProgressStrings.download_emoji, true);
                        has_updated_this_run = true;
                    }
                }
            },
    ...
    return error.NullPointer;
}, alias.slice(), ProgressStrings.download_emoji, true);
                        has_updated_this_run = true;
                    }
                }
            },
...
// safe-transpile: optional unwrap requires manual review
            const patch_hash = if (this.lockfile.patched_dependencies.get(h).?.patchfileHash()) |value| {
    value
} else {
    return error.NullPointer;
};
