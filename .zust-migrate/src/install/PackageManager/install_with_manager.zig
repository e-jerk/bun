...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
                    ...
                    const all_name_hashes: []PackageNameHash = brk: {
                        ...
                        safe.SimdUtils.copy(all_name_hashes[0..manager.lockfile.overrides.map.entries.len], manager.lockfile.overrides.map.keys());
                        safe.SimdUtils.copy(all_name_hashes[manager.lockfile.overrides.map.entries.len..], lockfile.overrides.map.keys());
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    if (manager.options.lockfile_only) {
        ...
        if (manager.options.do.summary) {
            ...
            Output.printStartEndStdout(ctx.start_time, @import("std-fs-compat").nanoTimestamp());
            Output.pretty("\n", .{});
        }
        Output.flush();
        return;
