...
pub fn doPatchCommit(
    ...
    var folder_path_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
    var lockfile: *Lockfile = try safe.Box(Lockfile).init(allocator, undefined);
    ...
    var resolution_buf: [1024]u8 = .{};
    const _cache_dir: @import("std-fs-compat").FsDir = undefined, const _cache_dir_subpath: stringZ = undefined, const _changes_dir: safe.Slice(u8) = undefined, const _pkg: Package = undefined = switch (arg_kind) {
        .path => result: {
            ...
            const actual_package = switch (lockfile.package_index.get(package.name_hash) orelse {
                ...
                .ids => |ids| brk: {
                    for (ids.items) |id| {
                        ...
                        if (safe.SimdUtils.eql(resolution_label, version)) {
                            break :brk pkg;
                        }
                    }
    ...
    const changes_dir: safe.Slice(u8) = _changes_dir;
    ...
    const patchfile_contents = brk: {
        ...
        var buf3: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
        ...
        var cwdbuf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
        ...
        var gitbuf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
    ...
    safe.SimdUtils.copy(resolution_buf[resolution_label.len .. resolution_label.len + ".patch".len], ".patch");
    var patch_filename: safe.Slice(u8) = resolution_buf[0 .. resolution_label.len + ".patch".len];
    var deinit = false;
...
fn escapePatchFilename(allocator: std.mem.Allocator, name: []const u8) ?[]const u8 {
    ...
    for (name) |c| {
        ...
        safe.SimdUtils.copy(buf[i..][0..e.len], e);
        i += e.len;
    }
    return buf;
}
...
pub fn preparePatch(manager: *PackageManager) !void {
    ...
    var win_normalizer: if (bun.Environment.isWindows) bun.PathBuffer else struct {} = std.mem.zeroes(if (bun.Environment.isWindows) bun.PathBuffer else struct {});
    ...
    const cache_dir: @import("std-fs-compat").FsDir = undefined, const cache_dir_subpath: safe.Slice(u8) = undefined, const module_folder: safe.Slice(u8) = undefined, const pkg_name: safe.Slice(u8) = undefined = switch (arg_kind) {
        .path => brk: {
    ...
    if (not_in_workspace_root) {
        var bufn: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
...
fn detachModuleFolderFromSharedStore(module_folder: []const u8) void {
    ...
    var native_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
    const native = if (comptime Environment.isWindows) native: {
        safe.SimdUtils.copy(native_buf[0..module_folder.len], module_folder);
...
fn overwritePackageInNodeModulesFolder(
    ...
    const src_path: bun.AbsPath(.{ .sep = .auto, .unit = .os }) = src_path: {
        if (comptime Environment.isWindows) {
            var path_buf: bun.WPathBuffer = std.mem.zeroes(bun.WPathBuffer);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    while (i < pairs.items.len) : (i += 1) {
        ...
        if (i + 1 < pairs.items.len) {
            for (0..pairs.items[i + 1 ..].len) |__zust_i| {
    var p = &pairs.items[i + 1 ..][__zust_i];
                if (p[1] == pkgid) {
...
fn pathArgumentRelativeToRootWorkspacePackage(manager: *PackageManager, lockfile: *const Lockfile, argument: []const u8) ?[]const u8 {
    ...
    const rel_path: safe.Slice(u8) = workspace_res.value.workspace.slice(lockfile.buffers.string_bytes.items);
