...
pub const Action = union(enum) {
    ...
    @ptrCast(@alignCast(data.context));
        }
    } else void,
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    const count = backtrace(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(addrs), @intCast(addrs.len));
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        0 => {
...

            {
                ...
                if (!has_printed_message) {
                    ...
                    writer.writeAll("=" ** 60 ++ "\n") catch std.c.abort();
                    printMetadata(writer) catch std.c.abort();
                    ...
                    if (inside_native_plugin) |name| {
                        ...
                        const fmt =
                            ...
                            \\
                        ;
                        writer.print(Output.prettyFmt(fmt, true), .{native_plugin_name}) catch std.c.abort();
                        ...
                        const fmt =
                            ...
                            \\
                        ;
                        writer.print(Output.prettyFmt(fmt, true), .{name}) catch std.c.abort();
                        has_printed_message = true;
                    }
                } else {
                    if (Output.enable_ansi_colors_stderr) {
                        writer.writeAll(Output.prettyFmt("<red>", true)) catch std.c.abort();
                    }
                    writer.writeAll("oh no") catch std.c.abort();
                    if (Output.enable_ansi_colors_stderr) {
                        writer.writeAll(Output.prettyFmt("<r><d>: multiple threads are crashing<r>\n", true)) catch std.c.abort();
                    } else {
                ...
                if (reason != .out_of_memory or debug_trace) {
                    ...
                    writer.writeAll("panic") catch std.c.abort();

                    if (Output.enable_ansi_colors_stderr) {
                        writer.writeAll(Output.prettyFmt("<r><d>", true)) catch std.c.abort();
                    }

                    if (bun.cli.Cli.is_main_thread) {
                        writer.writeAll("(main thread)") catch std.c.abort();
                    } else switch (bun.Environment.os) {
                        .windows => {
                            var name: std.os.windows.PWSTR = std.mem.zeroes(std.os.windows.PWSTR);
                            ...
                            if (std.os.windows.HRESULT_CODE(result) == .SUCCESS and name[0] != 0) {
                                writer.print("({f})", .{bun.fmt.utf16(bun.span(name))}) catch std.c.abort();
                            } else {
                                writer.print("(thread {d})", .{bun.c.GetCurrentThreadId()}) catch std.c.abort();
                            }
                        },
                        .mac, .linux, .freebsd => {},
                    ...
                    writer.writeAll(": ") catch std.c.abort();
                    if (Output.enable_ansi_colors_stderr) {
                        writer.writeAll(Output.prettyFmt("<r>", true)) catch std.c.abort();
                    }
                    writer.print("{f}\n", .{reason}) catch std.c.abort();
                }

                if (current_action) |action| {
                    writer.print("Crashed while {f}\n", .{action}) catch std.c.abort();
                }

                var addr_buf: [20]usize = .{};
                var trace_buf: std.builtin.StackTrace = std.mem.zeroes(std.builtin.StackTrace);
                ...
                const trace = blk: {
                    ...
                    @import("std-fs-compat").captureStackTrace(desired_begin_addr, &trace_buf);

                    if (comptime bun.Environment.isGlibc) {
                        var addr_buf_libc: [20]usize = .{};
                ...
                if (debug_trace) {
                    ...
                    if (!has_printed_message) {
                        ...
                        writer.writeAll("oh no") catch std.c.abort();
                        if (Output.enable_ansi_colors_stderr) {
                            writer.writeAll(Output.prettyFmt("<r><d>:<r> ", true)) catch std.c.abort();
                        } else {
                            writer.writeAll(Output.prettyFmt(": ", true)) catch std.c.abort();
                        }
                        if (inside_native_plugin) |name| {
                            const native_plugin_name = name;
                            writer.print(Output.prettyFmt(
                                ...
                                \\
                                \\
                            , true), .{native_plugin_name}) catch std.c.abort();
                        } else if (bun.analytics.Features.unsupported_uv_function > 0) {
                            const name = unsupported_uv_function orelse "<unknown>";
                            const fmt =
                                \\Bun encountered a crash when running a NAPI module that tried to call
                    ...
                    if (Output.enable_ansi_colors_stderr) {
                        writer.print(Output.prettyFmt("<cyan>", true), .{}) catch std.c.abort();
                    }
                    ...
                    writer.writeAll(trace_str_buf.slice()) catch std.c.abort();
                ...
                !bun.isProcessReloadInProgressOnAnotherThread())
            {
                ...
                Output.prettyErrorln("<d>--- Bun is auto-restarting due to crash <d>[time: <b>{d}<r><d>] ---<r>", .{
                    @max(@import("std-fs-compat").milliTimestamp(), 0),
                });
                Output.flush();
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                    ...
                    .object = if (!safe.SimdUtils.eql(name, image_path)) name: {
            ...
            .mac => {
                ...
                while (i < image_count) : (i += 1) {
                    ...
                    while (it.next()) |cmd| switch (cmd.hdr.cmd) {
                        .SEGMENT_64 => {
                            const segment_cmd = if (cmd.cast(std.macho.segment_command_64)) |value| {
    value
} else {
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                ...
                std.posix.dl_iterate_phdr(&ctx, error{Found}, struct {
                    fn callback(info: *std.posix.dl_phdr_info, _: usize, context: *CtxTy) !void {
                        ...
                        for (0..phdrs.len) |__zust_i| {
    var phdr = &phdrs[__zust_i];
...
fn encodeTraceString(opts: TraceString, writer: anytype) !void {
    ...
    try writeU64AsTwoVLQs(writer, // safe-transpile: @bitCast requires manual review
    @bitCast(packed_features));

    var name_bytes: [1024]u8 = .{};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    const first = VLQ.encode(// safe-transpile: @bitCast requires manual review
    @bitCast(@as(u32, @intCast((addr & 0xFFFFFFFF00000000) >> 32))));
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    const second = VLQ.encode(// safe-transpile: @bitCast requires manual review
    @bitCast(@as(u32, @intCast(addr & 0xFFFFFFFF))));
...
fn report(url: []const u8) void {
    ...
    switch (bun.Environment.os) {
        .windows => {
            var process: std.os.windows.PROCESS_INFORMATION = std.mem.zeroes(std.os.windows.PROCESS_INFORMATION);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        .mac, .linux, .freebsd => {
            ...
            var buf2: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
            ...
            switch (result) {
                ...
                0 => {
                    ...
                    _ = std.c.execve(if (argv[0]) |value| {
    value
} else {
    return error.NullPointer;
}, &argv, std.c.environ);
...
pub fn dumpStackTrace(trace: std.builtin.StackTrace, limits: WriteStackTraceLimits) void {
    ...
    switch (bun.Environment.os) {
        .windows => attempt_dump: {
            ...
            writeStackTrace(trace, stderr, debug_info, @import("std-io-compat").detectConfig(@import("std-fs-compat").File.stderr()), limits) catch |err| {
...
fn spawnSymbolizer(program: [:0]const u8, alloc: std.mem.Allocator, trace: *const std.builtin.StackTrace) !void {
    ...
    var stderr_writer = @import("std-fs-compat").File.stderr().writerStreaming(&.{});
...
pub fn dumpCurrentStackTrace(first_address: ?usize, limits: WriteStackTraceLimits) void {
    ...
    @import("std-fs-compat").captureStackTrace(first_address orelse @returnAddress(), &stack);
...
pub const StoredTrace = struct {
    ...
    pub fn from(stack_trace: ?*std.builtin.StackTrace) StoredTrace {
        if (stack_trace) |stack| {
            ...
            safe.SimdUtils.copy(data[0..items], stack.instruction_addresses[0..items]);
            return .{
                .data = data,
...
pub fn appendPreCrashHandler(comptime T: type, ptr: *T, comptime handler: fn (*T) anyerror!void) !void {
    const wrap = struct {
        fn onCrash(opaque_ptr: *anyopaque) void {
            handler(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(opaque_ptr))) catch |err| {
...
pub const SourceAtAddress = struct {
    ...
    fn deinit(self: *@This(), debug_info: *debug.SelfInfo) void {
        if (self.source_location) |sl| // safe-transpile: free removed (memory owned by safe type);
    }
};
...
fn printLineFromFileAnyOs(out_stream: anytype, tty_config: @import("std-io-compat").TtyConfig, source_location: SourceLocation) !void {
    ...
    var reader_buf: [4096]u8 = .{};
    var file_reader = f.reader(&reader_buf);
    read_line: {
        var buf: [4096]u8 = .{};
        var amt_read = try file_reader.interface.readSliceShort(&buf);
        const line_start = seek: {
            ...
            while (next_line != source_location.line) {
                ...
                if (bun.strings.indexOfChar(slice, '\n')) |pos| {
                    next_line += 1;
                    if (pos == slice.len - 1) {
                        amt_read = try file_reader.interface.readSliceShort(&buf);
...
export fn CrashHandler__unsupportedUVFunction(name: ?[*:0]const u8) callconv(.c) void {
    ...
    std.debug.panic("unsupported uv function: {s}", .{if (name) |value| {
    value
} else {
    return error.NullPointer;
}});
...
export fn CrashHandler__setDlOpenAction(action: ?[*:0]const u8) void {
    if (action) |str| {
        ...
        bun.debugAssert(current_action != null and if (current_action) |value| {
    value
} else {
    return error.NullPointer;
} == .dlopen);
