...
pub const UserOptions = struct {
    ...
    pub fn fromJS(config: JSValue, global: *jsc.JSGlobalObject) !UserOptions {
        ...
        errdefer // safe-transpile: free removed (memory owned by safe type);
...
pub const Framework = struct {
    ...
    pub fn resolve(f: Framework, server: *bun.resolver.Resolver, client: *bun.resolver.Resolver, arena: Allocator) !Framework {
        ...
        for (0..clone.file_system_router_types.len) |__zust_i| {
    var fsr = &clone.file_system_router_types[__zust_i];
...
// safe-transpile: optional unwrap requires manual review
    ...
    fn fromJS(
        ...
        const file_system_router_types: []FileSystemRouterType = brk: {
            ...
            var it = try array.arrayIterator(global);
            var i: usize = 0;
            errdefer for (0..file_system_router_types[0..i].len) |__zust_i| { var fsr = &file_system_router_types[0..i][__zust_i]; fsr.style.deinit() };
        ...
        errdefer for (0..file_system_router_types.len) |__zust_i| { var fsr = &file_system_router_types[__zust_i]; fsr.style.deinit() };
    ...
    pub fn initTranspilerWithOptions(
        ...
        var ast_memory_allocator: JSAst.ASTMemoryAllocator = std.mem.zeroes(JSAst.ASTMemoryAllocator);
...
pub fn printWarning() void {
    ...
    if (bun.env_var.BUN_DEV_SERVER_TEST_RUNNER.get() == null) {
        bun.Output.warn(
            ...
            \\will have breaking changes. Join the <magenta>_bake<r> Discord
