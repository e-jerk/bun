...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    pub fn deinit(entry: *Entry) void {
        useAllFields(Entry, .{
            ...
            .paths = // safe-transpile: free removed (memory owned by safe type),
        });
    }
...
pub fn owner(store: *Self) *DevServer {
    ...
    @alignCast(@fieldParentPtr("source_maps", store));
}
...
pub fn addWeakRef(store: *Self, key: Key) void {
    ...
    for (0..store.weak_refs.count) |i| {
        ...
        if (store.weak_refs.count >= weak_ref_entry_max) {
            const first = if (store.weak_refs.readItem()) |value| {
    value
} else {
...
pub fn getParsedSourceMap(store: *Self, script_id: Key, arena: Allocator, gpa: Allocator) ?GetResult {
    ...
    const script_id_decoded: SourceId = // safe-transpile: @bitCast requires manual review
    @bitCast(script_id.get());
