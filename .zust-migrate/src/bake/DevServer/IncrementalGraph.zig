    ...
    for (g.bundled_files.keys(), g.bundled_files.values()) |k, v| {
                ...
                .current_chunk_parts = {
                    if (comptime side == .server) {
                        for (0..g.current_chunk_parts.items.len) |__zust_i| { var part = &g.current_chunk_parts.items[__zust_i]; part.deinit() };
                    }
                ...
                .current_chunk_source_maps = if (side == .server) {
                    for (0..g.current_chunk_source_maps.items.len) |__zust_i| {
    var source_map = &g.current_chunk_source_maps.items[__zust_i];
                        source_map.deinit();
                    }
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        pub fn htmlRouteBundleIndex(g: *const Self, index: FileIndex) RouteBundle.Index {
            return if (g.getFileByIndex(index).html_route_bundle_index) |value| {
    value
} else {
                                ...
                                if (js.source_map) |*source_map| {
                                    ...
                                    if (source_map.chunk.buffer.len() > 0) {
                                        break :blk .{ .some = PackedMap.newNonEmpty(
                                            &source_map.chunk,
                                            if (source_map.escaped_source.take()) |value| {
    value
} else {
    return error.NullPointer;
},
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        fn processEdgeAttachment(
            ...
            const imported_file_index: FileIndex = undefined, const kind = brk: {
            ...
            if (gop.found_existing) {
                ...
                new_imports[0] = lookup.edge_index.toOptional();
            } else {
                ...
                new_imports[0] = edge.toOptional();
                first_dep[0] = edge.toOptional();
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        pub fn insertFailure(
            ...
            const gop: Gop = undefined, const found_existing, const file_index = switch (mode) {
                .abs_path => brk: {
...
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn onFileDeleted(g: *Self, abs_path: []const u8, bv2: *bun.BundleV2) void {
            ...
            // source index map.
            for (0..&bv2.graph.build_graphs.values.len) |__zust_i| {
    var map = &&bv2.graph.build_graphs.values[__zust_i];
        ...
        pub fn reset(g: *Self) void {
            ...
            if (comptime side == .client) {
                ...
                for (0..g.current_chunk_source_maps.items.len) |__zust_i| { var sourcemap = &g.current_chunk_source_maps.items[__zust_i]; sourcemap.deinit() };
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    @alignCast(@fieldParentPtr(@tagName(side) ++ "_graph", g));
        }
