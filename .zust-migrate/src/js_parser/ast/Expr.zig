...
pub fn getByIndex(expr: *const Expr, index: u32, index_str: string, allocator: std.mem.Allocator) ?Expr {
    switch (expr.data) {
        ...
        .e_object => |object| {
            for (0..object.properties.sliceConst().len) |__zust_i| {
    var prop = &object.properties.sliceConst()[__zust_i];
                const key = &(prop.key orelse continue);
                switch (key.data) {
                    .e_string => |str| {
...
// safe-transpile: optional unwrap requires manual review
    ...
    const ptr = safe.Box(bun.StringArrayHashMap(string)).init(allocator, undefined) catch unreachable;
    ptr[0] = map;
    return ptr;
}
    ...
    switch (Type) {
        E.Array => {
            return Expr{
                ...
                .data = Data{
                    .e_array = brk: {
                        const item = safe.Box(Type).init(allocator, undefined) catch unreachable;
                        item[0] = st;
                        break :brk item;
                    },
                },
...
pub const Data = union(Tag) {
    ...
    pub fn clone(this: Expr.Data, allocator: std.mem.Allocator) !Data {
        return switch (this) {
            .e_array => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_array))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_array = item };
            },
            .e_unary => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_unary))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_unary = item };
            },
            .e_binary => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_binary))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_binary = item };
            },
            .e_class => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_class))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_class = item };
            },
            .e_new => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_new))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_new = item };
            },
            .e_function => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_function))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_function = item };
            },
            .e_call => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_call))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_call = item };
            },
            .e_dot => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_dot))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_dot = item };
            },
            .e_index => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_index))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_index = item };
            },
            .e_arrow => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_arrow))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_arrow = item };
            },
            .e_jsx_element => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_jsx_element))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_jsx_element = item };
            },
            .e_object => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_object))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_object = item };
            },
            .e_spread => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_spread))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_spread = item };
            },
            .e_template => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_template))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_template = item };
            },
            .e_reg_exp => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_reg_exp))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_reg_exp = item };
            },
            .e_await => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_await))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_await = item };
            },
            .e_yield => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_yield))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_yield = item };
            },
            .e_if => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_if))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_if = item };
            },
            .e_import => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_import))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_import = item };
            },
            .e_big_int => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_big_int))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_big_int = item };
            },
            .e_string => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_string))).init(allocator, undefined);
                item[0] = el.*;
                return .{ .e_string = item };
            },
            .e_inlined_enum => |el| {
                const item = try safe.Box(std.meta.Child(@TypeOf(this.e_inlined_enum))).init(allocator, undefined);
                item[0] = el.*;
    ...
    pub fn writeToHasher(this: Expr.Data, hasher: anytype, symbol_table: anytype) void {
        ...
        switch (this) {
            ...
            .e_unary => |e| {
                writeAnyToHasher(hasher, @as(u8, // safe-transpile: @bitCast requires manual review
    @bitCast(e.flags)));
