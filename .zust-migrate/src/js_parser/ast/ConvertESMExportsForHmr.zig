...
pub fn convertStmt(ctx: *ConvertESMExportsForHmr, p: anytype, stmt: Stmt) !void {
    ...
    var decl_ptr = &st.decls.slice()[__zust_i];
...
// safe-transpile: optional unwrap requires manual review
                ...
                if (ReactRefresh.isComponentishName(name)) {
                    ...
                    try ctx.export_props.append(p.allocator, .{
                        ...
                        .value = Expr.initIdentifier(if (symbol.ref) |value| {
    value
} else {
...
// safe-transpile: optional unwrap requires manual review
                            .s_class => |class| if (class.class.class_name.?.ref) |value| {
    value
} else {
    return error.NullPointer;
},
// safe-transpile: optional unwrap requires manual review
                            .s_function => |func| if (func.func.name.?.ref) |value| {
    value
} else {
    return error.NullPointer;
},
...
// safe-transpile: optional unwrap requires manual review
                .value = Expr.initIdentifier(if (st.class.class_name.?.ref) |value| {
    value
} else {
    return error.NullPointer;
}, stmt.loc),
...
// safe-transpile: optional unwrap requires manual review
    ...
    return error.NullPointer;
        ...
        .s_export_clause => |st| {
            for (st.items) |item| {
                const ref = if (item.name.ref) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        .s_export_from => |st| {
            ...
            for (0..st.items.len) |__zust_i| {
    var item = &st.items[__zust_i];
...
pub fn finalize(ctx: *ConvertESMExportsForHmr, p: anytype, all_parts: []js_ast.Part) !void {
    if (ctx.export_star_props.items.len > 0) {
        if (ctx.export_props.items.len == 0) {
            ...
            safe.SimdUtils.copy(ctx.export_props.items[0..export_star_len], ctx.export_star_props.items);
        }
    }
    ...
    for (0..all_parts[0 .. all_parts.len - 1].len) |__zust_i| {
    var part = &all_parts[0 .. all_parts.len - 1][__zust_i];
