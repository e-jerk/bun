                ...
                switch (stmt.data) {
                    .s_function => |func| {
                        if (func.func.flags.contains(.is_export)) {
                            const locref = if (func.func.name) |value| {
    value
} else {
...
// safe-transpile: optional unwrap requires manual review
    ...
    return error.NullPointer;
                    ...
                    .s_class => |class| {
                        if (class.is_export) {
                            const locref = if (class.class.class_name) |value| {
    value
} else {
    return error.NullPointer;
};
// safe-transpile: optional unwrap requires manual review
    ...
    return error.NullPointer;
                    ...
                    inline .s_namespace, .s_enum => |ns| {
                        if (ns.is_export) {
                            if (p.ref_to_ts_namespace_member.get(if (ns.name.ref) |value| {
    value
} else {
    return error.NullPointer;
})) |member_data| {
...
// safe-transpile: optional unwrap requires manual review
    ...
    return error.NullPointer;
            ...
            if (!opts.is_typescript_declare) {
                ...
                try p.ref_to_ts_namespace_member.put(p.allocator, if (name.ref) |value| {
    value
} else {
    return error.NullPointer;
}, ns_member_data);
        ...
        pub fn parseTypescriptEnumStmt(p: *P, loc: logger.Loc, opts: *ParseStatementOptions) anyerror!Stmt {
            ...
            if (!opts.is_typescript_declare) {
                ...
                p.current_scope.ts_namespace = ts_namespace;
                bun.handleOom(p.ref_to_ts_namespace_member.putNoClobber(p.allocator, if (name.ref) |value| {
    value
} else {
    return error.NullPointer;
}, enum_member_data));
