                    ...
                    if (p.options.bundle) {
                        if (p.import_items_for_namespace.getPtr(id.ref)) |import_items| {
                            const ref = (import_items.get(name) orelse brk: {
                                ...
                                bun.handleOom(p.module_scope.generated.append(p.allocator, if (new_item.ref) |value| {
    value
} else {
    return error.NullPointer;
}));
                                ...
                                p.is_import_item.put(p.allocator, if (new_item.ref) |value| {
    value
} else {
    return error.NullPointer;
}, {}) catch unreachable;
...
// safe-transpile: optional unwrap requires manual review
                                    ...
                                    const visited_value = p.visitExpr(if (prop.value) |value| {
    value
} else {
    return error.NullPointer;
});
                                    ...
                                    const ref = if (named_export_entry.value_ptr.loc_ref.ref) |value| {
    value
} else {
    return error.NullPointer;
};
    ...
    return error.NullPointer;
                ...
                .e_import_identifier => |id| {
                    ...
                    if (p.options.bundle and !p.is_control_flow_dead) {
                        const use = if (p.symbol_uses.getPtr(id.ref)) |value| {
    value
} else {
    return error.NullPointer;
};
