...
// safe-transpile: optional unwrap requires manual review
    ...
    for (sorted, existing_items.keys()) |*result, alias| {
                        ...
                        for (sorted) |alias| {
                            const item = if (existing_items.get(alias)) |value| {
    value
} else {
    return error.NullPointer;
};
    ...
    return error.NullPointer;
                            ...
                            const name_ref = if (name.ref) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
                        ...
                        try p.recordExport(name.loc, original_name, if (name.ref) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: optional unwrap requires manual review
                        try p.recordExport(name.loc, p.symbols.items[name.ref.?.innerIndex()].original_name, if (name.ref) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: optional unwrap requires manual review
            ...
            .s_export_clause => |st| {
                for (st.items) |item| {
                    try p.recordExport(item.alias_loc, item.alias, if (item.name.ref) |value| {
    value
} else {
    return error.NullPointer;
});
