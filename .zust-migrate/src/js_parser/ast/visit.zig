    ...
    return struct {
        ...
        pub fn visitArgs(p: *P, args: []G.Arg, opts: VisitArgsOpts) void {
            ...
            if (strict_loc != null and !has_simple_args) {
                p.log.addRangeError(p.source, p.source.rangeOfString(if (strict_loc) |value| {
    value
} else {
    return error.NullPointer;
}), "Cannot use a \"use strict\" directive in a function with a non-simple parameter list") catch unreachable;
            }
...
// safe-transpile: optional unwrap requires manual review
            ...
            for (0..args.len) |__zust_i| {
    var arg = &args[__zust_i];
        ...
        pub fn visitTSDecorators(p: *P, decs: ExprNodeList) ExprNodeList {
            for (0..decs.slice().len) |__zust_i| {
    var dec = &decs.slice()[__zust_i];
                dec.* = p.visitExpr(dec.*);
            }
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    for (bound_array.items[0..array.items.len], array.items.slice()) |item, *child_expr| {
                            if (item.binding.data == .b_missing) {
                                child_expr[0] = p.newExpr(E.Missing{}, expr.loc);
                                continue;
                            }
                ...
                .b_object, .b_array => {
                    if (comptime allow_macros) {
                        if (could_be_macro and decl.value != null) {
                            p.visitBindingAndExprForMacro(decl.binding, if (decl.value) |value| {
    value
} else {
    return error.NullPointer;
});
    ...
    var dec = &st.decls.slice()[__zust_i];
    ...
    var property = &bind.properties[__zust_i];
        ...
        pub fn visitClass(noalias p: *P, name_scope_loc: logger.Loc, noalias class: *G.Class, default_name_ref: Ref) Ref {
            ...
            if (class.class_name) |name| {
                p.recordDeclaredSymbol(if (name.ref) |value| {
    value
} else {
    return error.NullPointer;
}) catch unreachable;
    ...
    return error.NullPointer;
                ...
                const name_str: safe.Slice(u8) = if (default_name_ref.isNull()) "_this" else "_default";
        ...
        pub fn visitStmts(p: *P, stmts: *ListManaged(Stmt), kind: StmtsKind) anyerror!void {
...

            {
                ...
                if (p.scopes_in_order_for_enum.count() > 0) {
                    var found: usize = 0;
                    for (0..stmts.items.len) |__zust_i| {
    var stmt = &stmts.items[__zust_i];
                        if (stmt.data == .s_enum) {
...
// safe-transpile: optional unwrap requires manual review
            ...
            if (kind != .switch_stmt and p.shouldLowerUsingDeclarations(stmts.items)) {
                ...
                stmts[0] = ctx.finalize(p, stmts.items, p.current_scope.parent == null);
            }
            ...
            if (p.current_scope.parent != null and !p.current_scope.contains_direct_eval) {
                ...
                if (p.const_values.count() > 0) {
                    ...
                    for (0..items.len) |__zust_i| {
    var stmt = &items[__zust_i];
                        switch (stmt.data) {
                    ...
                    inner: while (output.items.len > 0) {
                        ...
                        switch (prev_statement.data) {
                            .s_local => {
                                ...
                                const last: *Decl = if (local.decls.last()) |value| {
    value
} else {
    return error.NullPointer;
};
                                ...
                                switch (last.binding.data) {
                                    .b_identifier => |ident| {
                                        ...
                                        if (symbol.use_count_estimate == 1) {
                                            if (p.substituteSingleUseSymbolInStmt(stmt, id, if (last.value) |value| {
    value
} else {
    return error.NullPointer;
})) {
                ...
                switch (stmt.data) {
                    ...
                    .s_return => |ret| {
                        ...
                        if (output.items.len > 0 and ret.value != null and p.options.runtimeMergeAdjacentExpressionStatements()) {
                            var prev_stmt = &output.items[output.items.len - 1];
                            if (prev_stmt.data == .s_expr and !prev_stmt.isSuperCall()) {
                                ret.value = prev_stmt.data.s_expr.value.joinWithComma(if (ret.value) |value| {
    value
} else {
    return error.NullPointer;
}, p.allocator);
