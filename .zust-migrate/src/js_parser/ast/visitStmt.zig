pub fn VisitStmt(
        ...
        const visitors = struct {
            pub fn s_import(noalias p: *P, noalias stmts: *ListManaged(Stmt), noalias stmt: *Stmt, noalias data: *S.Import) !void {
                ...
                if (data.default_name) |default_name| {
                    try p.recordDeclaredSymbol(if (default_name.ref) |value| {
    value
} else {
    return error.NullPointer;
                ...
                if (data.items.len > 0) {
                    for (0..data.items.len) |__zust_i| {
    var item = &data.items[__zust_i];
    ...
    var item = &data.items[__zust_i];
            ...
            pub fn s_export_from(noalias p: *P, noalias stmts: *ListManaged(Stmt), noalias stmt: *Stmt, noalias data: *S.ExportFrom) !void {
                ...
                if (p.options.features.replace_exports.count() > 0) {
                    ...
                    for (data.items) |item| {
                        const old_ref = if (item.name.ref) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
                        ...
                        if (p.current_scope.parent == null and p.will_wrap_module_in_try_catch_for_using) {
                            ...
                            decls[0] = .{
                                .binding = p.b(B.Identifier{ .ref = if (data.default_name.ref) |value| {
    value
} else {
    return error.NullPointer;
                        ...
                        if (mark_for_replace) {
                            const entry = if (p.options.features.replace_exports.getPtr("default")) |value| {
    value
} else {
    return error.NullPointer;
};
                    ...
                    .stmt => |s2| switch (s2.data) {
                        .s_function => |func| {
                            const name = if (func.func.name) |func_loc|
                                p.loadNameFromRef(if (func_loc.ref) |value| {
    value
} else {
    return error.NullPointer;
})
...
// safe-transpile: optional unwrap requires manual review
    ...
    return error.NullPointer;
                                ...
                                (ReactRefresh.isComponentishName(name) or bun.strings.eqlComptime(name, "default")))
                            {
                                ...
                                } else if (data.default_name.ref) |value| {
    value
} else {
    return error.NullPointer;
};
                        ...
                        .s_class => |class| {
                            _ = p.visitClass(s2.loc, &class.class, if (data.default_name.ref) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: optional unwrap requires manual review
                    p.options.features.replace_exports.count() > 0 and p.isExportToEliminate(if (data.func.name.?.ref) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: optional unwrap requires manual review
                const name_ref = if (data.func.name.?.ref) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
// safe-transpile: optional unwrap requires manual review
                        p.newExpr(E.Identifier{ .ref = if (data.func.name.?.ref) |value| {
    value
} else {
    return error.NullPointer;
} }, data.func.name.?.loc),
                    ...
                    if (p.options.features.server_components.wrapsExports() and data.func.flags.contains(.is_export)) {
                        ...
                        const name = if (data.func.name) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
                    p.options.features.replace_exports.count() > 0 and p.isExportToEliminate(if (data.class.class_name.?.ref) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: optional unwrap requires manual review
                    const ref = if (data.class.class_name.?.ref) |value| {
    value
} else {
    return error.NullPointer;
};
                    ...
                    stmts.append(
                        Stmt.assign(
                            p.newExpr(
                                E.Dot{
                                    .target = p.newExpr(
                                        E.Identifier{ .ref = if (p.enclosing_namespace_arg_ref) |value| {
    value
} else {
    return error.NullPointer;
} },
...
// safe-transpile: optional unwrap requires manual review
                            ...
                            p.newExpr(
// safe-transpile: optional unwrap requires manual review
                                E.Identifier{ .ref = if (data.class.class_name.?.ref) |value| {
    value
} else {
    return error.NullPointer;
} },
// safe-transpile: optional unwrap requires manual review
            ...
            pub fn s_label(noalias p: *P, noalias stmts: *ListManaged(Stmt), noalias stmt: *Stmt, noalias data: *S.Label) !void {
                ...
                const name = p.loadNameFromRef(if (data.name.ref) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
                ...
                if (data.is_export and p.enclosing_namespace_arg_ref != null) {
                    for (0..data.decls.slice().len) |__zust_i| {
    var d = &data.decls.slice()[__zust_i];
                        if (d.value) |val| {
    ...
    var d = &data.decls.slice()[__zust_i];
                ...
                if (data.is_export and p.options.features.server_components.wrapsExports()) {
                    for (0..data.decls.slice().len) |__zust_i| try_annotate: {
    var decl = &data.decls.slice()[__zust_i];
...
// safe-transpile: optional unwrap requires manual review
            ...
            pub fn s_if(noalias p: *P, noalias stmts: *ListManaged(Stmt), noalias stmt: *Stmt, noalias data: *S.If) !void {
                ...
                if (p.options.features.minify_syntax) {
                    if (effects.ok) {
                        if (effects.value) {
                            if (data.no == null or !SideEffects.shouldKeepStmtInDeadControlFlow(if (data.no) |value| {
    value
} else {
    return error.NullPointer;
}, p.allocator)) {
                            ...
                            if (!SideEffects.shouldKeepStmtInDeadControlFlow(data.yes, p.allocator)) {
                                ...
                                return try p.appendIfBodyPreservingScope(stmts, if (data.no) |value| {
    value
} else {
    return error.NullPointer;
});
                ...
                if (data.init.data == .s_local) {
                    ...
                    if (data.init.data == .s_local and data.init.data.s_local.kind.isUsing() and p.options.features.lower_using) {
                        ...
                        if (data.body.data == .s_block) {
                            safe.SimdUtils.copy(statements[1..], data.body.data.s_block.stmts);
                        } else {
    ...
    for (data.cases, 0..) |case, i| {
            ...
            pub fn s_enum(noalias p: *P, noalias stmts: *ListManaged(Stmt), noalias stmt: *Stmt, noalias data: *S.Enum, was_after_after_const_local_prefix: bool) !void {
                ...
                if (p.current_scope == p.module_scope and p.options.bundle) {
                    try p.top_level_enums.append(p.allocator, if (data.name.ref) |value| {
    value
} else {
    return error.NullPointer;
});
                ...
                try p.recordDeclaredSymbol(if (data.name.ref) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: optional unwrap requires manual review
                ...
                for (0..data.values.len) |__zust_i| {
    var value = &data.values[__zust_i];
    ...
    return error.NullPointer;
            ...
            pub fn s_namespace(noalias p: *P, noalias stmts: *ListManaged(Stmt), noalias stmt: *Stmt, noalias data: *S.Namespace) !void {
                p.recordDeclaredSymbol(if (data.name.ref) |value| {
    value
} else {
    return error.NullPointer;
}) catch unreachable;
