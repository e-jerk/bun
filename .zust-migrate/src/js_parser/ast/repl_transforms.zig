                        ...
                        if (func.func.name) |name_loc| {
                            try hoisted_stmts.append(p.s(S.Local{
                                ...
                                .decls = Decl.List.fromOwnedSlice(bun.handleOom(allocator.dupe(G.Decl, &.{
                                    G.Decl{
                                        .binding = p.b(B.Identifier{ .ref = if (name_loc.ref) |value| {
    value
} else {
...
// safe-transpile: optional unwrap requires manual review
                            ...
                            const func_id = p.newExpr(E.Identifier{ .ref = if (name_loc.ref) |value| {
    value
} else {
    return error.NullPointer;
} }, name_loc.loc);
    ...
    return error.NullPointer;
                            ...
                            const class_id = p.newExpr(E.Identifier{ .ref = if (name_loc.ref) |value| {
    value
} else {
    return error.NullPointer;
} }, name_loc.loc);
                    ...
                    .s_import => |import_data| {
                        ...
                        if (import_data.star_name_loc) |_| {
                            ...
                            try hoisted_stmts.append(p.s(S.Local{
                                ...
                                .decls = Decl.List.fromOwnedSlice(bun.handleOom(allocator.dupe(G.Decl, &.{
                                    G.Decl{
                                        .binding = p.b(B.Identifier{ .ref = if (default_name.ref) |value| {
    value
} else {
    return error.NullPointer;
} }, default_name.loc),
                            ...
                            if (import_data.items.len > 0) {
                                ...
                                const assign = p.newExpr(E.Binary{
                                    .op = .bin_assign,
                                    .left = p.newExpr(E.Identifier{ .ref = if (default_name.ref) |value| {
    value
} else {
    return error.NullPointer;
} }, default_name.loc),
        ...
        fn convertNamedImports(
            ...
            for (import_data.items) |item| {
                try hoisted_stmts.append(p.s(S.Local{
                    ...
                    .decls = Decl.List.fromOwnedSlice(bun.handleOom(allocator.dupe(G.Decl, &.{
                        G.Decl{
                            .binding = p.b(B.Identifier{ .ref = if (item.name.ref) |value| {
    value
} else {
    return error.NullPointer;
} }, item.name.loc),
                ...
                const item_assign = p.newExpr(E.Binary{
                    .op = .bin_assign,
                    .left = p.newExpr(E.Identifier{ .ref = if (item.name.ref) |value| {
    value
} else {
    return error.NullPointer;
} }, item.name.loc),
