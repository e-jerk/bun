                ...
                .e_unary => |e| {
                    if (e.op == .un_not) {
                        ...
                        if (e.value.data == .e_unary and e.value.data.e_unary.op == .un_not) {
                            expr[0] = e.value.data.e_unary.value;
                            continue;
                        }
                    ...
                    if (spread.kind == .spread) {
                        ...
                        for (properties_slice) |prop_| {
                            ...
                            if (prop_.kind != .spread) {
                                const value = simplifyUnusedExpr(p, if (prop.value) |value| {
    value
} else {
...
// safe-transpile: optional unwrap requires manual review
                    ...
                    result = result.joinWithComma(
// safe-transpile: optional unwrap requires manual review
                        simplifyUnusedExpr(p, if (prop.value) |value| {
    value
} else {
    return error.NullPointer;
}) orelse prop.value.?.toEmpty(),
                        p.allocator,
                    );
                }
