                ...
                if (in.assign_target != .none) {
                    if (p.symbols.items[result.ref.innerIndex()].kind == .constant) { // TODO: silence this for runtime transpiler
                        ...
                        notes[0] = logger.Data{
                            ...
                            .location = logger.Location.initOrNull(p.source, js_lexer.rangeOfIdentifier(p.source, if (result.declare_loc) |value| {
    value
} else {
            ...
            pub fn e_jsx_element(p: *P, expr: Expr, _: ExprIn) Expr {
                ...
                switch (comptime jsx_transform_type) {
                    .react => {
                        ...
                        for (0..all_props.len) |__zust_i| {
    var property = &all_props[__zust_i];
...
// safe-transpile: optional unwrap requires manual review
                        ...
                        if (ref != null and !p.options.features.is_macro_runtime) {
                            if (p.macro.refs.get(if (ref) |value| {
    value
} else {
    return error.NullPointer;
})) |macro_ref_data| {
                                p.ignoreUsage(if (ref) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: optional unwrap requires manual review
                ...
                for (0..e_.parts.len) |__zust_i| {
    var part = &e_.parts[__zust_i];
    ...
    return error.NullPointer;
                ...
                while (p.binary_expression_stack.items.len > stack_bottom) {
                    v = if (p.binary_expression_stack.pop()) |value| {
    value
} else {
    return error.NullPointer;
};
            ...
            pub fn e_index(p: *P, expr: Expr, in: ExprIn) Expr {
                ...
                switch (e_.index.data) {
                    .e_private_identifier => |_private| {
                        ...
                        var r: logger.Range = std.mem.zeroes(logger.Range);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            pub fn e_dot(p: *P, expr: Expr, in: ExprIn) Expr {
                ...
                if (p.define.dots.get(e_.name)) |parts| {
                    for (0..parts.len) |__zust_i| {
    var define = &parts[__zust_i];
                        if (p.isDotDefineMatch(expr, define.parts)) {
                            if (in.assign_target == .none) {
                                // Substitute user-specified defines
    ...
    var item = &items[__zust_i];
            ...
            pub fn e_object(p: *P, expr: Expr, in: ExprIn) Expr {
                ...
                var has_proto = false;
                for (0..e_.properties.slice().len) |__zust_i| {
    var property = &e_.properties.slice()[__zust_i];
                    if (property.kind != .spread) {
            ...
            pub fn e_call(p: *P, expr: Expr, in: ExprIn) Expr {
                ...
                const is_macro_ref: bool = if (comptime allow_macros) brk: {
                    ...
                    break :brk possible_macro_ref != null and p.macro.refs.contains(if (possible_macro_ref) |value| {
    value
} else {
    return error.NullPointer;
});
...

                {
                    ...
                    for (0..e_.args.slice().len) |__zust_i| {
    var arg = &e_.args.slice()[__zust_i];
                        arg.* = p.visitExpr(arg.*);
                    }
                ...
                if (comptime allow_macros) {
                    if (is_macro_ref and !p.options.features.is_macro_runtime) {
                        ...
                        const macro_ref_data = if (p.macro.refs.get(ref)) |value| {
    value
} else {
    return error.NullPointer;
};
