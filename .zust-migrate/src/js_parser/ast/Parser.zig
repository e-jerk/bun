pub const Parser = struct {
    ...
    fn _scanImports(self: *Parser, comptime ParserType: type, scan_pass: *ScanPassResult) anyerror!void {
        ...
        //
        if (comptime ParserType.parser_features.typescript) {
            for (0..scan_pass.import_records.items.len) |__zust_i| {
    var import_record = &scan_pass.import_records.items[__zust_i];
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn toLazyExportAST(this: *Parser, expr: Expr, comptime runtime_api_call: []const u8, symbols: Symbol.List) !js_ast.Result {
        ...
        stmts[0] = Stmt{
            .data = .{
                .s_lazy_export = brk: {
                    const data = try safe.Box(Expr.Data).init(allocator, undefined);
                    data[0] = final_expr.data;
                    break :brk data;
                },
            },
            .loc = expr.loc,
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    ...
    fn _parse(noalias self: *Parser, comptime ParserType: type) !js_ast.Result {
        ...
        if (Environment.allow_assert) {
            bun.assert(binary_expression_stack_heap.fixed_buffer_allocator.ownsPtr(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(p.binary_expression_stack.items)));
            bun.assert(binary_expression_simplify_stack_heap.fixed_buffer_allocator.ownsPtr(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(p.binary_expression_simplify_stack.items)));
        }
        ...
        if (p.shouldUnwrapCommonJSToESM()) {
            if (p.imports_to_convert_from_require.items.len > 0) {
                ...
                for (p.imports_to_convert_from_require.items) |deferred_import| {
                    ...
                    bun.handleOom(p.module_scope.generated.append(p.allocator, if (deferred_import.namespace.ref) |value| {
    value
} else {
    return error.NullPointer;
                    ...
                    import_part_stmts[0] = Stmt.alloc(
                        ...
                        S.Import{
                            ...
                            .namespace_ref = if (deferred_import.namespace.ref) |value| {
    value
} else {
    return error.NullPointer;
},
                    ...
                    declared_symbols.appendAssumeCapacity(.{ .ref = if (deferred_import.namespace.ref) |value| {
    value
} else {
    return error.NullPointer;
}, .is_top_level = true });
            ...
            if (p.commonjs_named_exports.count() > 0) {
                ...
                break_optimize: {
                    if (!p.commonjs_named_exports_deoptimized) {
                        ...
                        for (0..export_refs.len) |__zust_i| {
    var export_ref = &export_refs[__zust_i];
...
// safe-transpile: optional unwrap requires manual review
            ...
            {
                for (0..parts.items.len) |__zust_i| {
    var part = &parts.items[__zust_i];
                    // Specially handle modules shaped like this:
                    //
                    //    doSomeStuff();
        ...
        if (p.isDeoptimizedCommonJS()) {
            ...
            if (p.options.features.commonjs_at_runtime) {
                ...
                const import_record: ?*const ImportRecord = brk: {
                    for (0..p.import_records.items.len) |__zust_i| {
    var import_record = &p.import_records.items[__zust_i];
    ...
    var import_record = &p.import_records.items[__zust_i];
        ...
        if (p.options.features.inject_jest_globals) outer: {
            ...
            for (0..p.import_records.items.len) |__zust_i| {
    var item = &p.import_records.items[__zust_i];
        ...
        if (p.has_called_runtime) {
            var runtime_imports: [RuntimeImports.all.len]u8 = .{};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        if (p.server_components_wrap_ref.isValid()) {
            ...
            const sc = if (fw.server_components) |value| {
    value
} else {
    return error.NullPointer;
};
