...
// safe-transpile: function uses raw slice parameter — consider safe.String
        ...
        fn makeStaticBlock(p: *P, expr: Expr, l: logger.Loc) Property {
            ...
            sb[0] = .{ .loc = l, .stmts = bun.BabyList(Stmt).fromOwnedSlice(stmts) };
        ...
        fn memberTarget(p: *P, target_expr: Expr, prop: Property) Expr {
            const key_expr = if (prop.key) |value| {
    value
} else {
...
// safe-transpile: function returns small constant slice — consider safe.String
        ...
        fn rewriteExpr(p: *P, expr: *Expr, kind: RewriteKind) void {
            ...
            switch (expr.data) {
                ...
                .e_call => |e| {
                    ...
                    for (0..e.args.slice().len) |__zust_i| { var a = &e.args.slice()[__zust_i]; rewriteExpr(p, a, kind) };
                },
                .e_new => |e| {
                    rewriteExpr(p, &e.target, kind);
                    for (0..e.args.slice().len) |__zust_i| { var a = &e.args.slice()[__zust_i]; rewriteExpr(p, a, kind) };
                ...
                .e_array => |e| {
                    for (0..e.items.slice().len) |__zust_i| { var item = &e.items.slice()[__zust_i]; rewriteExpr(p, item, kind) };
                },
                .e_object => |e| {
                    for (0..e.properties.slice().len) |__zust_i| {
    var prop = &e.properties.slice()[__zust_i];
                ...
                .e_template => |e| {
                    ...
                    for (0..e.parts.len) |__zust_i| { var part = &e.parts[__zust_i]; rewriteExpr(p, &part.value, kind) };
                },
                ...
                .e_index => |e| {
                    ...
                    if (e.index.data == .e_private_identifier) {
                        if (map.get(e.index.data.e_private_identifier.ref.innerIndex())) |info| {
                            expr[0] = privateGetExpr(p, e.target, info, expr.loc);
                            return;
                        }
                    }
                ...
                .e_binary => |e| {
                    if (e.op == .bin_assign and e.left.data == .e_index) {
                        if (e.left.data.e_index.index.data == .e_private_identifier) {
                            if (map.get(e.left.data.e_index.index.data.e_private_identifier.ref.innerIndex())) |info| {
                                ...
                                expr[0] = privateSetExpr(p, e.left.data.e_index.target, info, e.right, expr.loc);
                                return;
                            }
                        }
                    }
                    if (e.op == .bin_in and e.left.data == .e_private_identifier) {
                        if (map.get(e.left.data.e_private_identifier.ref.innerIndex())) |info| {
                            rewritePrivateAccessesInExpr(p, &e.right, map);
                            expr[0] = callRt(p, expr.loc, "__privateIn", &[_]Expr{
    ...
    for (orig_args, 0..) |*arg, ai| {
                    ...
                    rewritePrivateAccessesInExpr(p, &e.target, map);
                    for (0..e.args.slice().len) |__zust_i| { var arg = &e.args.slice()[__zust_i]; rewritePrivateAccessesInExpr(p, arg, map) };
                },
                ...
                .e_array => |e| {
                    for (0..e.items.slice().len) |__zust_i| { var item = &e.items.slice()[__zust_i]; rewritePrivateAccessesInExpr(p, item, map) };
                },
                .e_object => |e| {
                    for (0..e.properties.slice().len) |__zust_i| {
    var prop = &e.properties.slice()[__zust_i];
                ...
                .e_template => |e| {
                    ...
                    for (0..e.parts.len) |__zust_i| { var part = &e.parts[__zust_i]; rewritePrivateAccessesInExpr(p, &part.value, map) };
                },
    ...
    var stmt_item = &stmts[__zust_i];
        ...
        fn lowerImpl(p: *P, stmt: Stmt, mode: StdDecMode) []Stmt {
            ...
            var class_name_loc: logger.Loc = std.mem.zeroes(logger.Loc);
            ...
            if (is_expr) {
                ...
                expr_var_decls.append(.{ .binding = p.b(B.Identifier{ .ref = if (expr_class_ref) |value| {
    value
} else {
    return error.NullPointer;
} }, loc) }) catch unreachable;
                if (class.class_name) |cn| {
                    class_name_ref = if (cn.ref) |value| {
    value
} else {
    return error.NullPointer;
};
                    ...
                    class_name_ref = if (expr_class_ref) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
                class_name_ref = if (class.class_name.?.ref) |value| {
    value
} else {
    return error.NullPointer;
};
// safe-transpile: optional unwrap requires manual review
            ...
            if (class.extends != null) {
                ...
                if (is_expr) {
                    expr_var_decls.append(.{ .binding = p.b(B.Identifier{ .ref = if (base_ref) |value| {
    value
} else {
    return error.NullPointer;
} }, loc) }) catch unreachable;
            ...
            if (class_decorators.len > 0) {
                ...
                if (is_expr) {
                    expr_var_decls.append(.{ .binding = p.b(B.Identifier{ .ref = if (class_dec_ref) |value| {
    value
} else {
    return error.NullPointer;
} }, loc) }) catch unreachable;
                    class_dec_assign_expr = assignTo(p, if (class_dec_ref) |value| {
    value
} else {
    return error.NullPointer;
}, arr, loc);
                } else {
                    class_dec_stmt = varDecl(p, if (class_dec_ref) |value| {
    value
} else {
    return error.NullPointer;
}, arr, loc);
    ...
    for (class.properties, 0..) |*prop, prop_idx| {
                ...
                if (prop.flags.contains(.is_computed) and prop.key != null and prop.ts_decorators.len > 0) {
                    ...
                    pre_eval_stmts.append(varDecl(p, key_ref, if (prop.key) |value| {
    value
} else {
    return error.NullPointer;
}, loc)) catch unreachable;
// safe-transpile: optional unwrap requires manual review
            ...
            // Replace class name refs in pre-eval expressions for inner binding
            {
                ...
                if (!replacement_ref.eql(class_name_ref)) {
                    ...
                    for (0..pre_eval_stmts.items.len) |__zust_i| {
    var pre_stmt = &pre_eval_stmts.items[__zust_i];
    ...
    return error.NullPointer;
            ...
            const base_assign_expr: ?Expr = if (is_expr and base_ref != null)
                assignTo(p, if (base_ref) |value| {
    value
} else {
    return error.NullPointer;
}, if (class.extends) |value| {
    value
} else {
    return error.NullPointer;
}, loc)
...
// safe-transpile: optional unwrap requires manual review
                    ...
                    {
                        const nk_expr = if (prop.key) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: optional unwrap requires manual review
                        ...
                        get_fn[0] = .{ .body = .{ .stmts = get_body, .loc = loc } };
                        ...
                        set_fn[0] = .{ .args = setter_fn_args, .body = .{ .stmts = set_body, .loc = loc } };
...
// safe-transpile: optional unwrap requires manual review
    ...
    return error.NullPointer;
                ...
                const target_ref = if (is_expr and expr_class_ref != null) if (expr_class_ref) |value| {
    value
} else {
    return error.NullPointer;
} else class_name_ref;
                ...
                if (is_private and private_storage_ref != null) {
                    dec_args[4] = useRef(p, if (private_storage_ref) |value| {
    value
} else {
    return error.NullPointer;
}, loc);
                    if (dec_arg_count == 6) {
                        if (k >= 1 and k <= 3) {
                            ...
                            dec_args[5] = useRef(p, if (private_storage_ref) |value| {
    value
} else {
    return error.NullPointer;
}, loc);
                ...
                if (k >= 4) {
                    ...
                    if (!emitted_private_adds.contains(priv_inner2)) {
                        ...
                        emitPrivateAdd(p, prop.flags.contains(.is_static), if (private_storage_ref) |value| {
    value
} else {
    return error.NullPointer;
}, null, loc, &constructor_inject_stmts, &static_private_add_blocks);
                    }
                    if (prop.flags.contains(.is_static)) {
                        static_non_field_elements.append(element) catch unreachable;
            ...
            if (private_lowered_map.count() > 0) {
                for (0..new_properties.items.len) |__zust_i| {
    var nprop = &new_properties.items[__zust_i];
                ...
                for (0..instance_init_entries.items.len) |__zust_i| {
    var entry = &instance_init_entries.items[__zust_i];
                    if (entry.prop.initializer) |*ini| rewritePrivateAccessesInExpr(p, ini, &private_lowered_map);
                }
                for (0..static_init_entries.items.len) |__zust_i| {
    var entry = &static_init_entries.items[__zust_i];
                    if (entry.prop.initializer) |*ini| rewritePrivateAccessesInExpr(p, ini, &private_lowered_map);
                }
                for (extracted_static_blocks.items) |sb|
                    rewritePrivateAccessesInStmts(p, sb.stmts.slice(), &private_lowered_map);
                for (0..static_non_field_elements.items.len) |__zust_i| { var elem = &static_non_field_elements.items[__zust_i]; rewritePrivateAccessesInExpr(p, elem, &private_lowered_map) };
                for (0..instance_non_field_elements.items.len) |__zust_i| { var elem = &instance_non_field_elements.items[__zust_i]; rewritePrivateAccessesInExpr(p, elem, &private_lowered_map) };
                for (0..static_field_decorate.items.len) |__zust_i| { var elem = &static_field_decorate.items[__zust_i]; rewritePrivateAccessesInExpr(p, elem, &private_lowered_map) };
                for (0..instance_field_decorate.items.len) |__zust_i| { var elem = &instance_field_decorate.items[__zust_i]; rewritePrivateAccessesInExpr(p, elem, &private_lowered_map) };
            ...
            if (class_decorators.len > 0) {
                ...
                const class_name_str: safe.Slice(u8) = if (original_class_name_for_decorator) |name|
                    name
                ...
                cls_dec_args[4] = if (is_expr)
                    useRef(p, if (expr_class_ref) |value| {
    value
} else {
    return error.NullPointer;
}, loc)
                        ...
                        .field_or_accessor => {
                            ...
                            if (entry.is_accessor or entry.is_private) {
                                const wm_ref_expr = if (entry.is_accessor and !entry.is_private)
                                    if (entry.prop.value) |value| {
    value
} else {
    return error.NullPointer;
}
                                else
                                    if (entry.prop.key) |value| {
    value
} else {
    return error.NullPointer;
};
    ...
    var nprop = &new_properties.items[__zust_i];
            ...
            if (is_expr) {
                ...
                // _class = class { ... }
                comma_parts.append(assignTo(p, if (expr_class_ref) |value| {
    value
} else {
    return error.NullPointer;
}, p.newExpr(class.*, loc), loc)) catch unreachable;

                comma_parts.appendSlice(suffix_exprs.items) catch unreachable;

                // Final value
                const final_ref = if (class_decorators.len > 0) class_name_ref else if (expr_class_ref) |value| {
    value
} else {
    return error.NullPointer;
};
