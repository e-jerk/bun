...
// safe-transpile: function returns small constant slice — consider safe.String
        fn extractDynamicSpecifierShape(p: *P, arg: Expr, buf: *std.array_list.Managed(u8)) ![]const u8 {
            if (arg.data.as(.e_template)) |tmpl| {
                ...
                for (0..tmpl.parts.len) |__zust_i| {
    var part = &tmpl.parts[__zust_i];
        ...
        pub fn checkDynamicSpecifier(p: *P, arg: Expr, loc: logger.Loc, comptime kind: []const u8) !void {
            ...
            if (!p.options.allow_unresolved.allows(shape)) {
                ...
                if (shape.len > 0) {
                    ...
                    for (0..display.len) |__zust_i| if (c.* == 0) {
    var c = &display[__zust_i];
                        c.* = '*';
                    };
...
// safe-transpile: optional unwrap requires manual review
                                        if (name.ref.?.eql(default_export_ref) or
                                            ...
                                            p.named_imports.contains(if (name.ref) |value| {
    value
} else {
    return error.NullPointer;
}) or
                                            p.is_import_item.get(if (name.ref) |value| {
    value
} else {
    return error.NullPointer;
}) != null)
...
// safe-transpile: optional unwrap requires manual review
    ...
    for (imports, clause_items) |alias, *clause_item| {
                ...
                clause_item[0] = js_ast.ClauseItem{
                    .alias = alias_name,
...
// safe-transpile: function uses raw slice parameter — consider safe.String
                ...
                switch (expr.data) {
                    ...
                    .e_new => |new| {
                        ...
                        if (replacement_can_be_removed) {
                            for (0..new.args.slice().len) |__zust_i| {
    var arg = &new.args.slice()[__zust_i];
                    ...
                    .e_call => |e| {
                        ...
                        if (replacement_can_be_removed or e.optional_chain == null) {
                            for (0..e.args.slice().len) |__zust_i| {
    var arg = &e.args.slice()[__zust_i];
                                switch (p.substituteSingleUseSymbolInExpr(arg.*, ref, replacement, replacement_can_be_removed)) {
                                    .continue_ => {},
                                    .success => |result| {
                                        arg.* = result;
                    ...
                    .e_array => |e| {
                        for (0..e.items.slice().len) |__zust_i| {
    var item = &e.items.slice()[__zust_i];
    ...
    var property = &e.properties.slice()[__zust_i];
                            // Check the key
                    ...
                    .e_template => |e| {
                        ...
                        for (0..e.parts.len) |__zust_i| {
    var part = &e.parts[__zust_i];
        ...
        fn ensureRequireSymbol(p: *P) void {
            ...
            p.runtime_imports.put("__require", if (p.runtime_imports.__require) |value| {
    value
} else {
    return error.NullPointer;
});
                ...
                if (scope.parent) |parent_scope| {
                    nextMember: while (iter.next()) |res| {
                        ...
                        if (parent_scope.kind == .catch_binding and symbol.kind != .hoisted) {
                            ...
                            if (parent_scope.getMemberWithHash(name, if (hash) |value| {
    value
} else {
    return error.NullPointer;
})) |existing_member| {
                        ...
                        while (__scope) |_scope| {
                            ...
                            if (_scope.getMemberWithHash(name, if (hash) |value| {
    value
} else {
    return error.NullPointer;
})) |member_in_scope| {
                                    ...
                                    (Symbol.isKindFunction(existing_kind) and (scope_kind == .entry or scope_kind == .function_body)))
                                {
                                    ...
                                    const entry = _scope.getOrPutMemberWithHash(p.allocator, name, if (hash) |value| {
    value
} else {
    return error.NullPointer;
}) catch unreachable;
    ...
    return error.NullPointer;
                            ...
                            if (_scope.kindStopsHoisting()) {
                                const entry = _scope.getOrPutMemberWithHash(allocator, name, if (hash) |value| {
    value
} else {
    return error.NullPointer;
}) catch unreachable;
...
// safe-transpile: optional unwrap requires manual review
    ...
    for (ex.items.slice(), 0..) |_, i| {
                ...
                .e_object => |ex| {
                    ...
                    for (0..ex.properties.slice().len) |__zust_i| {
    var item = &ex.properties.slice()[__zust_i];
        ...
        pub fn processImportStatement(p: *P, stmt_: S.Import, path: ParsedPath, loc: logger.Loc, was_originally_bare_import: bool) anyerror!Stmt {
            ...
            if (is_macro) {
                ...
                if (stmt.default_name) |name_loc| {
                    const name = p.loadNameFromRef(if (name_loc.ref) |value| {
    value
} else {
    return error.NullPointer;
});
                ...
                for (stmt.items) |item| {
                    const name = p.loadNameFromRef(if (item.name.ref) |value| {
    value
} else {
    return error.NullPointer;
});
            ...
            if (strings.eqlComptime(path.text, "bun:bundle")) {
                ...
                for (0..stmt.items.len) |__zust_i| {
    var item = &stmt.items[__zust_i];
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    var item = &stmt.items[__zust_i];
        ...
        pub fn createDefaultName(p: *P, loc: logger.Loc) !js_ast.LocRef {
            ...
            try scope.generated.append(p.allocator, if (name.ref) |value| {
    value
} else {
    return error.NullPointer;
});
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        pub fn requireInitializers(noalias p: *P, comptime kind: S.Local.Kind, decls: []G.Decl) anyerror!void {
            ...
            for (0..decls.len) |__zust_i| {
    var decl = &decls[__zust_i];
                if (decl.value == null) {
...
// safe-transpile: optional unwrap requires manual review
        ...
        pub fn declareBinding(p: *P, kind: Symbol.Kind, binding: *BindingNodeIndex, opts: *ParseStatementOptions) anyerror!void {
            switch (binding.data) {
                ...
                .b_array => |bind| {
                    for (0..bind.items.len) |__zust_i| {
    var item = &bind.items[__zust_i];
                        p.declareBinding(kind, &item.binding, opts) catch unreachable;
                    }
                },
                .b_object => |bind| {
                    for (0..bind.properties.len) |__zust_i| {
    var prop = &bind.properties[__zust_i];
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        pub fn appendPart(noalias p: *P, parts: *ListManaged(js_ast.Part), stmts: []Stmt) anyerror!void {
            ...
            if (p.relocated_top_level_vars.items.len > 0) {
                ...
                for (0..p.relocated_top_level_vars.items.len) |__zust_i| {
    var local = &p.relocated_top_level_vars.items[__zust_i];
        ...
        fn bindingCanBeRemovedIfUnusedWithoutDCECheck(p: *P, binding: Binding) bool {
            switch (binding.data) {
                .b_array => |bi| {
                    for (0..bi.items.len) |__zust_i| {
    var item = &bi.items[__zust_i];
                ...
                .b_object => |bi| {
                    for (0..bi.properties.len) |__zust_i| {
    var property = &bi.properties[__zust_i];
            ...
            for (stmts) |stmt| {
                switch (stmt.data) {
                    ...
                    .s_local => |st| {
                        ...
                        for (0..st.decls.slice().len) |__zust_i| {
    var decl = &st.decls.slice()[__zust_i];
...
// safe-transpile: optional unwrap requires manual review
        ...
        pub fn classCanBeRemovedIfUnused(p: *P, class: *G.Class) bool {
            ...
            for (0..class.properties.len) |__zust_i| {
    var property = &class.properties[__zust_i];
        ...
        fn exprCanBeRemovedIfUnusedWithoutDCECheck(p: *P, expr: *const Expr) bool {
            switch (expr.data) {
                ...
                .e_array => |ex| {
                    for (0..ex.items.slice().len) |__zust_i| {
    var item = &ex.items.slice()[__zust_i];
                ...
                .e_object => |ex| {
                    for (0..ex.properties.slice().len) |__zust_i| {
    var property = &ex.properties.slice()[__zust_i];
                ...
                .e_call => |ex| {
                    ...
                    if (ex.can_be_unwrapped_if_unused != .never) {
                        for (0..ex.args.slice().len) |__zust_i| {
    var arg = &ex.args.slice()[__zust_i];
                ...
                inline else => |field| {
                    const ref: Ref = brk: {
                        if (p.jsx_imports.getWithTag(kind) == null) {
                            ...
                            bun.handleOom(p.module_scope.generated.append(p.allocator, if (loc_ref.ref) |value| {
    value
} else {
    return error.NullPointer;
}));
                            p.is_import_item.put(p.allocator, if (loc_ref.ref) |value| {
    value
} else {
    return error.NullPointer;
}, {}) catch unreachable;
                            @field(p.jsx_imports, @tagName(field)) = loc_ref;
                            break :brk if (loc_ref.ref) |value| {
    value
} else {
    return error.NullPointer;
};
                        ...
                        break :brk if (p.jsx_imports.getWithTag(kind)) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        pub fn lowerClass(
            ...
            switch (stmtorexpr) {
                .stmt => |stmt| {
                    ...
                    for (0..class.properties.len) |__zust_i| {
    var prop = &class.properties[__zust_i];
...
// safe-transpile: optional unwrap requires manual review
// safe-transpile: optional unwrap requires manual review
                        args[1] = p.newExpr(E.Identifier{ .ref = if (class.class_name.?.ref) |value| {
    value
} else {
    return error.NullPointer;
...
// safe-transpile: optional unwrap requires manual review
// safe-transpile: optional unwrap requires manual review
                            p.newExpr(E.Identifier{ .ref = if (class.class_name.?.ref) |value| {
    value
} else {
    return error.NullPointer;
} }, class.class_name.?.loc),
...
// safe-transpile: optional unwrap requires manual review
                        p.recordUsage(if (class.class_name.?.ref) |value| {
    value
} else {
    return error.NullPointer;
});
// safe-transpile: optional unwrap requires manual review
    ...
    return error.NullPointer;
        ...
        fn serializeMetadata(noalias p: *P, ts_metadata: TypeScript.Metadata) !Expr {
            return switch (ts_metadata) {
                ...
                .m_dot => |_refs| {
                    ...
                    while (i > 0) {
                        current_expr[0] = p.newExpr(E.Dot{
                    ...
                    current_expr[0] = try p.checkIfDefinedHelper(dot_identifier);
        ...
        fn wrapIdentifierNamespace(
            ...
            const enclosing_ref = if (p.enclosing_namespace_arg_ref) |value| {
    value
} else {
    return error.NullPointer;
};
            ...
            switch (define_data.value) {
                .e_identifier => {
                    return p.handleIdentifier(
                        ...
                        if (define_data.original_name()) |value| {
    value
} else {
    return error.NullPointer;
},
...
// safe-transpile: optional unwrap requires manual review
// safe-transpile: optional unwrap requires manual review
            while (_scope != null and !_scope.?.kindStopsHoisting()) : (_scope = if (_scope.?.parent) |value| {
    value
} else {
    return error.NullPointer;
}) {
    ...
    return error.NullPointer;
                ...
                return if (p.runtime_imports.at(name)) |value| {
    value
} else {
    return error.NullPointer;
};
        ...
        pub fn popAndFlattenScope(p: *P, scope_index: usize) void {
            ...
            var parent = if (to_flatten.parent) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            for (p.top_level_enums.items) |ref| {
                const entry = if (p.ref_to_ts_namespace_member.getEntry(ref)) |value| {
    value
} else {
    return error.NullPointer;
};
                ...
                for (stmts) |stmt| {
                    switch (stmt.data) {
                        .s_local => |local| {
                            ...
                            for (0..local.decls.slice().len) |__zust_i| {
    var decl = &local.decls.slice()[__zust_i];
                                if (decl.value) |*decl_value| {
    ...
    return error.NullPointer;
        ...
        pub fn handleImportMetaHotAcceptCall(p: *@This(), call: *E.Call) void {
            ...
            switch (call.args.at(0).data) {
                ...
                .e_array => |arr| for (0..arr.items.slice().len) |__zust_i| {
    var item = &arr.items.slice()[__zust_i];
...
// safe-transpile: optional unwrap requires manual review
                ...
                break :init &(if (ctx_storage.*) |value| {
    value
} else {
    return error.NullPointer;
});
        ...
        pub fn handleReactRefreshPostVisitFunctionBody(p: *P, stmts: *ListManaged(Stmt), hook: *ReactRefresh.HookContext) void {
            ...
            if (stmts.items.len == stmts.capacity) {
                ...
                safe.SimdUtils.copy(new_stmts[1..], stmts.items);
                stmts.deinit();
                stmts[0] = ListManaged(Stmt).fromOwnedSlice(p.allocator, new_stmts);
            } else {
                        ...
                        switch (stmt.data) {
                            .s_export_clause => |clause| {
                                for (clause.items) |item| {
                                    if (p.named_imports.getEntry(if (item.name.ref) |value| {
    value
} else {
    return error.NullPointer;
})) |_import| {
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    for (part.stmts, remaining_stmts[0..part.stmts.len]) |src, *dest| {
            ...
            var top_level = try safe.OffsetPtr.init(allocator, &top_level_symbols_to_parts); defer top_level.deinit();
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
            ...
            return .{
                ...
                .require_ref = if (p.runtime_imports.__require != null)
                    if (p.runtime_imports.__require) |value| {
    value
} else {
    return error.NullPointer;
}
