...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                                const right: u8 = @intCast(@as(u32, // safe-transpile: @bitCast requires manual review
    @bitCast(floatToInt32(vals[1]))) % 32);
                                const result: i32 = // safe-transpile: @bitCast requires manual review
    @bitCast(std.math.shl(i32, left, right));
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    @bitCast(floatToInt32(vals[1]))) % 32);
                                const result: i32 = // safe-transpile: @bitCast requires manual review
    @bitCast(std.math.shr(i32, left, right));
                                return p.newExpr(E.Number{
                                    .value = @floatFromInt(result),
                                }, v.loc);
                            }
                    ...
                    .bin_u_shr => {
                        if (p.should_fold_typescript_constant_expressions) {
                            if (Expr.extractNumericValues(e_.left.data, e_.right.data)) |vals| {
                                const left: u32 = // safe-transpile: @bitCast requires manual review
    @bitCast(floatToInt32(vals[0]));
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                                const right: u8 = @intCast(@as(u32, // safe-transpile: @bitCast requires manual review
    @bitCast(floatToInt32(vals[1]))) % 32);
