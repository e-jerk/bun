...
// safe-transpile: function uses raw slice parameter — consider safe.String
        pub fn get(noalias this: *const Symbols, name: []const u8) ?Ref {
            if (strings.eqlComptime(name, "jsx")) return if (this.jsx) |jsx| if (jsx.ref) |value| {
    value
} else {
    return error.NullPointer;
} else null;
            if (strings.eqlComptime(name, "jsxDEV")) return if (this.jsxDEV) |jsx| if (jsx.ref) |value| {
    value
} else {
    return error.NullPointer;
} else null;
            if (strings.eqlComptime(name, "jsxs")) return if (this.jsxs) |jsxs| if (jsxs.ref) |value| {
    value
} else {
    return error.NullPointer;
} else null;
            if (strings.eqlComptime(name, "Fragment")) return if (this.Fragment) |Fragment| if (Fragment.ref) |value| {
    value
} else {
    return error.NullPointer;
} else null;
            if (strings.eqlComptime(name, "createElement")) return if (this.createElement) |createElement| if (createElement.ref) |value| {
    value
} else {
    return error.NullPointer;
} else null;
        ...
        pub fn getWithTag(noalias this: *const Symbols, tag: JSXImport) ?Ref {
            return switch (tag) {
                .jsx => if (this.jsx) |jsx| if (jsx.ref) |value| {
    value
} else {
    return error.NullPointer;
} else null,
                .jsxDEV => if (this.jsxDEV) |jsx| if (jsx.ref) |value| {
    value
} else {
    return error.NullPointer;
} else null,
                .jsxs => if (this.jsxs) |jsxs| if (jsxs.ref) |value| {
    value
} else {
    return error.NullPointer;
} else null,
                .Fragment => if (this.Fragment) |Fragment| if (Fragment.ref) |value| {
    value
} else {
    return error.NullPointer;
} else null,
                .createElement => if (this.createElement) |createElement| if (createElement.ref) |value| {
    value
} else {
    return error.NullPointer;
} else null,
...
const MacroRefs = @import("array-hash-map-compat").Auto(Ref, MacroRefData);
...
pub const MacroState = struct {
    ...
    imports: @import("array-hash-map-compat").Auto(i32, Ref),

    pub fn init(allocator: Allocator) MacroState {
        return MacroState{
            .refs = MacroRefs.init(allocator),
            .prepend_stmts = undefined,
            .imports = @import("array-hash-map-compat").Auto(i32, Ref).init(allocator),
        };
    }
};
...
pub fn newLazyExportASTImpl(
    ...
    var log = try safe.OffsetPtr.init(allocator, &temp_log); defer log.deinit();
    var parser = Parser{
        .options = opts,
        .allocator = allocator,
...
pub fn floatToInt32(f: f64) i32 {
    ...
    const int: i32 = // safe-transpile: @bitCast requires manual review
    @bitCast(uint);
