...
pub const InlinedEnumValue = struct {
    ...
    const pure_nan: f64 = // safe-transpile: @bitCast requires manual review
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
            ...
            .number => |num| @as(u64, // safe-transpile: @bitCast requires manual review
    @bitCast(purifyNaN(num))) + double_encode_offset,
        } };
        if (Environment.allow_assert) {
            bun.assert(switch (encoded.decode()) {
                ...
                .number => |num| @as(u64, // safe-transpile: @bitCast requires manual review
    @bitCast(num)) ==
                    @as(u64, // safe-transpile: @bitCast requires manual review
    @bitCast(purifyNaN(decoded.number))),
            });
        }
        return encoded;
    }
    ...
    pub fn decode(encoded: InlinedEnumValue) Decoded {
        if (encoded.raw_data > 0x0000FFFFFFFFFFFF) {
            return .{ .number = // safe-transpile: @bitCast requires manual review
    @bitCast(encoded.raw_data - double_encode_offset) };
        } else {
...
pub fn NewBatcher(comptime Type: type) type {
    ...
    @ptrCast(&this.head.eat1(value).ptr));
        }
