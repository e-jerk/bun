...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                            ...
                            'u' => {
                                ...
                                if (c3 == '{') {
                                    ...
                                    variableLength: var __zust_loop_counter: u64 = 0;
    while (true) {
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        fn parseStringLiteralInner(lexer: *LexerType, comptime quote: CodePoint) !InnerStringLiteral {
            ...
            stringLiteral: var __zust_loop_counter: u64 = 0;
    while (true) {
        __zust_loop_counter += 1;
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
                ...
                switch (lexer.code_point) {
                    ...
                    '#' => {
                        ...
                        if (lexer.start == 0 and lexer.source.contents.len > 1 and lexer.source.contents[1] == '!') {
                            ...
                            hashbang: var __zust_loop_counter: u64 = 0;
    while (true) {
        __zust_loop_counter += 1;
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
                        ...
                        switch (lexer.code_point) {
                            ...
                            '-' => {
                                ...
                                if (lexer.code_point == '>' and lexer.has_newline_before) {
                                    ...
                                    singleLineHTMLCloseComment: var __zust_loop_counter: u64 = 0;
    while (true) {
        __zust_loop_counter += 1;
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
                    ...
                    '/' => {
                        ...
                        switch (lexer.code_point) {
                            ...
                            '*' => {
                                ...
                                multiLineComment: var __zust_loop_counter: u64 = 0;
    while (true) {
        __zust_loop_counter += 1;
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
        ...
        pub fn toEString(lexer: *LexerType) !js_ast.E.String {
            switch (lexer.string_literal_raw_format) {
                ...
                .utf16 => {
                    ...
                    return js_ast.E.String.init(@as([]const u16, // safe-transpile: @alignCast requires manual review
    @alignCast(std.mem.bytesAsSlice(u16, lexer.string_literal_raw_content))));
                },
                .needs_decode => {
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    while (true) {
        ...
        pub fn parseJSXStringLiteral(lexer: *LexerType, comptime quote: u8) !void {
            ...
            string_literal: var __zust_loop_counter: u64 = 0;
    while (true) {
        __zust_loop_counter += 1;
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
...
// safe-transpile: optional unwrap requires manual review
                            try lexer.decodeJSXEntities(text[first_non_whitespace.?..if (after_last_non_whitespace) |value| {
    value
} else {
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                    ...
                    (out.items.ptr + out.items.len)[0..2].* = [_]u16{
                        @as(
                            u16,
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
                            @truncate(@as(u32, // safe-transpile: @bitCast requires manual review
    @bitCast(@as(i32, 0xD800) + ((cursor.c >> 10) & 0x3FF)))),
                        ),
                        @as(
                            u16,
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
                            @truncate(@as(u32, // safe-transpile: @bitCast requires manual review
    @bitCast(@as(i32, 0xDC00) + (cursor.c & 0x3FF)))),
                        ),
                    };
        ...
        fn parseNumericLiteralOrDot(noalias lexer: *LexerType) !void {
            ...
            if (base != 0) {
                ...
                integerLiteral: var __zust_loop_counter: u64 = 0;
    while (true) {
        __zust_loop_counter += 1;
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
...
fn skipToInterestingCharacterInMultilineComment(text_: []const u8) ?u32 {
    ...
    while (text_end_ptr != text.ptr) {
        ...
        const any_significant =
            @as(V1x16, // safe-transpile: @bitCast requires manual review
    @bitCast(vec > strings.max_16_ascii)) |
            @as(V1x16, // safe-transpile: @bitCast requires manual review
    @bitCast(star == vec)) |
            @as(V1x16, // safe-transpile: @bitCast requires manual review
    @bitCast(carriage == vec)) |
            @as(V1x16, // safe-transpile: @bitCast requires manual review
    @bitCast(newline == vec));

        if (@reduce(.Max, any_significant) > 0) {
            const bitmask = @as(u16, // safe-transpile: @bitCast requires manual review
    @bitCast(any_significant));
