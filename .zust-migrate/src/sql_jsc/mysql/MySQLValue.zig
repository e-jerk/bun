...
pub const Value = union(enum) {
    ...
    pub fn toData(
        ...
        var buffer: [15]u8 = .{}; // Large enough for all fixed-size types
        ...
        switch (this.*) {
            ...
            .float => |f| writer.writeInt(u32, // safe-transpile: @bitCast requires manual review
    @bitCast(f), .little) catch undefined,
            .double => |d| writer.writeInt(u64, // safe-transpile: @bitCast requires manual review
    @bitCast(d), .little) catch undefined,
            inline .date, .time => |d| {
