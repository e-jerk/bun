...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
    ...
    pub fn deinit(this: *Row, allocator: std.mem.Allocator) void {
        for (0..this.values.len) |__zust_i| {
    var value = &this.values[__zust_i];
            value.deinit();
        }
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                        cell[0] = SQLDataCell{ .tag = .uint4, .value = .{ .uint4 = @intCast(val) } };
                        return;
                    }
                    if (this.bigint) {
                        cell[0] = SQLDataCell{ .tag = .uint8, .value = .{ .uint8 = val } };
                        return;
                    }
                } else {
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
                        cell[0] = SQLDataCell{ .tag = .int4, .value = .{ .int4 = @intCast(val) } };
                        return;
                    }
                    if (this.bigint) {
                        cell[0] = SQLDataCell{ .tag = .int8, .value = .{ .int8 = val } };
                        return;
                    }
                }

                const slice = value.slice();
                cell[0] = SQLDataCell{ .tag = .string, .value = .{ .string = if (slice.len > 0) bun.String.cloneUTF8(slice).value.WTFStringImpl else null }, .free_value = 1 };
            },
            .MYSQL_TYPE_JSON => {
                const slice = value.slice();
                cell[0] = SQLDataCell{ .tag = .json, .value = .{ .json = if (slice.len > 0) bun.String.cloneUTF8(slice).value.WTFStringImpl else null }, .free_value = 1 };
            },
            ...
            .MYSQL_TYPE_DATE, .MYSQL_TYPE_DATETIME, .MYSQL_TYPE_TIMESTAMP => {
                ...
                cell[0] = SQLDataCell{ .tag = .date, .value = .{ .date = date } };
            },
            .MYSQL_TYPE_BIT => {
                // BIT(1) is a special case, it's a boolean
                if (column.column_length == 1) {
                    const slice = value.slice();
                    cell[0] = SQLDataCell{ .tag = .bool, .value = .{ .bool = if (slice.len > 0 and slice[0] == 1) 1 else 0 } };
                } else {
                    cell[0] = SQLDataCell.raw(value);
                }
            },
            else => {
    ...
    fn decodeText(this: *Row, allocator: std.mem.Allocator, comptime Context: type, reader: NewReader(Context)) AnyMySQLError.Error!void {
        ...
        errdefer {
            for (0..cells.len) |__zust_i| {
    var value = &cells[__zust_i];
                value.deinit();
            }
            // safe-transpile: free removed (memory owned by safe type);
        }
    ...
    for (cells, 0..) |*value, index| {
            if (decodeLengthInt(reader.peek())) |result| {
                ...
                if (result.value == 0xfb) {
                    ...
                    value[0] = SQLDataCell{ .tag = .null, .value = .{ .null = 0 } };
                } else {
                    if (this.raw) {
                        var data = try reader.encodeLenString();
                        defer data.deinit();
                        value[0] = SQLDataCell.raw(&data);
                    } else {
                        reader.skip(result.bytes_read);
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
...
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
            ...
            value[0] = try decodeBinaryValue(this.globalObject, column.column_type, column.column_length, this.raw, this.bigint, column.flags.UNSIGNED, column.flags.BINARY, column.character_set, Context, reader);
            value.index = switch (column.name_or_index) {
                // The indexed columns can be out of order.
                .index => |idx| idx,
