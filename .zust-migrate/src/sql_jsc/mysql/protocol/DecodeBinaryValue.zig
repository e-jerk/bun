...
pub fn decodeBinaryValue(globalObject: *jsc.JSGlobalObject, field_type: types.FieldType, column_length: u32, raw: bool, bigint: bool, unsigned: bool, binary: bool, character_set: u16, comptime Context: type, reader: NewReader(Context)) !SQLDataCell {
    ...
    return switch (field_type) {
        .MYSQL_TYPE_TINY => {
            ...
            const ival: i8 = // safe-transpile: @bitCast requires manual review
    @bitCast(val);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        .MYSQL_TYPE_FLOAT => {
            ...
            return SQLDataCell{ .tag = .float8, .value = .{ .float8 = @as(f32, // safe-transpile: @bitCast requires manual review
    @bitCast(try reader.int(u32))) } };
        },
        .MYSQL_TYPE_DOUBLE => {
            if (raw) {
            ...
            return SQLDataCell{ .tag = .float8, .value = .{ .float8 = // safe-transpile: @bitCast requires manual review
    @bitCast(try reader.int(u64)) } };
        },
        .MYSQL_TYPE_TIME => {
