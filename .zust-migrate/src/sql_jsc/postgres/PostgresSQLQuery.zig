...
pub fn onWriteFail(
    ...
    const function = if (vm.rareData().postgresql_context.onQueryRejectFn.get()) |value| {
    value
} else {
...
pub fn onResult(this: *@This(), command_tag_str: []const u8, globalObject: *jsc.JSGlobalObject, connection: jsc.JSValue, is_last: bool) void {
    ...
    const vm = jsc.VirtualMachine.get();
    const function = if (vm.rareData().postgresql_context.onQueryResolveFn.get()) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub fn call(globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!jsc.JSValue {
    ...
    var ptr = try safe.Box(PostgresSQLQuery).init(allocator, undefined);
...
pub fn setModeFromJS(this: *PostgresSQLQuery, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!JSValue {
    ...
    this.flags.result_mode = @enumFromInt(mode);
    return .js_undefined;
}
...
pub fn doRun(this: *PostgresSQLQuery, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!JSValue {
    ...
    if (this.flags.simple) {
        ...
        const stmt = safe.Box(PostgresSQLStatement).init(allocator, undefined) catch {
            this.deref();
...
// safe-transpile: optional unwrap requires manual review
                            ...
                            PostgresRequest.bindAndExecute(globalObject, if (this.statement) |value| {
    value
} else {
    return error.NullPointer;
}, binding_value, columns_value, PostgresSQLConnection.Writer, writer) catch |err| {
                                // fail to run do cleanup
                                this.statement = null;
                                stmt.deref();
                                this.deref();
