...
fn parseArray(bytes: []const u8, bigint: bool, comptime arrayType: types.Tag, globalObject: *jsc.JSGlobalObject, offset: ?*usize, comptime is_json_sub_array: bool) !SQLDataCell {
    ...
    var stack_buffer: [16 * 1024]u8 = .{};

    errdefer {
        for (0..array.items.len) |__zust_i| {
    var cell = &array.items[__zust_i];
            cell.deinit();
        }
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    if (offset) |offset_ptr| {
        offset_ptr[0] = bytes.len - slice.len;
    }
    ...
    switch (oid) {
        ...
        inline .int4_array, .float4_array => |tag| {
            if (binary) {
                ...
                const dimensions_raw: int4 = // safe-transpile: @bitCast requires manual review
    @bitCast(bytes[0..4].*);
                const contains_nulls: int4 = // safe-transpile: @bitCast requires manual review
    @bitCast(bytes[4..8].*);
                ...
                const array_len: i32 = @byteSwap(@as(i32, // safe-transpile: @bitCast requires manual review
    @bitCast(bytes[12..16].*)));
                if (array_len < 0) {
                    return error.InvalidBinaryData;
                }
                ...
                return SQLDataCell{
                    ...
                    .value = .{
                        .typed_array = .{
                            ...
                            .ptr = if (elements.len > 0) // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(elements.ptr) else null,
// safe-transpile: @truncate requires manual review — consider safe.CheckedInt(T).init(@truncate)
        ...
        .time, .timetz => |tag| {
            ...
            if (binary) {
                if (tag == .time and bytes.len == 8) {
                    ...
                    const microseconds = @byteSwap(@as(i64, // safe-transpile: @bitCast requires manual review
    @bitCast(bytes[0..8].*)));
                    ...
                    const microseconds = @byteSwap(@as(i64, // safe-transpile: @bitCast requires manual review
    @bitCast(bytes[0..8].*)));
                    const tz_offset_seconds = @byteSwap(@as(i32, // safe-transpile: @bitCast requires manual review
    @bitCast(bytes[8..12].*)));
...
fn pg_ntoT(comptime IntSize: usize, i: anytype) std.meta.Int(.unsigned, IntSize) {
    ...
    const T = @TypeOf(i);
    if (@typeInfo(T) == .array) {
        return pg_ntoT(IntSize, @as(std.meta.Int(.unsigned, IntSize), // safe-transpile: @bitCast requires manual review
    @bitCast(i)));
    }
...
pub fn parseBinary(comptime tag: types.Tag, comptime ReturnType: type, bytes: []const u8) AnyPostgresError!ReturnType {
    switch (comptime tag) {
        .float8 => {
            return @as(f64, // safe-transpile: @bitCast requires manual review
    @bitCast(try parseBinary(.int8, i64, bytes)));
        },
        .int8 => {
            // pq_getmsgfloat8
            if (bytes.len != 8) return error.InvalidBinaryData;
            return @byteSwap(@as(i64, // safe-transpile: @bitCast requires manual review
    @bitCast(bytes[0..8].*)));
        },
        .int4 => {
            // pq_getmsgint
            switch (bytes.len) {
                ...
                2 => {
                    return pg_ntoh16(@as(u16, // safe-transpile: @bitCast requires manual review
    @bitCast(bytes[0..2].*)));
                },
                4 => {
                    return // safe-transpile: @bitCast requires manual review
    @bitCast(pg_ntoh32(@as(u32, @bitCast(bytes[0..4].*))));
                },
                else => {
    ...
    @bitCast(bytes[0..2].*)));
                ...
                4 => {
                    return pg_ntoh32(@as(u32, // safe-transpile: @bitCast requires manual review
    @bitCast(bytes[0..4].*)));
                },
                else => {
                    return error.UnsupportedIntegerSize;
        ...
        .int2 => {
            ...
            switch (bytes.len) {
                ...
                2 => {
                    ...
                    const value: u16 = // safe-transpile: @bitCast requires manual review
    @bitCast(bytes[0..2].*);
                    // Convert from big-endian to native-endian (we always use little endian)
                    return // safe-transpile: @bitCast requires manual review
    @bitCast(@byteSwap(value)); // Cast to signed 16-bit integer (i16)
                },
                else => {
                    return error.UnsupportedIntegerSize;
                },
    ...
    @bitCast(try parseBinary(.int4, i32, bytes)));
        },
