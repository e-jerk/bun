...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        if (value.isEmptyOrUndefinedOrNull()) {
            ...
            try writer.int4(// safe-transpile: @bitCast requires manual review
    @bitCast(@as(i32, -1)));
            continue;
        }
        ...
        if (tag.isBinaryFormatSupported() and value.isString()) .text else tag) {
            ...
            .int4 => {
                const l = try writer.length();
                try writer.int4(// safe-transpile: @bitCast requires manual review
    @bitCast(try value.coerceToInt32(globalObject)));
                try l.writeExcludingSelf();
            },
            .int4_array => {
                const l = try writer.length();
    ...
    @bitCast(try value.coerceToInt32(globalObject)));
            ...
            .float8 => {
                const l = try writer.length();
                try writer.f64(// safe-transpile: @bitCast requires manual review
    @bitCast(try value.toNumber(globalObject)));
                try l.writeExcludingSelf();
            },
