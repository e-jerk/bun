...
var file_path_buf: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
...
pub const Arguments = struct {
    ...
    pub fn parse(allocator: std.mem.Allocator) !Arguments {
        ...
        var raw_args: std.ArrayListUnmanaged(string) = std.mem.zeroes(std.ArrayListUnmanaged(string));
...
var stdout_: std.fs.File = std.mem.zeroes(std.fs.File);
var stderr_: std.fs.File = std.mem.zeroes(std.fs.File);
pub fn main() anyerror!void {
    ...
    var channel = try safe.Box(HTTP.HTTPChannel).init(allocator, undefined);
    channel[0] = HTTP.HTTPChannel.init();
    ...
    while (repeat_i < args.repeat + 1) : (repeat_i += 1) {
        ...
        while (i < args.count) : (i += 1) {
            ...
            response_body[0] = try MutableString.init(default_allocator, 1024);
