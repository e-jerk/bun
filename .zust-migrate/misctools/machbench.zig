...
var waker: bun.Async.Waker = std.mem.zeroes(bun.Async.Waker);
...
pub fn machMain(runs: usize) anyerror!void {
    ...
    while (remaining_runs > 0) : (remaining_runs -= 1) {
        var threads: [thread_count]std.Thread = .{};
        var j: usize = 0;
...
var user_waker: bun.Async.UserFilterWaker = std.mem.zeroes(bun.Async.UserFilterWaker);
