...
const CompressionFramework = struct {
    ...
    pub fn load() !void {
        ...
        compression_encode_scratch_buffer_size = // safe-transpile: @alignCast requires manual review
    @alignCast(@ptrCast(std.c.dlsym(handle, "compression_encode_scratch_buffer_size").?));
        compression_encode_buffer = // safe-transpile: @alignCast requires manual review
    @alignCast(@ptrCast(std.c.dlsym(handle, "compression_encode_buffer").?));
        compression_decode_scratch_buffer_size = // safe-transpile: @alignCast requires manual review
    @alignCast(@ptrCast(std.c.dlsym(handle, "compression_decode_scratch_buffer_size").?));
        compression_decode_buffer = // safe-transpile: @alignCast requires manual review
    @alignCast(@ptrCast(std.c.dlsym(handle, "compression_decode_buffer").?));
        compression_stream_init = // safe-transpile: @alignCast requires manual review
    @alignCast(@ptrCast(std.c.dlsym(handle, "compression_stream_init").?));
        compression_stream_process = // safe-transpile: @alignCast requires manual review
    @alignCast(@ptrCast(std.c.dlsym(handle, "compression_stream_process").?));
        compression_stream_destroy = // safe-transpile: @alignCast requires manual review
    @alignCast(@ptrCast(std.c.dlsym(handle, "compression_stream_destroy").?));
    }
...
pub fn main() anyerror!void {
    ...
    var output_file: std.fs.File = std.mem.zeroes(std.fs.File);
    var input_file: std.fs.File = std.mem.zeroes(std.fs.File);
    ...
    if (operation == .ENCODE) {
        switch (try CompressionFramework.compress(input_bytes, if (algorithm) |value| {
    value
} else {
    return error.NullPointer;
        ...
        switch (try CompressionFramework.decompress(input_bytes, if (algorithm) |value| {
    value
} else {
    return error.NullPointer;
}, true, writer.writer())) {
