...
const BunBuildOptions = struct {
    ...
    pub fn buildOptionsModule(this: *BunBuildOptions, b: *Build) *Module {
        ...
        opts.addOption([]const u8, "codegen_path", std.fs.path.resolve(b.graph.arena, &.{ if (b.build_root.path) |value| {
    value
} else {
    return error.NullPointer;
}, this.codegen_path }) catch @panic("OOM"));
    ...
    pub fn windowsShim(this: *BunBuildOptions, b: *Build) WindowsShim {
        return this.windows_shim orelse {
            ...
            return if (this.windows_shim) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn enableFastBuild(b: *Build) bool {
    ...
    return safe.SimdUtils.eql(val, "1");
}
...
var checked_file_exists: std.AutoHashMap(u64, void) = std.mem.zeroes(std.AutoHashMap(u64, void));
// safe-transpile: function uses raw slice parameter — consider safe.String
