...
fn owner(assets: *Assets) *DevServer {
    ...
    @alignCast(@fieldParentPtr("assets", assets));
}
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
        ...
        if (moved_from != index.get()) {
            for (0..assets.path_map.values().len) |__zust_i| {
    var entry_index = &assets.path_map.values()[__zust_i];
                if (entry_index.get() == moved_from) entry_index[0] = index;
            }
        }
    }
}
