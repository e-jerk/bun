...
pub fn runWithBody(ctx: *ErrorReportRequest, body: []const u8, r: AnyResponse) !void {
    ...
    defer for (0..parsed_source_maps.values().len) |__zust_i| {
    var value = &parsed_source_maps.values()[__zust_i];
        if (value.*) |*v| v.deinit(temp_alloc);
    };

    var runtime_lines: ?[5][]const u8 = null;
    var first_line_of_interest: usize = 0;
    var top_frame_position: jsc.ZigStackFramePosition = std.mem.zeroes(jsc.ZigStackFramePosition);
    var region_of_interest_line: u32 = 0;
    for (0..frames.items.len) |__zust_i| {
    var frame = &frames.items[__zust_i];
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    StaticRoute.sendBlobThenDeinit(r, &.fromArrayList(out), .{
        .mime_type = &.other,
        .server = if (ctx.dev.server) |value| {
    value
} else {
...
fn extractJsonEncodedSourceCode(contents: []const u8, target_line: u32, comptime n: usize, arena: Allocator) !?[n][]const u8 {
    ...
    for (0..&result.len) |__zust_i| {
    var decoded_line = &&result[__zust_i];
        ...
        if (has_extra_escapes) {
            ...
            decoded_line[0] = bytes.items;
        } else {
            decoded_line[0] = encoded_line;
        }
