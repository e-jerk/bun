...
dump_dir: if (bun.FeatureFlags.bake_debugging_features) ?@import("std-fs-compat").FsDir else void,
...
pub fn init(options: Options) bun.JSOOM!*DevServer {
    ...
    var dump_dir = if (bun.FeatureFlags.bake_debugging_features)
        if (options.dump_sources) |dir| dir: {
            var cwd_dir = @import("std-fs-compat").FsDir{ .fd = std.c.AT.FDCWD };
            break :dir cwd_dir.makeOpenPath(dir, .{}) catch |err| {
...
pub fn deinit(dev: *DevServer) void {
    ...
    useAllFields(DevServer, .{
        ...
        .route_bundles = {
            for (0..dev.route_bundles.items.len) |__zust_i| {
    var rb = &dev.route_bundles.items[__zust_i];
                rb.deinit(alloc);
            }
        ...
        .barrel_needed_exports = {
            ...
            while (it.next()) |entry| {
                ...
                while (inner_it.next()) |k| // safe-transpile: free removed (memory owned by safe type);
                inner.deinit(alloc);
                // safe-transpile: free removed (memory owned by safe type);
            }
        ...
        .source_maps = {
            for (0..dev.source_maps.entries.values().len) |__zust_i| {
    var value = &dev.source_maps.entries.values()[__zust_i];
        ...
        .watcher_atomics = for (0..&dev.watcher_atomics.events.len) |__zust_i| {
    var event = &&dev.watcher_atomics.events[__zust_i];
...
fn onJsRequest(dev: *DevServer, req: *Request, resp: AnyResponse) void {
    ...
    if (is_map) {
        const source_id: SourceMapStore.SourceId = // safe-transpile: @bitCast requires manual review
...
fn onAssetRequest(dev: *DevServer, req: *Request, resp: AnyResponse) void {
    ...
    var out: [@sizeOf(u64)]u8 = .{};
    assert((std.fmt.hexToBytes(&out, hex) catch
        return notFound(resp)).len == @sizeOf(u64));
    const hash: u64 = // safe-transpile: @bitCast requires manual review
    // safe-transpile: @bitCast requires manual review
    @bitCast(out);
...
// safe-transpile: optional unwrap requires manual review
    ...
    fn onFailure(this: *RequestEnsureRouteBundledCtx) bun.JSError!void {
        try this.dev.sendSerializedFailures(
            .{ .http = this.resp },
            (&(if (this.dev.routeBundlePtr(this.route_bundle_index).data.framework.evaluate_failure) |value| {
    value
} else {
    return error.NullPointer;
}))[0..1],
                ...
                server_handler.ctx.setAdditionalOnAbortCallback(.{
                    ...
                    .deref_fn = struct {
                        fn deref_fn(ptr: *anyopaque) void {
                            var self: *DeferredRequest = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
...
var __zust_loop_counter: u64 = 0;
    ...
    if (__loop_limit_1 > 1_000_000) break;
            ...
            route = dev.router.routePtr(framework_bundle.route_index);
            {
                const relative_path_buf = bun.path_buffer_pool.get();
                defer bun.path_buffer_pool.put(relative_path_buf);
                var route_name = bun.String.cloneUTF8(dev.relativePath(relative_path_buf, keys[fromOpaqueFileId(.server, if (route.file_page.unwrap()) |value| {
    value
} else {
    return error.NullPointer;
}).get()]));
...
fn onFrameworkRequestWithBundle(
    ...
    var params: FrameworkRouter.MatchedParams = std.mem.zeroes(FrameworkRouter.MatchedParams);
...
fn onHtmlRequestWithBundle(dev: *DevServer, route_bundle_index: RouteBundle.Index, resp: AnyResponse, method: bun.http.Method) void {
    ...
    const blob = html.cached_response orelse generate: {
        ...
        break :generate if (html.cached_response) |value| {
    value
} else {
    return error.NullPointer;
};
...
fn generateJavaScriptCodeForHTMLFile(
    ...
    const w = @import("std-io-compat").allocatingWriterFromArrayList(sfa, &array);
...
pub fn onJsRequestWithBundle(dev: *DevServer, bundle_index: RouteBundle.Index, resp: AnyResponse, method: bun.http.Method) void {
    ...
    const client_bundle = route_bundle.client_bundle orelse generate: {
        ...
        break :generate if (route_bundle.client_bundle) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub const DeferredRequest = struct {
    ...
    fn onAbortWrapper(this: *anyopaque) void {
        const self: *DeferredRequest = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
...
pub fn startAsyncBundle(
    ...
    if (dev.inspector()) |agent| {
        ...
        defer for (0..trigger_files.items.len) |__zust_i| {
    var str = &trigger_files.items[__zust_i];
            str.deref();
        };
    ...
    const ast_memory_allocator = try safe.Box(bun.ast.ASTMemoryAllocator).init(allocator, undefined);
    ...
    // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(dev.bundler_options.plugin),
        },
        alloc,
        .{ .js = dev.vm.eventLoop() },
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    // safe-transpile: @bitCast requires manual review
    @bitCast(removed.getOwner().encode()), .little);
            removed.deinit(dev);
        }
...
fn generateClientBundle(dev: *DevServer, route_bundle: *RouteBundle) bun.OOM![]u8 {
    ...
    var react_fast_refresh_id: safe.Slice(u8) = "";
...
fn generateCssJSArray(dev: *DevServer, route_bundle: *RouteBundle) bun.JSError!jsc.JSValue {
    ...
    // safe-transpile: for with index access requires manual review
    for (names, 0..) |item, i| {
        var buf: [asset_prefix.len + @sizeOf(u64) * 2 + "/.css".len]u8 = .{};
...
pub fn finalizeBundle(
    ...
    const current_bundle = &if (dev.current_bundle) |value| {
    value
} else {
    return error.NullPointer;
};
...
// zust: use safe.String or safe.GuardedSlice for slice operations
    ...
    for (0..result.htmlChunks().len) |__zust_i| {
    var chunk = &result.htmlChunks()[__zust_i];
        const index = bun.ast.Index.init(chunk.entry_point.source_index);
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    for (0..result.cssChunks().len) |__zust_i| {
    var chunk = &result.cssChunks()[__zust_i];
    ...
    @ptrCast(dev.vm.global),
        ...
        } else c.BakeLoadServerHmrPatch(// safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(dev.vm.global), bun.String.cloneLatin1(server_bundle)) catch |err| {
            dev.vm.printErrorLikeObjectToConsole(dev.vm.global.takeException(err));
            @panic("Error thrown while evaluating server code. This is always a bug in the bundler.");
        };
// safe-transpile: optional unwrap requires manual review
// safe-transpile: optional unwrap requires manual review
    ...
    const w = @import("std-io-compat").writer(&hot_update_payload);
...
var __zust_loop_counter: u64 = 0;
    ...
    if (__loop_limit_4 > 1_000_000) break;
        ...
        } else .{ false, @import("std-fs-compat").Timer.start() catch @panic("timers unsupported") };
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
    ...
    dev.route_bundles.appendAssumeCapacity(.{
        ...
        .client_script_generation = blk: {
            var buf: [4]u8 = .{};
            std.c.arc4random_buf(&buf, 4);
    ...
    index_location[0] = bundle_index.toOptional();
    return bundle_index;
}
...
// safe-transpile: function uses raw slice parameter — consider safe.String
// safe-transpile: function uses raw slice parameter — consider safe.String
pub fn dumpBundle(dump_dir: @import("std-fs-compat").FsDir, graph: bake.Graph, rel_path: []const u8, chunk: []const u8, wrap: bool) !void {
    const buf = bun.path_buffer_pool.get();
    defer bun.path_buffer_pool.put(buf);
    ...
    var inner_dir = try dump_dir.makeOpenPath(if (bun.Dirname.dirname(u8, name)) |value| {
    value
} else {
    return error.NullPointer;
}, .{});
    ...
    const file = try inner_dir.createFile(bun.path.basename(name), @import("std-fs-compat").FsDir.CreateFileOptions{ .mode = 0 });
    defer file.close();
    var file_buffer: [1024]u8 = .{};
...
// safe-transpile: function uses raw slice parameter — consider safe.String
// safe-transpile: function uses raw slice parameter — consider safe.String
pub noinline fn dumpBundleForChunk(dev: *DevServer, dump_dir: @import("std-fs-compat").FsDir, side: bake.Side, key: []const u8, code: []const u8, wrap: bool, is_ssr_graph: bool) void {
    const cwd = dev.root;
    var a: bun.PathBuffer = std.mem.zeroes(bun.PathBuffer);
    var b: [bun.MAX_PATH_BYTES * 2]u8 = .{};
...
pub fn emitMemoryVisualizerMessageTimer(timer: *EventLoopTimer, _: *const bun.timespec) void {
    if (!bun.FeatureFlags.bake_debugging_features) return;
    const dev: *DevServer = // safe-transpile: @alignCast requires manual review
    // safe-transpile: @alignCast requires manual review
    @alignCast(@fieldParentPtr("memory_visualizer_timer", timer));
    assert(dev.magic == .valid);
...
fn toOpaqueFileId(comptime side: bake.Side, index: IncrementalGraph(side).FileIndex) OpaqueFileId {
    if (Environment.allow_assert) {
        return OpaqueFileId.init(// safe-transpile: @bitCast requires manual review
    // safe-transpile: @bitCast requires manual review
    @bitCast(SafeFileId{
...
fn fromOpaqueFileId(comptime side: bake.Side, id: OpaqueFileId) IncrementalGraph(side).FileIndex {
    if (Environment.allow_assert) {
        const safe: SafeFileId = // safe-transpile: @bitCast requires manual review
    // safe-transpile: @bitCast requires manual review
    @bitCast(id.get());
...
fn dumpStateDueToCrash(dev: *DevServer) !void {
    ...
    var filepath_buf: [@min(4096, bun.MAX_PATH_BYTES)]u8 = .{};
    const filepath = std.fmt.bufPrintZ(&filepath_buf, "incremental-graph-crash-dump.{d}.html", .{@import("std-fs-compat").timestamp()}) catch "incremental-graph-crash-dump.html";
    const file = switch (bun.sys.open(filepath, bun.O.CREAT | bun.O.WRONLY | bun.O.TRUNC, 0o644)) {
        .result => |fd| fd,
        .err => |err| {
    ...
    var buf: [bun.base64.encodeLenFromSize(4096)]u8 = .{};
    var it = std.mem.window(u8, payload.items, 4096, 4096);
    while (it.next()) |chunk| {
        try writeAllFd(file, buf[0..bun.base64.encode(&buf, chunk)]);
    }

    try writeAllFd(file, "\"), c => c.charCodeAt(0));\n");
    try writeAllFd(file, end);
...
// safe-transpile: function uses raw slice parameter — consider safe.String
    pub fn append(entry_points: *EntryPointList, alloc: Allocator, abs_path: []const u8, flags: Flags) !void {
        ...
        if (gop.found_existing) {
            const T = if (@typeInfo(Flags).@"struct".backing_integer) |value| {
    value
} else {
    return error.NullPointer;
};
            gop.value_ptr.* = // safe-transpile: @bitCast requires manual review
    // safe-transpile: @bitCast requires manual review
    @bitCast(@as(T, @bitCast(gop.value_ptr.*)) | @as(T, @bitCast(flags)));
        } else {
            gop.value_ptr.* = flags;
        }
    }
...
fn bundleNewRouteJSFunctionImpl(global: *bun.jsc.JSGlobalObject, request_ptr: *anyopaque, url_bunstr: bun.String) bun.JSError!bun.jsc.JSValue {
    ...
    const request: *bun.webcore.Request = // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(@alignCast(request_ptr));
