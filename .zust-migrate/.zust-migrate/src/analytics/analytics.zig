...
pub const packed_features_list = brk: {
    ...
    var names: [decls.len][:0]const u8 = .{};
    var i = 0;
    for (decls) |decl| {
...
pub const PackedFeatures = @Type(.{
    .@"struct" = .{
        ...
        .backing_integer = u64,
        .fields = brk: {
            var fields: [64]std.builtin.Type.StructField = .{};
            var i: usize = 0;
...
var random: std.rand.DefaultPrng = std.mem.zeroes(std.rand.DefaultPrng);
...
pub const GenerateHeader = struct {
    pub const GeneratePlatform = struct {
        ...
        pub var linux_os_name: std.c.utsname = std.mem.zeroes(std.c.utsname);
        var platform_: analytics.Platform = std.mem.zeroes(analytics.Platform);
        pub const Platform = analytics.Platform;
        var linux_kernel_version: Semver.Version = std.mem.zeroes(Semver.Version);
...
// zust: use safe.String or safe.GuardedSlice for slice operations
        ...
        var freebsd_os_name: if (Environment.isFreeBSD) bun.c.struct_utsname else void = std.mem.zeroes(if (Environment.isFreeBSD) bun.c.struct_utsname else void);
