    ...
    while (true) {
        ...
        if (__zust_loop_counter > 1_000_000) return error.InfiniteLoop;
            ...
            if ((changelist[0].flags & std.c.EV.ERROR) != 0 and changelist[0].data != 0) {
                return if (bun.sys.Maybe(void).errnoSys(changelist[0].data, .kevent)) |value| {
    value
} else {
    return error.NullPointer;
};
...
// safe-transpile: @intCast requires manual review — consider safe.CheckedInt(T).init(@intCast)
            ...
            switch (rc) {
                ...
                std.math.minInt(@TypeOf(rc))...-1 => return if (bun.sys.Maybe(void).errnoSys(@intFromEnum(errno), .kevent)) |value| {
    value
} else {
    return error.NullPointer;
},
    ...
    return error.NullPointer;
            ...
            if (rc >= 2 and (changelist[1].flags & std.c.EV.ERROR) != 0 and changelist[1].data != 0) {
                return if (bun.sys.Maybe(void).errnoSys(changelist[1].data, .kevent)) |value| {
    value
} else {
    return error.NullPointer;
};
...
pub const LinuxWaker = struct {
    ...
    pub fn wait(this: Waker) void {
        var bytes: usize = 0;
        _ = std.posix.read(this.fd.cast(), @as(*[8]u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&bytes))) catch 0;
    }
    ...
    pub fn wake(this: *const Waker) void {
        ...
        _ = std.posix.write(
            this.fd.cast(),
            @as(*[8]u8, // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    // safe-transpile: @ptrCast requires manual review — add @alignCast if alignment is guaranteed
    @ptrCast(&bytes)),
        ) catch 0;
    }
