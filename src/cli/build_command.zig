pub const BuildCommand = struct {
    pub fn exec(ctx: Command.Context, fetcher: ?*BundleV2.DependenciesScanner) !void {
        ...
        const src_root_dir: string = brk1: {
            ...
            var posix_path = std.posix.toPosixPath(path) catch |err| {
                Output.prettyErrorln("<r><red>{s}<r> path too long {f}", .{ @errorName(err), bun.fmt.quote(path) });
                Global.exit(1);
            };
            var dir = bun.openDirForPath(&posix_path) catch |err| {
                Output.prettyErrorln("<r><red>{s}<r> opening root directory {f}", .{ @errorName(err), bun.fmt.quote(path) });
                Global.exit(1);
            };
            defer dir.close();

            break :brk1 bun.getFdPath(bun.FD.fromNative(dir.fd), &src_root_dir_buf) catch |err| {
