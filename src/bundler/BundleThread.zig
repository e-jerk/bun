    ...
    return struct {
        ...
        pub const singleton = struct {
            ...
            fn loadOnceImpl() void {
                const bundle_thread = bun.handleOom(zust.Box(Self).init(bun.default_allocator, undefined));
        ...
        fn generateInNewThread(completion: *CompletionStruct, generation: bun.Generation) !void {
            ...
            var ast_memory_allocator = try zust.Box(js_ast.ASTMemoryAllocator).init(allocator, undefined);
            ast_memory_allocator.* = .{ .allocator = allocator };
            ast_memory_allocator.reset();
            ast_memory_allocator.push();

            const transpiler = try zust.Box(bun.Transpiler).init(allocator, undefined);
