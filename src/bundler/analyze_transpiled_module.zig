...
pub const ModuleInfoDeserialized = struct {
    ...
    pub fn create(source: []const u8, gpa: std.mem.Allocator) !*ModuleInfoDeserialized {
        ...
        const res = try safe.Box(ModuleInfoDeserialized).init(gpa, undefined);
        errdefer _ = res.deinit();
...
pub const ModuleInfo = struct {
    ...
    pub fn create(gpa: std.mem.Allocator, is_typescript: bool) !*ModuleInfo {
        const res = try safe.Box(ModuleInfo).init(gpa, undefined);
        res.ptr.* = ModuleInfo.init(gpa, is_typescript);
        return res.ptr;
    }
