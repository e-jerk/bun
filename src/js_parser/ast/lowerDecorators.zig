...
pub fn LowerDecorators(
    ...
    return struct {
        ...
        fn makeStaticBlock(p: *P, expr: Expr, l: logger.Loc) Property {
            ...
            const sb = bun.handleOom(zust.Box(G.ClassStaticBlock, 0, 0, 0).init(p.allocator, undefined));
            ...
            for (class.properties, 0..) |*prop, prop_idx| {
                if (prop.ts_decorators.len == 0) {
                    ...
                    if (prop.kind == .auto_accessor) {
                        ...
                        const get_fn = bun.handleOom(zust.Box(G.Fn, 0, 0, 0).init(p.allocator, undefined));
                        ...
                        const set_fn = bun.handleOom(zust.Box(G.Fn, 0, 0, 0).init(p.allocator, undefined));
