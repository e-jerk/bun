...
pub const Archiver = struct {
    ...
    pub fn getOverwritingFileList(
        ...
        const dir: @import("std-fs-compat").FsDir = brk: {
            ...
            if (std.fs.path.isAbsolute(root)) {
                break :brk bun.openDirAbsolute(root) catch return;
            } else {
                break :brk bun.openDirA(std.Io.Dir.cwd(), root) catch return;
            }
        };
                        ...
                        .directory => {
                            ...
                            if (comptime Environment.isWindows) {
                                ...
                                if (rc != 0) {
                                    ...
                                    bun.makePath(std.Io.Dir{ .handle = dir.fd }, std.fs.path.dirname(path_slice) orelse return error.Fail) catch {};
    ...
    pub fn extractToDisk(
        ...
        var dir: @import("std-fs-compat").FsDir = brk: {
            ...
            if (std.fs.path.isAbsolute(root)) {
                break :brk try bun.openDirAbsolute(root);
            } else {
                break :brk try bun.openDirA(std.Io.Dir.cwd(), root);
            }
        };
