pub const PackageInstaller = struct {
    ...
    pub const LazyPackageDestinationDir = union(enum) {
        ...
        pub fn close(this: *LazyPackageDestinationDir) void {
            switch (this.*) {
                .dir => {
                    if (this.dir.handle != std.c.AT.FDCWD) {
                        @import("std-fs-compat").dirClose(this.dir);
                    }
                },
    ...
    pub fn installPackageWithNameAndResolution(
        ...
        if (needs_install) {
            ...
            const destination_dir = this.node_modules.makeAndOpenDir(this.root_node_modules_folder) catch |err| {
                if (log_level != .silent) {
                ...
                .symlink, .workspace => installer.installFromLink(this.skip_delete, destination_dir),
                    ...
                    if (resolution.tag == .root or (resolution.tag == .folder and !this.lockfile.isWorkspaceTreeId(this.current_tree_id))) {
                        ...
                        installer.cache_dir = (bun.openDirA(this.root_node_modules_folder, dirname) catch |err|
                            break :result .fail(err, .opening_cache_dir, @errorReturnTrace())).toDir();
