...
pub const HmrTopic = enum(u8) {
    ...
    pub const Bits = blk: {
        ...
        var types: [enum_fields.len]type = undefined;
        var attrs: [enum_fields.len]std.builtin.Type.StructField.Attributes = undefined;
        for (enum_fields, 0..) |e, i| {
            names[i] = e.name;
            types[i] = bool;
            attrs[i] = .{ .default_value_ptr = &false };
        }
        break :blk @Struct(.@"packed", @Int(.unsigned, max_count), &names, &types, &attrs);
    };
};
